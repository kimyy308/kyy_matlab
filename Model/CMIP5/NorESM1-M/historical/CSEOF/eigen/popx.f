C *****************************************************************************
C
C       PROGRAM NAME : popx.f
C       PROGRAMMER : Dr. Kwang Y. Kim
C       CODE IDENTIFICATION : POPX/VERSION 1.0
C       CODE CLASSIFICATION : scientific computer code
C       CREATION DATE : August 21, 1997
...
C          T(t) = A T(t-1) + N(t-1)
C
C     where A is called the system matrix.  The time series T(t) denotes 
C     a set of principal component (PC) time series of a regular EOF 
C     analysis.  The dimension of the problem is determined by the
C     truncation order.  Now, POPs (principal oscillation patterns),
C     P, are the eigenvectors of the system matrix A.  Then, the POP 
C     coefficients Z are given by
C
C          Z = (P')* T
C
...
C     may expect complex eigenvalues and complex eigenfunctions.
      ...
      PARAMETER (NTMAX=600, NMAX=100, MST=615)
      DIMENSION TSER(NTMAX,NMAX), ARR(NTMAX), AVG(NMAX),
     &          DMTRX(NMAX,NMAX), SIG0(NMAX,NMAX), SIG1(NMAX,NMAX)
      DIMENSION COV(NMAX,NMAX), WR(NMAX), WI(NMAX), Z(NMAX,NMAX)
...
C ------- Lagged Covariance Matrices
      ...
      DO 20 I=1,NMD
        ...
        SUM = 0.0
        DO K=1,NPTS-LTAU
          SUM = SUM + TSER(K+LTAU,I)*TSER(K,J)
        END DO
...
C ------- Call Eigenvalue Routines
      CALL BALANC(NMAX,NMD,COV,LOW,IGH,SCALE)
      CALL ELMHES(NMAX,NMD,LOW,IGH,COV,INDX)
      CALL ELTRAN(NMAX,NMD,LOW,IGH,COV,INDX,Z)
      CALL HQR2(NMAX,NMD,LOW,IGH,COV,WR,WI,Z,IERR)
...
65      FORMAT(5X,5E13.5,/)
        WRITE(8,70) (EGF(K,I), K=1,NMD)
        WRITE(9,70) (EGFA(K,I), K=1,NMD)
70      FORMAT(6E13.5)
        NMODE = I
75    CONTINUE
80    PRINT *, NMODE
...
C ------- Write POP Patterns
     ...
     &    (MOD(IOUT/16,2).EQ.0) .AND. (MOD(IOUT/32,2).EQ.0))  GO TO 90
      ...
      IF ((MOD(IOUT,2).EQ.0) .AND. (MOD(IOUT/2,2).EQ.0))  GO TO 90
...
C          SUMC = 0.0
C          DO K=1,NMD
C            SUMT1 = SUMT1 + TSER(I,K)*REAL(EGF(K,J))
C            SUMT2 = SUMT2 + TSER(I,K)*AIMAG(EGF(K,J))
C            SUM1 = SUM1 + REAL(EGF(K,J))*REAL(EGF(K,J))
C            SUM2 = SUM2 + AIMAG(EGF(K,J))*AIMAG(EGF(K,J))
C            SUMC = SUMC + REAL(EGF(K,J))*AIMAG(EGF(K,J))
C          END DO
C          IF (SUM2.EQ.0.) THEN
C            PCTS(I) = SUMT1 / SUM1
C          ELSE IF (SUM1.EQ.0.) THEN
C            PCTS(I) = JIMAG*SUMT2 / SUM2
C          ELSE
C            PCTS(I) = ((SUMT1*SUM2 - SUMT2*SUMC)
C     &                + JIMAG*(SUMT2*SUM1 - SUMT1*SUMC))
C     &              / (SUM1*SUM2 - SUMC**2)
C          END IF 
          SUM1 = 0.0
          SUM2 = 0.0
