...
C          T(t) = A T(t-1) + N(t-1)
C
C     where A is called the system matrix.  POPs (principal oscillation
C     patterns), P, are the eigenvectors of the system matrix A.  Then, 
...
C     may expect complex eigenvalues and complex eigenfunctions.
     ...
     &            DMTRX(:,:), SIG0(:,:), SIG1(:,:)
      ALLOCATABLE COV(:,:), WR(:), WI(:), Z(:,:)
      ALLOCATABLE SCALE(:), INDX(:)
      ALLOCATABLE W(:), V(:,:)
      ALLOCATABLE EFLD(:), PERD(:)
      COMPLEX, DIMENSION(:,:), ALLOCATABLE :: EGF, EGFA
      COMPLEX, DIMENSION(:), ALLOCATABLE :: EGV, PCTS
      COMPLEX JIMAG, CSUM
...
C ------- Allocate Dynamic Arrays
      ...
      ALLOCATE(ARR(NPTS))
      ALLOCATE(AVG(IDTR,NPTS))
      ALLOCATE(DMTRX(NST,NST))
      ALLOCATE(SIG0(NST,NST))
      ALLOCATE(SIG1(NST,NST))
      ...
      ALLOCATE(EFLD(NST))
      ALLOCATE(PERD(NST))
      ALLOCATE(EGF(NST,NST))
      ALLOCATE(EGFA(NST,NST))
      ALLOCATE(EGV(NST))
      ALLOCATE(PCTS(NPTS))
...
C ------- Read Input File
     ...
     &         ACCESS='DIRECT', RECL=NPTS)
          DO K=1,NST
            READ(4,REC=K)  (TSER(I,K), I=1,NPTS)
          END DO
        ELSE
          OPEN(UNIT=4, FILE=FILNM1, STATUS='OLD',
     &         ACCESS='DIRECT', RECL=NST)
          DO I=1,NPTS
            READ(4,REC=L)  (TSER(I,K), K=1,NST)
          END DO
        END IF
      CASE ('SEQ')
        IF (IRD.EQ.1) THEN
      ...
      CASE DEFAULT
        ...
        ELSE
          DO I=1,NPTS
          DO L=1,NY
            READ(4,FORMT1)  (TSER(I,K+(L-1)*NX), K=1,NX)
          END DO
          END DO
        END IF
      END SELECT
...
C ------- Remove Mean
      IF (IDTR.NE.0) THEN
        ...
        DO IM=1,IDTR
          ...
          AVG(IM,J) = SUM
          DO I=IM,NPTS,IDTR
            TSER(I,J) = TSER(I,J) - AVG(IM,J)
          END DO
        END DO
C          WRITE(11,'(6E13.5)')  (AVG(J), J=1,NST)
        END DO
      END IF
...
C ------- Lagged Covariance Matrices
      ...
      DO 20 I=1,NST
        ...
        END DO
        SIG0(I,J) = SUM/FLOAT(NPTS)
        SUM = 0.0
        DO K=1,NPTS-LTAU
          KK = K+LTAU
          SUM = SUM + TSER(KK,I)*TSER(K,J)
        END DO
        SIG1(I,J) = SUM/FLOAT(NPTS)
20    CONTINUE

C ------- System Matrix
      IF (ISOL.EQ.0) THEN
        CALL LUDCMP(SIG0,NST,NST,INDX,SIGN)
        DO I=1,NST
          DO J=1,NST
            DMTRX(I,J) = 0.0
          END DO
        ...
        END DO
        DO J=1,NST
          CALL LUBKSB(SIG0,NST,NST,INDX,DMTRX(1,J))
        END DO

      ELSE
        CALL SVDCMP(SIG0,NST,NST,NST,NST,W,V)
        WMAX = 0.0
        DO J=1,NST
...
C ------- Open Output Files
      ...
      OPEN(UNIT=9, FILE='amode.d', STATUS='UNKNOWN')
...
C ------- Call Eigenvalue Routines
      CALL BALANC(NST,NST,COV,LOW,IGH,SCALE)
      CALL ELMHES(NST,NST,LOW,IGH,COV,INDX)
      CALL ELTRAN(NST,NST,LOW,IGH,COV,INDX,Z)
      CALL HQR2(NST,NST,LOW,IGH,COV,WR,WI,Z,IERR)
      CALL BALBAK(NST,NST,LOW,IGH,SCALE,NST,Z)
...
C ----------> rotate such that the first mode is real
        ...
        END DO
C ----------> alternate way of computing normalization factor
C        SUM1 = 0.0
C        SUM2 = 0.0
C        SUMC = 0.0
C        DO I=1,NST
...
C ------- Sorting by Eigenvalues
      IF (ISRT.EQ.1)  CALL EIGSRT(EGV,EGF,EGFA,EFLD,PERD,NPOP,NST,NST)
...
65      FORMAT(5X,5E13.5,/)
        DO L=1,NY
          LL = (L-1)*NX
          WRITE(8,70) (EGF(K+LL,I)*SCL, K=1,NX)
          WRITE(9,70) (EGFA(K+LL,I)*SCL, K=1,NX)
70        FORMAT(6E13.5)
        END DO
        NMODE = I
75    CONTINUE
