C *****************************************************************************
C
C       PROGRAM NAME : cpop.f
C       PROGRAMMER : Dr. Kwang Y. Kim
C       CODE IDENTIFICATION : CPOP/VERSION 1.0
C       CODE CLASSIFICATION : scientific computer code
C       CREATION DATE : June 12, 1998
...
C     may expect complex eigenvalues and complex eigenfunctions.
      ...
      PARAMETER (NTMAX=600, NMAX=615, NN=2*NMAX)
     ...
     &          ZR(NMAX,NMAX), ZI(NMAX,NMAX), ORTR(NMAX), ORTI(NMAX)
      ...
      COMPLEX EGF(NMAX,NMAX), EGV(NMAX), PCTS(NTMAX), JIMAG
...
C ------- Remove Mean
      DO J=1,NST
        ...
        END DO
      END DO
C      WRITE(9,'(5E15.7)')  (AVG(J), J=1,NST)
...
C ------- System Matrix
      IF (ISOL.EQ.0) THEN
        CALL LUDCMP(SIG0,2*NST,NN,INDX,SIGN)
        DO I=1,NST
          II = I+NST
          DO J=1,2*NST
            DMTRX(I,J) = 0.0
        ...
        END DO
        DO J=1,NST
          CALL LUBKSB(SIG0,2*NST,NN,INDX,DMTRX(1,J))
        END DO

      ELSE
        CALL SVDCMP(SIG0,2*NST,2*NST,NN,NN,W,V)
        WMAX = 0.0
        DO J=1,2*NST
...
C ------- Call Eigenvalue Routines
      CALL CBAL(NMAX,NST,COVR,COVI,LOW,IGH,SCALE)
      CALL CORTH(NMAX,NST,LOW,IGH,COVR,COVI,ORTR,ORTI)
      CALL COMQR2(NMAX,NST,LOW,IGH,ORTR,ORTI,COVR,COVI,WR,WI,ZR,ZI,IERR)
      CALL CBABK2(NMAX,NST,LOW,IGH,SCALE,NST,ZR,ZI)
