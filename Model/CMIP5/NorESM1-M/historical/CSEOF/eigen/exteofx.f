C *****************************************************************************
C
C        PROGRAM NAME : exteofx.f
C        PROGRAMMER : Dr. Kwang Y. Kim
C        CODE IDENTIFICATION NUMBER = EXTEOFX/VERSION 1.0
C        CODE CLASSIFICATION = Scientific Computer Code
C        CREATION DATE = August 18, 1997
...
C
C     In this version of the code, PC time series are computed first as
C     eigenfunctions of temporal covariance matrix since the time dimension
C     is less than the space dimension.
...
C ------- Allocate Dynamic Array
      ALLOCATE(TSER(NST,NTOT))
      ALLOCATE(AVG(NST,IDTR))
      ALLOCATE(ARR(NTOT))
      ALLOCATE(PC(NST,0:ML))
      ALLOCATE(COV(NPTS,NPTS))
      ALLOCATE(D(NPTS))
      ALLOCATE(V(NPTS,NPTS))
      ALLOCATE(INDX(NPTS))
...
C ------- Read Input File
     ...
     &         ACCESS='DIRECT', RECL=NTOT)
          DO K=1,NST
            READ(4,REC=K)  (TSER(K,I), I=1,NTOT)
          END DO
        ELSE
          OPEN(UNIT=4, FILE=FILNMI, STATUS='OLD',
     &         ACCESS='DIRECT', RECL=NST)
          DO I=1,NTOT
            READ(4,REC=I)  (TSER(K,I), K=1,NST)
          END DO
        END IF
      CASE ('SEQ')
        IF (IRD.EQ.1) THEN
      ...
      CASE DEFAULT
        ...
        IF (IRD.EQ.1) THEN
          DO K=1,NST
            READ(4,FORMTI)  (TSER(K,I), I=1,NTOT)
          END DO
        ELSE
          DO I=1,NTOT
          DO L=1,NY
            READ(4,FORMTI)  (TSER(K+(L-1)*NX,I), K=1,NX)
          END DO
          END DO
        END IF
      END SELECT
...
C ------- Covariance Matrix (Lag Extended)
      ...
      DO 20 J=I,NPTS
        ...
        DO 15 LAG=0,NL,LD
        DO 15 K=1,NST
          SUM = SUM + TSER(K,I+LAG)*TSER(K,J+LAG)
15      CONTINUE
        COV(I,J) = SUM/FLOAT(NST)/FLOAT(ML+1)
        COV(J,I) = COV(I,J)
20    CONTINUE
...
C ------- Call Eigenvalue Routines
      CALL JACOBI(COV,NPTS,NPTS,D,V,NROT)
      CALL EIGSRT(D,V,NPTS,NPTS)
...
C ------- Normalization
        ...
        DO 70 L=0,ML
        DO 70 K=1,NST
          SUM = SUM + PC(K,L)*PC(K,L)
70      CONTINUE
        STD = SQRT(SUM/FLOAT(ML+1))
        DO 75 LAG=0,ML
          DO K=1,NST
            PC(K,LAG) = PC(K,LAG)/STD
          END DO
          DO L=1,NY
            LL = (L-1)*NX
            WRITE(10,'(6E13.5)')  (PC(K+LL,LAG)*SCL, K=1,NX)
          END DO
75      CONTINUE
80    CONTINUE
