C ***************************************************************************** 
C 
C        PROGRAM NAME : cseofx.f 
C        PROGRAMMER : Dr. Kwang Y. Kim 
C        CODE IDENTIFICATION = CSEOFX/VERSION 1.0 
C        CODE CLASSIFICATION = Scientific Computer Code 
C        CREATION DATE = April 10, 1996
...
C     time series at each sampling station.  In addition, PC time series
C     are computed first as eigenfunctions of temporal covariance matrix
C     since the time dimension is less than the space dimension of the 
C     problem.
C
...
5     FORMAT(A50)
      ...
      READ *, IDTR
      PRINT *, '  Type the number of points for covariance computation.'
      READ *, NCOR
...
C ------- Allocate space for each array
      ...
      ALLOCATE(TAVG(ICYC,NST))
      ALLOCATE(TCPY(NTOT))
      ALLOCATE(TCOF(NTOT,ICYC,NST))
      ALLOCATE(CF(NTOT,0:ICYC))
      ALLOCATE(SF(NTOT,0:ICYC))
      ALLOCATE(TCOFF(ICYC*NST,NTOT))
      ALLOCATE(COV(NCOR,NCOR))
      ALLOCATE(D(NCOR))
      ALLOCATE(V(NCOR,NCOR))
      ALLOCATE(INDX(NCOR))
      ALLOCATE(PC(MPTS))
      ALLOCATE(EOF(ICYC))
      ALLOCATE(EOFP(ICYC,NST))
      ALLOCATE(WGTS(NTOT))
      ...
      IF (IDTR.NE.0) THEN
        ...
        DO IM=1,IDTR
          ...
          DO I=IM,NTOT,IDTR
            SUM = SUM + TSER(I,L)
          END DO
          TAVG(IM,L) = SUM/FLOAT((NTOT-IM)/IDTR+1)
          DO I=IM,NTOT,IDTR
...
20    CONTINUE
      REWIND(UNIT=10)
      DO 25 K=1,MPTS*NST
        READ(10)  (TCOFF(K,I), I=1,NTOT)
25    CONTINUE
      GO TO 40

C ------- Read coefficient time series
30    CONTINUE
      IF (IOPT.NE.1)  GO TO 60
...
40    CONTINUE
      ...
      DO 50 J=I,NCOR
        SUM = 0.0
        DO 45 K=1,MPTS*NST
          SUM = SUM + TCOFF(K,I)*TCOFF(K,J)
45      CONTINUE
        COV(I,J) = SUM/FLOAT(NST)/FLOAT(MPTS)
        COV(J,I) = COV(I,J)
50    CONTINUE
      OPEN(UNIT=31, FILE='covm.d', STATUS='UNKNOWN', FORM='UNFORMATTED')
      DO J=1,NCOR
        WRITE(31)  (COV(I,J), I=1,NCOR)
      END DO
      GO TO 70
...
C ------- Total variance
      ...
      END DO
      WRITE(7,75) TVAR
75    FORMAT(5X,'TOTAL VARIANCE = ',E15.7,///)

C ------- Call eigenfunction routines
      CALL JACOBI(COV,NCOR,NCOR,D,V,NROT)
      CALL EIGSRT(D,V,NCOR,NCOR)
...
C ------- Write eigenmodes and modal contributions
      ...
      DO IM=1,NCOR
        ...
        SUM = SUM + VAR
        WRITE(7,80)  VAR, SUM
80      FORMAT(5X,'VARIANCE AND CUMULATIVE VARIANCE = ',2E16.7)
        WRITE(8,85)  (V(I,IM), I=1,NCOR)
85      FORMAT(6E13.5)
        NMODE = IM
        IF (SUM*100. GE. PVAR)  GO TO 90
      END DO
90    PRINT *, NMODE
...
C ------- Compute Bloch functions
      ...
      DO IM=1,MIN(NMODE,NPRT)
C ---------- PC patterns
        DO L=1,NST
          DO K=1,MPTS
            SUM = 0.0
            DO I=1,NCOR
              SUM = SUM + V(I,IM)*TCOF(I,K,L)
            END DO
            PC(K) = SUM
          END DO
C ---------- Bloch functions
          ...
          DO K=2,MPTS
            ...
            IF (MOD(K,2).EQ.0) THEN
              DO I=1,ICYC
                ANG = FRQ*FLOAT(I-1)
                EOF(I) = EOF(I) + FACT*PC(K)*COS(ANG)
              END DO
            ELSE
              DO I=1,ICYC
                ANG = FRQ*FLOAT(I-1)
                EOF(I) = EOF(I) + FACT*PC(K)*SIN(ANG)
              END DO
            END IF
          END DO
          DO I=1,ICYC
...
C ---------- Normalization
        ...
        DO 95 I=1,ICYC
          SUM = SUM + EOFP(I,L)**2
95      CONTINUE
C        write(81,'(i5,2e13.5)')  im, sum, sum/d(im)
        STD = SQRT(SUM/FLOAT(NST)/FLOAT(ICYC))
        DO I=1,ICYC
          DO L=1,NST
            EOFP(I,L) = EOFP(I,L)/STD*SCL
          END DO
          WRITE(32)  (EOFP(I,L), L=1,NST)
        END DO
      END DO
