C *****************************************************************************
C
C       PROGRAM NAME : ceof.f
C       PROGRAMMER : Dr. Kwang Y. Kim
C       CODE IDENTIFICATION NUMBER = CEOF/VERSION 1.0
C       CODE CLASSIFICATION = Scientific Computer Code
C       CREATION DATE = July 31, 1997
C       REVISION DATE = October 4, 2002
C       REVISION INFORMATION = f90 version (October 4, 2002)
C
C *****************************************************************************
...
C     up to the essential phase.
      ...
      COMPLEX, DIMENSION(:), ALLOCATABLE :: PCTS
...
C ------- Allocate Dynamic Arrays
      ...
      ALLOCATE(AMP(NST,NST))
      ALLOCATE(PHS(NST,NST))
      ALLOCATE(THLB(NPTS))
      ALLOCATE(ARR(NPTS))
...
C ------- New Complex Variable
        DO I=1,NPTS
          TCMP(I,J) = TSER(I,J) + JIMAG*THLB(I)
        END DO
      END DO
...
65      FORMAT(5X,'VARIANCE AND CUMULATIVE VARIANCE = ',2E16.7,/)
        IF (MOD(IOUT,2).EQ.1) THEN
          DO L=1,NY
            LL = (L-1)*NX
            WRITE(8,70) (W(K+LL,I)*SCL, K=1,NX)
70          FORMAT(6E13.5)
          END DO
        END IF
        IF (MOD(IOUT/2,2).EQ.1) THEN
          DO L=1,NY
            LL = (L-1)*NX
            WRITE(8,70) (AMP(K+LL,I)*SCL, K=1,NX)
          END DO
          DO L=1,NY
            LL = (L-1)*NX
            WRITE(8,70) (PHS(K+LL,I), K=1,NX)
          END DO
        END IF
        NMODE = I
...
C ------- PC Time Series
      ...
      DO 90 J=1,MIN(NST,NPRT)
        DO I=1,NPTS
          SUMC = (0.,0.)
          DO K=1,NST
            SUMC = SUMC + TCMP(I,K)*CONJG(W(K,J))
          END DO
          PCTS(I) = SUMC
        END DO
