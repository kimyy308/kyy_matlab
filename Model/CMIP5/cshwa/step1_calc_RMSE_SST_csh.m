...
%mod
ocean_mod_tos = zeros(24,ocean_num,nn);
for k = 1:24
    for i = 1:ocean_num
        for j = 1:30
            idx = nwp_idx{i};
            ocean_mod_tos(k,i,j) = nansum(model_tos3(cshwa_num(k),j,idx))/ocean_area(i); 
        end
    end
end
...
clear nc;
save(strcat(result_path2,'/','processing_result_tos_NWP_csh.mat'));
