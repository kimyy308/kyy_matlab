...
%mod
ocean_mod_heat = zeros(mm,ocean_num,nn);
for k = 1:mm
    for i = 1:ocean_num
        for j = 1:30
            idx = nwp_idx{i};
            ocean_mod_heat(k,i,j) = nansum(model_heat3(k,j,idx))/ocean_area(i); 
        end
    end
end
...
clear nc;
save(strcat(result_path,'/','processing_result_heatcontent_NWP_csh_bc.mat'));
