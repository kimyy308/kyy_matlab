...
load compare_result_zos_fix_1.mat
...
p_bc(i,:)=polyfit([1:30],squeeze(marine_mod_zos(i,2,:))',1)
y_bc=polyval(p_bc(i,:),[1:30]);
plot([1:30],y_bc,'r');
title(model_name(i))
set(gca,'fontsize',20,'fontweight','bold');
xlabel('year')
...
ylim([-0.05 0.05])
set(gca,'ytick',[-0.05:0.01:0.05])
saveas(gca,strcat('time_mod_BC_v2_',num2str(i),'.png'))
end
close all;
...
p_nwp(i,:)=polyfit([1:30],squeeze(marine_mod_zos(i,2,:))',1)
y_nwp=polyval(p_nwp(i,:),[1:30]);
plot([1:30],y_nwp,'r');
title(model_name(i))
set(gca,'fontsize',20,'fontweight','bold');
xlabel('year')
...
ylim([-0.05 0.05])
set(gca,'ytick',[-0.05:0.01:0.05])
saveas(gca,strcat('time_mod_NWP_v2_',num2str(i),'.png'))
end
close all;

save('trend_for_cmip5_41_model.mat','p_nwp','p_bc','model_name');
