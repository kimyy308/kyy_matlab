...
result_path2 = '/home/ygkim/ipcc/data/tos/new';
...
ocean_num = 2;
 %---2.2. calculate NWP tos time series
 %obs
ocean_obs_tos = zeros(ocean_num,30);
ocean_area = zeros(ocean_num,1);
for i = 1:ocean_num
    idx = nwp_idx{i};
    ocean_area(i) = len*len*nansum(ratio(idx));
    for j = 1:30
        ocean_obs_tos(i,j) = nansum(obs_tos3(j,idx))/ocean_area(i);
    end
end
%mod
ocean_mod_tos = zeros(mm,ocean_num,nn);
for k = 1:mm
    for i = 1:ocean_num
        for j = 1:30
            idx = nwp_idx{i};
            ocean_mod_tos(k,i,j) = nansum(model_tos3(k,j,idx))/ocean_area(i); 
        end
    end
end
...
ocean_RMSE = zeros(mm,ocean_num);
ocean_bias = zeros(mm,ocean_num);
for i = 1:mm
    for j = 1:ocean_num
        ocean_RMSE(i,j) = sqrt(squeeze(nanmean((squeeze(ocean_mod_tos(i,j,:))-ocean_obs_tos(j,:)').^2)));
        ocean_bias(i,j)    = squeeze(nanmean(squeeze(ocean_mod_tos(i,j,:))-ocean_obs_tos(j,:)')) ;
    end
end
...
%obs_trend
 ...
 %mod trend
 x = [1:30]';
ocean_trend_mod = zeros(mm,ocean_num);
for i = 1:mm
     for j = 1:ocean_num
         data = squeeze(ocean_mod_tos(i,j,:));
         [a,b] = polyfit(x,data,1);
...
clear nc;
save(strcat(result_path2,'/','processing_result_tos_NWP.mat'));
