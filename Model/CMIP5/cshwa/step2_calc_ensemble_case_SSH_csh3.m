...
result_path2 = '/home/ygkim/ipcc/data/zos/new'; %result path
...
load(strcat(result_path2,'/processing_result_zos_NWP.mat'));
...
% ocean_mod_zos = ocean_mod_zos(zos_idx-3,:,:);


%------STEP2. ensemble mean process ------%
%[sort_idx, sort_num] = sort(total_RMSE_new) ;   %set sort parameter
% cshwa_num = [3,5,7,11,13,15,16,17,19,20,21,14,25,26,28,29,31,32,33,35,37,38,39,40]; %cshwa
ens_ocean_mod_zos2 = zeros(14,2,30);
ens_ocean_RMSE2 = zeros(14,2);
ens_ocean_bias2 = zeros(14,2);
ens_ocean_trend_mod2 = zeros(14,2);

%%%%%%%%% cshwa 14 model index pick up
k=0
% sort_cal = sort_num +3;
...
csh_name = {'CSIRO-Mk3-6-0', 'GFDL-ESM2G', 'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'MIROC5', 'MIROC-ESM-CHEM','NorESM1-M', 'bcc-csm1-1-m', 'CNRM-CM5','CanESM2', 'FGOALS-g2', 'MPI-ESM-LR', 'MPI-ESM-MR', 'MRI-CGCM3'};
for i = 1:14
...
for k = 1:14
    ...
    ens_ocean_mod_zos = squeeze(mean(ocean_mod_zos(fin_idx,:,:),1));
    %----3.2. calc RMSE----%
     %calculate RMSE and bias

    for i = 1:2
            ens_ocean_RMSE(i) = sqrt(squeeze(nanmean((squeeze(ens_ocean_mod_zos(i,:))-ocean_obs_zos(i,:)).^2)));
            ens_ocean_bias(i)    = squeeze(nanmean(squeeze(ens_ocean_mod_zos(i,:))-ocean_obs_zos(i,:))) ;
    end
    ...
    ens_ocean_trend_mod = zeros(1,2);
         for j = 1:2
             data = squeeze(ens_ocean_mod_zos(j,:));
             [a,b] = polyfit(x,data,1);
             ens_ocean_trend_mod(j) = a(1);
         end
    
    %save each ens results
    ens_ocean_mod_zos2(k,:) = ens_ocean_mod_zos(:);
...
%save data
save(strcat(result_path2,'/','processing_result_v2_zos_NWP_csh3.mat'));
