...
dirs.r='/mnt/lustre/proj/earth.system.predictability/ASSM_EXP_timeseries/archive'; %dirs.root
% dirs.yoshi_root='/proj/yoshi/DATA/CESM2_ODA';
dirs.arc=[dirs.r, fs, 'archive']; %dirs.archive
% dirs.svr='/mnt/lustre/proj/kimyy/Model/CESM2/ESP/LENS2/assm_ini/all_layer';  %dirs.saveroot
dirs.svr='/mnt/lustre/proj/kimyy/Model/CESM2/ESP/ASSM_EXP/all_layer';
cfg.y=1960:2014; %config.years
cfg.m=1:12; %config.months
% cfg.sce='HIST'; %cfg.scenname
cfg.gr='f09_g17'; %cfg.gridname
config.obsn={'en4.2_ba', 'oras4_ba'};
% config.obsn={'projdv7.3_ba', 'en4.2_ba', 'oras4_ba'};
...
cfg_o.st = 'ALOHA';
cfg_o.st_lon = 360.0-158.0;
cfg_o.st_lat = 22.0+45.0/60.0;

% config_obs.staname = 'CalCOFI';
% config_obs.sta_lon = [360-124.323, 360-117.28]; % degrees west to 0~360 degrees east
% config_obs.sta_lat = [29.846, 35.088]; % degrees north
...
for vind=1:length(cfg.v)
    ...
    for oind=1:length(cfg.obs)
        t.onm=cfg.obs{oind};
        system(['ls -d ', dirs.r, '/*/ | grep ', t.onm, ' | rev | cut -c 2-5 | rev > ~/tmpn']); %save ensemble name list (nonuniq)
    
        cfg.ens=unique(textread('~/tmpn', '%s')); %tmp.ensnamelist
        cfg.l_e= length(cfg.ens); %cfg.l_e
        t.e_str=' ';
        for eind=1:cfg.l_e
            t.e_str=[t.e_str, ', ', t.onm,'-',cfg.ens{eind}];
        end

        t.er_s=0; %t.error_status;
    
        cfg.svnm=strcat(dirs.svr, fs, cfg_o.st, fs, t.cmp, fs, t.v, fs, ...
                ['CESM2_', cfg_o.st, '_', t.v, '_', t.onm, '_', cfg.ys, '.nc']); %cfg.savefilename (will be saved)
                            ...
                            if t.y==min(cfg.y)
                                ...
                                switch t.cmp
                                    ...
                                    case {'atm', 'lnd', 'rof'}
                                        ...
                                        if strcmp(t.cmp, 'lnd')
                                            CESM2_grid.land_mask=ncread(t.gn, 'landmask');
                                            CESM2_grid.land_mask(CESM2_grid.landmask==0)=NaN;
                                        end
                                ...
                                if strcmp(cfg_o.st, 'GLO')
                                    ...
                                    switch t.cmp
                                        ...
                                        [CESM2_grid.id_w, CESM2_grid.id_e, CESM2_grid.id_s, CESM2_grid.id_n] = ...
                                            Func_0012_findind_Y(1, [cfg_o.st_lon, cfg_o.st_lat], ...
                                        ...
                                        case {'atm', 'rof'}
                                         [CESM2_grid.id_w, CESM2_grid.id_e, CESM2_grid.id_s, CESM2_grid.id_n] = ...
                                            ...
                                            CESM2_grid.z_t=ncread(t.gn, 'lev');  % (cm)
                                        case {'lnd'}
                                         [CESM2_grid.id_w, CESM2_grid.id_e, CESM2_grid.id_s, CESM2_grid.id_n] = ...
                                            Func_0012_findind_Y(1, [cfg_o.st_lon, cfg_o.st_lat], ...
                                            CESM2_grid.lon_t.*CESM2_grid.land_mask, ...
                                            CESM2_grid.lat_t.*CESM2_grid.land_mask, 'CESM2'); % find valid lon, lat index near station
                                    end
                                end
    %                             [CESM2_grid.id_e, CESM2_grid.id_n]=size(CESM2_grid.lon_t);
                                ...
                                CESM2_grid.l_z= length(CESM2_grid.z_t);
    ...
    %                             CESM2_grid.z_t_3d= permute(repmat(CESM2_grid.z_t',1,CESM2_grid.l_x,CESM2_grid.l_y),[2,3,1]);
                                ...
                                switch t.dim
                                    case '4d'
                                        CESM2_data.(t.v)(1:CESM2_grid.l_x, 1:CESM2_grid.l_y, 1:CESM2_grid.l_z, 1:t.i_tmax, eind) = ...
                                            ncread(t.fn, t.v, [CESM2_grid.id_w, CESM2_grid.id_s, 1, t.i_tmin], ...
                                            [CESM2_grid.l_x, CESM2_grid.l_y, inf, t.i_tmax]);
                                    case '3d'
                                        CESM2_data.(t.v)(1:CESM2_grid.l_x, 1:CESM2_grid.l_y, 1:t.i_tmax, eind) = ...
                                ...
                                if t.f_y_max < max(cfg.y)
                                    ...
                                    switch t.dim
                                        case '4d'
                                            CESM2_data.(t.v)(1:CESM2_grid.l_x, 1:CESM2_grid.l_y, 1:CESM2_grid.l_z, t.i_t:t.i_t+t.i_tmax-1, eind)= ...
                                                ncread(t.fn, t.v, [CESM2_grid.id_w, CESM2_grid.id_s, 1, 1], ...
                                                [CESM2_grid.l_x, CESM2_grid.l_y, inf, t.i_tmax]);
                                        case '3d'
                                            CESM2_data.(t.v)(1:CESM2_grid.l_x, 1:CESM2_grid.l_y, t.i_t:t.i_t+t.i_tmax-1, eind)= ...
        ...
        if (exist(cfg.svnm)==0)
            ...
            switch t.cmp
                case 'ocn'
                    ...
                    lev_dimid = netcdf.defDim(ncid,'z_t', CESM2_grid.l_z);
                case {'atm', 'lnd', 'rof'}
                    lon_dimid = netcdf.defDim(ncid, 'lon', CESM2_grid.l_x);
                    lat_dimid = netcdf.defDim(ncid, 'lat', CESM2_grid.l_y);
                    lev_dimid = netcdf.defDim(ncid,'lev', CESM2_grid.l_z);
            end
    ...
    %             netcdf.putAtt(ncid,timevarid,'bounds','time_bound');           
            ...
            switch t.cmp
                case {'ocn'}
                    levvarid = netcdf.defVar(ncid, 'z_t', 'NC_DOUBLE', lev_dimid);
                    netcdf.putAtt(ncid,levvarid,'units','cm');
                case {'atm', 'lnd', 'rof'}
                    ...
                    levvarid = netcdf.defVar(ncid, 'lev', 'NC_DOUBLE', lev_dimid);
                    netcdf.putAtt(ncid,levvarid,'units','hPa');
            end
            ...
            switch t.dim
                ...
                case '4d'
                    tempvarid=netcdf.defVar(ncid, t.v, 'NC_FLOAT', [lon_dimid lat_dimid lev_dimid time_dimid, ens_dimid]);
                        netcdf.putAtt(ncid,tempvarid,'long_name', [t.v]);
                        netcdf.defVarChunking(ncid, tempvarid, 'CHUNKED', [CESM2_grid.l_x/10, CESM2_grid.l_y/10 1 cfg.l_t/10 1]);
                        netcdf.defVarDeflate(ncid, tempvarid, true, true, 1);
            end
    ...
    %             netcdf.putAtt(ncid,gm_tempvarid,'units','degC');
            ...
            netcdf.putVar(ncid, levvarid, 0, CESM2_grid.l_z , CESM2_grid.z_t);
            switch t.cmp
                case {'atm', 'lnd', 'rof'}
                    netcdf.putVar(ncid, lonvarid, 0, CESM2_grid.l_x , CESM2_grid.lon);
                    netcdf.putVar(ncid, latvarid, 0, CESM2_grid.l_y , CESM2_grid.lat);
            end
            netcdf.putVar(ncid, tempvarid, [0 0 0 0 0], [CESM2_grid.l_x CESM2_grid.l_y CESM2_grid.z_t cfg.l_t cfg.l_e], CESM2_data.(t.v));
