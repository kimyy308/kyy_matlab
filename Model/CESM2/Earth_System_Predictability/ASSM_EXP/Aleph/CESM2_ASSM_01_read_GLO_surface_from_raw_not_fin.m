...
config.obsnames={'projdv7.3_ba', 'en4.2_ba', 'oras4_ba'};
...
config.ensnames={'10p1', '10p2', '10p3', '10p4', '10p5', ...
...
for varind=1:length(config.varnames)
    ...
    t.cmp = Func_0025_CESM2_cmpname_var(tmp.varname); %t.component
        ...
        if (exist(config.savename) ==0)
            for ensind=1:length(config.ensnames)
                ...
                for yind=1:length(config.years)
                    ...
                    if tmp.year<=2014
                        config.scenname='HISTsmbb';
                    elseif tmp.year<=2015
                        config.scenname='SSP370smbb';
                    end
                    
                    dirs.datadir=[dirs.archive, filesep, 'b.e21.B', config.scenname, '.', ...
                        config.gridname, '.assm.', tmp.obsname, '-', tmp.ensname, filesep, ...
                        t.cmp, filesep, 'hist'];
                    %% read grid
                    if ensind==1
                        [tmp.error_status, tmp.value]=system(['ls ', dirs.datadir, '| grep once']);
                        t.gn = [dirs.datadir, filesep, tmp.value(1:end-1)];
