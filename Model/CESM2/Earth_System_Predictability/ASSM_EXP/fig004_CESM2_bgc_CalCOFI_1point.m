...
%     tmp.alldata(tmp.alldata==-999)=NaN;
    ...
    OBS.latmean=32;
    tic;
    tmp.tmpsavename_2nd=[dirs.obssavedir, '/obs_2_1point', '_', tmp.varname, '.mat'];
    if (exist(tmp.tmpsavename_2nd) ==0)
        for ti=1:OBS.len_Depth_ID_uniq   %% 470 sec, 512?
            ...
            tmp.lon=OBS.lon(OBS.data_ind);
            tmp.lat=OBS.lat(OBS.data_ind);
            %% quality control
            ...
            if length(tmp.depth_valid(tmp.uniqind))<=1 || tmp.lon(1) < OBS.lonmean-1 || tmp.lon(1) > OBS.lonmean+1 ...
                    || tmp.lat(1) < OBS.latmean-1 || tmp.lat(1) > OBS.latmean+1
