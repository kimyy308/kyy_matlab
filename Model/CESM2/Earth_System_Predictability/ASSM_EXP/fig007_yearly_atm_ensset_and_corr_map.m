...
cfg.vars={'CLOUD', 'OMEGA', 'Q', 'RELHUM', 'U', 'V', 'AODDUST', 'dry_deposition_NOy_as_N', 'dry_deposition_NHx_as_N', ...
    'dst_a1SF', 'dst_a1_SRF', 'dst_a2SF', 'dst_a2DDF', 'dst_a2SFWET', 'dst_a2_SRF', 'dst_a3SF', ...
    'dst_a3_SRF', 'FSNS', 'FSDS', 'PRECT', 'PSL', 'SFdst_a1', 'SFdst_a2', 'SFdst_a3', 'TAUX', 'TAUY', ...
    'TS', 'U10', 'wet_deposition_NHx_as_N', 'wet_deposition_NOy_as_N'};
% abc=dir('/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/ASSM_EXP/archive_yearly_analysis/atm')
% cfg.vars=abc(3:end).name;
% 
% cfg.obs={'en4.2_ba', 'projdv7.3_ba'};
for vari= 1:length(cfg.vars)
    name.var=cfg.vars{vari};
    dirs.assmroot=['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/ASSM_EXP/archive_yearly_analysis/atm/', name.var];
    dirs.figroot=['/Volumes/kyy_raid/kimyy/Figure/CESM2/ESP/ASSM_EXP/archive_yearly_analysis/atm/', name.var];
...
%% Signal to Noise Ratio (SNR) map  --------------------------------------
        ...
        dirs.figdir= [dirs.figroot, filesep, 'ens_set'];
        if (strcmp(cfg.dim_var, '4d'))
            cfg.figname=[dirs.figdir, filesep, name.fign, '_', name.var, '_lev', str.lev, '.tif'];
            tmp.title=[name.var, ', ', name.fign, ', ', str.lev];
        elseif (strcmp(cfg.dim_var, '3d'))
            cfg.figname=[dirs.figdir, filesep, name.fign, '_', name.var, '.tif'];
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_y_det_', name.fign, '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
...
%% ens_mean_magnitude map  -----------------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_y_det_', 'time_mean_ensmean', '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
            [grid.tlat, grid.tlong] = meshgrid(grid.lat, grid.lon);
            if (strcmp(cfg.dim_var, '4d'))
...
%% ens_spread map  -----------------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_y_det_', 'time_mean_spread', '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
            [grid.tlat, grid.tlong] = meshgrid(grid.lat, grid.lon);
            if (strcmp(cfg.dim_var, '4d'))
...
%% ens_amplitude map  -----------------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_y_det_', 'time_std_ensmean', '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
            [grid.tlat, grid.tlong] = meshgrid(grid.lat, grid.lon);
            if (strcmp(cfg.dim_var, '4d'))
...
%ttest  p= 1-tcdf(r*sqrt((mu)/(1-r^2)), mu)
        ...
        name.fign = 'corr_mean';
        cfg.dim_var=Func_0026_CESM2_dim_var(name.var);
        dirs.figdir= [dirs.figroot, filesep, 'corr_set'];
        if (strcmp(cfg.dim_var, '4d'))
            cfg.figname=[dirs.figdir, filesep, name.fign, '_', name.var, '_lev', str.lev, '.tif'];
            tmp.title=[name.var, ', ', name.fign, ', ', str.lev];
        elseif (strcmp(cfg.dim_var, '3d'))
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_y_det_', 'corr_ensmean', '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
            [grid.tlat, grid.tlong] = meshgrid(grid.lat, grid.lon);
            if (strcmp(cfg.dim_var, '4d'))
...
%% correlation (max) map  --------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_y_det_', 'corr_ensmax', '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
            [grid.tlat, grid.tlong] = meshgrid(grid.lat, grid.lon);
            if (strcmp(cfg.dim_var, '4d'))
...
%% correlation (min) map  --------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_y_det_', 'corr_ensmin', '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
            [grid.tlat, grid.tlong] = meshgrid(grid.lat, grid.lon);
            if (strcmp(cfg.dim_var, '4d'))
...
%% correlation (std) map  --------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_y_det_', 'corr_ensstd', '.nc'];
            grid.lon=ncread(fname.(name.fign), 'lon');
            grid.lat=ncread(fname.(name.fign), 'lat');
            [grid.tlat, grid.tlong] = meshgrid(grid.lat, grid.lon);
            if (strcmp(cfg.dim_var, '4d'))
