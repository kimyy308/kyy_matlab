...
% abc=dir('/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/ASSM_EXP/archive_analysis/ocn')
% cfg.vars=abc(3:end).name;

for vari= 1:length(cfg.vars)
    name.var=cfg.vars{vari};
    dirs.assmroot=['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/ASSM_EXP/archive_analysis/ocn/', name.var];
    dirs.figroot=['/Volumes/kyy_raid/kimyy/Figure/CESM2/ESP/ASSM_EXP/archive_analysis/ocn/', name.var];
...
%% Signal to Noise Ratio (SNR) map  --------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_', name.fign, '.nc'];
...
%% ens_mean_magnitude map  -----------------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_', 'time_mean_ensmean', '.nc'];
            grid.tlong=ncread(fname.grid, 'TLONG');
            grid.tlat=ncread(fname.grid, 'TLAT');
            if (strcmp(cfg.dim_var, '4d'))
                tmp.C=ncread(fname.(name.fign), name.var, [1 1 tmp.lev 1], [inf inf 1 1]);
...
%% ens_spread map  -----------------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_', 'time_mean_spread', '.nc'];
            grid.tlong=ncread(fname.grid, 'TLONG');
            grid.tlat=ncread(fname.grid, 'TLAT');
            if (strcmp(cfg.dim_var, '4d'))
                tmp.C=ncread(fname.(name.fign), name.var, [1 1 tmp.lev 1], [inf inf 1 1]);
...
%% ens_amplitude map  -----------------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.ens, tmp.fs, name.var, '_', 'time_std_ensmean', '.nc'];
            grid.tlong=ncread(fname.grid, 'TLONG');
            grid.tlat=ncread(fname.grid, 'TLAT');
            if (strcmp(cfg.dim_var, '4d'))
                tmp.C=ncread(fname.(name.fign), name.var, [1 1 tmp.lev 1], [inf inf 1 1]);
...
%ttest  p= 1-tcdf(r*sqrt((mu)/(1-r^2)), mu)
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_', 'corr_ensmean', '.nc'];
            grid.tlong=ncread(fname.grid, 'TLONG');
            grid.tlat=ncread(fname.grid, 'TLAT');
            if (strcmp(cfg.dim_var, '4d'))
                tmp.C=ncread(fname.(name.fign), name.var, [1 1 tmp.lev 1], [inf inf 1 1]);
...
%             tmp.C(tmp.C<1)=1;
            tmp.maxC=max(max(tmp.C));
            tmp.prc95= prctile(tmp.C(:), 95);
            fig_cfg=fig_param_set('jet', [0 1], tmp.dropboxpath);  % cmap, clim, droppath
%              fig_cfg.c_map(1,:)=[0.8 0.8 0.8]; % 1< : grayed
            fig_cfg.fig_name=tmp.title;
            fig_h = figure('name',fig_cfg.fig_name,'PaperUnits','inches', ...
                'PaperPosition',fig_cfg.fig_size,'position',fig_cfg.fig_size*get(groot,'ScreenPixelsPerInch')+[200,200,0,0],'visible','off');
...
%% correlation (max) map  --------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_', 'corr_ensmax', '.nc'];
            grid.tlong=ncread(fname.grid, 'TLONG');
            grid.tlat=ncread(fname.grid, 'TLAT');
            if (strcmp(cfg.dim_var, '4d'))
                tmp.C=ncread(fname.(name.fign), name.var, [1 1 tmp.lev 1], [inf inf 1 1]);
...
%% correlation (min) map  --------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_', 'corr_ensmin', '.nc'];
            grid.tlong=ncread(fname.grid, 'TLONG');
            grid.tlat=ncread(fname.grid, 'TLAT');
            if (strcmp(cfg.dim_var, '4d'))
                tmp.C=ncread(fname.(name.fign), name.var, [1 1 tmp.lev 1], [inf inf 1 1]);
...
%% correlation (std) map  --------------------------------------
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.corr, tmp.fs, name.var, '_', 'corr_ensstd', '.nc'];
            grid.tlong=ncread(fname.grid, 'TLONG');
            grid.tlat=ncread(fname.grid, 'TLAT');
            if (strcmp(cfg.dim_var, '4d'))
                tmp.C=ncread(fname.(name.fign), name.var, [1 1 tmp.lev 1], [inf inf 1 1]);
