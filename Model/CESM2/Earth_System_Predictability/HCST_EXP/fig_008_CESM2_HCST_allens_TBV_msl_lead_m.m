...
dirs.obsroot=['/Volumes/kyy_raid/kimyy/Reanalysis/ERA5/', cfg.obs_var, '/monthly_reg_cam'];
...
%     tmp.assmvar_ano_lm_rm=[cfg.region, 'm_', tmp.varname, '_assm_ano_',  '_l', tmp.lmonth_str, '_rm'];
    ...
    tmp.rm_window=1;
%     tmp.rm_window=[0 11]; % forward (12 month)
...
%         tmp.assm=data.(tmp.assmvar_ano_lm_rm);
    ...
    %% correlation coefficient of NINO3.4 ind, function of lead year (3-mon running mean)
        cfg.figname=[dirs.figdir, filesep, cfg.var, '_', cfg.region, 'm_hcst_obs_70-20m', '_corr_leadm', '.tif'];
...
%         tmp.assm=data.(tmp.assmvar_ano_lm_rm);
    ...
    %% correlation coefficient of NINO3.4 ind, function of lead year (3-mon running mean)
        cfg.figname=[dirs.figdir, filesep, cfg.var, '_', cfg.region1, 'm_hcst_obs_70-20m', '_corr_leadm', '.tif'];
%             bar(0:cfg.proj_year*12-1,data.([cfg.region, 'm_', varn.varname, '_corr']),  'linewidth', 2)
...
%         tmp.assm=data.(tmp.assmvar_ano_lm_rm);
    ...
    %% correlation coefficient of NINO3.4 ind, function of lead year (3-mon running mean)
        cfg.figname=[dirs.figdir, filesep, cfg.var, '_', cfg.region2, 'm_hcst_obs_70-20m', '_corr_leadm', '.tif'];
%             bar(0:cfg.proj_year*12-1,data.([cfg.region, 'm_', varn.varname, '_corr']),  'linewidth', 2)
...
cfg.figname=[dirs.figdir, filesep, 'spaghetti_', cfg.var, '_',cfg.region, 'm_hcst_obs_70-20m', '_corr_leadm', '.tif'];
xlabel('Year'); ylabel('()');
set(gca, 'fontsize', 20)
grid minor
...
cfg.figname=[dirs.figdir, filesep, 'spaghetti_', cfg.var, '_', cfg.region1, 'm_hcst_obs_70-20m', '_corr_leadm', '.tif'];
xlabel('Year'); ylabel('()');
set(gca, 'fontsize', 20)
grid minor
set(gcf, 'PaperPosition', [0, 0, 8, 4]);
...
cfg.figname=[dirs.figdir, filesep, 'spaghetti_', cfg.var, '_', cfg.region2, 'm_hcst_obs_70-20m', '_corr_leadm', '.tif'];
xlabel('Year'); ylabel('()');
set(gca, 'fontsize', 20)
grid minor
set(gcf, 'PaperPosition', [0, 0, 8, 4]);
...
axis tight
hold off
xlabel('Year'); ylabel('Pa');
set(gca, 'fontsize', 20)
grid minor
set(gcf, 'PaperPosition', [0, 0, 8, 4]);
cfg.figname=[dirs.figdir, filesep, 'spaghetti_', cfg.var, '_', cfg.region1, '_hcst_obs_70-20m', '.tif'];
saveas(gcf,cfg.figname,'tif');
RemoveWhiteSpace([], 'file', cfg.figname);
close all;
...
grid minor
set(gcf, 'PaperPosition', [0, 0, 8, 4]);
cfg.figname=[dirs.figdir, filesep, 'spaghetti_', cfg.var, '_', cfg.region2, '_hcst_obs_70-20m', '.tif'];
saveas(gcf,cfg.figname,'tif');
RemoveWhiteSpace([], 'file', cfg.figname);
close all;
