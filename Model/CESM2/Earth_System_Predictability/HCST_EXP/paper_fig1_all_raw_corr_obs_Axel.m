...
corrval_lens2=load(['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/statistics/corr_raw/corr_lens2_', cfg.var, '_v1_v1.mat']);
    ...
    [fig_cfg.c_map_continuous, tmp.err_stat] = Func_0009_get_colormaps('bwr', tmp.dropboxpath);
...
for subi=1:1
    fig_cfg.fig_size = [0,0,14,10]; %% paper size (original)
    fig_cfg.ax_size = [1, 2.5, 12, 6];
    fig_cfg.cb_size = [1, 0.5, 12, 0.3];
    fig_cfg.title_pos = [0.5,1.02];
end



%% SUBPLOT(3,3,3); corr, obs <-> HCST, median(individual)  (LY2~5)
for subi=1:1
...
%     tmp=rmfield(tmp,'p');
    ...
    fig_cfg.fig_name='$$ \hspace{1mm}  M(r_{O,I}^{\tau=2 \textendash 5}) $$';
    ...
    %% map setting
%     subplot(3,3,3);
    ax_m = axesm('MapProjection',fig_cfg.map_proj,'grid','on','fontsize',24, ...
        'fontname','freeserif'); 
...
%     subplot(3,3,2,ax_m);
    ...
    set(ax_m,'Units','inches','Position',fig_cfg.ax_size);
        ...
        'fontsize',30,'fontname','freeserif','interpreter','latex')
...
%         tmp.tmppos(2)=-fig_cfg.ax_size(4)+1.67; % y position correction
    ...
    labelcorr=[8, 9, 10, 11, 12, 13, 14, 3.1, 3.4];
    labelcorr=[8.8, 8.5, 8.2, 8.1, 8.1, 8.1, 8.2, 8.5, 8.8]+1.2;
...
%% caxis & colorbar (ACC)
    ...
    cfg.figname=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/Axel_hq_figures', ...
        filesep, 'Axel_fig_obs','_',cfg.var, '_discrete_with_cbar.tif'];
    print(fig_h, cfg.figname, '-r500', '-dpng');
...
%% caxis & colorbar (ACC)
    caxis(ax_m, fig_cfg.c_lim); 
    colormap(ax_m, fig_cfg.c_map_continuous);
    cb = colorbar(ax_m,'units','inches','Location', 'northoutside', 'position',fig_cfg.cb_size + [0, 1, 0, 0]);
    set(cb,'fontsize',15,'fontname','freeserif','TickDir','both');
    cb_title=title(cb,'$$ r $$','fontsize', 22, 'Position', [880, 0, 0]); % hor, ver, ?
    set(cb_title, 'interpreter', 'latex');
    ...
    if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
    cfg.figname=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/Axel_hq_figures', ...
        filesep, 'Axel_fig_obs','_',cfg.var, '_continuous_with_cbar.tif'];
    print(fig_h, cfg.figname, '-r500', '-dpng');
%     RemoveWhiteSpace([], 'file', cfg.figname);
    close all;
