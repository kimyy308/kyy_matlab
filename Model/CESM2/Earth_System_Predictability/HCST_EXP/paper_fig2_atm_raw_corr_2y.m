...
for subi=1:1
    ...
    recasted_val_hcst=NaN(size(corrval_hcst.assm_hcst_em.ly2.val));
...
%     corrval_hcst.assm_hcst.ly2.val_median

    tmp.C=squeeze(recasted_val_hcst.*grid.lmask);
    tmp.C=tmp.C([end, 1:end],:);

    % significance test
    sig_n=size(corrval_hcst.data.ly2.([cfg.var,'_ym']),3);
...
for subi=1:1
    ...
    for reci=1:length(corrval_assm.grid.valid_ind)
        tmp.A(corrval_assm.grid.valid_ind_i(reci), corrval_assm.grid.valid_ind_j(reci))=corrval_hcst.assm_hcst.ly2.val_median(1,reci);
    end
    ...
    for reci=1:length(corrval_assm.grid.valid_ind)
        tmp.a=corrval_hcst.assm_hcst.ly2.val(:,reci);
...
%         Func_0036_corr_diff_ttest(tmp.A, tmp.B, size(corrval_hcst.data.ly2.([cfg.var,'_ym']),3), size(corrval_lens2.data.([cfg.var,'_ym']),3));
...
%     tmp.C(tmp.p2>0.1)=NaN;
    ...
    fig_cfg.fig_name='$$ (d) \hspace{1mm}  M(r_{A,I}^{\tau=1}) -  M(r_{A,U}^{\tau=1}) $$';
...
%     tmp.a=corrval_hcst.assm_hcst.ly2.val_median;
...
%     aimax=size(corrval_hcst.assm_hcst.ly2.val,1);
%     bimax=size(corrval_lens2.assm_lens2.val,1);
%     for reci=1:length(corrval_assm.grid.valid_ind)
%         for ai=1:aimax
%             for bi=1:bimax
%                 tmp.c((ai-1)*bimax+bi)=corrval_hcst.assm_hcst.ly2.val(ai,reci)-corrval_lens2.assm_lens2.val(bi,reci);
%             end
%         end
...
%     if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
    cfg.figname=['/Users/kimyy/Desktop/backup/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'fig2','_ATM_2y', '.tif'];
