...
dirs.lens2root=['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/LENS2/archive_analysis/atm/', cfg.var];
...
%% all data read for AR1
for iyear=min(cfg.iyears):max(cfg.iyears)
    tmp.iyear_str=num2str(iyear, '%04i');
    for mon=1:12
        tmp.mon_str=num2str(mon, '%02i');
        cfg.obsfnm = [dirs.obsroot, tmp.fs, cfg.region, tmp.fs, 'M_', cfg.region, '_', 'ersst_reg_cesm2.v5.',tmp.iyear_str,tmp.mon_str, '.nc'];
        data.obs_all((iyear-min(cfg.iyears))*12+mon)=ncread(cfg.obsfnm, 'sst');
    end
end
tmp.diff=diff(data.obs_all);
tmp.ano_T=data.obs_all(1:end-1)' - mean(data.obs_all(1:end-1)');
data.lambda=regress(tmp.ano_T, tmp.diff');
% plot(data.obs_all(1:end-1)')
% tmp.ab(1)=data.obs_all(1);
% for i=1:length(data.obs_all)-1
%     tmp.ab(i+1)=tmp.ab(i)+tmp.ab(i)*data.lambda;
% end
% hold off
% scatter(data.obs_all(1:end-1)', tmp.diff')
% plot(tmp.ab(i+1))

%% AR1 (corr)
% [tmp.corr_AC_lag1, tmp.corr_AC_lag1_p] = ...
%  corrcoef(squeeze(data.obs_all(1:end-1)), squeeze(data.obs_all(2:end)));
% data.(['AC_lag1'])=tmp.corr_AC_lag1(1,2);
% data.(['AC_lag1_p'])=tmp.corr_AC_lag1_p(1,2);
...
for iyear=min(cfg.iyears):max(cfg.iyears)
    ...
    tmp.lens2var = ['nino34m_', tmp.varname, '_lens2_',  'i', tmp.iyear_str];
%     tmp.assmvar = ['nino34m_', tmp.varname, '_assm_',  '_i', tmp.iyear_str];
    ...
    %% AR1 (integration)
    tmp.AR1var=['nino34m_', tmp.varname, '_AR1_',  'i', tmp.iyear_str];

    data.(tmp.AR1var)= ...
    Func_0030_AR1_prog(data.(tmp.obsvar)(1)-mean(data.obs_all(1:end-1)'), data.lambda, 59, 0);
...
%         data.(tmp.modelvar_lm)(data.(tmp.modelvar_lm)==0)=NaN; % to remove value on land
        
        % LENS2
        tmp.lens2var_lm=['nino34m_', tmp.varname, '_lens2',  '_l', tmp.lmonth_str];
        tmp.mdata= data.(tmp.lens2var)(lmonth+1);
        data.(tmp.lens2var_lm)(tmp.mind)=tmp.mdata;
        
        %AR1
        tmp.AR1var_lm=['nino34m_', tmp.varname, '_AR1',  '_l', tmp.lmonth_str];         
        tmp.mdata= data.(tmp.AR1var)(lmonth+1);
        data.(tmp.AR1var_lm)(tmp.mind)=tmp.mdata;
...
%     tmp.assmvar_lm=['nino34m_', tmp.varname, '_assm_',  '_l', tmp.lmonth_str];
    ...
    tmp.lens2var_ano_lm=['nino34m_', tmp.varname, '_lens2_ano_',  '_l', tmp.lmonth_str];    
    tmp.obsvar_ano_lm=['nino34m_', tmp.varname, '_obs_ano_',  '_l', tmp.lmonth_str];
    tmp.AR1var_ano_lm=['nino34m_', tmp.varname, '_AR1_ano',  '_l', tmp.lmonth_str];
...
%     tmp.assmvar_ano_lm=['nino34m_', tmp.varname, '_assm_ano_',  '_l', tmp.lmonth_str];
    ...
    tmp.lens2_ltm=mean(data.(tmp.lens2var_lm)(tmp.avgwin_min:tmp.avgwin_max));    
    tmp.obs_ltm=mean(data.(tmp.obsvar_lm)(tmp.avgwin_min:tmp.avgwin_max));
    tmp.AR1_ltm=mean(data.(tmp.AR1var_lm)(tmp.avgwin_min:tmp.avgwin_max));
...
%     tmp.assm_ltm=mean(data.(tmp.assmvar_lm)(tmp.avgwin_min:tmp.avgwin_max));
    ...
    data.(tmp.lens2var_ano_lm)=data.(tmp.lens2var_lm)-tmp.lens2_ltm;    
    data.(tmp.obsvar_ano_lm)=data.(tmp.obsvar_lm)-tmp.obs_ltm;
%     data.(tmp.obsvar_ano_lm)=data.(tmp.obsvar_lm);
    data.(tmp.AR1var_ano_lm)=data.(tmp.AR1var_lm)-tmp.AR1_ltm;
...
%     data.(tmp.assmvar_ano_lm)=data.(tmp.assmvar_lm)-tmp.assm_ltm;
    ...
    tmp.lens2var_ano_lm_rm=['nino34m_', tmp.varname, '_lens2_ano_',  '_l', tmp.lmonth_str, '_rm'];    
    tmp.obsvar_ano_lm_rm=['nino34m_', tmp.varname, '_obs_ano_',  '_l', tmp.lmonth_str, '_rm'];
    tmp.AR1var_ano_lm_rm=['nino34m_', tmp.varname, '_AR1_ano_',  '_l', tmp.lmonth_str, '_rm'];
...
%% movmean for averaged rm_window years -> wrong, so rm_window should set to 1
    ...
    data.(tmp.lens2var_ano_lm_rm)=movmean(data.(tmp.lens2var_ano_lm), tmp.rm_window, 'Endpoints', 'fill');    
    data.(tmp.obsvar_ano_lm_rm)=movmean(data.(tmp.obsvar_ano_lm), tmp.rm_window, 'Endpoints', 'fill');
    data.(tmp.AR1var_ano_lm_rm)=movmean(data.(tmp.AR1var_ano_lm), tmp.rm_window, 'Endpoints', 'fill');
...
%         tmp.assmvar_ano_lm_rm=['nino34m_', tmp.varname, '_assm_ano_',  '_l', tmp.lmonth_str, '_rm'];
        ...
        tmp.lens2=data.(tmp.lens2var_ano_lm_rm);        
...
%         tmp.tind= (cfg.iyears-min(cfg.iyears))*12+1+lmonth;
        ...
        tmp.corrcoef=corrcoef(tmp.lens2(isfinite(tmp.obs)), tmp.obs(isfinite(tmp.obs)));
        data.(['nino34m_', tmp.varname, '_corr_lens2'])(lmonth+1)=tmp.corrcoef(1,2);

        %% corr, AR1
        tmp.AR1var_lm=['nino34m_', tmp.varname, '_AR1',  '_l', tmp.lmonth_str];    
        tmp.AR1=data.(tmp.AR1var_lm);
        tmp.corrcoef=corrcoef(tmp.AR1(isfinite(tmp.obs)), tmp.obs(isfinite(tmp.obs)));
        data.(['nino34m_AR1_', tmp.varname, '_corr'])(lmonth+1)=tmp.corrcoef(1,2);
    ...
    %% correlation coefficient of NINO3.4 ind, function of lead month
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_',str.iy_min, '-', str.iy_max, 'm', '_corr_leadm', '.tif'];
%             bar(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr']),  'linewidth', 2)
%     bar(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_corr'])(1:cfg.proj_year*12),  'linewidth', 2);
    plot(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_corr'])(1:cfg.proj_year*12),  '-o', 'linewidth', 2,  'color','k');
    hold on
    plot(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_corr_lens2'])(1:cfg.proj_year*12), '-o', 'linewidth', 2,  'color','g');
    plot(0:cfg.proj_year*12-1, data.(['nino34m_AR1_', tmp.varname, '_corr'])(1:cfg.proj_year*12), '-o', 'linewidth', 2,  'color',[0.8 0.8 0.8]);
    line([0, cfg.proj_year*12-1], [0 0], 'color', 'k', 'linewidth', 2)
    hold off

    xlabel('Lead month'); ylabel(['ACC,', ' Niño 3.4']);
    set(gca, 'fontsize', 20);
    grid minor
    ylim([-0.4 1]);
    legend({'HCST', 'LENS2', 'AR1'});
    axis tight
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_Elnino_',str.iy_min, '-', str.iy_max, 'm', '_corr_leadm', '.tif'];
    bar(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_corr_Elnino'])(1:cfg.proj_year*12),  'linewidth', 2);

    xlabel('Lead month'); ylabel(['ACC,', 'Nino3.4 (E-init)']);
    set(gca, 'fontsize', 20);
    grid minor
    ylim([-0.4 1]);
    saveas(gcf,cfg.figname,'tif');
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_Lanina_',str.iy_min, '-', str.iy_max, 'm', '_corr_leadm', '.tif'];
    bar(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_corr_Lanina'])(1:cfg.proj_year*12),  'linewidth', 2);

    xlabel('Lead month'); ylabel(['ACC,', 'Nino3.4 (L-init)']);
    set(gca, 'fontsize', 20);
    grid minor
    ylim([-0.4 1]);
    saveas(gcf,cfg.figname,'tif');
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_Neutral_',str.iy_min, '-', str.iy_max, 'm', '_corr_leadm', '.tif'];
    bar(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_corr_Neutral'])(1:cfg.proj_year*12),  'linewidth', 2);
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_All_',str.iy_min, '-', str.iy_max, 'm', '_corr_leadm', '.tif'];
    plot(0:cfg.proj_year*12-1,data.(['nino34m_AR1_', tmp.varname, '_corr'])(1:cfg.proj_year*12),...
        '-o', 'color', [0.8 0.8 0.8], 'linewidth', 2);
    hold on
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr'])(1:cfg.proj_year*12),...
        '-o', 'color', 'k', 'linewidth', 2);
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr_Elnino'])(1:cfg.proj_year*12),...
        '-o', 'color', 'r', 'linewidth', 2);
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr_Lanina'])(1:cfg.proj_year*12),...
        '-o', 'color', 'b', 'linewidth', 2);
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr_Neutral'])(1:cfg.proj_year*12),...
        '-o', 'color', 'g', 'linewidth', 2);
    


    xlabel('Lead month'); ylabel(['ACC,', 'Nino3.4']);
    set(gca, 'fontsize', 20);
    grid minor
    ylim([-0.4 1]);
    legend({'All(AR1)', 'All', 'E-start', 'L-start', 'N-start'})
    saveas(gcf,cfg.figname,'tif');
    RemoveWhiteSpace([], 'file', cfg.figname);
    close all;
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_',str.iy_min, '-', str.iy_max, 'm', '_nRMSE_leadm', '.tif'];
    bar(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_RMSE'])(1:cfg.proj_year*12),  'linewidth', 2);
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_',str.iy_min, '-', str.iy_max, 'm', '_nRMSE_Elnino_leadm', '.tif'];
    bar(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_RMSE_Elnino'])(1:cfg.proj_year*12),  'linewidth', 2);
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_',str.iy_min, '-', str.iy_max, 'm', '_nRMSE_Lanina_leadm', '.tif'];
    bar(0:cfg.proj_year*12-1, data.(['nino34m_', tmp.varname, '_RMSE_Lanina'])(1:cfg.proj_year*12),  'linewidth', 2);
    ...
    cfg.figname=[dirs.figdir, filesep, 'nino34m_v2_hcst_obs_',str.iy_min, '-', str.iy_max, 'm', '_nRMSE_All_leadm', '.tif'];
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_RMSE'])(1:cfg.proj_year*12),  ...
        '-o', 'color', 'k', 'linewidth', 2);
    hold on
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_RMSE_Elnino'])(1:cfg.proj_year*12),  ...
        '-o', 'color', 'r', 'linewidth', 2);
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_RMSE_Lanina'])(1:cfg.proj_year*12),  ...
        '-o', 'color', 'b', 'linewidth', 2);
    plot(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_RMSE_Neutral'])(1:cfg.proj_year*12),  ...
        '-o', 'color', 'g', 'linewidth', 2);
    xlabel('Lead month'); ylabel(['nRMSE.,', 'Nino3.4']);
    set(gca, 'fontsize', 20);
    grid minor
...
% end
    ...
    cfg.figname=[dirs.figdir, filesep, 'SMYLE_nino34m_v2_hcst_obs_',str.iy_min, '-', str.iy_max, 'm', '_corr_leadm', '.tif'];
%             bar(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr']),  'linewidth', 2)
    ...
    cfg.figname=[dirs.figdir, filesep, 'SMYLE_nino34m_v2_hcst_obs_',str.iy_min, '-', str.iy_max, 'm', '_nRMSE_leadm', '.tif'];
%             bar(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr']),  'linewidth', 2)
...
tmp.plottime=min(cfg.iyears)+1/24:1/12:max(cfg.iyears)-1/24+12*cfg.proj_year*1/12;
plot(tmp.plottime, tmp.obs, 'k', 'linewidth',2)
hold off
axis tight
cfg.figname=[dirs.figdir, filesep, 'spaghetti_nino34m_v2_hcst_obs_oneline', '_', str.iy_min, '-', str.iy_max, '_leadm', '.tif'];
...
grid minor
set(gcf, 'PaperPosition', [0, 0, 8, 4]);
cfg.figname=[dirs.figdir, filesep, 'spaghetti_v2_', 'SST_hcst_obs_', str.iy_min, '-', str.iy_max,'_proj_',cfg.proj_year,  'y.tif'];
saveas(gcf,cfg.figname,'tif');
RemoveWhiteSpace([], 'file', cfg.figname);
close all;
...
%% spaghetti plot (SST AR1)
for iyear=min(cfg.iyears):max(cfg.iyears)
    tmp.iyear_str=num2str(iyear);
    tmp.plottime=iyear+1/24 : 1/12 : iyear+cfg.proj_year - 1/24;
    tmp.modelvar=['nino34m_', tmp.varname, '_AR1_',  'i', tmp.iyear_str];
    tmp.obsvar=['nino34m_', tmp.varname, '_obs_',  'i', tmp.iyear_str];
    
    plot(tmp.plottime,data.(tmp.obsvar)-mean(data.obs_all(1:end-1)'),'k', 'linewidth',1);
    hold on
...
grid minor
set(gcf, 'PaperPosition', [0, 0, 8, 4]);
cfg.figname=[dirs.figdir, filesep, 'spaghetti_v2_', 'SST_AR1_obs_', str.iy_min, '-', str.iy_max,'_proj_',cfg.proj_year,  'y.tif'];
legend({'obs','damp'})
saveas(gcf,cfg.figname,'tif');
RemoveWhiteSpace([], 'file', cfg.figname);
close all;
...
for lmonth=0:cfg.proj_year*12-1
    tmp.lmonth_str=num2str(lmonth, '%03i');
%     tmp.modelvar_ano_lm_rm=['nino34m_', tmp.varname, '_model_ano_',  '_l', tmp.lmonth_str, '_rm'];
    tmp.modelvar_ano_lm_rm=['nino34m_', tmp.varname, '_AR1_ano',  '_l', tmp.lmonth_str];
    tmp.plottime=min(cfg.iyears)+1/24+lmonth*1/12 : 1 : max(cfg.iyears)-1/24+lmonth*1/12;
    tmp.md=data.(tmp.modelvar_ano_lm_rm);
    tmp.plottime=cfg.iyears+1/24+lmonth*(1/12);
%     plot(tmp.plottime, tmp.md, 'b');
...
% 
%         xlabel('Lead month'); ylabel(['ACC,', 'Nino3.4' , ', assm-ba']);
%         set(gca, 'fontsize', 20)
%         grid minor
%         ylim([-0.4 1])
%         saveas(gcf,cfg.figname,'tif');
