...
%     data.time_y_extended=[config.iyears, max(config.iyears)+1:max(config.iyears)+config.proj_year-1];
 ...
 %% normal time series and correlation coefficient (combined plot, hcst&obs)
    for varind=1:length(config.varnames)
        ...
        for lyear=0:3
            tmp.lyear_str=num2str(lyear, '%02i');
            config.figname=[dirs.figdir, filesep, 'nino34m_hcst_obs_', tmp.varname, '_l', tmp.lyear_str, '.tif'];
...
%             end
            ...
            %% bias plot
            config.figname=[dirs.figdir, filesep, 'nino34m_hcst_obs_bias_', tmp.varname, '_l', tmp.lyear_str, '.tif'];
            tmp.md=data.(['nino34m_', tmp.varname, '_model_', tmp.obsname_simple, '_l', tmp.lyear_str]) ...
                - data.(['nino34m_', tmp.varname, '_model_ltm_', tmp.obsname_simple, '_l', tmp.lyear_str]);
            tmp.obs=data.(['nino34m_', tmp.varname, '_obs_', tmp.obsname_simple]) ...
                - data.(['nino34m_', tmp.varname, '_obs_ltm_', tmp.obsname_simple]);
            tmp.ind_finite=(isfinite(tmp.md(1:length(data.time_leap))));
            tmp.bias=tmp.md(tmp.ind_finite)-tmp.obs(tmp.ind_finite);
            
%             plot(data.time_leap_extended(tmp.ind_finite),tmp.bias,  '-o', 'color', 'r', 'linewidth', 2)
            bar(data.time_leap_extended(tmp.ind_finite),tmp.bias, 1, 'r')
            xlim([min(data.time_leap_extended), max(data.time_leap_extended)])
            datetick('x', 'yymmm', 'keeplimits')
%             switch tmp.varname
%                 case 'temp'
...
%             end
            ...
            hold on;
            xlabel('Time'); 
            ylabel([tmp.varname, '-bias-l', tmp.lyear_str]); 
            legend(['HCST-bias-l', tmp.lyear_str], 'Location', 'NorthWest');
            set(gca, 'fontsize', 20)
            grid minor
            hold off
...
%             close all;
    ...
    for varind=1:length(config.varnames)
        ...
        for lyear=0:3
            tmp.lyear_str=num2str(lyear, '%02i');
            config.figname=[dirs.figdir, filesep, 'nino34m_hcst_assm_', tmp.varname, '_l', tmp.lyear_str, '.tif'];
            tmp.md=data.(['nino34m_', tmp.varname, '_model_', tmp.obsname_simple, '_l', tmp.lyear_str]) ...
                - data.(['nino34m_', tmp.varname, '_model_ltm_', tmp.obsname_simple, '_l', tmp.lyear_str]);
...
%             end
            ...
            %% bias plot
            config.figname=[dirs.figdir, filesep, 'nino34m_hcst_assm_bias_', tmp.varname, '_l', tmp.lyear_str, '.tif'];
            tmp.md=data.(['nino34m_', tmp.varname, '_model_', tmp.obsname_simple, '_l', tmp.lyear_str]) ...
                - data.(['nino34m_', tmp.varname, '_model_ltm_', tmp.obsname_simple, '_l', tmp.lyear_str]);
            tmp.assm=data.(['nino34m_', tmp.varname, '_assm_', tmp.obsname_simple]) ...
                - data.(['nino34m_', tmp.varname, '_assm_ltm_', tmp.obsname_simple]);
            tmp.ind_finite=(isfinite(tmp.md(1:length(data.time_leap))));
            tmp.bias=tmp.md(tmp.ind_finite)-tmp.assm(tmp.ind_finite);
