% %  Created 16-May-2025 by Yong-Yub Kim
...
%     fig_cfg.cb_size = [3, 1, 12, 0.3];
    ...
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_HCST_em.nc'];
    autocorr=ncread(autocorrfile, cfg.var);
    sig_n_star=sig_n.*(1-autocorr)./(1+autocorr);
...
%     corrval_hcst.assm_hcst.ly1.val_median
    ...
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_HCST.nc'];
    autocorr_i=ncread(autocorrfile, cfg.var);
    autocorr=NaN(size(recasted_idx_hcst));
    for loni=size(autocorr_i,1)
        for lati=size(autocorr_i,2)
            autocorr(loni,lati)=autocorr_i(loni,lati,recasted_idx_hcst(loni,lati));
        end
    end
...
for subi=1:1
    ...
    recasted_val_hcst=NaN(size(corrval_hcst.assm_hcst_em.ly1.val));
    for reci=1:length(corrval_assm.grid.valid_ind)
        recasted_val_hcst(corrval_assm.grid.valid_ind_i(reci), corrval_assm.grid.valid_ind_j(reci))=...
            corrval_hcst.assm_hcst.ly1.val_median(1,reci);
    end
...
for subi=1:1
    ...
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_HCST_ds_4yr.nc'];
    autocorr_i=ncread(autocorrfile, cfg.var);
    autocorr=NaN(size(recasted_idx_hcst));
    for loni=size(autocorr_i,1)
        for lati=size(autocorr_i,2)
...
for subi=1:1
    ...
    % significance test
    sig_n=size(corrval_hcst.data.ly1.([cfg.var,'_ym']),3); % normal n
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_LE_em.nc'];
    autocorr=ncread(autocorrfile, cfg.var);
    sig_n_star=sig_n.*(1-autocorr)./(1+autocorr);
    sig_n_star=sig_n_star([end, 1:end],:);
    sig_n_star(sig_n_star<=2)=NaN;
...
%     corrval_hcst.assm_hcst.ly1.val_median
    ...
    for reci=1:length(corrval_assm.grid.valid_ind)
        tmp.a=corrval_lens2.assm_lens2_em.val_sr(:,reci);
        [~, tmp.a_order] = sort(tmp.a,'ascend'); 
        tmp.a_idx   = tmp.a_order(length(tmp.a)/2);
        tmp.a_idx_hc = mod(tmp.a_idx,20); tmp.a_idx_hc(tmp.a_idx_hc==0)=20;
    ...
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_LE.nc'];
    autocorr_i=ncread(autocorrfile, cfg.var);
    autocorr=NaN(size(recasted_idx_lens2));
    for loni=size(autocorr_i,1)
        for lati=size(autocorr_i,2)
            autocorr(loni,lati)=autocorr_i(loni,lati,recasted_idx_lens2(loni,lati));
        end
    end
...
%     corrval_hcst.assm_hcst.ly1.val_median
    ...
    for reci=1:length(corrval_assm.grid.valid_ind)
        tmp.a=corrval_lens2.assm_lens2.val(:,reci);
        [~, tmp.a_order] = sort(tmp.a,'ascend'); 
        tmp.a_idx   = tmp.a_order(length(tmp.a)/2);
        tmp.a_idx_hc = mod(tmp.a_idx,20); tmp.a_idx_hc(tmp.a_idx_hc==0)=20;
        recasted_idx_lens2(corrval_assm.grid.valid_ind_i(reci), corrval_assm.grid.valid_ind_j(reci)) = tmp.a_idx_hc;
...
for subi=1:1
    ...
    for reci=1:length(corrval_assm.grid.valid_ind)
        tmp.a=corrval_lens2.assm_lens2_4ym.val(:,reci);
        [~, tmp.a_order] = sort(tmp.a,'ascend'); 
        tmp.a_idx   = tmp.a_order(length(tmp.a)/2);
        tmp.a_idx_hc = mod(tmp.a_idx,20); tmp.a_idx_hc(tmp.a_idx_hc==0)=20;
        recasted_idx_lens2(corrval_assm.grid.valid_ind_i(reci), corrval_assm.grid.valid_ind_j(reci)) = tmp.a_idx_hc;
    ...
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_LE_ds_4yr.nc'];
    autocorr_i=ncread(autocorrfile, cfg.var);
    autocorr=NaN(size(recasted_idx_lens2));
    for loni=size(autocorr_i,1)
        for lati=size(autocorr_i,2)
...
for subi=1:1
    ...
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_HCST_em.nc'];
    autocorr=ncread(autocorrfile, cfg.var);
    sig_n_star1=sig_n.*(1-autocorr)./(1+autocorr);
    sig_n_star1=sig_n_star1([end, 1:end],:);
    sig_n_star1(sig_n_star1<=2)=NaN;

    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_LE_em.nc'];
    autocorr=ncread(autocorrfile, cfg.var);
    sig_n_star2=sig_n.*(1-autocorr)./(1+autocorr);
...
for subi=1:1
    ...
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_HCST.nc'];
    autocorr1=ncread(autocorrfile, cfg.var);
    autocorrfile=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_LE.nc'];
    autocorr2=ncread(autocorrfile, cfg.var);
    ...
    for reci=1:length(corrval_assm.grid.valid_ind)
        ...
        sig_n_star1=sig_n.*(1-autocorr1_tmp)./(1+autocorr1_tmp);
        sig_n_star1(sig_n_star1<=2)=NaN;
        ...
        sig_n_star2=sig_n.*(1-autocorr2_tmp)./(1+autocorr2_tmp);
        sig_n_star2(sig_n_star2<=2)=NaN;
...
for subi=1:1
    ...
    for reci=1:length(corrval_assm.grid.valid_ind)
        ...
        tmp.a_idx_hc = mod(tmp.a_idx,20); tmp.a_idx_hc(tmp.a_idx_hc==0)=20;
        autocorr1_tmp=autocorr1(corrval_assm.grid.valid_ind_i(reci), corrval_assm.grid.valid_ind_j(reci), tmp.a_idx_hc);
        sig_n_star1=(sig_n-3).*(1-autocorr1_tmp)./(1+autocorr1_tmp);
        sig_n_star1(sig_n_star1<=2)=NaN;
        ...
        tmp.b_idx_le = mod(tmp.b_idx,50); tmp.b_idx_le(tmp.b_idx_le==0)=50;
        autocorr2_tmp=autocorr2(corrval_assm.grid.valid_ind_i(reci), corrval_assm.grid.valid_ind_j(reci), tmp.b_idx_le);
        sig_n_star2=(sig_n-3).*(1-autocorr2_tmp)./(1+autocorr2_tmp);
        sig_n_star2(sig_n_star2<=2)=NaN;
...
%     set(MyBox,'String','Burned Area')
    ...
    set(text_column2,'String', 'EM-IM-based (Lead Year 1)')
    ...
    set(text_column3,'String', 'IM-based (Lead Year 1)')
    ...
    set(text_column4,'String', 'IM-based (Lead Year 2–5)')
    ...
    cfg.figname=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'eDOF_Siegert_title_fig1','_v2_',cfg.var, '.tif'];
    ...
    cfg.figname2=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'eDOF_Siegert_title_fig1','_v2_',cfg.var, '.eps'];
