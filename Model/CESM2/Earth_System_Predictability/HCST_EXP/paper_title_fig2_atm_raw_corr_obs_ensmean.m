...
%     fig_cfg.fig_size = [0,0,13,14]; %% paper size (original)
    ...
    tmp.C=squeeze(corrval_hcst.obs_hcst_em.ly1.val.*grid.lmask);
...
% %                 corrval_assm.data_obs.([cfg.var,'_ym'])(loni,lati,:),'alpha', 0.1, 'Rows', 'complete');
% %             [tmp.le_corr,tmp.le_p,tmp.le_rl,tmp.le_ru] = ...
% %                 corrcoef(corrval_assm.data_obs.([cfg.var,'_ym'])(loni,lati,:), ...
% %                 corrval_lens2.data.([cfg.var,'_ym'])(loni,lati,:), 'alpha', 0.1, 'Rows', 'complete');
...
% % 
% %             tmp.tt(loni, lati)=ttest2(tmp.hc_rnd,tmp.le_rnd,'alpha', fig_cfg.p_lim); % same = 0 diff =1
% % %             tmp.tt(loni, lati)=ttest2(tmp.hc_rnd,tmp.le_rnd,'alpha', 0.05); % same = 0 diff =1
% %             
% %         end
% %     end
...
for subi=1:1
    ...
    fig_cfg.fig_name='$$ (c) \hspace{1mm}  r_{O,E(I)}^{\tau=1} $$';
...
% %     end
    ...
    fig_cfg.fig_name='$$ (d) \hspace{1mm}  r_{O,E(I)}^{\tau=1} -  r_{O,E(U)}^{\tau=1} $$';
...
%     set(text_row1,'String','TWS')
    ...
    cfg.figname=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'title_fig2','_ATM_obs_ensmean', '.tif'];
    ...
    cfg.figname2=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'title_fig2','_ATM_obs_ensmean', '.eps'];
