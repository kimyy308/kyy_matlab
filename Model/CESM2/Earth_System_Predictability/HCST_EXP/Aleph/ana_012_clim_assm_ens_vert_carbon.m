...
for vari=1:length(cfg.vars)
    ...
    if grid.regions(1)==grid.regions(2)
        tmp.gg=round((grid.id_w+grid.id_e)/2);
        grid.id_w=tmp.gg;
        grid.id_e=tmp.gg;
    end
    if grid.regions(3)==grid.regions(4)
        tmp.gg=round((grid.id_s+grid.id_n)/2);
        grid.id_s=tmp.gg;
        grid.id_n=tmp.gg;
    end
...
%% variables initialization
        data.([tmp.varname, '_lens2'])=NaN(grid.cut_nlon, grid.cut_nlat, cfg.vlayer_cnt, cfg.len_t_y);
        data.([tmp.varname, '_obs'])=NaN(grid.cut_nlon, grid.cut_nlat, cfg.vlayer_cnt, cfg.len_t_y);
        data.([tmp.varname, '_assm'])=NaN(grid.cut_nlon, grid.cut_nlat, cfg.vlayer_cnt, cfg.len_t_y);
...
%                                 tmp.dd=tmp.dd./86400./eomday(tmp.fy,mi)./grid.area;
                        ...
                        tmp.ydata_obs(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt, mi) = tmp.dd;
                        netcdf.close(ncid);
                        
                    else
                        tmp.ydata_obs(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt, mi) = NaN;                    
                    end
                end
                tmp.ymean_obs(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt)=mean(tmp.ydata_obs,4);

            else
                tmp.ymean_obs(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt) = NaN;          
            end
...
%                      ddd=mean(tmp.dd,3,'omitnan'); % depth mean
                     tmp.ydata_assm(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt) = tmp.dd; %depth averaged value             
                else
                    tmp.ydata_assm(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt) = netcdf.getVar(ncid,tmpvarid, ...
...
%% save variables as structure
            ...
            data.([tmp.varname, '_obs'])(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt, iyear-min(cfg.iyears)+1)= tmp.ymean_obs;
            data.([tmp.varname, '_assm'])(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt, iyear-min(cfg.iyears)+1)= tmp.ymean_assm;
        ...
        %                 disp(tmp.fname_period);
                ...
                if length(tmp.dimids)>3
                     ...
                     tmp.dd(abs(tmp.dd)>1e30)=NaN;
%                      ddd=mean(tmp.dd,3,'omitnan'); % depth mean
                     tmp.ydata_lens2(1:grid.cut_nlon,1:grid.cut_nlat, 1:cfg.vlayer_cnt, (ty-1)*12+1:(ty-1)*12+ind_count) = tmp.dd;
                else
...
%                 netcdf.close(ncid);
            ...
            tmp.ydata2_lens2=reshape(tmp.ydata_lens2, [grid.cut_nlon, grid.cut_nlat, cfg.vlayer_cnt, 12, cfg.len_t_y]);
            tmp.ymean_lens2=squeeze(mean(tmp.ydata2_lens2,4));
            data.([tmp.varname, '_lens2'])(1:grid.cut_nlon,1:grid.cut_nlat,1:cfg.vlayer_cnt,:)= tmp.ymean_lens2;
...
%         end
   ...
   %% get RMSE with obs (drift-removed)
        data.([tmp.varname, '_lens2_rmse_obs']) = ...
            sqrt( mean( ...
            (data.([tmp.varname, '_lens2_err_obs']) ...
            - mean(data.([tmp.varname, '_lens2_err_obs']), 4, 'omitnan')).^2, ...
            4, 'omitnan') );
        data.([tmp.varname, '_assm_rmse_obs']) = ...
            sqrt( mean( ...
            (data.([tmp.varname, '_assm_err_obs']) ...
            - mean(data.([tmp.varname, '_assm_err_obs']), 4, 'omitnan')).^2, ...
            4, 'omitnan') );
    
    %% get hindcast climatology as the function of lead year
    data.([tmp.varname, '_assm_clim']) = mean(data.([tmp.varname, '_assm'])(:,:,:,:),4, 'omitnan'); %1964 ~ 2020
    data.([tmp.varname, '_lens2_clim']) = mean(data.([tmp.varname, '_lens2'])(:,:,:,:),4, 'omitnan'); %1964 ~ 2020
    data.([tmp.varname, '_obs_clim']) = mean(data.([tmp.varname, '_obs'])(:,:,:,:),4, 'omitnan'); %1964 ~ 2020
...
% % %     fprintf('%7.1f sec\n', toc(lap_time) );
        ...
        for loni=1:grid.cut_nlon
            for lati=1:grid.cut_nlat
                for zi=1:cfg.vlayer_cnt
                     if (isnan(data.([tmp.varname, '_assm'])(loni,lati,zi,1))~=1 && sum(data.([tmp.varname, '_lens2'])(loni,lati,zi,:), 'omitnan')~=0)
        
                     %% corr lens2 ~ assm
                         tmp.lens2 = squeeze(data.([tmp.varname, '_lens2'])(loni,lati,zi,:));
                         tmp.data_assm = squeeze(data.([tmp.varname, '_assm'])(loni,lati,zi,:));
                         [tmp.corr, tmp.corr_p]=corrcoef(tmp.lens2(isfinite(tmp.data_assm)), tmp.data_assm(isfinite(tmp.data_assm)));
    
                         data.([tmp.varname, '_corr_assm_lens2'])(loni,lati,zi)=tmp.corr(1,2);
                         data.([tmp.varname, '_corr_assm_lens2_p'])(loni,lati,zi)=tmp.corr_p(1,2);
    
                     %% corr obs ~ assm
                         tmp.data_assm = squeeze(data.([tmp.varname, '_assm'])(loni,lati,zi,:));
                         tmp.data_obs = squeeze(data.([tmp.varname, '_obs'])(loni,lati,zi,:));
                         ...
                         data.([tmp.varname, '_corr_obs_assm'])(loni,lati,zi)=tmp.corr(1,2);
                         data.([tmp.varname, '_corr_obs_assm_p'])(loni,lati,zi)=tmp.corr_p(1,2);
    
                     %% corr obs ~ lens2
                            tmp.data_lens2 = squeeze(data.([tmp.varname, '_lens2'])(loni,lati,zi,:));
                         ...
                         data.([tmp.varname, '_corr_obs_lens2'])(loni,lati,zi)=tmp.corr(1,2);
                         data.([tmp.varname, '_corr_obs_lens2_p'])(loni,lati,zi)=tmp.corr_p(1,2);          
        
                     else
                         %% obs ~ ASSM (NaN)
                         data.([tmp.varname, '_corr_obs_assm'])(loni,lati,zi)=NaN;
                         data.([tmp.varname, '_corr_obs_assm_p'])(loni,lati,zi)=NaN;
                         
                         %% obs ~ LENS2 (NaN)
                         data.([tmp.varname, '_corr_obs_lens2'])(loni,lati,zi)=NaN;
                         data.([tmp.varname, '_corr_obs_lens2_p'])(loni,lati,zi)=NaN;
                     end
                end
            end
        end
