% %  Created 04-Oct-2023 by Yong-Yub Kim
...
%% HCST mean
            ...
            tmp.varname, '_', cfg.gnm, '.hcst.', cfg.casename, cfg.obs_fname_module, ...
            'ly_', num2str(cfg.ly_s), '_', num2str(cfg.ly_e), '.nc'];
...
%% HCST ensstd (spread)
            ...
            cfg.mod_fnm=[dirs.datadir, tmp.fs, 'ens_all_i',tmp.iyear_str, tmp.fs, ...
                tmp.varname, '_', cfg.gnm, '.hcst.', cfg.casename_std, cfg.obs_fname_module, ...
            'ly_', num2str(cfg.ly_s), '_', num2str(cfg.ly_e), '.nc'];
...
%             if tmp.fy <= cfg.max_iy
                cfg.lens2_fnm=[dirs.lens2dir, tmp.fs, ...
                    tmp.varname, '_y_ensmean_', tmp.iyear_str, '_ly_', num2str(cfg.ly_s), '_', num2str(cfg.ly_e), '.nc'];
...
%             if tmp.fy <= cfg.max_iy
                cfg.lens2_fnm=[dirs.lens2dir, tmp.fs, ...
                    tmp.varname, '_y_ensstd_', tmp.iyear_str, '_ly_', num2str(cfg.ly_s), '_', num2str(cfg.ly_e), '.nc'];
...
%% OBS
        ...
        for lyear_f=cfg.ly_s:cfg.ly_e
%             tmp.fy=iyear+lyear-1;
%             tmp.fy_str=num2str(tmp.fy, '%04i');
%             if tmp.fy <= cfg.max_iy
            tmp.fy=iyear+lyear_f-1;
            tmp.fy_str=num2str(tmp.fy, '%04i');
                for mi=1:12
                    ...
                    elseif strcmp(cfg.obs_name, 'GPCC')==1 || strcmp(cfg.obs_name, 'ORNL_DAAC')==1 || strcmp(cfg.obs_name, 'VODCA2GPP')==1
...
%                         cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'netcdf_regrid/s_vgpm', tmp.fs, cfg.obs_fname_mid, tmp.fy_str,tmp.m_str, '.nc'];
                    ...
                    if (exist(cfg.obs_fnm)~=0 && (iyear+cfg.ly_e-1<=max(cfg.obs_iyears2) && iyear+cfg.ly_s-1>=min(cfg.obs_iyears2)))
                        ncid=netcdf.open(cfg.obs_fnm, 'NOWRITE');
                        tmpvarid=netcdf.inqVarID(ncid,cfg.obs_varname);
                        tmp.dd =  netcdf.getVar(ncid,tmpvarid);
                        tmp.dd=double(tmp.dd);
                        if (strcmp(cfg.obs_name, 'ERA5') || strcmp(tmp.varname, 'TLAI')==1)
                        ...
                        elseif (strcmp(tmp.varname, 'GPP')==1)
                            tmp.dd=tmp.dd./86400;
                        elseif strcmp(tmp.varname, 'FAREA_BURNED')
%                                 tmp.dd=tmp.dd./86400./eomday(tmp.fy,mi)./grid.area;
                                tmp.dd=tmp.dd./86400./grid.area;
%                                 tmp.miss_val=tmp.dd(1,1);
%                                 tmp.dd(tmp.dd==tmp.miss_val)=NaN;
                        elseif strcmp(tmp.varname, 'COL_FIRE_CLOSS')
                            tmp.dd=tmp.dd./86400./eomday(tmp.fy,mi);
                        end
                        tmp.ydata_obs(1:grid.nlon,1:grid.nlat,(lyear_f-cfg.ly_s)*12+mi) = tmp.dd;
                        netcdf.close(ncid);
                        
                    else
                        tmp.ydata_obs(1:grid.nlon,1:grid.nlat,(lyear_f-cfg.ly_s)*12+mi) = NaN;                    
                    end
                end
...
%% ASSM
%             if tmp.fy <= cfg.max_iy
                cfg.assm_fnm=[dirs.assmdir, tmp.fs, tmp.varname, '_ensmean_',  tmp.iyear_str, '_ly_', num2str(cfg.ly_s), '_', num2str(cfg.ly_e), '.nc'];
...
%% ASSM std
%             if tmp.fy <= cfg.max_iy
                cfg.assm_fnm=[dirs.assmdir, tmp.fs, tmp.varname, '_ensstd_', tmp.iyear_str, '_ly_', num2str(cfg.ly_s), '_', num2str(cfg.ly_e), '.nc'];
...
%% save variables as structure
        ...
        for loni=1:size(data.([tmp.varname, '_obs']),1)
            for lati=1:size(data.([tmp.varname, '_obs']),2)
                if sum(isfinite(data.([tmp.varname, '_obs'])(loni,lati,:)))<floor((length(cfg.obs_iyears2)-(cfg.ly_e-cfg.ly_s)).*0.8)
...
%        if lyear==0
            cfg.clim_tlen_AR1 = 1:cfg.len_t_y-1; % for AR1 integration, it should start from 1960 to integrate 1965, LY5 
            cfg.clim_tlen_AR1 = cfg.clim_tlen-1; % for AR1 integration, it should start from 1960 to integrate 1965, LY5 
...
%         end
   ...
   %% AR1 parameter estimation, getting prognostic value (assm based), should be estimated from anomaly
        data2.([tmp.varname, '_AR1', '_l', tmp.lyear_str])=NaN(grid.nlon, grid.nlat, length(cfg.clim_tlen_AR1));  %% AR1 initialization
        data2.([tmp.varname, '_obs_AR1', '_l', tmp.lyear_str])=NaN(grid.nlon, grid.nlat, length(cfg.clim_tlen_AR1));  %% AR1 initialization
...
%                         tmp.init_finite=data2_l0_AR1.assm_ano_full(loni, lati, cfg.clim_ys-1960-lyear:cfg.clim_ye-1960-lyear);
                        tmp.init_finite=data2_l0_AR1.assm_ano_full(loni, lati, cfg.clim_ys-1965+cfg.ly_e:cfg.clim_ye-1965+cfg.ly_e);
...
%                     prog=tmp.assm_ano(loni,lati,1:end-1); %initialize  
                      prog=data2_l0_AR1.assm_ano_full(loni, lati, cfg.clim_ys-1965+cfg.ly_e:cfg.clim_ye-1965+cfg.ly_e); %initialize
...
%                         tmp.init_finite=tmp.obs_ano(loni,lati,1:end-1);
                        tmp.init_finite=data2_l0_AR1.obs_ano_full(loni, lati, cfg.clim_ys-1965+cfg.ly_e:cfg.clim_ye-1965+cfg.ly_e);
                        tmp.init_finite=tmp.init_finite(isfinite(tmp.init_finite));
                        tmp.init_finite= tmp.init_finite-mean(tmp.init_finite);
                        [tmp.coef, tmp.noise] = aryule(squeeze(tmp.init_finite), 1);
...
%                     prog=tmp.obs_ano(loni,lati,1:end-1);        
                    prog=data2_l0_AR1.obs_ano_full(loni, lati, cfg.clim_ys-1965+cfg.ly_e:cfg.clim_ye-1965+cfg.ly_e); 
                    for i=1:lyear+1
