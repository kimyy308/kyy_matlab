...
%     cfg.season = {'MAM2', 'JJA2', 'SON2'};
    ...
    clear tmp.ydata tmp.ydata_lens2 tmp.ydata_obs tmp.ydata_obs2 tmp.ydata_assm
    %% lead season loop
...
%% read variables
        for iyear=min(cfg.iyears):max(cfg.iyears)
            ...
            tmp.iyear_str2=num2str(iyear+1, '%04i');
...
%% HCST mean
            ...
            if strcmp(tmp.season(1:3), 'DJF') || strcmp(tmp.season(4), '2')
                cfg.mod_fnm=[dirs.datadir, tmp.fs, 'ens_all_i',tmp.iyear_str, tmp.fs, tmp.season, tmp.fs, ...
                tmp.varname, '_', cfg.gnm, '.hcst.', cfg.casename, cfg.obs_fname_module, tmp.iyear_str2, '_', tmp.season, '.nc'];
            end
...
%% HCST ensstd (spread)
            ...
            if strcmp(tmp.season(1:3), 'DJF') || strcmp(tmp.season(4), '2')
                cfg.mod_fnm=[dirs.datadir, tmp.fs, 'ens_all_i',tmp.iyear_str, tmp.fs, tmp.season, tmp.fs, ...
                tmp.varname, '_', cfg.gnm, '.hcst.', cfg.casename_std, cfg.obs_fname_module,tmp.iyear_str2,'_',tmp.season '.nc'];
            end
            ncid=netcdf.open(cfg.mod_fnm, 'NOWRITE');
             tmpvarid=netcdf.inqVarID(ncid,tmp.fvarname);
            [tmp.fvarname,tmp.xtype,tmp.dimids,tmp.natts]= netcdf.inqVar(ncid,tmpvarid);
...
%                     end
                    ...
                    if exist(cfg.obs_fnm_sst)~=0
                        ...
                        tmp.dd=double(tmp.dd);
                         tmp.dd(abs(tmp.dd)>1e30)=NaN;
                        tmp.ydata_obs2(1:grid.nlon,1:grid.nlat,mi) = tmp.dd;
                        netcdf.close(ncid);
                    else
                        tmp.ydata_obs2(1:grid.nlon,1:grid.nlat,mi) = NaN;                    
                    end
                end
...
%             if iyear <= cfg.max_iy
                ...
                if strcmp(tmp.season(1:3), 'DJF')
                    cfg.assm_fnm=[dirs.assmdir, tmp.fs, tmp.season, tmp.fs, tmp.varname, '_ensmean_', tmp.iyear_str2, '_', tmp.season, '.nc'];
                end
...
%             if iyear <= cfg.max_iy
                ...
                if strcmp(tmp.season(1:3), 'DJF')
                    cfg.assm_fnm=[dirs.assmdir, tmp.fs, tmp.season, tmp.fs, tmp.varname, '_ensstd_', tmp.iyear_str2, '_', tmp.season, '.nc'];                    
                end
...
%                          && sum(data2.([tmp.varname, '_model_ano_', tmp.season])(loni,lati,:),'omitnan')~=0)
                if (~isnan(data2.([tmp.varname, '_assm_ano', '_', tmp.season])(loni,lati,1)))
                 ...
                 %% corr obs (anomaly based)
                     tmp.data2_obs = data2.([tmp.varname, '_obs_ano','_', tmp.season])(loni,lati,:);
                     ...
                     [tmp.corr_det, tmp.corr_det_p]=corrcoef(tmp.data2_det(isfinite(tmp.data2_assm_det)), tmp.data2_obs_det(isfinite(tmp.data2_assm_det)));
