...
tmp.hostname=tmp.hostname(1:end-1);switch tmp.hostname
...
% cfg.var='TS'; %SST PRECT PSL TS SSH sumChl
 ...
 cfg.vars = {'photoC_TOT_zint_100m', 'sumChl', ...
...
tmp.lyear_str=[num2str(cfg.ly_s-1,'%02i'), '_', num2str(cfg.ly_e-1,'%02i')];
    ...
    for stai=1:length(sta_lonlat)
        ...
        plot(cfg.iyears+lyear,squeeze(data_all.([tmp.varname, '_model'])(grid.id_w,grid.id_s,:)), 'linewidth', 2)
        hold on
        plot(cfg.iyears+lyear,squeeze(data_all.([tmp.varname, '_assm'])(grid.id_w,grid.id_s,:)), 'linewidth', 2)
        plot(cfg.iyears+lyear,squeeze(data_all.([tmp.varname, '_lens2'])(grid.id_w,grid.id_s,:)), 'linewidth', 2)
        plot(cfg.iyears+lyear,squeeze(data_all.([tmp.varname, '_obs'])(grid.id_w,grid.id_s,:)), 'linewidth', 2)
        
        hold off
        legend ('HCST', 'ASSM', 'LENS2', 'OBS', 'Location', 'Northwest')
