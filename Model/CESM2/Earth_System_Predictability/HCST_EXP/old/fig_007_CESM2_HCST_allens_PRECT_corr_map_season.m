...
%         tmp.fy_str=num2str(tmp.fy, '%04i');
        ...
        for mon=1:length(tmp.mons)
            ...
            tmp.sdata(:,:,mon)=ncread(cfg.mod_fnm, tmp.varname) .* 1000 .* 86400.0 ; %m/s -> mm/day
            
            cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'GPCP_reg_cesm2.v5.',tmp.fy_str,tmp.mon_str, '.nc'];
            tmp.sdata_obs(:,:,mon)=ncread(cfg.obs_fnm, 'precip');
        end
