...
%     data.([tmp.varname, '_assm'])=NaN(grid.nlon, grid.nlat, cfg.len_t_y);
    ...
    for iyear=min(cfg.iyears):max(cfg.iyears)
        ...
        for mon=1:12
            ...
            cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'ERA5_', cfg.obs_var, '_reg_cesm2.',tmp.iyear_str,tmp.mon_str, '.nc'];  %read as iyear, plot as +lyear
%             cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'ERA5_', cfg.obs_var, '_reg_cesm2.',tmp.fy_str,tmp.mon_str, '.nc'];
            
            if  exist(cfg.obs_fnm)==0
                tmp.ydata_obs(:,:,mon)=NaN;
            else
                tmp.ydata_obs(:,:,mon)=ncread(cfg.obs_fnm, cfg.obs_var);
            end
        end
...
%             hh = hatchfill2(hp,'hatchstyle','single','HatchAngle',45,'HatchDensity',150,'HatchColor','k','HatchLineWidth',0.5);
    ...
    dirs.figdir= [dirs.figroot, filesep, cfg.casename_m, filesep, tmp.varname, '_corr_obs_map'];
    if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
    cfg.figname=[dirs.figdir, filesep, cfg.var, '_corr_obs_det_map_', tmp.varname, '_l', tmp.lyear_str, 'y.tif'];
    print(fig_h, cfg.figname, '-dpng');
    RemoveWhiteSpace([], 'file', cfg.figname);
    fprintf('%7.1f sec\n', toc(lap_time) );
    close all;
