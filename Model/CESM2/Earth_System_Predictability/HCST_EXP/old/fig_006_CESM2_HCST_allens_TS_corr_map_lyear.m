...
%     data.([tmp.varname, '_assm'])=NaN(grid.nlon, grid.nlat, cfg.len_t_y);
    ...
    for iyear=min(cfg.iyears):max(cfg.iyears)
        ...
        for mon=1:12
            ...
            tmp.ydata(:,:,mon)=ncread(cfg.mod_fnm, tmp.varname); % m/s -> mm /day
            
            cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'HadCRUT5_reg_cesm2.',tmp.iyear_str,tmp.mon_str, '.nc'];
            tmp.ydata_obs(:,:,mon)=ncread(cfg.obs_fnm, 'tas_mean');
