...
%         tmp.fy_str=num2str(tmp.fy, '%04i');
        ...
        for mon=1:length(tmp.mons)
            ...
            tmp.sdata(:,:,mon)=ncread(cfg.mod_fnm, tmp.varname); %m/s -> mm/day
            
            cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'HadCRUT5_reg_cesm2.',tmp.fy_str,tmp.mon_str, '.nc'];
            tmp.sdata_obs(:,:,mon)=ncread(cfg.obs_fnm, 'tas_mean');
