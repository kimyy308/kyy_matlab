...
dirs.hcstroot=['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/HCST_EXP/archive/ocn/', cfg.var];
dirs.obsroot=['/Volumes/kyy_raid/kimyy/Observation/OC_CCI/monthly_reg_cam'];
dirs.figroot=['/Volumes/kyy_raid/kimyy/Figure/CESM2/ESP/HCST_EXP/archive/ocn/', cfg.var];

cfg.iyears=1998:2020;
% cfg.months=1:12;
% cfg.scnm='HIST';
cfg.gnm='f09_g17';
% cfg.assm_factor='10';
...
% /mnt/lustre/proj/kimyy/Model/CESM2/ESP/HCST_EXP/archive/ocn/SST/ens_all/ens_all_i2020
% SST_f09_g17.hcst.ens_all_i2020.pop.h.2024-12.nc
...
%         tmp.fy_str=num2str(tmp.fy, '%04i');
        ...
        for mon=1:length(tmp.mons)
            ...
            tmp.varname, '_', cfg.gnm, '.hcst.', cfg.casename, '.pop.h.', tmp.fy_str, '-', tmp.mon_str, '.nc'];
            tmp.sdata(:,:,mon)=ncread(cfg.mod_fnm, tmp.varname);
            
%             cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'OC_CCI_reg_cesm2.',tmp.iyear_str,tmp.mon_str, '.nc'];
            cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'OC_CCI_reg_cesm2.',tmp.fy_str,tmp.mon_str, '.nc'];

            tmp.sdata_obs(:,:,mon)=ncread(cfg.obs_fnm, 'chlor_a');
