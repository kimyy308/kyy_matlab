...
%             data.(['gm_', tmp.varname, '_sq_err_', tmp.obsname_simple, '_i', tmp.iyear_str])=ncread(config.datafilename, ['gm_sq_err_', tmp.varname]);
            ...
            data.(['time_proj_', tmp.iyear_str])=iyear:iyear+9;
...
%             for lyear=1:config.proj_year
%                 tmp.lyear_str=num2str(lyear, '%02i');
%                 tmp.ymean= mean(ncread(config.datafilename, ['gm_', tmp.varname], [(lyear-1)*12+1], 12));
%                 data.(['gm_', tmp.varname, '_model_', tmp.obsname_simple, '_l', tmp.lyear_str])(iyear-min(config.iyears)+lyear)= tmp.ymean;
...
%             end
            tmp.data= ncread(config.datafilename, ['gm_', tmp.varname], [1], inf);
            tmp.data_rsp=reshape(tmp.data, [12, length(tmp.data)/12]);
            tmp.ymean=mean(tmp.data_rsp, 1);
            data.(['gm_', tmp.varname, '_model_', tmp.obsname_simple, '_i', tmp.iyear_str])=tmp.ymean;
