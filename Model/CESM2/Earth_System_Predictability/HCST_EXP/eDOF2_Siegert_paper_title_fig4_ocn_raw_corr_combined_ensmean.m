% %  Created 24-May-2025 by Yong-Yub Kim
...
%% subplot(3,4,2); corr, ASSM<->HCST - ASSM <-> LENS2, ensmean (LY1)
for subi=1:1
...
%% subplot(3,4,5); corr, ASSM <-> HCST, ensmean (LY1)
for subi=1:1
...
%% subplot(3,4,6); corr, ASSM<->HCST - ASSM <-> LENS2, ensmean (LY1)
for subi=1:1
...
%% subplot(3,4,9); corr, ASSM <-> HCST, ensmean (LY1)
for subi=1:1
...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (i) \hspace{1mm}  r_{E(A),E(I)}^{\tau=1} $$';
...
%% subplot(3,4,10); corr, ASSM<->HCST - ASSM <-> LENS2, ensmean (LY1)
for subi=1:1
...
%     tmp.C(tmp.p2>0.1)=NaN;
    ...
    fig_cfg.fig_name='$$ (j) \hspace{1mm}  r_{E(A),E(I)}^{\tau=1} -  r_{E(A),E(U)}^{\tau=1} $$';
...
%% subplot(3,4,3); corr, ASSM <-> HCST, ensmean (LY25)
for subi=1:1
%     fig_cfg.fig_size = [0,0,13,14]; %% paper size (original)
    ...
    tmp.C=squeeze(corrval_hcst.assm_hcst_em_4ym.val);
    tmp.C=tmp.C([end, 1:end],:);

    % significance test
    sig_n=size(corrval_hcst.data.([cfg.var,'_4ym']),3);
    autocorrfile1=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_ODA_ds_4yr_em.nc'];
    autocorr1=ncread(autocorrfile1, cfg.var);
    autocorrfile2=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_HCST_ds_4yr_em.nc'];
    autocorr2=ncread(autocorrfile2, cfg.var);
    sig_n_star=sig_n.*(1-autocorr1.*autocorr2)./(1+autocorr1.*autocorr2); % 2nd order
    sig_n_star=sig_n_star([end, 1:end],:);
    sig_n_star(sig_n_star<=2)=NaN;
...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (c) \hspace{1mm}  r_{E(A),E(I)}^{\tau=2 \textendash 5} $$';
%     fig_h = figure('name',fig_cfg.fig_name,'PaperUnits','inches', ...
%         'PaperPosition',fig_cfg.fig_size,'position',fig_cfg.fig_size*get(groot,'ScreenPixelsPerInch')+[200,200,0,0],'visible','on');
% 
    %% map setting
...
%% subplot(3,4,4); corr, ASSM<->HCST - ASSM <-> LENS2, ensmean (LY25)
for subi=1:1
    ...
    tmp.A=corrval_hcst.assm_hcst_em_4ym.val;
    tmp.B=corrval_lens2.assm_lens2_em_4ym.val;
    
    r12_fn=['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/tmp_python/HCST_skills_HCST-LE/', ...
        'corr_',cfg.var,'_LE_HCST_ly25_em.nc'];
    r12=ncread(r12_fn, cfg.var);
    sig_n=size(corrval_hcst.data.([cfg.var,'_4ym']),3);
    sig_n_k=ceil(sig_n/2);
    ...
    autocorrfile2=[dirs.autocorrroot, '/', 'autocorr_', cfg.var, '_LE_ds_4yr_em.nc'];
    autocorr2=ncread(autocorrfile2, cfg.var);
    sig_n_star2=sig_n.*(1-autocorr1.*autocorr2)./(1+autocorr1.*autocorr2);
    sig_n_star2=sig_n_star2([end, 1:end],:);
    sig_n_star2(sig_n_star2<=2)=NaN;
...
%     tmp.C(tmp.p2>0.1)=NaN;
    ...
    fig_cfg.fig_name='$$ (d) \hspace{1mm}  r_{E(A),E(I)}^{\tau=2 \textendash 5} -  r_{E(A),E(U)}^{\tau=2 \textendash 5} $$';
...
%% subplot(3,4,7); corr, ASSM <-> HCST, ensmean (LY25)
for subi=1:1
...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (g) \hspace{1mm}  r_{E(A),E(I)}^{\tau=2 \textendash 5} $$';
...
%% subplot(3,4,8); corr, ASSM<->HCST - ASSM <-> LENS2, ensmean (LY1)
for subi=1:1
...
%     tmp.C(tmp.p2>0.1)=NaN;
    ...
    fig_cfg.fig_name='$$ (h) \hspace{1mm}  r_{E(A),E(I)}^{\tau=2 \textendash 5} -  r_{E(A),E(U)}^{\tau=2 \textendash 5} $$';
...
%% subplot(3,4,11); corr, ASSM <-> HCST, ensmean (LY1)
for subi=1:1
...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (k) \hspace{1mm}  r_{E(A),E(I)}^{\tau=2 \textendash 5} $$';
...
%% subplot(3,4,12); corr, ASSM<->HCST - ASSM <-> LENS2, ensmean (LY1)
for subi=1:1
...
%     tmp.C(tmp.p2>0.1)=NaN;
    ...
    fig_cfg.fig_name='$$ (l) \hspace{1mm}  r_{E(A),E(I)}^{\tau=2 \textendash 5} -  r_{E(A),E(U)}^{\tau=2 \textendash 5} $$';
...
%     set(title_main,'String', 'Ocean EM-based Skills (Potential Predictability)')
    set(title_main,'String', 'Ocean EM-based Skills (Attainable Skill)')
...
%     if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
    cfg.figname=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'eDOF2_Siegert_title_fig4','_OCN_comb_ensmean', '.tif'];
    ...
    cfg.figname2=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'eDOF2_Siegert_title_fig4','_OCN_comb_ensmean', '.eps'];
