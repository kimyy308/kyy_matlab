...
config.varnames={'salt'};  %temp, salt
            ...
            for loni=1:grid.nlon
                for lati=1:grid.nlat
                     if (isnan(data.([tmp.varname, '_obs_', tmp.obsname_simple])(loni,lati,1))~=1)
                         ...
                         tmp.md=squeeze(data.([tmp.varname, '_model_', tmp.obsname_simple, '_l', tmp.lyear_str])(loni,lati,1+lyear:end-config.proj_year+1));
                         tmp.od=squeeze(data.([tmp.varname, '_obs_', tmp.obsname_simple])(loni,lati,1+lyear:end));
                         
                         [f_exp, gof_exp] = fit(data.time_y',squeeze(tmp.md),'poly1');
                         data.([tmp.varname, '_trend_model_', tmp.obsname_simple, '_l', tmp.lyear_str])(loni,lati)=f_exp.p1;
                         [f_exp, gof_exp] = fit(data.time_y',squeeze(tmp.od),'poly1');
                         data.([tmp.varname, '_trend_obs_', tmp.obsname_simple, '_l', tmp.lyear_str])(loni,lati)=f_exp.p1;
                     else
                         data.([tmp.varname, '_trend_model_', tmp.obsname_simple, '_l', tmp.lyear_str])(loni,lati)=NaN;
                         data.([tmp.varname, '_trend_obs_', tmp.obsname_simple, '_l', tmp.lyear_str])(loni,lati)=NaN;                         
                     end
                end
            end
            disp('abc')
...
%             tmp.X(tmp.X>180)= tmp.X(tmp.X>180)-360;
            ...
            tmp.C=data.([tmp.varname, '_trend_model_', tmp.obsname_simple, '_l', tmp.lyear_str]).*10.0;
...
%             tmp.C_2(tmp.C_H<fig_config.p_lim)=NaN;
            ...
            fig_config.fig_name=['l',tmp.lyear_str, ', trend, ', tmp. varname];
...
%             title(cb,'^oC/dec','fontsize',12);
            title(cb,[tmp.var_unit, '/dec'],'fontsize',12);
            ...
            %% save
            dirs.figdir= [dirs.figroot, filesep, config.casename_m, filesep, 'GMSV', filesep, 'trend_map'];
            if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
            config.figname=[dirs.figdir, filesep, 'trend_map_model', tmp.varname, '_l', tmp.lyear_str, '.tif'];
...
%             tmp.X(tmp.X>180)= tmp.X(tmp.X>180)-360;
            tmp.Y=grid.tlat([end, 1:end],:);
            tmp.C=data.([tmp.varname, '_trend_obs_', tmp.obsname_simple, '_l', tmp.lyear_str]).*10.0;
            tmp.C=tmp.C([end, 1:end],:);
%             tmp.C_H=data.([tmp.varname, '_corr_p_', tmp.obsname_simple, '_l', tmp.lyear_str]);
%             tmp.C_H=tmp.C_H([end, 1:end],:);
%             tmp.C_2=tmp.C;
%             tmp.C_2(tmp.C_H<fig_config.p_lim)=NaN;
            ...
            fig_config.fig_name=['trend, ', tmp. varname];
            fig_h = figure('name',fig_config.fig_name,'PaperUnits','inches', ...
                'PaperPosition',fig_config.fig_size,'position',fig_config.fig_size*get(groot,'ScreenPixelsPerInch')+[200,200,0,0],'visible','off');
            %% map setting
            ax_m = axesm('MapProjection',fig_config.map_proj,'grid','on','fontsize',14, ...
...
%             ylabel(cb,[replace(tmp.varname,'_',' '), '(',tmp.var_unit,')']);
            ...
            dirs.figdir= [dirs.figroot, filesep, config.casename_m, filesep, 'GMSV', filesep, 'trend_map'];
            if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
            config.figname=[dirs.figdir, filesep, 'trend_map_obs', tmp.varname, '_l', tmp.lyear_str, '.tif'];
            print(fig_h, config.figname, '-dpng');
            RemoveWhiteSpace([], 'file', config.figname);
            fprintf('%7.1f sec\n', toc(lap_time) );
