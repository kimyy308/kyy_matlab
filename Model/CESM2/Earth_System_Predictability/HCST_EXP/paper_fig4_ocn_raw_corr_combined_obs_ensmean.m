...
%     fig_cfg.cb_size = [2, 1, 9, 0.3];
    ...
    tmp.C=squeeze(corrval_hcst.obsdet_hcst_em.ly1.val);
...
for subi=1:1
    ...
    tmp.A=corrval_hcst.obsdet_hcst_em.ly1.val;
    tmp.B=corrval_lens2.obsdet_lens2_em.val;
    ...
    for loni=1:size(corrval_hcst.obs_hcst_em.ly1.val,1)
        for lati=1:size(corrval_hcst.obs_hcst_em.ly1.val,2)
            tmp.obs=corrval_assm.data_obs.([cfg.var,'_ym'])(loni,lati,:);
            tmp.hcst=corrval_hcst.data.ly1.([cfg.var,'_ym'])(loni,lati,:);
            tmp.hcst(isnan(tmp.obs))=NaN;
            tmp.lens2=corrval_lens2.data.([cfg.var,'_ym'])(loni,lati,:);
            tmp.lens2(isnan(tmp.obs))=NaN;
            [tmp.hcst_det, tmp.hcst_tr] = Func_0028_detrend_linear_1d(tmp.hcst, 'omitnan');
            [tmp.obs_det, tmp.hcst_tr] = Func_0028_detrend_linear_1d(tmp.obs, 'omitnan');
            [tmp.lens2_det, tmp.hcst_tr] = Func_0028_detrend_linear_1d(tmp.lens2, 'omitnan');

            [tmp.hc_corr,tmp.hc_p,tmp.hc_rl,tmp.hc_ru] = ...
                corrcoef(tmp.hcst_det, ...
                tmp.obs_det,'alpha', 0.34, 'Rows', 'complete');
            [tmp.le_corr,tmp.le_p,tmp.le_rl,tmp.le_ru] = ...
                corrcoef(tmp.obs_det, ...
                tmp.lens2_det, 'alpha', 0.34, 'Rows', 'complete');
...
%             [tmp.lens2_det, tmp.hcst_tr] = Func_0028_detrend_linear_1d(tmp.lens2, 'omitnan');
            ...
            [tmp.hc_corr,tmp.hc_p,tmp.hc_rl,tmp.hc_ru] = ...
                corrcoef(tmp.hcst, ...
                tmp.obs,'alpha', 0.34, 'Rows', 'complete');
            [tmp.le_corr,tmp.le_p,tmp.le_rl,tmp.le_ru] = ...
                corrcoef(tmp.obs, ...
                tmp.lens2, 'alpha', 0.34, 'Rows', 'complete');
            tmp.hc_sigma=(tmp.hc_ru-tmp.hc_rl)/2;
            tmp.le_sigma=(tmp.le_ru-tmp.le_rl)/2;
%             for i=1:100
%                 tmp.hc_rnd(i)=normrnd(tmp.hc_corr(1,2),tmp.hc_sigma(1,2));
...
for subi=1:1
    ...
    tmp.C=squeeze(corrval_hcst.obsdet_hcst_em_4ym.val);
    tmp.C=tmp.C([end, 1:end],:);
...
for subi=1:1
    ...
    tmp.A=corrval_hcst.obsdet_hcst_em_4ym.val;
    tmp.B=corrval_lens2.obsdet_lens2_em_4ym.val;
    
    %% normal distributed sample -> ttest
    for loni=1:size(corrval_hcst.obs_hcst_em.ly1.val,1)
        for lati=1:size(corrval_hcst.obs_hcst_em.ly1.val,2)
            tmp.obs=corrval_assm.data_obs.([cfg.var,'_4ym'])(loni,lati,:);
            tmp.hcst=corrval_hcst.data.([cfg.var,'_4ym'])(loni,lati,:);
            tmp.hcst(isnan(tmp.obs))=NaN;
            tmp.lens2=corrval_lens2.data.([cfg.var,'_4ym'])(loni,lati,:);
            tmp.lens2(isnan(tmp.obs))=NaN;
            [tmp.hcst_det, tmp.hcst_tr] = Func_0028_detrend_linear_1d(tmp.hcst, 'omitnan');
            [tmp.obs_det, tmp.hcst_tr] = Func_0028_detrend_linear_1d(tmp.obs, 'omitnan');
            [tmp.lens2_det, tmp.hcst_tr] = Func_0028_detrend_linear_1d(tmp.lens2, 'omitnan');
    ...
    for loni=1:size(corrval_hcst.obs_hcst_em.ly1.val,1)
        for lati=1:size(corrval_hcst.obs_hcst_em.ly1.val,2)
            [tmp.hc_corr,tmp.hc_p,tmp.hc_rl,tmp.hc_ru] = ...
                ...
                corrval_assm.data_obs.([cfg.var,'_4ym'])(loni,lati,:),'alpha', 0.34, 'Rows', 'complete');
            [tmp.le_corr,tmp.le_p,tmp.le_rl,tmp.le_ru] = ...
                corrcoef(corrval_assm.data_obs.([cfg.var,'_4ym'])(loni,lati,:), ...
                corrval_lens2.data.([cfg.var,'_4ym'])(loni,lati,:), 'alpha', 0.34, 'Rows', 'complete');
            tmp.hc_sigma=(tmp.hc_ru-tmp.hc_rl)/2;
            tmp.le_sigma=(tmp.le_ru-tmp.le_rl)/2;
%             for i=1:100
%                 tmp.hc_rnd(i)=normrnd(tmp.hc_corr(1,2),tmp.hc_sigma(1,2));
