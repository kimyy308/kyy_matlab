...
%     fig_cfg.cb_size = [2, 1, 9, 0.3];
    ...
    fig_cfg.cb_size = [4, 1, 20, 0.3]; % original
    ...
    for xi = 1:size(corrval_hcst.obs_hcst.ly1.val, 2)
        for yi = 1:size(corrval_hcst.obs_hcst.ly1.val, 3)
            data_slice = corrval_hcst.obs_hcst.ly1.val(:, xi, yi);
            tmp.C(xi, yi) = Func_0039_mode_highest(data_slice, edges);
        end
    end
    corrval_hcst.obs_hcst.ly1.val_mode=tmp.C;

    for xi = 1:size(corrval_hcst.obs_hcst.ly1.val, 2)
        for yi = 1:size(corrval_hcst.obs_hcst.ly1.val, 3)
            data_slice = corrval_lens2.obs_lens2.val(:, xi, yi);
            corrval_lens2.obs_lens2.val_mode(xi, yi) = Func_0039_mode_highest(data_slice, edges);
        end
    end
...
for subi=1:1
    ...
    tmp.A=corrval_hcst.obs_hcst.ly1.val_mode;
    tmp.B=corrval_lens2.obs_lens2.val_mode;
...
%     fig_cfg.fig_size = [0,0,13,14]; %% paper size (original)
    ...
    for xi = 1:size(corrval_hcst.obs_hcst.ly1.val, 2)
        for yi = 1:size(corrval_hcst.obs_hcst.ly1.val, 3)
            data_slice = corrval_hcst.obs_hcst_4ym.val(:, xi, yi);
            tmp.C(xi, yi) = Func_0039_mode_highest(data_slice, edges);
        end
    end
    corrval_hcst.obs_hcst_4ym.val_mode=tmp.C;

    for xi = 1:size(corrval_hcst.obs_hcst.ly1.val, 2)
        for yi = 1:size(corrval_hcst.obs_hcst.ly1.val, 3)
            data_slice = corrval_lens2.obs_lens2_4ym.val(:, xi, yi);
            corrval_lens2.obs_lens2_4ym.val_mode(xi, yi) = Func_0039_mode_highest(data_slice, edges);
        end
    end
    tmp.C=tmp.C([end, 1:end],:);
...
for subi=1:1
    ...
    tmp.A=corrval_hcst.obs_hcst_4ym.val_mode;
    tmp.B=corrval_lens2.obs_lens2_4ym.val_mode;

    %% double-sample t-test
    sig_n=sum(isfinite(corrval_assm.data_obs.([cfg.var,'_4ym'])),3);
    tmp.tt=Func_0038_compare_correlation(tmp.A,tmp.B,sig_n,sig_n);
...
for subi=1:1
    ...
    pcolor(corrval_hcst.obs_hcst_4ym.val_mode'); shading flat; colorbar;
    histogram(corrval_hcst.obs_hcst_4ym.val(:,237,128), -1:0.1:1); set(gca, 'fontsize', 20);
    ylabel('# of corr'); xlabel('interval'); title('x=237, y=128');

    histogram(corrval_hcst.obs_hcst_4ym.val(:,237,129), -1:0.1:1); set(gca, 'fontsize', 20);
    ylabel('# of corr'); xlabel('interval'); title('x=237, y=129');
...
%     text(loc_column_first-1, loc_row_first-7, 'SSH2', 'Units', 'inches', 'fontsize', 22)
...
%     if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
    cfg.figname=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'title_fig4','_mode_OCN_comb_obs', '.tif'];
...
%     RemoveWhiteSpace([], 'file', cfg.figname);
    cfg.figname2=['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'title_fig4','_mode_OCN_comb_obs', '.eps'];
