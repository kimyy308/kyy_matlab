...
%     fig_cfg.fig_size = [0,0,19,14.5]; %% paper size (original)
    fig_cfg.fig_size = [0,0,19,24]; %% paper size (original)

%     fig_cfg.cb_size = [1, 13, 17, 0.3];
    fig_cfg.cb_size = [1, 18, 17, 0.3];
    ...
    ax_m_1=subplot(20,20,1);
    fig_cfg.ax_size = [loc_column_first, loc_row_first+5, 4.5, 4.5];
    period=1:figi;
    fig_weig=1;
    ...
    cb_title=title(cb,'$$ \tau $$','fontsize', 22, 'Position', [1040, 0, 0]); % hor, ver, ?

    set(cb_title, 'interpreter', 'latex');
    
    %scatter
    ax_m_2=subplot(20,20,2);
...
%         'MarkerEdgeColor', 'none', 'LineWidth', 3);
    ...
    ax_m_3=subplot(20,20,3);
    fig_cfg.ax_size = [loc_column_first + 6, loc_row_first+5, 4.5, 4.5];
    period=1:figi;
    fig_weig=1;
...
%         'MarkerEdgeColor', 'none', 'LineWidth', 3);
    ...
    ax_m_5=subplot(20,20,5);
    fig_cfg.ax_size = [loc_column_first + 12, loc_row_first+5, 4.5, 4.5];
    period=1:figi;
    fig_weig=1;
...
%         'MarkerEdgeColor', 'none', 'LineWidth', 3);
    ...
    ax_m_7=subplot(20,20,7);
    caxis([0 max(period_org)/100]);
    fig_cfg.ax_size = [loc_column_first, loc_row_first-6, 17, 4];
...
%     bp1=boxplot(ax_m_7, HCST.X.corr(:,1:cfg.pred_len), (1:cfg.pred_len).*0.1-0.1, 'BoxStyle', 'outline', 'symbol', '', 'Whisker', 2);
    ...
    ylim([-0.2 1])
    title1=title('(e) ACC(\tau)', 'fontsize', 20);
    grid on
    ...
    %% timeseries (middle)
    ax_m_9=subplot(20,20,9);
%     caxis([0 max(period_org)/100]);
    fig_cfg.ax_size = [loc_column_first-0.1, loc_row_first-0.5, 16.4, 4];
    set(ax_m_9, 'Parent', fig_h)
    set(ax_m_9,'Units','inches','Position',fig_cfg.ax_size);
    
    tmp.corr_em=HCST.X.corr_em;
    if figi<=480
        tmp.corr_em((figi/10)+2:end)=NaN;
    end
%     sc_1=scatter(ax_m_9, (1:cfg.pred_len), tmp.corr_em);
%     set(sc_1, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'k')
%     set(sc_1, 'SizeData', 50.*fig_weig)
    
    hold on
%     lineplot_1=plot(ax_m_9, (1:cfg.pred_len), tmp.corr_em, 'k-', 'linewidth',2);
    if figi/10 <=50
        for ei=1:100
            lineplot_0=plot(ax_m_9, [50:0.1:200]+figi/100-50, squeeze(HCST.X.data_lt(ei, figi/10,:)), 'color', [0.9 0.9 0.9], 'linewidth',1);
            if ei<100
                set(get(get(lineplot_0,'Annotation'),'LegendInformation'),'IconDisplayStyle','off'); %legend off
            end
        end
%         lineplot_3=plot(ax_m_9, [50:0.1:200]+figi/100, squeeze(HCST.X.data_lt(8,figi/10,:)), 'b--', 'linewidth',0.5);
        lineplot_1=plot(ax_m_9, [50:0.1:200]+figi/100-50, CTL.X(0+figi/10:end-51+figi/10), 'k-', 'linewidth',1);
        lineplot_2=plot(ax_m_9, [50:0.1:200]+figi/100-50, HCST.X.data_lt_em(figi/10,:), 'r-', 'linewidth',1);
    else
        for ei=1:100
            lineplot_0=plot(ax_m_9, [50:0.1:200]+5-50, squeeze(HCST.X.data_lt(ei, 50,:)), 'color', [0.9 0.9 0.9], 'linewidth',1);
            if ei<100
                set(get(get(lineplot_0,'Annotation'),'LegendInformation'),'IconDisplayStyle','off'); %legend off
            end
        end
%         lineplot_3=plot(ax_m_9, [50:0.1:200]+5, squeeze(HCST.X.data_lt(8,50,:)), 'b--', 'linewidth',1);
        lineplot_1=plot(ax_m_9, [50:0.1:200]+5-50, CTL.X(50:end-1), 'k-', 'linewidth',1);
        lineplot_2=plot(ax_m_9, [50:0.1:200]+5-50, HCST.X.data_lt_em(50,:), 'r-', 'linewidth',1);
    end
    
%     [hLg]=legend(ax_m_9, [lineplot_1, lineplot_2, lineplot_3], ...
%         {'observation', 'ensmean', 'individual'}, ...
%         'Fontsize', 15.*fig_weig);  %% for median
    [hLg]=legend(ax_m_9, [lineplot_1, lineplot_2, lineplot_0], ...
        {'observation', 'ensmean', 'individuals'}, ...
        'Fontsize', 15.*fig_weig);  %% for median
            
    ylim([-25 25])
    xlim([50 210]-50)
    if figi/10 <=50
        title1=title(['(d) x ( l\tau = ', num2str(figi/100), ' )'], 'fontsize', 20);
    else
        title1=title(['(d) x ( l\tau = 5', ' )'], 'fontsize', 20);
    end
...
%     grid minor
    ...
    set(ax_m_9,'fontsize',18);
    set(ax_m_9,'YAxisLocation', 'right');

    cfg.figdir='/Users/kimyy/Desktop/backup/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper/Figureset_raw/Lorenz_anime_v3/';
