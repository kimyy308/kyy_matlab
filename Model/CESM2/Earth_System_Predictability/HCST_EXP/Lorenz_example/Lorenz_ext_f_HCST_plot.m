...
set(gca,'fontsize',15.*fig_weig);
grid on;
s=text(56,45,['Lorenz Model: ', '(x+f)', ' versus ', 't'], 'fontsize', 22.*fig_weig, ...
...
ylabel('$$ x(t) + f(t) $$', 'Interpreter', 'latex', 'fontsize', 25.*fig_weig)
xlabel('$$ t $$', 'Interpreter', 'latex', 'fontsize', 25.*fig_weig)
...
legend([plot_legend], ...
    {'$$ C  $$', '$$ Y_{I}[lt=0] $$', ['$$ Y_{I}[lt=', num2str(cfg.pred_len), ']  $$'], '$$ Y_{UI} $$'}, ...
    'location', 'northoutside', 'Orientation', 'horizontal', 'Fontsize', 22.*fig_weig, ...
    'interpreter', 'latex');
...
for ti=1:cfg.pred_len*100
    ...
    tmp.corr=corrcoef(data_comb.X(tmp.tmin:100:tmp.tmax),  ...
        HCST_X.data_lt_em(ti,tmp.ts:tmp.te));  % (lt=500, 60~85)
    ...
    tmp.cov=cov(data_comb.X(tmp.tmin:100:tmp.tmax),  ...
        squeeze(HCST_X.data_lt_em(ti,tmp.ts:tmp.te)));  % (lt=500, 60~85)
    HCST_X.cov_em(ti)=tmp.cov(1,2);
    for ensi=1:cfg.ensnum
        tmp.corr=corrcoef(data_comb.X(tmp.tmin:100:tmp.tmax),  ...
            HCST_X.data_lt(ensi,ti,tmp.ts:tmp.te));  % (lt=500, 60~85)
        HCST_X.corr(ensi,ti)=tmp.corr(1,2);
        HCST_X.sig_t_indv(ensi,ti)=std(HCST_X.data_lt(ensi,ti,tmp.ts:tmp.te));
        tmp.cov=cov(data_comb.X(tmp.tmin:100:tmp.tmax),  ...
            squeeze(HCST_X.data_lt(ensi,ti,tmp.ts:tmp.te)));  % (lt=500, 60~85)
...
tmp.xticks{cfg.pred_len+2}='$$ Y_{UI} $$';
...
xlabel(axRH,'$$ lt $$', 'Interpreter', 'latex', 'fontsize', 25.*fig_weig)
grid(axRH)
