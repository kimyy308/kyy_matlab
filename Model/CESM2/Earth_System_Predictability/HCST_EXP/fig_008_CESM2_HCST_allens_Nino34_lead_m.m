...
%         tmp.assm=data.(tmp.assmvar_ano_lm_rm);
    ...
    %% correlation coefficient of NINO3.4 ind, function of lead year (3-mon running mean)
        cfg.figname=[dirs.figdir, filesep, 'nino34m_hcst_obs_91-20m', '_corr_leadm_3rm', '.tif'];
...
%% correlation coefficient of NINO3.4 ind, function of lead year (3-mon running mean)
cfg.figname=[dirs.figdir, filesep, 'spaghetti_nino34m_hcst_obs_91-20m', '_corr_leadm_3rm', '.tif'];
%             bar(0:cfg.proj_year*12-1,data.(['nino34m_', tmp.varname, '_corr']),  'linewidth', 2)
...
grid minor
set(gcf, 'PaperPosition', [0, 0, 8, 4]);
cfg.figname=[dirs.figdir, filesep, 'spaghetti_', 'SST_hcst_obs_91-20m', '.tif'];
saveas(gcf,cfg.figname,'tif');
RemoveWhiteSpace([], 'file', cfg.figname);
close all;
