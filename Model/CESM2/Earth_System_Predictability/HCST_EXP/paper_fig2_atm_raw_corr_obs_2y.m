...
for subi=1:1
    ...
    tmp.C=squeeze(corrval_hcst.obs_hcst.ly2.val_median.*grid.lmask);
...
for subi=1:1
    ...
    tmp.A=corrval_hcst.obs_hcst.ly2.val_median;
    tmp.B=corrval_lens2.obs_lens2.val_median;
    
    %% double-sample t-test
    tmp.tt=NaN(size(corrval_lens2.assm_lens2_em.val));
    for loni=1:size(corrval_hcst.obs_hcst.ly2.val,2)
        for lati=1:size(corrval_hcst.obs_hcst.ly2.val,3)
            tmp.a=corrval_hcst.obs_hcst.ly2.val(:,loni,lati);
...
for subi=1:1
    ...
    tmp.C=squeeze(corrval_hcst.obs_hcst.ly2.val_median);
    tmp.C=tmp.C([end, 1:end],:);
...
%% caxis & colorbar (dACC)
    ...
    cfg.figname=['/Users/kimyy/Desktop/backup/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'fig2','_ATM_obs_2y', '.tif'];
