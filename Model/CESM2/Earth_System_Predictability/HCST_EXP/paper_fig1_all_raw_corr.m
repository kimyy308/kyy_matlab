...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (b) \hspace{1mm}  M(r_{A,I}^{\tau=1}) $$';
...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (d) \hspace{1mm}  r_{E(A),E(U)}^{\tau=1} $$';
    %% map setting
    subplot(3,3,4);
...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (e) \hspace{1mm}  M(r_{A,U}^{\tau=1}) $$';
...
%     tmp.C(tmp.p>0.1)=NaN; % 90% significant
    ...
    fig_cfg.fig_name='$$ (f) \hspace{1mm}  M(r_{A,U}^{\tau=2 \textendash 5}) $$';
...
% % %     % if lower hc is higher than upper le -> significantly higher
% % %     % if upper hc is lower than lower le -> significantly lower
% % %     for loni=1:size(corrval_hcst.assm_hcst_em.ly1.val,1)
% % %         for lati=1:size(corrval_hcst.assm_hcst_em.ly1.val,2)
% % %             [tmp.hc_corr,tmp.hc_p,tmp.hc_rl,tmp.hc_ru] = ...
% % %                 corrcoef(corrval_hcst.data.ly1.SST_ym(loni,lati,:), ...
% % %                 corrval_assm.data.SST_ym(loni,lati,:),'alpha', fig_cfg.p_lim);
% % %             [tmp.le_corr,tmp.le_p,tmp.le_rl,tmp.le_ru] = ...
% % %                 corrcoef(corrval_assm.data.SST_ym(loni,lati,:), ...
% % %                 corrval_lens2.data.SST_ym(loni,lati,:), 'alpha', fig_cfg.p_lim);
% % %             tmp.diff=tmp.A(loni,lati)-tmp.B(loni,lati);
% % %             if tmp.diff>=0
% % %                 if (tmp.hc_rl(1,2)-tmp.le_ru(1,2))>0
% % %                     tmp.tt(loni,lati)=1; % significantly diff
% % %                 else
% % %                     tmp.tt(loni,lati)=NaN;
% % %                 end
% % %             elseif tmp.diff<0
% % %                 if (tmp.hc_ru(1,2)-tmp.le_rl(1,2))<0
% % %                     tmp.tt(loni,lati)=1; % significantly diff
% % %                 else
% % %                     tmp.tt(loni,lati)=NaN;
% % %                 end
    ...
    for loni=1:size(corrval_hcst.assm_hcst_em.ly1.val,1)
        for lati=1:size(corrval_hcst.assm_hcst_em.ly1.val,2)
            [tmp.hc_corr,tmp.hc_p,tmp.hc_rl,tmp.hc_ru] = ...
                ...
                corrval_assm.data.([cfg.var,'_ym'])(loni,lati,:),'alpha', 0.1);
            [tmp.le_corr,tmp.le_p,tmp.le_rl,tmp.le_ru] = ...
                corrcoef(corrval_assm.data.([cfg.var,'_ym'])(loni,lati,:), ...
                corrval_lens2.data.([cfg.var,'_ym'])(loni,lati,:), 'alpha', 0.1);
...
%     tmp.C(tmp.p2>0.1)=NaN;
    ...
    fig_cfg.fig_name='$$ (g) \hspace{1mm}  r_{E(A),E(I)}^{\tau=1} - r_{E(A),E(U)}^{\tau=1} $$';
    %% map setting
    subplot(3,3,7);
...
%     tmp.C(tmp.p2>0.1)=NaN;
    ...
    fig_cfg.fig_name='$$ (h) \hspace{1mm}  M(r_{A,I}^{\tau=1}) -  M(r_{A,U}^{\tau=1}) $$';
...
%     tmp.C2=tmp.tt;
    ...
    fig_cfg.fig_name='$$ (i) \hspace{1mm}  M(r_{A,I}^{\tau=2 \textendash 5}) - M(r_{A,U}^{\tau=2 \textendash 5})$$';
...
%     cb.TickLabelPosition = 'top';
    ...
    cfg.figname=['/Users/kimyy/Desktop/backup/Research/Postdoc/03_IBS/2022_predictability_assimilation_run/paper', ...
        filesep, 'Figureset_raw', filesep, 'fig1','_',cfg.var, '.tif'];
