...
%     data.([tmp.varname, '_assm'])=NaN(grid.nlon, grid.nlat, cfg.len_t_y);
    ...
    for iyear=min(cfg.iyears):max(cfg.iyears)
        ...
        %% winter filename
        cfg.mod_fnm=[dirs.datadir, tmp.fs, cfg.casename, tmp.fs, 'winter', tmp.fs, ...
        tmp.varname, '_', cfg.gnm, '.hcst.', cfg.casename, '.cam.h0.', tmp.fy_str, '-', 'djfm', '.nc'];
        tmp.ymean=ncread(cfg.mod_fnm, tmp.varname);
        
        cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'winter', tmp.fs, ...
            'ERA5_', cfg.obs_var, '_reg_cesm2.',tmp.iyear_str,'-djfm', '.nc'];  %read as iyear, plot as +lyear
%             cfg.obs_fnm=[dirs.obsroot, tmp.fs, 'ERA5_', cfg.obs_var, '_reg_cesm2.',tmp.fy_str,tmp.mon_str, '.nc'];
        
        if  exist(cfg.obs_fnm)==0
            tmp.ymean_obs(:,:)=NaN;
        else
            tmp.ymean_obs(:,:)=ncread(cfg.obs_fnm, cfg.obs_var);
        end
...
data.([tmp.varname, '_corr_obs', '_l1_3'])= ( data.([tmp.varname, '_corr_obs', '_l01']) + ...
    data.([tmp.varname, '_corr_obs', '_l02']) + ...
    data.([tmp.varname, '_corr_obs', '_l03'])) ./ 3;


% tmp.yearset={'01', '02', '3_4', '5_9'};
tmp.yearset={'01', '02', '03', '1_3'};
...
%             hh = hatchfill2(hp,'hatchstyle','single','HatchAngle',45,'HatchDensity',150,'HatchColor','k','HatchLineWidth',0.5);
    ...
    %% save
    dirs.figdir= [dirs.figroot, filesep, cfg.casename_m, filesep, tmp.varname, '_corr_obs_map', filesep, 'winter'];
    if ~exist(dirs.figdir,'dir'), mkdir(dirs.figdir); end
    cfg.figname=[dirs.figdir, filesep, cfg.var, '_corr_obs_map_', tmp.varname, '_l', tmp.lyear_str, 'y.tif'];
