...
for vari= 1:length(cfg.vars)
    ...
    dirs.assmroot=['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/ATM_ASSM_EXP/', ...
        name.exp, tmp.fs, 'archive_analysis/', name.comp, tmp.fs, name.freq, tmp.fs, name.var, tmp.fs, name.obs];
    dirs.assm_refroot=['/Volumes/kyy_raid/kimyy/Model/CESM2/ESP/ATM_ASSM_EXP/', ...
        name.exp_ref, tmp.fs, 'archive_analysis/', name.comp, tmp.fs, name.freq, tmp.fs, name.var, tmp.fs, name.obs];
...
%         name.exp, tmp.fs, 'archive_analysis/', name.comp, tmp.fs, name.var];
    ...
    dirs.MEAN=[dirs.assmroot, tmp.fs, 'MEAN'];
    dirs.MEAN_ref=[dirs.assm_refroot, tmp.fs, 'MEAN'];
...
%% MEAN map  --------------------------------------
        ...
        dirs.figdir= [dirs.figroot, filesep, 'MEAN'];
        ...
        if ~exist(cfg.figname)
            ...
            fname.(name.fign)=[dirs.MEAN, tmp.fs, ...
                'MEAN_', name.var, '_', name.obs, '.', name.outf, '.nc'];
            fname_ref.(name.fign)=[dirs.MEAN_ref, tmp.fs, ...
                'MEAN_', name.var, '_', name.obs, '.', name.outf, '.nc'];
