...
for vari=1:length(cfg.vars)
    ...
    for lyear=1:cfg.proj_year-1
        ...
        data.([tmp.varname, '_AR1', '_l', tmp.lyear_str])=NaN(grid.nlon, grid.nlat, cfg.len_t_y+cfg.proj_year-1);  %% AR1 initialization
    ...
    %             tmp.varname, '_', cfg.gnm, '.lens2.', cfg.casename, cfg.obs_fname_module, tmp.fy_str, '-', tmp.mon_str, '.nc'];
    ...
    %                  data.([tmp.varname, '_corr_assm_p', '_l', tmp.lyear_str])(loni,lati)=NaN;
        ...
        tmp.matname = [dirs.lens2root, tmp.fs, ...
            cfg.var, '_', cfg.casename_m, '_l', tmp.lyear_str, num2str(min(cfg.obs_iyears)), '_', num2str(max(cfg.obs_iyears)), '.mat'];
        LENS2.corr_obs=data.([tmp.varname, '_corr_obs', '_l', tmp.lyear_str]);
        LENS2.corr_obs_p=data.([tmp.varname, '_corr_obs_p', '_l', tmp.lyear_str]);
        LENS2.corr_obs_det=data.([tmp.varname, '_corr_obs_det', '_l', tmp.lyear_str]);        
        LENS2.corr_obs_det_p=data.([tmp.varname, '_corr_obs_det_p', '_l', tmp.lyear_str]);
