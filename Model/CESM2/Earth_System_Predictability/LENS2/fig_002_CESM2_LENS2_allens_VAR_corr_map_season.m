...
for vari=1:length(cfg.vars)
    ...
    dirs.figroot=['/Volumes/kyy_raid/kimyy/Figure/CESM2/ESP/LENS2/archive_analysis/', cfg.comp,'/', cfg.var];
...
%     cfg.season = {'FMA', 'MAM', 'JJA', 'SON', 'DJF', 'AMJ', 'JAS', 'OND', 'JFM', 'INI'};
%     cfg.season = {'INI'};
    ...
    % cfg.casename_m=[cfg.gridname, '.lens2.', tmp.obsname, '-', cfg.assm_factor, 'p', cfg.ens_member];
    % cfg.casename=[cfg.casename_m, '_i', num2str(iyear)];
    % dirs.datadir= [dirs.lens2root, filesep, cfg.casename_m, filesep, 'GMSV'];
    
    % f09_g17.lens2.en4.2_ba-10p1_i2021.pop.h.once.nc
    
    % [tmp.error_status, tmp.value]=system(['ls ', dirs.datadir, '/*once*']);  % b.e21.BHISTsmbb.f09_g17.assm.oras4_ba-10p1.pop.h.once.nc
    tmp.gridname = [dirs.lens2root, tmp.fs, '../grid.nc'];
    ...
    % /mnt/lustre/proj/kimyy/Model/CESM2/ESP/LENS2/archive/atm/SST/ens_all/ens_all_i2020
    % SST_f09_g17.lens2.ens_all_i2020.cam.h0.2024-12.nc
    ...
    for iyear=min(cfg.iyears):max(cfg.iyears)
        ...
        for mon=1:12
            ...
            tmp.all_data_obs(:,:,(tmp.iind-1)*12+mon) = netcdf.getVar(ncid,tmpvarid);
            netcdf.close(ncid);
%             tmp.all_data_obs(:,:,(tmp.iind-1)*12+mon)=ncread(cfg.obs_fnm, cfg.obs_varname);
    ...
    for lss=1:length(cfg.season)
        ...
        dirs.datadir= [dirs.lens2root, filesep, cfg.casename_m, filesep];
    ...
    %         tmp.fy_str=num2str(tmp.fy, '%04i');
            ...
            for mon=1:length(tmp.mons)
                ...
                cfg.mod_fnm=[dirs.datadir, tmp.fs, ...
...
%                 tmp.sdata(:,:,mon)=ncread(cfg.mod_fnm, tmp.varname);
                ...
                tmp.sdata(:,:,mon) = netcdf.getVar(ncid,tmpvarid);
                netcdf.close(ncid);
...
%                 tmp.sdata_obs(:,:,mon)=ncread(cfg.obs_fnm, cfg.obs_varname);
                if exist(cfg.obs_fnm)~=0
                    ncid=netcdf.open(cfg.obs_fnm, 'NOWRITE');
                    tmpvarid=netcdf.inqVarID(ncid,cfg.obs_varname);
                    tmp.sdata_obs(:,:,mon) = netcdf.getVar(ncid,tmpvarid);
                    netcdf.close(ncid);
                else
                    tmp.sdata_obs(:,:,mon) = NaN;                    
                end

                if ( strcmp(cfg.var, 'sumChl') || strcmp(cfg.var, 'TS'))%for Chls, it uses all available data
                    if exist(cfg.obs_fnm)~=0
                        tmp.obs_mask(:,:)=tmp.sdata_obs(:,:,mon)./tmp.sdata_obs(:,:,mon);
                        tmp.sdata(:,:,mon) = tmp.sdata(:,:,mon) .* tmp.obs_mask(:,:); % masking with available observation
                    end
                end
                
                %% AR1 (integration)
                tmp.sdata_AR1(:,:,mon) = ...
                    Func_0030_AR1_prog(tmp.all_data_obs(:,:,(tmp.iind-1)*12+1), data.([tmp.varname, '_AC_lag1']), tmp.mon-1, 0);
    ...
    %         data.time=ncread(cfg.datafilename, 'time');
            ...
            data.([tmp.varname, '_AR1', '_', tmp.season])(:,:,tmp.iind)= tmp.smean_AR1;
    ...
    %                  data.([tmp.varname, '_corr_assm_p', '_', tmp.season])(loni,lati)=NaN;  
        ...
        tmp.matname = [dirs.lens2root, tmp.fs, ...
            cfg.var, '_', cfg.casename_m, '_', tmp.season, num2str(min(cfg.obs_iyears)), '_', num2str(max(cfg.obs_iyears)), '.mat'];
        LENS2.corr_obs=data.([tmp.varname, '_corr_obs', '_', tmp.season]);
        LENS2.corr_obs_p=data.([tmp.varname, '_corr_obs_p', '_', tmp.season]);
        LENS2.corr_obs_det=data.([tmp.varname, '_corr_obs_det', '_', tmp.season]);        
        LENS2.corr_obs_det_p=data.([tmp.varname, '_corr_obs_det_p', '_', tmp.season]);
        save(tmp.matname, 'LENS2');
    ...
    %     tmp.lss_str=tmp.yearset{lss_ind};
        ...
        fig_cfg.p_lim =0.1;
        fig_cfg.c_lim = [-1 1];
        [fig_cfg.c_map, tmp.err_stat] = Func_0009_get_colormaps('bwr', tmp.dropboxpath);
