...
filename='D:\Research\Ph_D_course\2021_YS_biogeochemical\global-reanalysis-phy-001-030-2006_2019_monthly.nc';
data_info=ncinfo(filename);
...
for x=1:size(uo,1)
    for y=1:size(uo,2)
        valid_depthnum=sum(isfinite(squeeze(clim_uo(x,y,1:15,1))));
        if valid_depthnum>0
...
for x=1:size(uo,1)
    for y=1:size(uo,2)
        valid_depthnum=sum(isfinite(squeeze(clim_uo(x,y,16:end,1))));
        if valid_depthnum>0
            for i=16:16+valid_depthnum-1
                subsurf_clim_uo(x,y,:)=squeeze(subsurf_clim_uo(x,y,:))+squeeze(clim_uo(x,y,i,:).*layer_thickness(i)./sum(layer_thickness(16:16+valid_depthnum-1)));
                subsurf_clim_vo(x,y,:)=squeeze(subsurf_clim_vo(x,y,:))+squeeze(clim_vo(x,y,i,:).*layer_thickness(i)./sum(layer_thickness(16:16+valid_depthnum-1)));
            end
        else
...
save('D:\Research\Ph_D_course\2021_YS_biogeochemical\glorys_2006_2019_ys_current.mat', 'clim_uo', 'clim_vo', 'depth', 'layer_thickness', 'surf_clim_uo', 'surf_clim_vo', 'subsurf_clim_uo', 'subsurf_clim_vo', 'longitude', 'latitude', 'lon2', 'lat2')
