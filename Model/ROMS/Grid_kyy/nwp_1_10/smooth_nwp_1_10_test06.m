...
% shading interp;
h(indw:inde,inds:indn)=h(indw:inde,inds:indn)+30;   %% +20 -> error2

%% tsugaru strait

smootharea=[139.9, 141.5, 40.7, 41.9];
[indw, inde, inds, indn]=findind_Y(dl,smootharea,lon_rho,lat_rho);
h(indw:inde,inds:indn)=h(indw:inde,inds:indn)+15;   %% + 20 -> error(spinup) +0 -> error2
...
m_grid('fontsize',20, 'box', 'fancy', 'tickdir', 'in');   %% for nwp = 25, for es = 20
hold on;
m_pcolor(lon_rho,lat_rho,h_raw);
shading flat;
% set colorbar 
ccc= colorbar;
colormap(jet);
