...
romstools_param
warning off
rawname=['E:\Data\Model\ROMS\nwp_1_10\input\test15\roms_grid_nwp_1_10_test15_not_smoothed.nc'];
system(['copy ',rawname,' ','E:\Data\Model\ROMS\nwp_1_10\input\test15\roms_grid_nwp_1_10_test15.nc'])
...
ncid=netcdf.open('E:\Data\Model\ROMS\nwp_1_10\input\test15\roms_grid_nwp_1_10_test15.nc', 'WRITE');
...
% shading interp;
h(indw:inde,inds:indn)=h(indw:inde,inds:indn)+20;   %% +20 -> error2

%% tsugaru strait

smootharea=[139.9, 141.5, 40.7, 41.9];
[indw, inde, inds, indn]=findind_Y(dl,smootharea,lon_rho,lat_rho);
h(indw:inde,inds:indn)=h(indw:inde,inds:indn)+20;   %% + 20 -> error(spinup) +0 -> error2

%% soya strait

smootharea=[141.6, 142.3, 45.3, 46.0];
[indw, inde, inds, indn]=findind_Y(dl,smootharea,lon_rho,lat_rho);
h(indw:inde,inds:indn)=h(indw:inde,inds:indn)+30;   %% +35 -> error2
...
[indw, inde, inds, indn]=findind_Y(dl,smootharea,lon_rho,lat_rho);
rtarget = 0.5; n_filter_deep_topo = 30; n_filter_final = 5;
h(indw:inde,inds:indn)=smoothgrid(h(indw:inde,inds:indn),maskr(indw:inde,inds:indn),hmin,hmax_coast,hmax, ...
  rtarget, n_filter_deep_topo, n_filter_final);
...
smootharea=[126.9, 129.8, 36.6, 41.6];
[indw, inde, inds, indn]=findind_Y(dl,smootharea,lon_rho,lat_rho);
...
% rtarget = 0.8; n_filter_deep_topo = 5; n_filter_final = 5;
rtarget = 0.8; n_filter_deep_topo = 1; n_filter_final = 0.1;  
h(indw:inde,inds:indn)=smoothgrid(h(indw:inde,inds:indn),maskr(indw:inde,inds:indn),hmin,hmax_coast,hmax, ...
  rtarget, n_filter_deep_topo, n_filter_final);


%% East Sea depth overwriting from test11

nc2=netcdf('E:\Data\Model\ROMS\nwp_1_10\input\test11\roms_grid_nwp_1_10_test11.nc','write');
h_test11=nc2{'h'}(:)';
close(nc2);

run('nwp_polygon_point');

mask_model(1:size(lon_rho,1),1:size(lon_rho,2))=1;
mask_model = double(inpolygon(lon_rho,lat_rho,espolygon(:,1),espolygon(:,2)));
mask_model(mask_model==0)=NaN;

h(~isnan(mask_model))=h_test11(~isnan(mask_model));
