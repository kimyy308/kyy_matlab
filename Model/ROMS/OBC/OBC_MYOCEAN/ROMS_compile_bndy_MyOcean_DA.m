...
%     cycle_length = 365.25;
    ...
    for mi = mm(1):mm(end);
        ...
        elseif (strcmp(system_name,'GLNXA64'))
            ...
            if mi==13
                filename = ['/data1/kimyy/Model/ROMS/nwp_1_10/forcing_matlab/OBC/',model_name,'/',testname,'/',model_name,'_Y', num2str(yyyy,4), 'M', num2str(1), '_',testname,'.nc'];
            elseif mi==14
                filename = ['/data1/kimyy/Model/ROMS/nwp_1_10/forcing_matlab/OBC/',model_name,'/',testname,'/',model_name,'_Y', num2str(yyyy,4), 'M', num2str(12), '_',testname,'.nc'];
            end
        end
            nc = netcdf(filename);
        ...
        for i_dir = 1:length(direction_list);
            ...
            for i_3d = 1:length(varis_3d);
                ...
                eval([var_3d, '_all(mi, :, :) = nc{', quot, var_3d, quot, '}(1,:,:,:);'])
            end
            for i_2d = 1:length(varis_2d);
                var_2d = [varis_2d{i_2d}, '_', direction];
                eval([var_2d, '_all(mi, :) = nc{', quot, var_2d, quot, '}(1,:,:);'])
            end
        end
        close(nc);
    end
