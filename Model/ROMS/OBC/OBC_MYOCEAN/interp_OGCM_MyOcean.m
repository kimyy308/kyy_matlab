...
nc_t = netcdf([OGCM_dir,OGCM_prefix,num2str(year,'%04i'),num2str(month,'%02i'),'.nc']);
nc_s = netcdf([OGCM_dir,OGCM_prefix,num2str(year,'%04i'),num2str(month,'%02i'),'.nc']);
nc_u = netcdf([OGCM_dir,OGCM_prefix,num2str(year,'%04i'),num2str(month,'%02i'),'.nc']);
nc_v = netcdf([OGCM_dir,OGCM_prefix,num2str(year,'%04i'),num2str(month,'%02i'),'.nc']);
nc_z = netcdf([OGCM_dir,OGCM_prefix,num2str(year,'%04i'),num2str(month,'%02i'),'.nc']);
...
zeta_scale_factor = 0.000305185094475746;
zeta_add_offset = 0.;
zeta = ext_data_OGCM_MyOcean(nc_z,lonZ,latZ,'zos',tin,lon,lat,1,missvalue,Roa,interp_method) ...
    * zeta_scale_factor + zeta_add_offset;
...
for k = 1:NZ
    ...
    u_scale_factor = 0.000610370188951492;
    v_scale_factor = 0.000610370188951492;
    t_scale_factor = 0.000732444226741791;
    s_scale_factor = 0.00152592547237873;
    u_add_offset = 0.;
    v_add_offset = 0.;
    t_add_offset = 21.;
    s_add_offset = -0.00152592547237873;
    
    u2d = ext_data_OGCM_MyOcean(nc_u,lonU,latU,'uo',tin,lon,lat,...
        k,missvalue,Roa,interp_method) * u_scale_factor + u_add_offset; 
    v2d = ext_data_OGCM_MyOcean(nc_v,lonV,latV,'vo',tin,lon,lat,...
        k,missvalue,Roa,interp_method) * v_scale_factor + v_add_offset;
    u(k,:,:) = rho2u_2d(u2d.*cosa+v2d.*sina);
    v(k,:,:) = rho2v_2d(v2d.*cosa-u2d.*sina);
    temp(k,:,:) = ext_data_OGCM_MyOcean(nc_t,lonT,latT,'thetao',tin,lon,lat,...
        k,missvalue,Roa,interp_method) * t_scale_factor + t_add_offset; %% Celcius degree
    salt(k,:,:) = ext_data_OGCM_MyOcean(nc_s,lonT,latT,'so',tin,lon,lat,...
        k,missvalue,Roa,interp_method) * s_scale_factor + s_add_offset; 
end
