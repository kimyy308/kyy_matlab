...
for tyear=2014:2018
    ...
    if exist('gl')~=1
        gl = grd_ROMS_SS('L_model', tyear);         % (1) Get Larger grid information
    end

    if exist('gn')~=1
        gn = grd_ROMS_SS('N_model', tyear);          % (2) Get Nested grid information
    end


    % (3) roms data file from a larger domain 
    filedir = ['E:\Data\Model\ROMS\nwp_1_10\test06\DA\',num2str(tyear,'%04i'),'\'];
    ...
    ref_year = tyear ;  %% Model data is 1993, then input 1992

    % (5) output file, boundary file for the nested grid
    out_head = 'E:\Data\Model\ROMS\ysecs\input\';
    ...
    out_file = [out_head,'bry_ysecs_',num2str(tyear,'%04i'),foot];
    ...
    donuts = 0;
    nc_create_roms_bndy_nest(out_file,gn.grd_file,'ysecs',[1 1 1 1],...
                            gn.theta_s,gn.theta_b,gn.hc,gn.N,...
                            time,cycle_length,0)  % create an nc file, write variables and close it.
    ...
    fm=0;
    tempyear=tyear;
    for f=start_file:end_file  % month
        fm=fm+1;
        ...
        elseif (file_type == 'year_mon_number')
            ...
            infile=[filedir, head, num2str(tempyear,'%04i'), '_', num2str(conv_mon,'%02i'),foot];
        end
    ...
    %                     zzr(1:nl,kk)=z0r+zeta(jj,ii).*(1.0 + z0r./gl.h(jj,ii));
                        zzr(1:nl,kk)=zlevs(gl.Vtransform,gl.Vstretching,gl.h(jj,ii),zeta(jj,ii),gl.theta_s,gl.theta_b,gl.hc,gl.N,'r');
    ...
    %                 izzr(1:nn)=iz0r+izeta.*(1.0 + iz0r./gn.h(j,i));
                    izzr(1:nn)=zlevs(gn.Vtransform,gn.Vstretching,gn.h(j,i),izeta,gn.theta_s,gn.theta_b,gn.hc,gn.N,'r');
