...
for year = 2014:2018
    for month = 1:12
        for varind=1:length(var)
            tempyear=year;
            tempmonth=month;
            bryname=['E:\Data\Model\ROMS\ysecs\input\bry_ysecs_', num2str(tempyear,'%04i'),'.nc'];
            ...
            title([varname, ',', num2str(tempyear,'%04i'), ',', calendar{tempmonth}]);
            ...
            saveas(gcf, ['D:\OneDrive - 서울대학교\MEPL\project\MICT_pollack\4th_year\Figure\YSECS\south_', ...
                varname,'_',num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i')], 'tif');
            
            ylim([-200 0])   
            saveas(gcf, ['D:\OneDrive - 서울대학교\MEPL\project\MICT_pollack\4th_year\Figure\YSECS\south_', ...
                varname,'_200_',num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i')], 'tif');
            close all;
        end
    end
end
