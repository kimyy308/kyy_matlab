...
nc_t = netcdf([OGCM_dir,OGCM_prefix,'_temp_',num2str(year),'.nc']);
nc_s = netcdf([OGCM_dir,OGCM_prefix,'_so_',num2str(year),'.nc']);
nc_u = netcdf([OGCM_dir,OGCM_prefix,'_uo_',num2str(year),'.nc']);
nc_v = netcdf([OGCM_dir,OGCM_prefix,'_vo_',num2str(year),'.nc']);
nc_z = netcdf([OGCM_dir,OGCM_prefix,'_zos_',num2str(year),'.nc']);
...
%
zeta = ext_data_OGCM_CMIP5(nc_z,lonZ,latZ,'zos',tin,lon,lat,1,missvalue,Roa,interp_method);
...
for k = 1:NZ
    ...
    u2d = ext_data_OGCM_CMIP5(nc_u,lonU,latU,'uo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    v2d = ext_data_OGCM_CMIP5(nc_v,lonV,latV,'vo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    u(k,:,:) = rho2u_2d(u2d.*cosa+v2d.*sina);
    v(k,:,:) = rho2v_2d(v2d.*cosa-u2d.*sina);
    temp(k,:,:) = ext_data_OGCM_CMIP5(nc_t,lonT,latT,'temp',tin,lon,lat,...
        k,missvalue,Roa,interp_method) - 273.15; %% Kelvin -> Celcius
    salt(k,:,:) = ext_data_OGCM_CMIP5(nc_s,lonT,latT,'salt',tin,lon,lat,...
        k,missvalue,Roa,interp_method); 
end
