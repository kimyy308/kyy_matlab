function interp_OGCM_SODA3_meanssh(OGCM_dir,OGCM_prefix,year,Ymin, Ymax, month,interp_method,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Updated 05-Apr-2018 by Y.Y.Kim
...
for tempY=Ymin : Ymax
    for tempM=1:12
        nc_ssh = netcdf([OGCM_dir,OGCM_prefix,num2str(tempY),'.nc']);
        zeta_temp = ext_data_OGCM_SODA3(nc_ssh,lonT,latT,'ssh',tempM,lon,lat,1,missvalue,Roa,interp_method);    
        if (exist('zeta')==0)
            zeta=zeros(size(zeta_temp));        
        end
        zeta=zeta+zeta_temp;
        close(nc_ssh)
    end
end
zeta=zeta/((Ymax-Ymin+1)*12);
%
