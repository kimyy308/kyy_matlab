...
% % Updated 24-Sep-2020 by Y.Y.Kim (roms)
...
% testnames ={'test61', 'test62'};

% % CMIP5 Model name
% model_name='ens10';
%model_names = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR'};
% model_names = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR'};
...
for model_names_index=1:length(model_names)
    ...
    for yind=1:length(year)
        ...
        elseif (strcmp(system_name,'GLNXA64'))
            bryname = ['/data1/kimyy/Model/ROMS/nwp_1_10/forcing_matlab/OBC/',scenario_name,'/',model_name,'/roms_bndy_',casename,'_',model_name,'_',num2str(yyyy), '_',testname,'.nc'];
        end
        ...
        for mi = mm(1):mm(end);
            ...
            elseif (strcmp(system_name,'GLNXA64'))
                filename = ['/data1/kimyy/Model/ROMS/nwp_1_10/forcing_matlab/OBC/',scenario_name,'/',model_name,'/',model_name,'_Y', num2str(yyyy,4), 'M', num2str(mi), '_',testname,'.nc'];
            end
                nc = netcdf(filename);
