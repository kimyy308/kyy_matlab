function interp_OGCM_CMIP5(OGCM_dir,OGCM_prefix,year,month,Roa,interp_method,...
...
nc_t = netcdf([OGCM_dir,OGCM_prefix,'_ocean_thetao_',num2str(year),'_v2.nc']);
nc_s = netcdf([OGCM_dir,OGCM_prefix,'_ocean_so_',num2str(year),'_v2.nc']);
nc_u = netcdf([OGCM_dir,OGCM_prefix,'_ocean_uo_',num2str(year),'_v2.nc']);
nc_v = netcdf([OGCM_dir,OGCM_prefix,'_ocean_vo_',num2str(year),'_v2.nc']);
nc_z = netcdf([OGCM_dir,OGCM_prefix,'_zos_',num2str(year),'_v2.nc']);
...
% % % nonsteric sealevel
% zeta = ext_data_OGCM_CMIP5_total(nc_z,lonZ,latZ,'zos',tin,lon,lat,1,missvalue,Roa,interp_method);

% % nonsteric + steric sealevel
zeta = ext_data_OGCM_CMIP5_total(nc_z,lonZ,latZ,'zostoga',tin,lon,lat,1,missvalue,Roa,interp_method);
...
for k = 1:NZ
    ...
    temp(k,:,:) = ext_data_OGCM_CMIP5_total(nc_t,lonT,latT,'temp',tin,lon,lat,...
        k,missvalue,Roa,interp_method) - 273.15; %% Kelvin -> Celcius
    salt(k,:,:) = ext_data_OGCM_CMIP5_total(nc_s,lonT,latT,'salt',tin,lon,lat,...
        k,missvalue,Roa,interp_method); 
end
