...
for yind=1:length(year)
    ...
    elseif (linux==1)
        bryname = ['/data1/kimyy/Model/ROMS/roms_nwp/nwp_1_40/forcing_matlab/OBC/SODA_3_4_2/','roms_bndy_',casename,'_SODA_3_4_2_',num2str(yyyy), '_',testname,'.nc'];
    end
    ...
    for mi = mm(1):mm(end);
        ...
        elseif (linux==1)
            filename = ['/data1/kimyy/Model/ROMS/roms_nwp/nwp_1_40/forcing_matlab/OBC/SODA_3_4_2/','SODA_Y', num2str(yyyy,4), 'M', num2str(mi), '_',testname,'.nc'];
        end
            nc = netcdf(filename);
