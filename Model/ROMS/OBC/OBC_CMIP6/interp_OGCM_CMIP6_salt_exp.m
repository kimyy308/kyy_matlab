function interp_OGCM_CMIP6(OGCM_dir,OGCM_prefix,year,month,interp_method,...
    lonU,latU,lonV,latV,lonT,latT,lonZ, latZ, Z,tin,...
     nc_bry,lon,lat,angle,h,tout, obc,Vtransform,Vstretching,model_name,scenario_name, ensemble_name, model_name_s, ensemble_name_s)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%
...
% Updated 16-Jun-2021 by Y-Y. Kim, getting misvalue from FillValue of attribute
...
nc_s = netcdf([OGCM_sdir,'so_',middle_name_s,'_',num2str(year,'%04i'),'.nc']);
...
% missvalue = nc_z{'zos'}._FillValue(:);
% missvalue = NaN;
missvalue=ncreadatt([OGCM_zdir,'zos_',middle_name,'_',num2str(year,'%04i'),'.nc'], 'zos', '_FillValue');
...
% % % nonsteric sealevel
% ssh_correct_val = northwestern pacific mean sealevel (x=115:164, y=15:52, 1985.01)
switch model_name
    case('CMCC-CM2-HR4')
...
zeta = ext_data_OGCM_CMIP6(nc_z,lonZ,latZ,'zos',tin,lon,lat,1,missvalue,Roa,interp_method) - ssh_correct_val;
% zeta = ext_data_OGCM_CMIP6(nc_z,lonZ,latZ,'zos',tin,lon,lat,1,missvalue,Roa,interp_method);

% pcolor(zeta); shading flat; colorbar
...
%     k
    ...
    u2d = ext_data_OGCM_CMIP6(nc_u,lonU,latU,'uo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
    v2d = ext_data_OGCM_CMIP6(nc_v,lonV,latV,'vo',tin,lon,lat,...
        k,missvalue,Roa,interp_method);
...
%         temp(k,:,:) = ext_data_OGCM_CMIP6(nc_t,lonT,latT,'thetao',tin,lon,lat,...
%            k,missvalue,Roa,interp_method) - 273.15; %% Kelvin -> Celcius
        temp(k,:,:) = ext_data_OGCM_CMIP6(nc_t,lonT,latT,'thetao',tin,lon,lat,...
           k,missvalue,Roa,interp_method); %% Celcius
%        pcolor(squeeze(temp(1,:,:))); shading flat; colorbar
        salt(k,:,:) = ext_data_OGCM_CMIP6(nc_s,lonT,latT,'so',tin,lon,lat,...
           k,missvalue,Roa,interp_method); 
    end
end
