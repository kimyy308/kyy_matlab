...
org_file='..\2008\monthly_mean\ocean_avg_mon_';
...
t_point=  3 ;  %test1
point_name={'part1', 'part2','part3'};
kts = [ 121.6 25.1 123.3 26.56 ...
        128.3 30.85 130.1 32.4 ...
        123.3 26.56 128.3 30.85 ];
...
% % point_name={'korea','kuroshio','tsugaru','soya'};
% kts = [128.883481062 35.2833167966 130.094555939 33.4263348468 ...
%     121.670863246  24.6796908592 123.743143796  23.5224419446 ...
%     140.106100315 41.5809032835 140.106100315 40.6658661097 ...
%     142.151473264  46.4790236999 142.043821201  45.1872096987];

% t_point= 1  ;  %test2
% point_name={'korea','ruku','Taiwan','kuroshio','tsugaru','soya'};
% kts = [130.150062895 33.717425942 128.910567456 34.9397165636];

% t_point= 1  ;  %test3 onshore
% point_name={'onshore'};
% kts = [121.6 25.1 130.1 32.4];
...
for st=1:t_point
    ...
    for yy = year_start : year_end ;
        ...
        for mm =  mon_start : mon_end
            ...
            %temp=nc{'salt'}(:); % salt(time, s_rho, eta_rho, xi_rho)
            close(nc)
            
            u = u.*mask3d_u;
            v = v.*mask3d_v;
            %temp = salt.*mask3d_rho; % zero for land, ** very important **
            ...
            %temp_u=0.5*(salt(:,:,1:L-1)+salt(:,:,2:L)); % each layer salt at u point
            %temp_u=0.5*(fresh(:,:,1:L-1)+fresh(:,:,2:L)); % each layer freshwater at u point
            ...
            for k=2:+1:N
                ...
                %temp_v=0.5*(salt(:,1:M-1,:)+salt(:,2:M,:)); % each layer salt at u point
                %temp_v=0.5*(fresh(:,1:M-1,:)+fresh(:,2:M,:)); % each layer freshwater at u point
...
trans=-trans;
outfile = ['roms_onshore_eachpart.txt'];
fid = fopen(outfile,'w+');
    fprintf(fid,'%%part1 part2 part3\n');
