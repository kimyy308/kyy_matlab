...
%       sum_fresh=0;
      ...
      for k=N:-1:lastlevel         % whole cell transport
         ...
         trans_section(month,k,index) = v(k,jj,i)*Hz_v(k,jj,i);
...
%          sum_temp=sum_temp+v(k,jj,i)*Hz_v(k,jj,i)*temp_v(k,jj,i);
      ...
      if (lastlevel == N)          % partial cell transport 
         ...
         trans_section(month,N,index) = v(N,jj,i)*partialH;
...
%          sum_temp=sum_temp+v(N,jj,i)*partialH*temp_v(N,jj,i);
      elseif (lastlevel ~= 1)         
         ...
         trans_section(month,k,index) = v(k-1,jj,i)*partialH;
...
%       sum_fresh=0;
      for k=N:-1:lastlevel         % whole cell transport
          ...
          trans_section(month,k,index) = u(k,j,ii)*Hz_u(k,j,ii);
...
%           sum_temp=sum_temp+u(k,j,ii)*Hz_u(k,j,ii)*temp_u(k,j,ii);
      ...
      if (lastlevel == N)          % partial cell transport 
           ...
           trans_section(month,N,index) = u(N,j,ii)*partialH;
...
%            sum_temp=sum_temp+u(N,j,ii)*partialH*temp_u(N,j,ii);
      elseif (lastlevel ~= 1)     
          ...
          sum=sum+u(k-1,j,ii)*partialH;
           trans_section(month,k,index) = u(k-1,j,ii)*partialH;
