...
%  Title 
%
    title='ROMS initial file from MyOcean analysis 1 day averaged result';



%  Data climatologies file names:
%
%    temp_daily_data : 5-day averaged MyOcean temperature
%    salt_daily_data : 5-day averaged MyOcean salinity
%

soda3_dir=['E:\Data\Reanalysis\MyOcean\'];
soda3_year=2019;
soda3_month=6;
soda3_day=12;

soda3_daily_data  = [soda3_dir,'global-analysis-forecast-phy-001-024_', num2str(soda3_year,'%04i'),num2str(soda3_month,'%02i'), num2str(soda3_day,'%02i'),'.nc']; 
...
disp(' Temperature...')
    ...
    ext_datas_ini_MyOcean(ininame,grdname,soda3_daily_data, ...
                'thetao','temp','r',tini);  %% for SODA 3.4.2 daily data

disp(' ')
disp(' Salinity...')

    ext_datas_ini_MyOcean(ininame,grdname,soda3_daily_data, ...
                 'so','salt','r',tini); %% for SODA 3.4.2 daily data

disp(' ')
disp(' Sea Surface Height...')

    ext_datas_ini_MyOcean(ininame,grdname,soda3_daily_data, ...
                'zos','zeta','r',tini);  %% for SODA 3.4.2 daily data
            
disp(' ')
disp(' U...')
    ext_datas_ini_MyOcean(ininame,grdname,soda3_daily_data, ...
                'uo','u','u',tini);  %% for SODA 3.4.2 daily data
            
disp(' ')
disp(' V...')
    ext_datas_ini_MyOcean(ininame,grdname,soda3_daily_data, ...
                'vo','v','v',tini);  %% for SODA 3.4.2 daily data
            
% Initial file
%
if (insitu2pot)  %% it doesn't run for MyOcean
  disp(' ')
