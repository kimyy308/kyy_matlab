...
season_particle=7;
    ...
    lv_u=squeeze(mean(comb_data_an.sustr(:,:,:,:),4));
    lv_u(lv_u==lv_u(1,13,110))=NaN; %% make land grids NaN;
    lv_u=permute(lv_u, [2,3,1]);
    [lv_u, error_status] = Func_0011_get_area_weighted_mean(lv_u, lon, lat);
    plot(lv_u)
    grid on
    ylim([-max(abs(lv_u)) max(abs(lv_u))])
    title('sustr clim')
    set(gca,'fontsize', 20)


    lv_u=squeeze(mean(comb_data_an.svstr(:,:,:,:),4));
    lv_u(lv_u==lv_u(1,13,110))=NaN; %% make land grids NaN;
    lv_u=permute(lv_u, [2,3,1]);
    [lv_u, error_status] = Func_0011_get_area_weighted_mean(lv_u, lon, lat);
    plot(lv_u)
