...
%             pmon=9;
            ...
            tmp.season=si(1);
            ax_s1=subplot(4,2,1);
            lv_l=squeeze(mean(comb_data_an_mov.(tmp.varn)(tmp.season,:,:,:),4));
            lv_l(lv_l==lv_l(13,110))=NaN; %% make land grids NaN;
            ...
            title([tmp.varn, ' mean, ', num2str(tmp.season), '-', num2str(tmp.season+2), 'M']);
%             mcax=max(abs(lv_l(:)));
%             caxis([-mcax mcax]);
            colormap(ax_s1, fig_cfg.c_map);
            colorbar;
            
            ax_s2=subplot(4,2,2);
            lv_l=squeeze(std(comb_data_an_mov.(tmp.varn)(tmp.season,:,:,:),1,4));
            lv_l(lv_l==lv_l(13,110))=NaN; %% make land grids NaN;
            m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
            m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
            m_grid;  
            m_gshhs_i('color',[1 1 1]);
            m_gshhs_i('patch',[0.7 0.7 0.7]);   % gray colored land
            title([tmp.varn, ' std, ', num2str(tmp.season), '-', num2str(tmp.season+2), 'M']);
            colormap(ax_s2, parula(20));
            colorbar;
...
%             caxis([-mcax mcax]);
            ...
            ax_s7=subplot(4,2,7);
            if si(4)>10
                lv_l=squeeze(mean(comb_data_an_mov.(tmp.varn)(tmp.season,:,:,1:end-1),4));
            else
                lv_l=squeeze(mean(comb_data_an_mov.(tmp.varn)(tmp.season,:,:,:),4));
            end
            lv_l(lv_l==lv_l(13,110))=NaN; %% make land grids NaN;
            ...
            m_gshhs_i('patch',[0.7 0.7 0.7]);   % gray colored land
            title([tmp.varn, ' mean, ', num2str(tmp.season), '-', num2str(tmp.season+2), 'M']);
            colormap(ax_s7, fig_cfg.c_map);
            colorbar;
            
            ax_s8=subplot(4,2,8);
            if si(4)>10
                lv_l=squeeze(std(comb_data_an_mov.(tmp.varn)(tmp.season,:,:,1:end-1),1,4));                
            else
                lv_l=squeeze(std(comb_data_an_mov.(tmp.varn)(tmp.season,:,:,:),1,4));
            end
            ...
            m_gshhs_i('patch',[0.7 0.7 0.7]);   % gray colored land
            title([tmp.varn, ' std, ', num2str(tmp.season), '-', num2str(tmp.season+2), 'M']);
            colormap(ax_s8,parula(20));
            colorbar;           
            
            dir.figtgdir=[dir.figdir, '/', 'mean_std'];
            mkdir(dir.figtgdir);
            
            cfg.figname=[dir.figtgdir, '/', 'm_std_',tmp.varn,'_season_',num2str(si), '_', str_d '.tif'];
            print(fig_h, cfg.figname, '-dpng');
            RemoveWhiteSpace([], 'file', cfg.figname);
    end
% end
...
%% basic figure
    ...
    m_gshhs_i('color',[1 1 1]);
    m_gshhs_i('patch',[0.7 0.7 0.7]);   % gray colored land
    title(['wstr', ' mean, ', num2str(tmp.season), '-', num2str(tmp.season+2), 'M']);
    ...
    cfg.figname=[dir.figtgdir, '/', 'm_','vec','_season_',num2str(si), '_', str_d '.tif'];
    print(fig_h, cfg.figname, '-dpng');
    RemoveWhiteSpace([], 'file', cfg.figname);
