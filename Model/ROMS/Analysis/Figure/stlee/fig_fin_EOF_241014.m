...
whos(fileInfo);
serialDate = (part_stlee.tt_t(end));      % Example serial date number
readableDate = datestr(serialDate);
disp(readableDate);       % Displays the converted date


part_stlee2.num_A_t=part_stlee.num_A_t(13:end);
part_stlee2.tt_t=part_stlee.tt_t(13:end);
tn=length(part_stlee2.num_A_t);
part_stlee2.num_A_t=reshape(part_stlee2.num_A_t, [12 tn/12]);
part_stlee2.tt_t=reshape(part_stlee2.tt_t, [12 tn/12]);
...
%% key figure (different aspect, stlee particle)
...
%% key figure (supplementary; wind 2mode, ssh 1mode)
            fntsize=15;
            close all;
            fig_cfg.fig_size=[0 0 14 8];
            fig_h = figure('name', 'EOF','PaperUnits','inches', ...
                    'PaperPosition',(fig_cfg.fig_size),'position', fig_cfg.fig_size*get(groot,'ScreenPixelsPerInch')+[200,200,0,0],'visible','off');
...
%             title([tmp.varn, ' lv, ', num2str(round(EOF_result_mov.(tmp.varn).var_exp(si,amod),2)), '%']);
            ...
            cfg.figname=[dir.figtgdir, '/', 'Fin_sup_figure_EOF_mov_', '_season_',num2str(si), '_', str_d, '_p_', num2str(pmon) '.tif'];
            print(fig_h, cfg.figname, '-dpng');
            RemoveWhiteSpace([], 'file', cfg.figname);
