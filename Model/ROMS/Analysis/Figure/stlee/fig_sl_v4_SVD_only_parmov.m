...
for lm=1:12
    for depthi=1:length(depths)
        ...
        [SVD_result.zeta.lv(lm,:,:,:), ...
            SVD_result.zeta.pcs(lm,:,:), ...
            SVD_result.svstr.lv(lm,:,:,:), ...
            SVD_result.svstr.pcs(lm,:,:), ...
            SVD_result.lambdas(lm,:), ...
            SVD_result.scf(lm,:)] = ...
            mca(double(squeeze(comb_data_an.zeta(lm,:,:,:)).*mask_model.*masks.(['d', str_d])), ...
            double(squeeze(comb_data_an.svstr(lm,:,:,:)).*mask_model.*masks.(['d', str_d])), ...
            tmp.svd_modes);

        %% movmean env
        if lm<=10
            tmp.left=double(squeeze(comb_data_an_mov.zeta(lm,:,:,:)).*mask_model.*masks.(['d', str_d]));
            tmp.left(tmp.left==tmp.left(16,109,1))=NaN;
            tmp.right=double(squeeze(comb_data_an_mov.svstr(lm,:,:,:)).*mask_model.*masks.(['d', str_d]));
            tmp.right(tmp.left==tmp.left(16,109,1))=NaN;
%             tmp.right=double(squeeze(comb_data_an_mov.zeta(lm,:,:,:)).*mask_model.*masks.(['d', str_d]));
%             tmp.right(tmp.right==tmp.right(16,109,1))=NaN;
%             tmp.left=double(squeeze(comb_data_an_mov.svstr(lm,:,:,:)).*mask_model.*masks.(['d', str_d]));
%             tmp.left(tmp.left==tmp.left(16,109,1))=NaN;

        [SVD_result_mov.zeta.lv(lm,:,:,:), ...
            SVD_result_mov.zeta.pcs(lm,:,:), ...
            SVD_result_mov.svstr.lv(lm,:,:,:), ...
            SVD_result_mov.svstr.pcs(lm,:,:), ...
            SVD_result_mov.lambdas(lm,:), ...
            SVD_result_mov.scf(lm,:)] = ...
            mca(tmp.left, tmp.right, tmp.svd_modes);

        else
            tmp.left=double(squeeze(comb_data_an_mov.zeta(lm,:,:,1:end-1)).*mask_model.*masks.(['d', str_d]));
            tmp.left(tmp.left==tmp.left(16,109,1))=NaN;
            tmp.right=double(squeeze(comb_data_an_mov.svstr(lm,:,:,1:end-1)).*mask_model.*masks.(['d', str_d]));
            tmp.right(tmp.left==tmp.left(16,109,1))=NaN;
%             tmp.right=double(squeeze(comb_data_an_mov.zeta(lm,:,:,1:end-1)).*mask_model.*masks.(['d', str_d]));
%             tmp.right(tmp.right==tmp.right(16,109,1))=NaN;
%             tmp.left=double(squeeze(comb_data_an_mov.svstr(lm,:,:,1:end-1)).*mask_model.*masks.(['d', str_d]));
%             tmp.left(tmp.left==tmp.left(16,109,1))=NaN;

            [SVD_result_mov.zeta.lv(lm,:,:,:), ...
            SVD_result_mov.zeta.pcs(lm,:,1:length(cfg.years)-1), ...
            SVD_result_mov.svstr.lv(lm,:,:,:), ...
            SVD_result_mov.svstr.pcs(lm,:,1:length(cfg.years)-1), ...
            SVD_result_mov.lambdas(lm,:), ...
            SVD_result_mov.scf(lm,:)] = ...
            mca(tmp.left, tmp.right, tmp.svd_modes);
            SVD_result_mov.zeta.pcs(lm,:,length(cfg.years))=NaN;
            SVD_result_mov.svstr.pcs(lm,:,length(cfg.years))=NaN;
        end
...
for pmon= [7, 6, 8]
    ...
    %             pmon=9;
        ...
        amod=1;
        subplot(4,4,1); % row column order
        lv_l=squeeze(SVD_result_mov.svstr.lv(si,:,:,amod));
        ...
        title(['svstr', ' lv, ', num2str(round(SVD_result_mov.scf(si,amod).*100,2)), '%']);
        mcax=max(abs(lv_l(:)));
        caxis([-mcax mcax]);
        colormap(fig_cfg.c_map);
        
        subplot(4,4,2);
        plot(cfg.years,squeeze(SVD_result_mov.svstr.pcs(si,amod,:)));
        tmp.corr1=corrcoef(part_tc, SVD_result_mov.svstr.pcs(si,amod,:), 'Rows', 'complete');
        tmp.corr2=corrcoef(part_wc, SVD_result_mov.svstr.pcs(si,amod,:), 'Rows', 'complete');
        tmp.corr3=corrcoef(part_ec, SVD_result_mov.svstr.pcs(si,amod,:), 'Rows', 'complete');
        title(['svstr', ' pcs, ', num2str(round(SVD_result_mov.scf(si,amod).*100,2)), '%, ', ...
        ...
        subplot(4,4,3); % row column order
        lv_l=squeeze(SVD_result_mov.zeta.lv(si,:,:,amod));
        m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
        % m_grid('fontsize', m_grid_fontsize-5, 'tickdir', m_grid_tickdir_type, 'xticklabels', [], 'xtick',(120:20:160), 'parent', ax1_1);  
        m_grid;  
        m_gshhs_i('color',[1 1 1]);
        m_gshhs_i('patch',[0.7 0.7 0.7]);   % gray colored land
        title(['zeta', ' lv, ', num2str(round(SVD_result_mov.scf(si,amod).*100,2)), '%']);
        mcax=max(abs(lv_l(:)));
        caxis([-mcax mcax]);
        colormap(fig_cfg.c_map);
        
        subplot(4,4,4);
        plot(cfg.years,squeeze(SVD_result_mov.zeta.pcs(si,amod,:)));
        tmp.corr1=corrcoef(part_tc, SVD_result_mov.zeta.pcs(si,amod,:), 'Rows', 'complete');
        tmp.corr2=corrcoef(part_wc, SVD_result_mov.zeta.pcs(si,amod,:), 'Rows', 'complete');
        tmp.corr3=corrcoef(part_ec, SVD_result_mov.zeta.pcs(si,amod,:), 'Rows', 'complete');
        title(['zeta', ' pcs, ', num2str(round(SVD_result_mov.scf(si,amod).*100,2)), '%, ', ...
            'r(tc):', num2str(round(tmp.corr1(1,2),2)), ...
            ', r(wc):', num2str(round(tmp.corr2(1,2),2)), ...
            ', r(ec):', num2str(round(tmp.corr3(1,2),2))]);
        grid on;
        ...
        amod=2;
        subplot(4,4,5); % row column order
        lv_l=squeeze(SVD_result_mov.svstr.lv(si,:,:,amod));
        m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
        % m_grid('fontsize', m_grid_fontsize-5, 'tickdir', m_grid_tickdir_type, 'xticklabels', [], 'xtick',(120:20:160), 'parent', ax1_1);  
        ...
        subplot(4,4,7); % row column order
        lv_l=squeeze(SVD_result_mov.zeta.lv(si,:,:,amod));
        m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
        % m_grid('fontsize', m_grid_fontsize-5, 'tickdir', m_grid_tickdir_type, 'xticklabels', [], 'xtick',(120:20:160), 'parent', ax1_1);  
        ...
        amod=3;
        subplot(4,4,9); % row column order
        lv_l=squeeze(SVD_result_mov.svstr.lv(si,:,:,amod));
        m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
        % m_grid('fontsize', m_grid_fontsize-5, 'tickdir', m_grid_tickdir_type, 'xticklabels', [], 'xtick',(120:20:160), 'parent', ax1_1);  
        ...
        subplot(4,4,11); % row column order
        lv_l=squeeze(SVD_result_mov.zeta.lv(si,:,:,amod));
        m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
        % m_grid('fontsize', m_grid_fontsize-5, 'tickdir', m_grid_tickdir_type, 'xticklabels', [], 'xtick',(120:20:160), 'parent', ax1_1);  
        ...
        amod=4;
        subplot(4,4,13); % row column order
        lv_l=squeeze(SVD_result_mov.svstr.lv(si,:,:,amod));
        m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
        % m_grid('fontsize', m_grid_fontsize-5, 'tickdir', m_grid_tickdir_type, 'xticklabels', [], 'xtick',(120:20:160), 'parent', ax1_1);  
        ...
        subplot(4,4,15); % row column order
        lv_l=squeeze(SVD_result_mov.zeta.lv(si,:,:,amod));
        m_proj('mercator','lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        m_pcolor(lon',lat',lv_l'); shading flat; colorbar;
        % m_grid('fontsize', m_grid_fontsize-5, 'tickdir', m_grid_tickdir_type, 'xticklabels', [], 'xtick',(120:20:160), 'parent', ax1_1);  
