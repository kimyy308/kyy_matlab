...
% all_region ={'NWP', 'AKP4'}
% all_region ={'ES_deep', 'SES_deep', 'NWES', 'NEES'};
all_region ={'NWES'};
...
%             figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\MICT_pollack\4th_year\figure\nwp_1_20\',testname,'\',regionname,'\'); % % where figure files will be saved
        ...
        % % %         flag configuration
        for folding=1:1
            fig_flags{1,1}='get model wind stress curl';
            fig_flags{2,1}='analysis of curl and mean energies';
...
% % %         get model data and cmemsstructed data      
        ...
        while (fig_flag)
            run(param_script);
            ind=1;
            matname = [savedir,testname,'_',regionname,'model_wind_curl_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat'];
                ...
                for yearij = 1:length(inputyear)
                    for monthij = 1:length(inputmonth)
                        ...
                        if (exist('lon')==0)
                            ...
                            angle = ncread(gridname, 'angle', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]) * 360.0/(2*pi); % radian -> degree
                            pm = ncread(gridname, 'pm', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
                            pn = ncread(gridname, 'pn', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
...
%                             shading flat;
                            ...
                            xlen=size(mask_rho,1);
                            ylen=size(mask_rho,2);
                        end

                        uwind_info = ncinfo(filename, 'Uwind');  %% [lon lat depth time] -> [1601 1201 33 1]

                        sustr = ncread(filename,'sustr',[lon_min(1) lat_min(1) 1], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
                        sustr=sustr.*mask_model;
                        svstr = ncread(filename,'svstr',[lon_min(1) lat_min(1) 1], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
                        svstr=svstr.*mask_model;
                        
                        sustr_proj = sustr .* cosd(angle) - svstr .* sind(angle);
                        svstr_proj = svstr .* cosd(angle) + sustr .* sind(angle);
                        xdist = 1./pm;
                        ydist = 1./pn;
                        ...
                        for i=1:xlen-2
                            for j=1:ylen-2
                                curl(i+1,j+1)= (svstr_proj(i+2,j+1)-svstr_proj(i,j+1)) / xdist(i+1,j+1)*2 + (sustr_proj(i+1,j+2)-sustr_proj(i+1,j)) / ydist(i+1,j+1)*2;
                            end
                        end
                        curl(1,2:ylen-2)=curl(2,2:ylen-2);
                        curl(xlen,2:ylen-2)=curl(xlen-1,2:ylen-2);
                        curl(2:xlen-2,1)=curl(2:xlen-2,2);
                        curl(2:xlen-2,ylen)=curl(2:xlen-2,ylen-1);
                        curl(1,1)=curl(2,2);
                        curl(1,ylen)=curl(2,ylen-1);
                        curl(xlen,1)=curl(xlen-1,2);
                        curl(xlen,ylen)=curl(xlen-1,ylen-1);
                        
%                         pcolor(lon,lat,sustr);
%                         shading flat;

                        len_lon_model = size(sustr,1);
                        len_lat_model = size(sustr,2);

                        if (exist('comb_spatial_meanmodel_sustr')==0)
                            comb_spatial_meanmodel_sustr=(zeros([len_lon_model,len_lat_model,12]));
                            comb_spatial_meanmodel_svstr=(zeros([len_lon_model,len_lat_model,12]));
                            comb_spatial_meanmodel_curl=(zeros([len_lon_model,len_lat_model,12]));
%                             del=(zeros([len_lon_model,len_lat_model,12]));
                        end
                        comb_sustr(:,:,ind) = single(sustr);
                        comb_svstr(:,:,ind) = single(svstr);
                        comb_curl(:,:,ind) = single(curl);
                        
                        mean_sustr(ind) = mean(sustr(:), 'omitnan');
                        mean_svstr(ind) = mean(svstr(:), 'omitnan');
                        mean_curl(ind) = mean(curl(:), 'omitnan');
                        
                        comb_spatial_meanmodel_sustr(:,:,monthij)=comb_spatial_meanmodel_sustr(:,:,monthij)+sustr/double(length(inputyear));
                        comb_spatial_meanmodel_svstr(:,:,monthij)=comb_spatial_meanmodel_svstr(:,:,monthij)+svstr/double(length(inputyear));
                        comb_spatial_meanmodel_curl(:,:,monthij)=comb_spatial_meanmodel_curl(:,:,monthij)+curl/double(length(inputyear));
...
%                         comb_spatial_meaninterped(:,:,monthij)=comb_spatial_meaninterped(:,:,monthij)+interped_data/double(length(inputyear));
                ...
                save(matname, 'mask_model', 'len_lon_model', 'len_lat_model', ...
                    'comb_spatial_meanmodel_sustr', 'comb_spatial_meanmodel_svstr', 'comb_spatial_meanmodel_curl', ...
                    'comb_sustr', 'comb_svstr', 'comb_curl',  ...
                    'mean_sustr', 'mean_svstr', 'mean_curl',  ...
