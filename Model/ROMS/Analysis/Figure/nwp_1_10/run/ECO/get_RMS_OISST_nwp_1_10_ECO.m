...
% % for ECO
regionname='ECO'
lonlat = [126.1 129.1 33.1 35.1];

figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\etc\eco\2019\figure\nwp_1_10\',regionname,'\'); % % where figure files will be saved
...
for yearij = 1:length(inputyear)
    for monthij = 1:length(inputmonth)
        ...
        if (exist('avhrr_lon')==0)
            ...
            avhrr_lon_min = find(avhrr_lon_west == min_avhrr_lon_west)-4;
            avhrr_lon_max = find(avhrr_lon_east == min_avhrr_lon_east)+4;
            avhrr_lat_min = find(avhrr_lat_south == min_avhrr_lat_south)-4;
            avhrr_lat_max = find(avhrr_lat_north == min_avhrr_lat_north)+4;
...
mslplot=plot(mean(comb_meanrms,1))
jpgname=strcat(outfile, '_', testname, '_climrms', '.jpg'); %% ~_year_month.jpg
xlabel('month')
ylabel('rms(^o)')
mrms=mean(comb_meanrms(:),'omitnan');
...
mslplot=plot(mean(comb_meanbias,1))
jpgname=strcat(outfile, '_', testname, '_climbias', '.jpg'); %% ~_year_month.jpg
xlabel('month')
ylabel('bias(^o)')
mbias=mean(comb_meanbias(:),'omitnan');
