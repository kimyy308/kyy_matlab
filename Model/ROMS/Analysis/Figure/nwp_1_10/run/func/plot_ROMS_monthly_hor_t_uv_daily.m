function status=plot_ROMS_monthly_hor_t_uv(testname, outfile, filedir, section, tempyear, inputmonth, inputday, shadlev, conlev, var,  param_script)
% clear all;close all;
%==========================================================================
...
for monthij=1:length(inputmonth)
    ...
    for dayij=1:length(inputday)
        ...
        if (tempmonth==1)
            filenum=num2str(tempday,'%04i');
        else
            filenum = num2str(eomday(tempyear, 1:tempmonth-1)+tempday, '%04i');
        end
        filename = strcat(filedir, ...
                    'ocean_avg_', filenum, '.nc');
    ...
    % % %     plot
        ...
        elseif (refdepth ==Nz)
            ...
            titlename = strcat('T. ', num2str(refdepth),'m (',num2str(tempday, '%02i'), 'd, ', char(calendarname(tempmonth)), ', ', num2str(tempyear),')');
        end
        ...
        jpgname=strcat(outfile, '_', testname, '_', num2str(tempyear,'%04i'), '_', num2str(tempmonth,'%02i'), '_', num2str(tempday,'%02i'), '.jpg'); %% ~_year_month.jpg
