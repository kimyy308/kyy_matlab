function status=plot_ROMS_monthly_surface_data_clim(testname, outfile, filedir, lonlat, inputyear, inputmonth, shadlev, conlev, var, param_script)
...
%         filename = strcat(filedir, num2str(inputyear,'%04i'), '\', ...
%                 testname, '_monthly_', num2str(inputyear,'%04i'), '_', ...
%                 num2str(tempmonth,'%02i'), '.nc');  ~ test37
        ...
        titlename = strcat(var, ' (',char(calendarname(tempmonth)), ', ', num2str(inputyear),')');
...
%         print(fig,jpgname,'-djpg');
        ...
        jpgname=strcat(outfile, '_', testname, '_', num2str(inputyear,'%04i'), '_', num2str(tempmonth,'%02i'), '.jpg'); %% ~_year_month.jpg
%         pause(2); %% prevent too short time between previous command and save command
        ...
        disp(' ')
        disp([num2str(inputyear), '_', num2str(tempmonth), '_', var, ' plot is created.'])
        disp(' ')
