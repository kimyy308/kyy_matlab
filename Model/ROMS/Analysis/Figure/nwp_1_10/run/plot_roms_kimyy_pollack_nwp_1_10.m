...
testname= 'test07'
year = [1993:2010]; % % put year which you want to plot [year year ...]
...
for i=1:length(year)   
    ...
    tempyear=year(i);
    outputdir = strcat(['E:\Data\Model\ROMS\nwp_1_10\',testname,'\run\',num2str(tempyear,'%04i'),'\']); % % where data files are
...
% 
    ...
    status=plot_ROMS_monthly_hor_t_uv(testname, hor_tuv_file, outputdir, [127 134 35 43 0 0], tempyear,month, [0 15], [-20 100], 'temp',  'fig_param_kyy_EKB_10');                                 
    
    hor_tuv_file =[figdir, 'EJS/EK/', 'hor_temp/', 'EK_hor_temp_0050m']; % % ~/SUV_EJS_testname_year_month.jpg
    status=plot_ROMS_monthly_hor_t_uv(testname, hor_tuv_file, outputdir, [127 134 35 43 -50 -50], tempyear,month, [0 15], [-20 100], 'temp',  'fig_param_kyy_EKB_10');                                 
end
