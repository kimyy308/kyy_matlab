...
%                 mean_later_data(mean_later_data==0)=NaN;
    ...
    cumsum_lon_sum_ref_data=cumsum(lon_sum_ref_data);
    cumsum_lon_sum_data=cumsum(lon_sum_data, 'omitnan');
...
%     lon_sum_data=sum(sum_data, 1, 'omitnan');
    ...
    xlabel('# of indivisuals')
    xlim([-1000 500])
%     ylim([37 41])
    set(gca, 'fontsize', m_pcolor_title_fontsize)
    titlename = strcat('meridional particle existence ratio, ',testname, ',(', ...
