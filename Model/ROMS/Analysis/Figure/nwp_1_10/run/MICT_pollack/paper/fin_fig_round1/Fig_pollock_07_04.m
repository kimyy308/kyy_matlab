...
figs.tifname = [dirs.figrawdir, 'Fig07_04.tif'];
...
RCM_info.months=[1,2];
RCM_info.temp_chekctime={'A', 'B', 'C', 'D', 'E', 'F'};
...
figs.fig_pos  = [0, ...
                 0, ...
                 figs.xlabel_width+figs.n_columns*(figs.xtick_width+figs.ax_width+figs.right_margin)+0.2, ...
                 figs.ylabel_height+figs.n_rows*(figs.ytick_height+figs.ax_height+figs.top_margin)]+0.3;
...
for mm = 1:figs.n_rows*figs.n_columns
    ...
    figs.subplot_bar_mer(mm).posAx = ...
        [figs.ax_bar_margin+ figs.ax_width+figs.xlabel_width+figs.xtick_width + (jj-1)*(figs.ax_width+figs.right_margin+figs.xtick_width), ...
        figs.ylabel_height+figs.ytick_height + (figs.n_rows-ii)*(figs.ax_height+figs.top_margin+figs.ytick_height), ...
        figs.ax_width_bar_mer, ...
        figs.ax_height_bar_mer];
    figs.subplot_bar_zonal(mm).posAx = ...
        [figs.xlabel_width+figs.xtick_width + (jj-1)*(figs.ax_width+figs.right_margin+figs.xtick_width), ...
        ...
        figs.ax_width_bar_zonal, ...
        figs.ax_height_bar_zonal];
    figs.subplot_bar_mer(mm).ax = axes(figs.fig, 'units', 'inches', 'position',figs.subplot_bar_mer(mm).posAx);
    figs.subplot_bar_zonal(mm).ax = axes(figs.fig, 'units', 'inches', 'position',figs.subplot_bar_zonal(mm).posAx);
    ...
    lonind=find(RCM_grid.lon_1d{mm}>=RCM_grid.domain(1) & RCM_grid.lon_1d{mm}<=RCM_grid.domain(2));
    figs.subplot_bar_zonal(mm).bar = bar(RCM_grid.lon_1d{mm}(lonind), RCM_data.lat_sum_data{mm}(lonind), 'parent', figs.subplot_bar_zonal(mm).ax);
...
%     grid(figs.subplot_bar_mer(mm).ax,'minor')
    ...
    figs.subplot_bar_zonal(mm).ax.FontSize = param.m_grid_fontsize-4;
    figs.subplot_bar_zonal(mm).ax.XTickLabel = [];
     if mod(mm,figs.n_columns)~=0
        figs.subplot_bar_zonal(mm).ax.YLim = [0 1500];
        figs.subplot_bar_zonal(mm).ax.YTick = [500 1000];
        figs.subplot_bar_zonal(mm).ax.YTickLabel = [500, 1000];
    end
    figs.subplot_bar_zonal(mm).ax.XMinorTick = 1;
    figs.subplot_bar_zonal(mm).ax.YMinorTick = 1;
    grid(figs.subplot_bar_zonal(mm).ax,'on')
end
