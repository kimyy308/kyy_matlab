...
tifname = [figrawdir, 'Fig09_02.tif'];
...
for yearij = 1:length(allyear)
    ...
    clear yearly_comb_mov_dist_lon_mean yearly_comb_mov_dist_lat_mean
...
mslplot{testnameind,2}=bar(yearly_mov_dist_lat_mean.*dist_value, 'FaceColor', [0.6 0.6 0.6], 'parent',ax{testnameind,1});
hold on
...
txt{testnameind,1}=text(480, -0.8.*dist_value, '(A) 15 days', 'FontSize', m_grid_fontsize); 
...
mslplot{testnameind,2}=bar(comb_mov_dist_lat_mean.*dist_value, 'FaceColor', [0.6 0.6 0.6], 'parent',ax{testnameind,1});
hold on
mslplot{testnameind,1}=plot(regime_ts_egg_mask.*dist_value,'k','parent',ax{testnameind,1});
ylabel(ax{testnameind,1},'Distance (km)')
...
% set(lgd,'Orientation','horizontal');
% set(lgd,'Location','Northwest');
txt{testnameind,1}=text(480, -0.8.*dist_value, '(B) 30 days', 'FontSize', m_grid_fontsize); 
