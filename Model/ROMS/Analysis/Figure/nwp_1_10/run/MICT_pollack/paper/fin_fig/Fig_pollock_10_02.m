...
clear wind_curl comb_wind_curl u_rho comb_u_rho v_rho comb_v_rho uwind comb_uwind vwind comb_vwind ...
    egg_mask comb_egg_mask sp_ground comb_sp_ground wind_curl2 comb_wind_curl2 temp_surf comb_temp_surf vel_mask ...
    comb_mov_dist_lon_mean comb_egg_mask
...
egg_half_1=mean(sum_ts_egg_mask(1:half_len), 'omitnan');
egg_half_2=mean(sum_ts_egg_mask(half_len+1:end), 'omitnan');
...
txt{testnameind,1}=text(30, 30, '(A) 15 days', 'FontSize', m_grid_fontsize); 
...
% set(lgd,'Orientation','horizontal');
% set(lgd,'Location','Northwest');
txt{testnameind,1}=text(30, 30, '(B) 30 days', 'FontSize', m_grid_fontsize); 
