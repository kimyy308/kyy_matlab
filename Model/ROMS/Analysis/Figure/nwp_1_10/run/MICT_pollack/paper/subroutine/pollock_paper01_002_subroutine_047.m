    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
        ...
        for yearij = 1:length(allyear)
            ...
            for monthij = 1:length(inputmonth)
                ...
                lastday_m=eomday(tempyear,tempmonth);
                disp([num2str(yearij), 'y_',num2str(monthij),'m'])  
                for dayij = 1:length(lastday_m)
                    tempday = dayij;
                    daystr=num2str(tempday, '%02i');
                    ncname = [savedir,yearstr, '\', '0030d_', yearstr, monthstr, daystr, '\output_0030d_',yearstr, monthstr, daystr,'.nc'];
                    lon_par=ncread(ncname, 'lon');
                    lon_par_1day=lon_par(:,49);
                    lon_par_tgtday=lon_par(:,49+diff_day*24);
                    lon_par_diff=lon_par_tgtday-lon_par_1day;
                    
                    lat_par=ncread(ncname, 'lat');
                    lat_par_1day=lat_par(:,49);
                    lat_par_tgtday=lat_par(:,49+diff_day*24);
                    for parij=1:length(lat_par_1day)
                        lon_par_dist(parij)=m_lldist([lon_par_1day(parij), lon_par_tgtday(parij)], [lat_par_1day(1), lat_par_1day(1)]);
                    end
                ...
                if(exist('curl_mask')==0)
                    ...
                    vel_polygon=[128, 37.5; 128, 39; 129.5, 39; 129.5, 37.5];
    ...
    %                 mslplot{2}=plot(comb_ocean_time,mean_ts_nwv, 'k','parent',axRH);
        ...
        mslplot{2}=plot(mean_ts_nwv, 'm','parent',axRH);
        ylabel(axLH,'# of eggs')
        ylabel(axRH,'Northwestward Velocity, m/s')
    ...
    %                 set(axRH,'color','none','yaxislocation','right','xtick', 1:60*length(inputmonth):length(comb_ocean_time),'xticklabel',datestr(comb_ocean_time(1:60*length(inputmonth):end), 'yyyy'),'position', ax_pos+[0 0.02 -0.01 -0.02]);
        ...
        title(['# of eggs', ',', 'NWV', ',', num2str(min(allyear),'%04i'),'-',num2str(max(allyear),'%04i')]);
    ...
    %                 datetick(axRH, 'x','yymm');
        ...
        lgd=legend([mslplot{1} mslplot{2}], '# of eggs', 'Northwest V');

        tempcorr=corrcoef(sum_ts_egg_mask, mean_ts_nwv);  %egg & nwv !!!
    %                 txt1=text(5, max(double(mean_ts_nwv))-diff(double([min(mean_ts_nwv), max(mean_ts_nwv)]))/32.0 ,['R = ', num2str(round(tempcorr(1,2),2))], 'FontSize', 20); 

        nan_sum_ts_egg_mask=sum_ts_egg_mask;
        nan_sum_ts_egg_mask(nan_sum_ts_egg_mask==0)=NaN;
        nan_mean_ts_nwv=mean_ts_nwv;
        nan_mean_ts_nwv(isnan(nan_sum_ts_egg_mask))=NaN;
        tempcorr2=corrcoef(nan_sum_ts_egg_mask(isfinite(nan_sum_ts_egg_mask)), nan_mean_ts_nwv(isfinite(nan_sum_ts_egg_mask)));

    %                 txt2=text(5, max(double(mean_ts_nwv))-diff(double([min(mean_ts_nwv), max(mean_ts_nwv)]))/8.0 ,['R = ', num2str(round(tempcorr2(1,2),2))], 'FontSize', 20); 
        ...
        nwv_half_1=mean(mean_ts_nwv(1:half_len));
        nwv_half_2=mean(mean_ts_nwv(half_len+1:end));
        txt2=text(5, max(double(mean_ts_nwv))-diff(double([min(mean_ts_nwv), max(mean_ts_nwv)]))/8.0 , ...
            ['nwv = ', num2str(round(nwv_half_1,2)), ' / ', num2str(round(nwv_half_2,2))], 'FontSize', 20); 
