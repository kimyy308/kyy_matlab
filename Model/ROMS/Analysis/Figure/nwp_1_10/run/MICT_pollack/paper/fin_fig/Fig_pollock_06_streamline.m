...
tifname = [figrawdir, 'Fig06_streamline.tif'];
...
clear u_early_RCM comb_u_early_RCM
for yearij = 1:length(earlyyear)
    ...
    for monthij = 1:length(inputmonth)
        ...
        u_early_RCM=ncread(ncname, 'u_rho');
        lastday_m=size(u_early_RCM,3);
        if (exist('comb_u_early_RCM')==0)
            comb_u_early_RCM=u_early_RCM;
        else
            comb_u_early_RCM(:,:,end+1:end+lastday_m)=u_early_RCM;
        end
    end
end
mean_u_early_RCM=mean(comb_u_early_RCM,3);

clear v_early_RCM comb_v_early_RCM
for yearij = 1:length(earlyyear)
    tempyear = earlyyear(yearij);
    for monthij = 1:length(inputmonth)
        tempmonth = inputmonth(monthij);
        ncname = [savedir,testname,'_',regionname,'model_pollock_',num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i'),'.nc'];
        disp([num2str(yearij), 'y_',num2str(monthij),'m'])
        v_early_RCM=ncread(ncname, 'v_rho');
        lastday_m=size(v_early_RCM,3);
        if (exist('comb_v_early_RCM')==0)
            comb_v_early_RCM=v_early_RCM;
        else
            comb_v_early_RCM(:,:,end+1:end+lastday_m)=v_early_RCM;
        end
    end
end
mean_v_early_RCM=mean(comb_v_early_RCM,3);
...
mean_u_early_RCM = mean_u_early_RCM .* mask_model;
mean_v_early_RCM = mean_v_early_RCM .* mask_model;
...
mean_u_early_RCM(ref_vec_x_range,ref_vec_y_range)=m_quiver_ref_u_value;
mean_v_early_RCM(ref_vec_x_range,ref_vec_y_range)=m_quiver_ref_v_value;
...
pc{testnameind,2}=m_quiver(lon_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
                    ...
                    mean_u_early_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
                    mean_v_early_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
...
m_text(m_quiver_ref_text_x_location, m_quiver_ref_text_y_location, m_quiver_ref_text, 'FontSize', m_quiver_ref_text_fontsize-8); 


figure;
% hlines=streamline(lon_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
%                     lat_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
%                     mean_u_early_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
%                     mean_v_early_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
%                     lon_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
%                     lat_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)');

% % % 
% % % % % %  using streamcolor
% % % mean_speed=sqrt(mean_u_early_RCM.^2+mean_v_early_RCM.^2);
% % % hold off
% % % m_quiver_x_interval=5;
% % % m_quiver_y_interval=5;
% % % hlines_col=Streamcolor(lon_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
% % %                     lat_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
% % %                     mean_u_early_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
% % %                     mean_v_early_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
% % %                     lon_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
% % %                     lat_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
% % %                     mean_speed(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)');
...
for yearij = 1:length(lateyear)
    ...
    for monthij = 1:length(inputmonth)
        ...
        ncname = [savedir,testname,'_',regionname,'model_pollock_',num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i'),'.nc'];
        disp([num2str(yearij), 'y_',num2str(monthij),'m'])
        u_late_RCM=ncread(ncname, 'u_rho');
...
for yearij = 1:length(lateyear)
    ...
    for monthij = 1:length(inputmonth)
        ...
        ncname = [savedir,testname,'_',regionname,'model_pollock_',num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i'),'.nc'];
        disp([num2str(yearij), 'y_',num2str(monthij),'m'])
        v_late_RCM=ncread(ncname, 'v_rho');
...
ref_vec_x_range = round(ref_vec_x_ind-(m_quiver_x_interval/2)) : round(ref_vec_x_ind-(m_quiver_x_interval/2))+m_quiver_x_interval/2+1;
ref_vec_y_range = round(ref_vec_y_ind-(m_quiver_y_interval/2)) : round(ref_vec_y_ind-(m_quiver_y_interval/2))+m_quiver_y_interval;
mean_u_late_RCM(ref_vec_x_range,ref_vec_y_range)=m_quiver_ref_u_value;
mean_v_late_RCM(ref_vec_x_range,ref_vec_y_range)=m_quiver_ref_v_value;
...
pc{testnameind,2}=m_quiver(lon_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
                    lat_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
                    mean_u_late_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
                    mean_v_late_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
                    'color',m_quiver_vector_color, 'AutoScale','off','LineWidth', m_quiver_LineWidth);
m_grid('fontsize', m_grid_fontsize, 'tickdir', m_grid_tickdir_type, 'box', m_grid_box_type,  ...
        'xticklabels', [129, 131], 'xtick',[129, 131], 'XaxisLocation', 'top',...
        'yticklabels', [], 'backcolor', 'none','parent', ax{testnameind,2});
...
mean_u_diff_RCM=mean_u_late_RCM-mean_u_early_RCM;
mean_v_diff_RCM=mean_v_late_RCM-mean_v_early_RCM;
mean_u_diff_RCM(ref_vec_x_range,ref_vec_y_range)=m_quiver_ref_u_value;
mean_v_diff_RCM(ref_vec_x_range,ref_vec_y_range)=m_quiver_ref_v_value;
...
pc{testnameind,2}=m_quiver(lon_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
                    lat_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)', ...
                    mean_u_diff_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
                    mean_v_diff_RCM(1:m_quiver_x_interval:end,1:m_quiver_y_interval:end)' * m_quiver_vector_size/1.5, ...
                    'color',m_quiver_vector_color, 'AutoScale','off','LineWidth', m_quiver_LineWidth);
m_grid('fontsize', m_grid_fontsize, 'tickdir', m_grid_tickdir_type, 'box', m_grid_box_type,  ...
        'xticklabels', [129, 131], 'xtick',[129, 131], 'XaxisLocation', 'top',...
        'yticklabels', [], 'backcolor', 'none','parent', ax{testnameind,2});
...
% for i=1:70
%     for j=1:88
%         tt_u(i,j)=ttest(comb_u_early_RCM(i,j,1:296), comb_u_late_RCM(i,j,1:296));
%         tt_v(i,j)=ttest(comb_v_early_RCM(i,j,1:296), comb_v_late_RCM(i,j,1:296));
%     end
% end
% pcolor(tt_u'); shading flat; colorbar;
% pcolor(tt_v'); shading flat; colorbar;
...
save ('D:\research\Ph_D_course\2021_Particle tracking of the walleye pollock\data_dryad\Data06_SUV.mat', ...
            'lon_RCM', 'lat_RCM', 'RCM_land', ...
            'mean_u_early_RCM', 'mean_v_early_RCM', ...
            'mean_u_late_RCM', 'mean_v_late_RCM', ...
            'mean_u_diff_RCM', 'mean_v_diff_RCM')
