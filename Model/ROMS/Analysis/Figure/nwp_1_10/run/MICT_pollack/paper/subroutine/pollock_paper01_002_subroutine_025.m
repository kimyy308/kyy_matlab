                ...
                for yearij = 1:length(allyear)
                    ...
                    for monthij = 1:length(inputmonth)
                        ...
                        if(exist('curl_mask')==0)
                            ...
                            curl_polygon=[127, 37.5; 127, 40.5; 130, 40.5; 130, 37.5];
                            curl_mask = double(inpolygon(lon_rho,lat_rho,curl_polygon(:,1),curl_polygon(:,2)));
                        end
...
%                 mslplot{2}=plot(comb_ocean_time,mean_ts_nwv, 'k','parent',axRH);
                mslplot{1}=plot(mean_ts_wind_curl,'k','parent',axLH);
                mslplot{2}=plot(mean_ts_nwv, 'm','parent',axRH);
                ylabel(axLH,'Wind Stress Curl, N/m^3')
...
%                 datetick(axRH, 'x','yymm');
                ...
                lgd=legend([mslplot{1} mslplot{2}], 'Wind stress curl', 'Northwest V');
                tempcorr=corrcoef(mean_ts_wind_curl, mean_ts_nwv);  
