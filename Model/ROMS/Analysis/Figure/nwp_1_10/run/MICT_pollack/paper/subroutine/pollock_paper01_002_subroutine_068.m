 if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
    ...
    dist_value = m_lldist([128 129], [39, 39]);
    ...
    axLH = gca;
    mslplot{2}=bar(comb_mov_dist_lon_mean.*dist_value, 'FaceColor', [0.6 0.6 0.6], 'parent',axLH);
    hold on
    mslplot{1}=plot(regime_ts_egg_mask.*dist_value,'k','parent',axLH);
    ylabel(axLH,'distance (km)')
    ...
    axis tight 
    ylim(axLH, [-0.5,3].*dist_value)
    set(axLH,'ycolor','k', 'box', 'off', 'FontSize',15);
    xlabel(axLH, 'Year');

    set(mslplot{1},'LineWidth',2);
    grid on
    lgd=legend([mslplot{1}], 'moved distance of individuals(lon)');
    set(lgd,'FontSize',15);
    set(lgd,'Position',[0.13 0.88, 0.775, 0.03]);
    set(lgd,'Orientation','horizontal');
    
    title(['longitudinal shift(',num2str(temp_checktime), 'd)',  ',', num2str(min(allyear),'%04i'),'-',num2str(max(allyear),'%04i'), ...
        ', ', num2str(min(inputmonth),'%02i'),'-', num2str(max(inputmonth),'%02i')]);
    
    half_len=length(comb_mov_dist_lon_mean)/2;
    egg_half_1=mean(comb_mov_dist_lon_mean(1:half_len), 'omitnan');
    egg_half_2=mean(comb_mov_dist_lon_mean(half_len+1:end), 'omitnan');
