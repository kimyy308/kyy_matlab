...
tifname = [figrawdir, 'Fig08_03.tif'];
...
barh(lat_1d, lon_sum_late_data_15d)
    ...
    ylim([36 41]); grid minor;
    set(ax{testnameind,1}, 'fontsize', m_grid_fontsize) 
txt{testnameind,1}=text(127.15, 36.5, '1988―1992', 'FontSize', m_grid_fontsize-2); 
txt{testnameind,1}=text(127.15, 37.1, '15 days', 'FontSize', m_grid_fontsize-2); 
...
txt{testnameind,1}=text(-872.85, 36.5, '(B)-(A)', 'FontSize', m_grid_fontsize-3); 
...
txt{testnameind,1}=text(-872.85, 36.5, '(E)-(D)', 'FontSize', m_grid_fontsize-3); 
