...
%                 mslplot{2}=plot(comb_ocean_time,mean_ts_nwv, 'k','parent',axRH);
                mslplot{1}=plot(-mean_ts_uwind,'k','parent',axLH);
...
%                 set(axRH,'color','none','yaxislocation','right','xtick', 1:60*length(inputmonth):length(comb_ocean_time),'xticklabel',datestr(comb_ocean_time(1:60*length(inputmonth):end), 'yyyy'),'position', ax_pos+[0 0.02 -0.01 -0.02]);
                ...
                set(axRH,'ycolor','m', 'box', 'off', 'FontSize',15);
                xlabel(axRH, 'Year');

                title(['Easterly ', ',', 'NWV', ',', num2str(min(allyear),'%04i'),'-',num2str(max(allyear),'%04i')]);
...
%                 datetick(axRH, 'x','yymm');
                ...
                lgd=legend([mslplot{1} mslplot{2}], 'Easterly', 'Northwest V');
                tempcorr=corrcoef(-mean_ts_uwind, mean_ts_nwv);  
...
%                 txt1=text(5, max(double(mean_ts_nwv))-diff(double([min(mean_ts_nwv), max(mean_ts_nwv)]))/32.0 ,['R = ', num2str(round(tempcorr(1,2),2))], 'FontSize', 20); 
                ...
                half_len=length(mean_ts_vwind)/2;
          
                wind_half_1=mean(-mean_ts_uwind(1:half_len));
                wind_half_2=mean(-mean_ts_uwind(half_len+1:end));
                txt1=text(5, max(double(mean_ts_nwv))-diff(double([min(mean_ts_nwv), max(mean_ts_nwv)]))/32.0 , ...
