    ...
    for yearij = 1:length(allyear)
        ...
        for monthij = 1:length(inputmonth)
            ...
            if(exist('curl_mask')==0)
                ...
                mean_diff_lon = mean(diff(lon_rho(:,1)));
                mean_diff_lat = mean(diff(lat_rho(1,:)));
%                 dA = (m_lldist([mean_diff_lon 0], [0 0])*1e3)^2 * cosd(lat_rho); %m^2
                dA = (m_lldist([mean_diff_lon 0], [0 0]))^2 * cosd(lat_rho); %km^2
...
%                 set(axRH,'ycolor','b', 'box', 'off', 'FontSize',15);
    ...
    title(['Spawning area (km^2)', ',', num2str(min(allyear),'%04i'),'-',num2str(max(allyear),'%04i'), ...
