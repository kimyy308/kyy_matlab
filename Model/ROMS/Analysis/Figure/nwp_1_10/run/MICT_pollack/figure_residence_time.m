...
for scaleind=1:length(scalename)
    ...
    for nmonth=1:length(totmonth)
        ...
        figname=['D:\OneDrive - 서울대학교\MEPL\project\MICT_pollack\3rd_year\figure\',testname,'\LTRANS\residence_time\', ...
            scalename{scaleind},'_',num2str(inputyear(1),'%04i'),'_',num2str(inputyear(end),'%04i'),'_', num2str(month,'%04i'),'.jpg'];
        resplot=plot(ftime, squeeze(comb_res_time(inputyear(1)-1982+1:inputyear(end)-1982+1,month)) ,'color','k','Marker','o','MarkerFaceColor','k','MarkerSize',4)
        xlabel('year')
...
%         yyaxis left
        resplot=plot(ftime, squeeze(mean(comb_res_time(inputyear(1)-1982+1:inputyear(end)-1982+1,:),2)) ,'color','k','Marker','o','MarkerFaceColor','k','MarkerSize',4)
        xlabel('year')
