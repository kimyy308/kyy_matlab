...
%     testname='avg_ens_10km_mean_monthly_'   % % need to change
    ...
    inputyear = [1982:2018]; % % put year which you want to plot [year year ...]
    ...
    switch(testname)
        ...
        %         avhrrdir='E:\Data\Observation\OISST\monthly\';
                filedir = strcat('E:\Data\Reanalysis\nwp_1_10_seo\avg_ens_10km_mean\'); % % where data files are
    ...
    %             mask_model = inpolygon(lon,lat,polygon_ind(:,1),polygon_ind(:,2));
            ...
            if (length(data_info.Dimensions)==4)
                ...
                data = ncread(filename,varname,[lon_min(1) lat_min(1) 20], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
            end
            data(data<-100)=NaN;
            data(data>100)=NaN;
            data(data==0)=NaN;
...
% %                 end
                ...
                if (exist('comb_spatial_meanrms')==0)
                    comb_spatial_meanrms=(zeros([length(avhrr_lon),length(avhrr_lat),12]));
...
%             end
    ...
    ncid = netcdf.create(strcat(filedir, testname,regionname,'_rms_clim_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc'),'NETCDF4');
    ...
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'type', ['NWP 1/10 Reanalysis_ ', testname, 'monthly SST RMS/BIAS file']);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
