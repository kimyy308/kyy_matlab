function status=plot_ROMS_monthly_surface_UV_data_2y_con(testname, outfile, filedir, section, tempyear, tempyear2, inputmonth, shadlev, conlev, var, param_script)
...
hc = ncread(filename,'hc')';
   ...
   for monthij = 1:length(inputmonth)
        ...
        filename = strcat(filedir, ...
                testname, '_monthly_', num2str(tempyear,'%04i'), '_', num2str(tempmonth,'%02i'), '.nc');
        filename2 = strcat(filedir,'\..\',num2str(tempyear2,'%04i'),'\', ...
    ...
    if (length(data_info.Dimensions)==4)
        ...
        data2 = ncread(filename2,varname,[lon_min(1) lat_min(1) data_info.Size(3) 1], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
%         u2 = ncread(filename2,'u',[lon_u_min(1) lat_u_min(1) data_info.Size(3) 1], [lon_u_max(1)-lon_u_min(1)+1 lat_u_max(1)-lat_u_min(1)+1 1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
%         v2 = ncread(filename2,'v',[lon_v_min(1) lat_v_min(1) data_info.Size(3) 1], [lon_v_max(1)-lon_v_min(1)+1 lat_v_max(1)-lat_v_min(1)+1 1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
    else
        ...
        data2 = ncread(filename,varname,[lon_min(1) lat_min(1) data_info.Size(3)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
%         u2 = ncread(filename,'u',[lon_u_min(1) lat_u_min(1) data_info.Size(3)], [lon_u_max(1)-lon_u_min(1)+1 lat_u_max(1)-lat_u_min(1)+1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
%         v2 = ncread(filename,'v',[lon_v_min(1) lat_v_min(1) data_info.Size(3)], [lon_v_max(1)-lon_v_min(1)+1 lat_v_max(1)-lat_v_min(1)+1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
    end
    data=squeeze(data);
    u=squeeze(u);
    v=squeeze(v);
...
%     v2=squeeze(v2);
    ...
    cut_data2 = permute(data2, [3 2 1]);  %% permute [x y z] -> [z y x]
...
%                         'color',m_quiver_vector_color, 'AutoScale','off','LineWidth', m_quiver_LineWidth);                    
    ...
    [C2,h22]=m_contour(cut_lon_rho,cut_lat_rho, cut_data2, conlev, 'LineColor', 'white', 'linewidth', m_contour_linewidth);
    clabel(C2,h22,'FontSize',m_contour_label_fontsize,'Color','w', ...
        'labelspacing',m_contour_labelspacing,'Rotation',m_contour_rotation,'fontweight',m_contour_fontweight);
