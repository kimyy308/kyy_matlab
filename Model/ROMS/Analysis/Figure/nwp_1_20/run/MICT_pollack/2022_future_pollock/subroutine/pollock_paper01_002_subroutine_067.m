...
%                 mean_data = sum(comb_egg_mask,3)/size(comb_egg_mask,3);
    ...
    EKB_lonlat= [127, 133, 36, 41];
    [indw, inde, inds, indn]=Func_0012_findind_Y(1/10,[127, 133, 36, 41],lon_rho,lat_rho); % southern EKB
    
    lon_rho = lon_rho(indw:inde, inds:indn);
    lat_rho = lat_rho(indw:inde, inds:indn);
    mean_data = mean_data(indw:inde, inds:indn);
    [mean_egg_mask, error_status] = Func_0011_get_area_weighted_mean(mean_data, lon_rho, lat_rho);
    
    testnameind=1;
    m_proj(m_proj_name,'lon',[EKB_lonlat(1) EKB_lonlat(2)],'lat',[EKB_lonlat(3) EKB_lonlat(4)]);
...
%                 set(ax{testnameind,1},'pos',pos_sb{testnameind});
    ...
    temp_surf=temp_surf(indw:inde, inds:indn);
