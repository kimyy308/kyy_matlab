...
% all_testname = {'test2127', 'test2128', 'test2129', 'test2130', 'test2131'};
% all_testname = {'test2117', 'test2118', 'test2119', 'test2120', 'test2121'};

all_testname = {'ens2203'};
...
%         inputyear = [2086]; % % put year which you want to plot [year year ...]
...
%                 tairname = [inputdir, 'SBC/old/nwp_1_10_', num2str(tempyear, '%04i'), '_', 'Tair.nc'];
                for monthij = 1:length(inputmonth)
                    ...
                    ncname = [savedir,testname,'_',tmp.regionname,'model_pollock_',num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i'),'.nc'];
...
%                                         '=ncread(ltransfilename, ', '''', 'lat', '''', ', [1, time_', checktime_str{checkti}, '], [inf 1]);']);
                                    ...
                                    ind_lon.(['d', checktime_str{checkti}])(1:length(lon_par.(['d', checktime_str{checkti}])))=NaN;
                                    ind_lat.(['d', checktime_str{checkti}])(1:length(lon_par.(['d', checktime_str{checkti}])))=NaN;
                                    
                                    for pari=1:length(lon_par.(['d', checktime_str{checkti}]))
                                        if lon_par.(['d', checktime_str{checkti}])(pari) > RCM_grid.domain(1) & ...
                                            lon_par.(['d', checktime_str{checkti}])(pari) < RCM_grid.domain(2) & ...
                                            lat_par.(['d', checktime_str{checkti}])(pari) > RCM_grid.domain(3) & ...
                                            lat_par.(['d', checktime_str{checkti}])(pari) < RCM_grid.domain(4)
                                                
%                                     eval(['for pari=1:length(lon_par_', checktime_str{checkti}, ')'])
%                                     eval(['if (lon_par_', checktime_str{checkti}, '(pari)> RCM_grid.domain(1) & ' ...
%                                         'lon_par_', checktime_str{checkti}, '(pari)', ' < RCM_grid.domain(2) & ' ...
%                                         'lat_par_', checktime_str{checkti}, '(pari)', ' > RCM_grid.domain(3) & ' ...
%                                         'lat_par_', checktime_str{checkti}, '(pari)', ' < RCM_grid.domain(4))']);
%                                     for lati=1:latcount
...
%                                         checktime_str{checkti}, '(:)==min(dist_lat', checktime_str{checkti},'(:)));']);
                                            dist_lon.(['d', checktime_str{checkti}])(1:loncount)=NaN;
                                            dist_lat.(['d', checktime_str{checkti}])(1:loncount)=NaN;
...
%                                             mov_dist_lon.(['d', checktime_str{checkti}])(mov_dist_lon.(['d', checktime_str{checkti}])==0)=NaN;
                            ...
                            idx_egg = find(-h >= -500 & -h <= -50 & temp_surf >= 2 & temp_surf <= 5);
                            egg_mask(idx_egg)=1;
...
%                             toc
                        ...
                        netcdf.defVarChunking(ncid, hvarid, 'CHUNKED', [loncount/10 latcount/10]);
                        netcdf.defVarDeflate(ncid, hvarid, true, true, 1);
...
%                         netcdf.putAtt(ncid,tairvarid,'units','^oC ');
                        ...
                        netcdf.defVarChunking(ncid, mask_parvarid, 'CHUNKED', [loncount/10 latcount/10 length(comb_time)/10, 1]);
                        netcdf.defVarDeflate(ncid, mask_parvarid, true, true, 1);
                        ...
                        netcdf.defVarChunking(ncid, mov_dist_lon_meanvarid, 'CHUNKED', [length(comb_time)/10, 1]);
                        netcdf.defVarDeflate(ncid, mov_dist_lon_meanvarid, true, true, 1);
                        ...
                        netcdf.defVarChunking(ncid, mov_dist_lat_meanvarid, 'CHUNKED', [length(comb_time)/10, 1]);
                        netcdf.defVarDeflate(ncid, mov_dist_lat_meanvarid, true, true, 1);
                        ...
                        netcdf.defVarChunking(ncid, mov_dist_lon_km_meanvarid, 'CHUNKED', [length(comb_time)/10, 1]);
                        netcdf.defVarDeflate(ncid, mov_dist_lon_km_meanvarid, true, true, 1);
                        ...
                        netcdf.defVarChunking(ncid, mov_dist_lat_km_meanvarid, 'CHUNKED', [length(comb_time)/10, 1]);
                        netcdf.defVarDeflate(ncid, mov_dist_lat_km_meanvarid, true, true, 1);
