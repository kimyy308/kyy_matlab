...
for obsi=1:size(OBS_data.cut_raw,3)/2
    tmp.ymean_obs = (OBS_data.cut_raw(:,:,2*obsi-1).*OBS_grid.mask_model + ...
        OBS_data.cut_raw(:,:,2*obsi).*OBS_grid.mask_model) / 2.0;
    RCM_all_data.('OISST').spa_yearly_mean(obsi) = ...
        Func_0011_get_area_weighted_mean(tmp.ymean_obs, ...
...
%% set figure file name
    tmp.tifname=strcat(dirs.figdir, 'all', '_yearly_ts_withOISST_', tmp.variable, '_', ...
        ...
        '_', RCM_info.season,'.tif'); %% ~_year_month.jpg
            ...
            %% OISST
            if max(RCM_info.years)<=2021
                mslplot2_all{testind+1}=plot(RCM_info.years,RCM_all_data.('OISST').spa_yearly_mean, 'k');
...
%             set(mslplot2_all{6},'Marker','pentagram');
            ...
            corrcoef(RCM_all_data.ens.spa_yearly_mean, RCM_all_data.('OISST').spa_yearly_mean)
