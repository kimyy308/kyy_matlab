% %  Updated    23-May-2022 by Yong-Yub Kim   % make
...
if testnameind==1
    ...
    for checkti=1:length(RCM_info.checktime)
        ...
        %% historical plot
        tmp.tifname=strcat(dirs.figdir_allmean, 'all_mean','_',tmp.regionname, '_loc_std_', num2str(tmp.checktime, '%02i'),'days', ...
        ...
        if (exist(tmp.tifname , 'file') ~= 2 || flags.fig_switch(flagi)==2)
            ...
            tmp.sp_median=median(tmp.mean_data(:), 'omitnan');
            tmp.sp_std=std(tmp.mean_data(:), 'omitnan');
            tmp.sp_std_mean=mean(tmp.std_data(:), 'omitnan');
            
            tmp.sp_quantile = quantile(tmp.mean_data(:), 0.90);
            tmp.agreement_data2=NaN(size(tmp.mean_data));
            tmp.agreement_data2(tmp.mean_data>tmp.sp_quantile)=1;
            
%             median(tmp.mean_data(:), 'omitnan')
%% sig_mask            
...
%             tmp.dot_lon_agr2=RCM_grid.lon_rho(tmp.agreement_data2==1);
%             tmp.dot_lat_agr2=RCM_grid.lat_rho(tmp.agreement_data2==1);
%             for agri=1:1:length(tmp.dot_lon_agr2)
%                 hold on
%                 tmp.dot=m_plot(tmp.dot_lon_agr2(agri), ...
%                 tmp.dot_lat_agr2(agri), ...
%                  'marker','x','color','b', ...
%                   'markerfacecolor', 'none', 'parent',ax{tmp.testnameind,2}, 'markersize', 1.5); 
%             end
...
%                   'markerfacecolor',bwrmap(colind,:), 'parent',ax{tmp.testnameind,2}); 
            ...
            tmp.titlename = strcat(num2str(tmp.checktime, '%02i'), 'd, egg std, ','allmean', ',(', ...
