...
%         yrmap = byrmap(129:256,:);
        [yrmap, error_status] = Func_0009_get_colormaps('yr', dropboxpath);        
...
% % %         switch region
        ...
        param_script =['C:\users\user/Dropbox/source/matlab/Model/ROMS/Analysis/Figure/nwp_1_20/run/fig_param/fig_param_kyy_', regionname, '.m'];
        figrawdir =strcat('D:\Research\Ph_D_course\2022_pollock_future\figure\', testname, '\'); % % where figure files will be saved
        filedir = strcat('D:\Data\Model\ROMS\nwp_1_20\', testname, '\pollock\'); % % where data files are
        savedir = strcat('D:\Data\Model\ROMS\nwp_1_20\', testname, '\pollock\');
        filedir_his = strcat('D:\Data\Model\ROMS\nwp_1_20\', testname_his, '\pollock\'); % % where data files are
        savedir_his = strcat('D:\Data\Model\ROMS\nwp_1_20\', testname_his, '\pollock\');
...
% % %       'temp diff plot between 80s and later'
        ...
        while (fig_flag)
            jpgname=strcat(clim_diff_outfile, '_', testname,'_',regionname, '_temp_diff_fut_his_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), 'y', ...
