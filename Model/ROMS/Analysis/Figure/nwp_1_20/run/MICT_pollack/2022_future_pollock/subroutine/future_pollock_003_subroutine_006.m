...
%             caxis([0, 1.0]);
       ...
       %% diff plot relative to spawning decrease
        ...
        %% diff plot (significant area(>34%) only
        tmp.tifname=strcat(dirs.figdir_allmean, 'all_mean','_',tmp.regionname, '_loc_num_', num2str(tmp.checktime, '%02i'),'days_',...
                    'diff_sigonly_', num2str(min(RCM_info.years_ssp),'%04i'),'_',num2str(max(RCM_info.years_ssp),'%04i'), 'y_', ...
                    num2str(min(RCM_info.years_his),'%04i'),'_',num2str(max(RCM_info.years_his),'%04i'), 'y_', ...
                    RCM_info.season, '.tif'); 
        ...
        if (exist(tmp.tifname , 'file') ~= 2 || flags.fig_switch(flagi)==2)
            ...
            tmp.his_sig_mask=tmp.mean_data_his;
            tmp.his_sig_mask(tmp.mean_data_ssp==0 & tmp.mean_data_his==0)=NaN;
            tmp.thres=prctile(tmp.his_sig_mask(isfinite(tmp.his_sig_mask)), 34); % <10%
            tmp.thres=median(tmp.his_sig_mask(:), 'omitnan');
%             tmp.thres=10; % <10%
            tmp.his_sig_mask(tmp.his_sig_mask<tmp.thres)=NaN;
            tmp.his_sig_mask(isfinite(tmp.his_sig_mask))=1;

%             pcolor(tmp.his_sig_mask'); shading flat; colorbar;
%             mean(tmp.his_sig_mask(:), 'omitnan');
%             median(tmp.his_sig_mask(:), 'omitnan');
%             pcolor(tmp.mean_data'); shading flat; colorbar;

        %% land fig    
            RCM_grid.mask_model2 = double(inpolygon(RCM_grid.lon_rho,RCM_grid.lat_rho,tmp.refpolygon(:,1),tmp.refpolygon(:,2)));
            RCM_grid.mask_model2(RCM_grid.mask_model2==0)=NaN;
            tmp.mean_data = tmp.mean_data .* RCM_grid.mask_model2 .* tmp.his_sig_mask;
       ...
       %% diff plot (%) sig only
        tmp.tifname=strcat(dirs.figdir_allmean, 'all_mean','_',tmp.regionname, '_loc_num_', num2str(tmp.checktime, '%02i'),'days_',...
                    'diff_percent_sigonly_', num2str(min(RCM_info.years_ssp),'%04i'),'_',num2str(max(RCM_info.years_ssp),'%04i'), 'y_', ...
                    num2str(min(RCM_info.years_his),'%04i'),'_',num2str(max(RCM_info.years_his),'%04i'), 'y_', ...
                    RCM_info.season, '.tif'); 
...
%             pcolor(tmp.mean_data'); shading flat; colorbar; colormap(jet); colormap(cmaps.bwr_10) caxis([-100 1])
%                 pcolor(tmp.mean_data'); shading flat; colorbar; colormap(cmaps.bwr_10); caxis([-100 100])
