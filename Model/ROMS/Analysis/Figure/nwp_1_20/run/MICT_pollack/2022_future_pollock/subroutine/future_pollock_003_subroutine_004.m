% %  Updated    08-May-2022 by Yong-Yub Kim   %


%% historical plot
tmp.tifname=strcat(dirs.figdir, tmp.testname_his,'_',tmp.regionname, 'ens_spawning_ground_probability_', ...
...
%% read grids, eggs    
    for yearij = 1:length(RCM_info.years_his)
        ...
        for monthij = 1:length(RCM_info.months)
            ...
            tmp.egg_mask=ncread(tmp.ncname, 'spawn_prob');
...
% % % %     less than 1% -> NaN
    tmp.mean_data(tmp.mean_data<1)=NaN;
...
%     caxis([0, 100]);
    ...
    tmp.titlename = strcat('ens sp prob, ',tmp.testname_his, ',(', ...
    ...
    spawn_prob=tmp.mean_data;
    tmp.matname=strcat(dirs.savedir_his, filesep, tmp.testname_his,'_',tmp.regionname, 'ens_spawning_ground_probability_', ...
            num2str(min(RCM_info.years_his),'%04i'),'_',num2str(max(RCM_info.years_his),'%04i'), 'y_', ...
            RCM_info.season, '.mat'); 
    save(tmp.matname, 'spawn_prob', 'RCM_grid');
    close all;
end
...
tmp.tifname=strcat(dirs.figdir, tmp.testname_ssp,'_',tmp.regionname, '_ens_spawning_ground_probability_', ...
...
%     caxis([0, 100]);
    ...
    tmp.titlename = strcat('ens sp prob, ',tmp.testname_ssp, ',(', ...
...
%     [indw, inde, inds, indn]=Func_0012_findind_Y(1/10,[127, 131, 37, 42],lon_rho,lat_rho);
    ...
    spawn_prob=tmp.mean_data;
    tmp.matname=strcat(dirs.savedir_ssp, filesep, tmp.testname_ssp,'_',tmp.regionname, 'ens_spawning_ground_probability_', ...
            num2str(min(RCM_info.years_ssp),'%04i'),'_',num2str(max(RCM_info.years_ssp),'%04i'), 'y_', ...
            RCM_info.season, '.mat'); 
    save(tmp.matname, 'spawn_prob', 'RCM_grid');
