...
if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
    ...
    for yearij = 1:length(refyear)
        ...
        for monthij = 1:length(inputmonth)
            ...
            ncname = [savedir_his,testname_his,'_',regionname,'model_pollock_',num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i'),'.nc'];
            disp([num2str(yearij), 'y_',num2str(monthij),'m'])
            ref_data=ncread(ncname, 'temp_surf');
...
%     mean_data_sDHB=mean_data;
%     mean_data_sDHB(lon_rho<=129.5)=NaN;
%     mean_data_sDHB(lat_rho>=41.0)=NaN;
%     mean_data_sDHB(lat_rho<=37.0)=NaN;
%     [mean_SST_SDHB, error_status] = Func_0011_get_area_weighted_mean(mean_data_sDHB, lon_rho, lat_rho);

    [mean_SST_diff, error_status] = Func_0011_get_area_weighted_mean(mean_data, lon_rho, lat_rho)

    pc{testnameind,2}=m_pcolor(lon_rho',lat_rho', mean_data','parent',ax{testnameind,2});
    colormap(ax{testnameind,2},yrmap);
    caxis([1, 6]);
    shading(gca,m_pcolor_shading_method);   
...
%     titlename = strcat('temp, ',testname, ',(fut-his', ...
...
%         num2str(min(inputmonth),'%02i'),'-', num2str(max(inputmonth),'%02i'), ')'); 
    ...
    disp(' ')
    disp([fig_flags{3,1}, ' plot is created.'])
    disp(' ')
