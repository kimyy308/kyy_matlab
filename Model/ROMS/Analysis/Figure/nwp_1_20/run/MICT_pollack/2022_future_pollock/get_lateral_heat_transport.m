...
%                                 RCM_grid.lat_rho(RCM_grid.lon_min(1):RCM_grid.lon_max(1)-1, RCM_grid.lat_min(1):RCM_grid.lat_max(1)-1);
                        ...
                        tmp.temp_ssp = ncread(tmp.filename_ssp, 'temp',[RCM_grid.lon_min(1) RCM_grid.lat_min(1) 1 1], ...
                            [RCM_grid.lon_max(1)-RCM_grid.lon_min(1)+1 RCM_grid.lat_max(1)-RCM_grid.lat_min(1)+1 1 1]);
                        ...
                        cut_mask=NaN(size(tmp.temp_ssp));
                        cut_mask(isfinite(tmp.temp_ssp))=1;
...
%                         tmp.unit_watt=1e+15; % param unit for watt
                        tmp.heat_trans_u_ssp= tmp.temp_ssp.*tmp.u_ssp.*(RCM_grid.depth(2)-RCM_grid.depth(1)) .*(tmp.c2).*cut_ydist; % Watt
                        tmp.heat_trans_v_ssp= tmp.temp_ssp.*tmp.v_ssp.*(RCM_grid.depth(2)-RCM_grid.depth(1)) .*(tmp.c2).*cut_xdist; % Watt
...
%                         pcolor(tmp.heat_trans_w_m2'); shading flat; colorbar; caxis([-2000 2000]);
                        ...
                        tmp.sum_seg_ssp(5)=sum(sum(tmp.heat_trans_w_ssp, 'omitnan'),'omitnan'); %vertical
                        tmp.sum_seg_ssp(6)=sum(sum(tmp.heat_trans_shflux_ssp, 'omitnan'),'omitnan'); %surface

                        
                        %% historical
                        
                        tmp.heat_trans_u_his= tmp.temp_his.*tmp.u_his.*(RCM_grid.depth(2)-RCM_grid.depth(1)) .*(tmp.c2).*cut_ydist; % Watt
                        tmp.heat_trans_v_his= tmp.temp_his.*tmp.v_his.*(RCM_grid.depth(2)-RCM_grid.depth(1)) .*(tmp.c2).*cut_xdist; % Watt
...
%                         pcolor(tmp.heat_trans_w_m2'); shading flat; colorbar; caxis([-2000 2000]);
                        ...
                        tmp.sum_seg_his(5)=sum(sum(tmp.heat_trans_w_his, 'omitnan'),'omitnan'); %vertical
