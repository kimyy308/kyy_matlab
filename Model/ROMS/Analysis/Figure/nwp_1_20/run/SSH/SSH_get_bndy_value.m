...
%     regionname=all_region{regionind}
    ...
    switch(varname)
        ...
        case('temp') %% North western Pacific
            ndim=4;
            data_units = 'Celsius'       
        case('salt') %% North western Pacific
            ndim=4;
            data_units = ' '
        otherwise
            ndim=4;
    ...
    for yearij = 1:length(inputyear)
        for monthij = 1:length(inputmonth)
            ...
            if (exist('lon')==0)
                ...
                if strcmp(varname,'u')
                    lon = ncread(filename,'lon_u',[1 1],[modelinfo.Dimensions(7).Length,1]);
                    lat = ncread(filename,'lat_u',[1 1],[1,modelinfo.Dimensions(6).Length]);
                elseif strcmp(varname,'v')
                    lon = ncread(filename,'lon_v',[1 1],[modelinfo.Dimensions(9).Length,1]);
                    lat = ncread(filename,'lat_v',[1 1],[1,modelinfo.Dimensions(8).Length]);
                else
                ...
                if strcmp(varname,'u')
                    lon = ncread(filename,'lon_u', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
                    lat = ncread(filename,'lat_u', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
                elseif strcmp(varname,'v')
                    lon = ncread(filename,'lon_v', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
                    lat = ncread(filename,'lat_v', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
                else
...
%     grid off
    ...
    if strcmp(varname,'u')
        xivarid=netcdf.defVar(ncid, 'xi_u', 'NC_DOUBLE', [xidimid]);
        netcdf.putAtt(ncid,xivarid,'long_name','xi_rho');

        etavarid=netcdf.defVar(ncid, 'eta_u', 'NC_DOUBLE', [etadimid]);
        netcdf.putAtt(ncid,etavarid,'long_name','eta_rho');

        s_rhovarid=netcdf.defVar(ncid, 's_u', 'NC_DOUBLE', [s_rhodimid]);
        netcdf.putAtt(ncid,s_rhovarid,'long_name','s_rho');

        lon_rhovarid=netcdf.defVar(ncid, 'lon_u', 'NC_DOUBLE', [xidimid etadimid]);
        netcdf.putAtt(ncid,lon_rhovarid,'long_name','lon_model');
        netcdf.putAtt(ncid,lon_rhovarid,'units','degree_east');

        lat_rhovarid=netcdf.defVar(ncid, 'lat_u', 'NC_DOUBLE', [xidimid etadimid]);
        netcdf.putAtt(ncid,lat_rhovarid,'long_name','lat_model');
        netcdf.putAtt(ncid,lat_rhovarid,'units','degree_north');
    elseif strcmp(varname,'v')
        xivarid=netcdf.defVar(ncid, 'xi_v', 'NC_DOUBLE', [xidimid]);
        netcdf.putAtt(ncid,xivarid,'long_name','xi_rho');

        etavarid=netcdf.defVar(ncid, 'eta_v', 'NC_DOUBLE', [etadimid]);
        netcdf.putAtt(ncid,etavarid,'long_name','eta_rho');

        s_rhovarid=netcdf.defVar(ncid, 's_v', 'NC_DOUBLE', [s_rhodimid]);
        netcdf.putAtt(ncid,s_rhovarid,'long_name','s_rho');

        lon_rhovarid=netcdf.defVar(ncid, 'lon_v', 'NC_DOUBLE', [xidimid etadimid]);
        netcdf.putAtt(ncid,lon_rhovarid,'long_name','lon_model');
        netcdf.putAtt(ncid,lon_rhovarid,'units','degree_east');

        lat_rhovarid=netcdf.defVar(ncid, 'lat_v', 'NC_DOUBLE', [xidimid etadimid]);
        netcdf.putAtt(ncid,lat_rhovarid,'long_name','lat_model');
        netcdf.putAtt(ncid,lat_rhovarid,'units','degree_north');
    else
