...
ind=1;
depth=NaN(306,240);
% point_MyOcean_data=NaN(306,240,1174);
comb_MyOcean_data=NaN(306,240,300); %
for yearij = 1:length(inputyear)
    for monthij = 1:length(inputmonth)
        ...
        if length(list_dep)==0
            ...
            point_temp2(i_dep+1:end)=point_temp2(i_dep);
            % ÇÏ³ªÇÏ³ª interpolation
            dep= 0:5:5730;
            interp_MyOcean_data=interp1(MyOcean_st(1:end),point_temp2(1:end),dep,'pchip');
            point_temp3 = interp_MyOcean_data;
            ii_dep=find(dep==end_depth);
            mean_temp=mean(point_temp3(1:ii_dep));
        end
        % point_MyOcean_data(aa,bb,:) = point_temp3;
        depth(aa,bb)=end_depth;
        comb_MyOcean_data(aa,bb,ind)=mean_temp;
            end
         end
        
        %   comb_MyOcean_data(:,:,ind,:) = point_MyOcean_data;
        %
...
%
%%
save([filedir,regionname,'Myocean_temp_1205','.mat'],'MyOcean_lon','MyOcean_lat','MyOcean_st','MyOcean_st2'...
    'comb_MyOcean_temp_divided','comb_MyOcean_data','depth','len_lon','len_lat','len_st','lonlat','varname',...
    'tt1','tt2');
% %
