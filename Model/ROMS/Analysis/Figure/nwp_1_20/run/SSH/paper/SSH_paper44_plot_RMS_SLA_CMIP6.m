...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
        filename = [filedir,testname,'_',regionname, ...
                    '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
...
% start-------------------- lowpass SLA RMS plot
        ...
        for nyeari=1:length(nyears)
            ...
            for nc_varnameij=1:length(nc_varname_prefixes)
                ...
                pngname=strcat(outfile, '_', testname,'_',regionname, '_sla_RMS_', nc_varname, '_',num2str(min(inputyear),'%04i'), ...
...
%                     eval(['m_pcolor(lon_cmems', '''', ',lat_cmems', '''',',squeeze(corr_', nc_varname, '(:,:)', '''', '));']);
    ...
    %                 caxis(colorbar_lev);
                    ...
                    matname = [filedir,'\',testname,'_',regionname, ...
...
% start-------------------- SLA_RMS plot
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_rms_', 'sla','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2)        
                run(param_script);
%                 for yearij=1:length(inputyear)
...
% start-------------------- SLA_filtered_RMS plot
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_rms_', 'sla','_filtered_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2)        
                run(param_script);
%                 for yearij=1:length(inputyear)
...
% start-------------------- SLA_RMS_seasonal plot
            ...
            for monthij=1:length(inputmonth)
                tempmonth=inputmonth(monthij);
                pngname=strcat(outfile, '_', testname,'_',regionname, '_',num2str(tempmonth, '%02i'),'_rms_', 'sla','_',num2str(min(inputyear),'%04i'), ...
                    '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
                if (exist(pngname , 'file') ~= 2)        
                    run(param_script);
...
% start-------------------- lowpass SLA RMS_seasonal plot
        ...
        for nyeari=1:length(nyears)
            ...
            for nc_varnameij=1:length(nc_varname_prefixes)
                ...
                for monthij=1:length(inputmonth)
                    tempmonth=inputmonth(monthij);
                    pngname=strcat(outfile, '_', testname,'_',regionname, '_', num2str(tempmonth, '%02i'), '_sla_RMS_', nc_varname, '_',num2str(min(inputyear),'%04i'), ...
                        '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
                    if (exist(pngname , 'file') ~= 2)
                    for yearij=1:length(inputyear)
                        tempyear=inputyear(yearij);
