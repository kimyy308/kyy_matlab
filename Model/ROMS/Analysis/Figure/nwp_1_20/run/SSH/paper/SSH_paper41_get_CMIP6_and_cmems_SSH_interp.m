...
% %  get reconstructed SSH. compare CMIP6 and reSSH. save. 
% all_region ={'AKP4'}
...
for testnameind=1:length(all_testname)
    for regionind=1:length(all_region)
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
            figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\SSH\4th_year\figure\CMIP6\',testname,'\',regionname,'\'); % % where figure files will be saved
        ...
        for yearij = 1:length(inputyear)
            for monthij = 1:length(inputmonth)
                ...
                filename = strcat(filedir, 'NWPCMIP6_ssh_trend_', num2str(inputyear(1),'%04i'), ...
                            '_',num2str(inputyear(end),'%04i'), '.nc');

                        
                % read CMIP6 data
                if (exist('lon')==0)
                    CMIP6info=ncinfo(filename);
%                     lon1= ncread(filename, 'lon');
%                     lat1= ncread(filename, 'lat');
%                     [lat, lon]=meshgrid(lat1, lon1);

                    if (exist('lon' , 'var') ~= 1)
                        lon = ncread(filename,'lon',[1, 1],[inf, inf]);
                        lat = ncread(filename,'lat',[1, 1],[inf, inf]);
                        [lon_min, lon_max, lat_min, lat_max] = findind_Y(1/20, lonlat(1:4), lon, lat, 1);
                    ...
                    switch(regionname)
                        case('NWP') %% North western Pacific
                            mask_CMIP6(1:size(lon,1),1:size(lon,2))=1;
                        otherwise
                            mask_CMIP6 = double(inpolygon(lon,lat,refpolygon(:,1),refpolygon(:,2)));
                            mask_CMIP6(mask_CMIP6==0)=NaN;
                    end
                end

                data_info = ncinfo(filename, 'CMIP6_ssh');  %% [lon lat depth time] -> [1601 1201 33 1]

                data = ncread(filename,'CMIP6_ssh',[lon_min(1) lat_min(1) (yearij-1)*12+monthij], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
                data=data.*mask_CMIP6;

                len_lon_CMIP6 = size(data,1);
                len_lat_CMIP6 = size(data,2);
                len_lon=len_lon_CMIP6;
                len_lat=len_lat_CMIP6;
                if (exist('comb_spatial_meanCMIP6')==0)
                    comb_spatial_meanCMIP6=(zeros([len_lon_CMIP6,len_lat_CMIP6,12]));
                end
                
                comb_data(:,:,ind) = single(data);

                comb_spatial_meanCMIP6(:,:,monthij)=comb_spatial_meanCMIP6(:,:,monthij)+data/double(length(inputyear));
...
%                     comb_spatial_meanCMIP6=(zeros([length(cmems_lon),length(cmems_lat),12]));
...
% % %         trend
        ...
        trend(1:len_lon_CMIP6,1:len_lat_CMIP6)=NaN;
        for i=1:len_lon_CMIP6
            for j=1:len_lat_CMIP6
...
% % %         trend (seasonal filtered)
        for t=1:length(inputyear)
            comb_data_filtered(:,:,(t-1)*12+1:(t-1)*12+12)=single(comb_data(:,:,(t-1)*12+1:(t-1)*12+12)-comb_spatial_meanCMIP6);
        end
        
        trend_filtered(1:len_lon_CMIP6,1:len_lat_CMIP6)=NaN;
        for i=1:len_lon_CMIP6
            for j=1:len_lat_CMIP6
...
% % %         climatological trend 
        comb_spatial_data=reshape(comb_data, [len_lon_CMIP6 len_lat_CMIP6 12 length(inputyear)]);
...
% % %   correlation coefficient between CMIP6 and cmems
        for i=1:len_lon
            for j=1:len_lat
...
% % %   correlation coefficient between CMIP6_filtered and cmems_filtered 
        for i=1:len_lon
            for j=1:len_lat
...
% % %   correlation coefficient between CMIP6_low_passed and cmems_low_passed
%         nyears=[2,3,5];
        for nyeari=1:length(nyears)
            nyear=nyears(nyeari);
            for i=1:len_lon
...
% % %   correlation coefficient between CMIP6_ssh_filtered_detrended and cmems_filtered_detrended
...
% %        2, 3, 5year detrended data lowpass filter 
        ...
        % % %   correlation coefficient between detrended CMIP6_low_passed and detrended cmems_low_passed
%         nyears=[2,3,5];
        for nyeari=1:length(nyears)
            nyear=nyears(nyeari);
            for i=1:len_lon
...
%         load([filedir,testname,'_',regionname,'ssh_trend_corr_coef_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
        ...
        lon_dimid = netcdf.defDim(ncid, 'lon', len_lon_CMIP6);
        lat_dimid = netcdf.defDim(ncid,'lat',len_lat_CMIP6);
        xidimid = netcdf.defDim(ncid, 'xi_rho', len_lon_CMIP6);
        etadimid = netcdf.defDim(ncid,'eta_rho',len_lat_CMIP6);
        ...
        netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
            'type', ['NWP 1/20 _ ', testname, 'CMIP6, recon monthly SSH analysis file']);
        netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        ...
        netcdf.putVar(ncid, lon_rhovarid, [0 0], [len_lon_CMIP6 len_lat_CMIP6], lon);
        netcdf.putVar(ncid, lat_rhovarid, [0 0], [len_lon_CMIP6 len_lat_CMIP6], lat);
        netcdf.putVar(ncid, raw_sshvarid, [0 0 0], [len_lon_CMIP6 len_lat_CMIP6 length(ftime)], comb_data);
        netcdf.putVar(ncid, ssh_filteredvarid, [0 0 0], [len_lon_CMIP6 len_lat_CMIP6 length(ftime)], comb_data_filtered);
        netcdf.putVar(ncid, trendvarid, [0 0], [len_lon_CMIP6, len_lat_CMIP6], trend);
        netcdf.putVar(ncid, trend_filteredvarid, [0 0], [len_lon_CMIP6, len_lat_CMIP6], trend_filtered);
        netcdf.putVar(ncid, mean_trendvarid, [0], [1], mean_trend);
        netcdf.putVar(ncid, mean_trend_filteredvarid, [0], [1], mean_trend_filtered);
        netcdf.putVar(ncid, clim_sshvarid, [0 0 0], [len_lon_CMIP6, len_lat_CMIP6 length(climtime)], comb_spatial_meanCMIP6);
        netcdf.putVar(ncid, clim_ssh_trendvarid, [0 0 0], [len_lon_CMIP6, len_lat_CMIP6 length(climtime)], trend_clim);
