...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
        ...
        load('C:\Users\kyy\Dropbox\source\matlab\Common\Figure\gmt_ocean_mod2.mat')  % % set colormap (gmt_ocean, nonwhite)
...
% start-------------------- BT plot
        for varind2=1:length(all_var2)
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_BT','.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2)        
                ...
                for yearij=1:length(inputyear2)
                    ...
                    for monthij=1:length(inputmonth)
                        ...
                        filename=[filedir, 'soda3.4.2_mn_ocean_reg_2017.nc'];
...
%                             lat_rho=ncread(filename, 'lat_rho');
                        ...
                        data = ncread(filename,'temp',[lon_min(1) lat_min(1) 1 tempmonth], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 inf 1]);  %% cut horizontal area [x,y,z] (wider than target area) 
                        numx=size(data,1);
                        numy=size(data,2);
                        for lonind=1:numx
                            for latind=1:numy
...
%                             lat_rho=ncread(filename, 'lat_rho');
                        ...
                        data = ncread(filename,'temp',[lon_min(1) lat_min(1) 1 tempmonth], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 inf 1]);  %% cut horizontal area [x,y,z] (wider than target area) 
                        st_ocean = ncread(filename, 'st_ocean');
                        numx=size(data,1);
                        numy=size(data,2);
                        for lonind=1:numx
                            for latind=1:numy
                                ...
                                botind=min(nanind)-1;
                                if (botind>0)
                                    bottomtemp(lonind,latind)=st_ocean(botind);
                                else
                                    bottomtemp(lonind,latind)=NaN;
                                end
                                clear nanind botind
