...
%     regionind2 = 1;
    ...
    for folding=1:1
            ...
            nc_varname_prefixes2={'interped'};
            nc_titlename_prefixes={'lowpass_corrected'};
            ...
            for nc_varnameij=1:length(nc_varname_prefixes)
                ...
                nc_varname2=[nc_varname_prefix2];
                eval(['corr=ncread(filename,','''','corr_', nc_varname2,'''',');']);
                corr=corr.*cmems_mask;
                mcorr(1,nc_varnameij)=mean(corr(:),'omitnan');
                eval(['corr=ncread(confilename,','''','corr_', 'corrected_interped','''',');']);
                corr=corr.*cmems_mask;
                mcorr_con_corrected(1,nc_varnameij)=mean(corr(:),'omitnan');
                eval(['corr=ncread(confilename,','''','corr_', nc_varname2,'''',');']);
                corr=corr.*cmems_mask;
                mcorr_con(1,nc_varnameij)=mean(corr(:),'omitnan');
                eval(['corr=ncread(sodafilename,','''','corr_', 'corrected_interped','''',');']);
                corr=corr.*cmems_mask;
                mcorr_soda_corrected(1,nc_varnameij)=mean(corr(:),'omitnan');
                eval(['corr=ncread(sodafilename,','''','corr_', nc_varname2,'''',');']);
                corr=corr.*cmems_mask;
            ...
            for nyeari=1:length(nyears)
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    ...
                    nc_varname_prefix2=nc_varname_prefixes2{nc_varnameij};
                    nc_varname2=[nc_varname_prefix2, '_', num2str(nyear),'y_lowpass'];
    ...
    %                     eval(['corr_',nc_varname, '=ncread(filename,','''','corr_', nc_varname,'''',');']);
                        ...
                        corr=corr.*cmems_mask;
                        mcorr_con_corrected(nyeari+1,nc_varnameij)=mean(corr(:),'omitnan');
                        eval(['corr=ncread(confilename,','''','corr_', nc_varname2,'''',');']);
                        corr=corr.*cmems_mask;
                        mcorr_con(nyeari+1,nc_varnameij)=mean(corr(:),'omitnan');
                        eval(['corr=ncread(sodafilename,','''','corr_', nc_varname,'''',');']);
                        corr=corr.*cmems_mask;
                        mcorr_soda_corrected(nyeari+1,nc_varnameij)=mean(corr(:),'omitnan');
                        eval(['corr=ncread(sodafilename,','''','corr_', nc_varname2,'''',');']);
                        corr=corr.*cmems_mask;
    ...
    % end-------------------- corr_ lowpassed get

    
    % % % start-------------------- correlation perfomance by lowpass filter window
    for folding=1:1
         jpgname=strcat(outfile, '_', testname, '_',regionname, 'rcm_glo_corr_corrected_lp_perfomance', '.jpg'); %% ~_year_month.jpg
    %         if (exist(jpgname , 'file') ~= 2)
            mslplot=plot(0:length(nyears),mcorr_soda(:,1),'b')
            hold on
            msl2plot=plot(0:length(nyears),mcorr_soda_corrected(:,1),'b')
            msl3plot=plot(0:length(nyears),mcorr_con(:,1),'g')
            msl4plot=plot(0:length(nyears),mcorr_con_corrected(:,1),'g')
    ...
    % % % end-------------------- correlation perfomance by lowpass filter
    % window RCM-GLO
    

    % % % start-------------------- correlation perfomance by lowpass
    % filter window RCM-SAT
    for folding=1:1
         jpgname=strcat(outfile, '_', testname, '_',regionname, 'rcm_sat_corr_corrected_lp_perfomance', '.jpg'); %% ~_year_month.jpg
    %         if (exist(jpgname , 'file') ~= 2)
            mslplot=plot(0:length(nyears),mcorr_con(:,1),'g')
            hold on
            msl2plot=plot(0:length(nyears),mcorr_con_corrected(:,1),'g')
            msl3plot=plot(0:length(nyears),mcorr(:,1),'r')
    ...
    %         datetick('x','yyyy','keepticks')
            ...
            set(gca,'FontSize',15);
            grid on
            lgd=legend('RCM-GLO','RCM-GLO-corrected', 'RCM-SAT');
            set(lgd,'FontSize',10);
            % set(lgd,'Position',[0.13 0.88, 0.775, 0.03]);
            set(gcf,'PaperPosition', [0 0 24 12]) 
            set(lgd,'Orientation','horizontal');
