...
for testnameind=1:length(all_testname)
    for regionind=1:length(all_region)
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
            figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\MICT_pollack\4th_year\figure\SODA\',testname,'\',regionname,'\'); % % where figure files will be saved
        ...
        for yearij = 1:length(inputyear)
            for monthij = 1:length(inputmonth)
                ...
                filename = strcat(filedir, 'NWP3soda_ssh_trend_', num2str(inputyear(1),'%04i'), ...
                            '_',num2str(inputyear(end),'%04i'), '.nc');
                sstfilename = strcat(filedir, 'NWP3soda_temp_trend_', num2str(inputyear(1),'%04i'), ...
                            '_',num2str(inputyear(end),'%04i'), '.nc');
...
%                     lat=lat(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
                ...
                data_info = ncinfo(filename, 'soda_ssh');  %% [lon lat depth time] -> [1601 1201 33 1]

                data = ncread(filename,'soda_ssh',[lon_min(1) lat_min(1) (yearij-1)*12+monthij], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
                data=data.*mask_soda;
...
% % %   correlation coefficient between model_low_passed and
% cmems_low_passed (corrected)
%         nyears=[2,3,5];
        for nyeari=1:length(nyears)
            nyear=nyears(nyeari);
            for i=1:len_lon
...
%         load([filedir,testname,'_',regionname,'ssh_trend_corr_coef_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
        ...
        nc_varname_prefixes={'cmems_', 'interped_', 'interped_sla_', 'corrected_interped_sla_', 'cmems_detrended_', 'interped_detrended_'};
        ...
        nc_varname_prefixes={'corr_interped_', 'corr_interped_detrended_', 'corr_corrected_interped_sla_'};
        for nc_varnameij=1:length(nc_varname_prefixes)
            nc_varname_prefix=nc_varname_prefixes{nc_varnameij};
            for nyeari=1:length(nyears)
                nyear=nyears(nyeari);
