...
%         cmems_msl=ncread(filename,'cmems_ssh');
        ...
        msldata{1}=ncread(filename,'raw_ssh');
        tr_model(1)=ncread(filename,'mean_trend');
...
%         std(mean(mean(comb_interped_data,'omitnan'),'omitnan'))
%         std(mean(mean(comb_cmems_data,'omitnan'),'omitnan'))
...
%         set(get(get(mslplot{5},'Annotation'),'LegendInformation'),'IconDisplayStyle','on');
        ...
        jpgname=strcat(outfile, '_', 'soda_model_all', '_',regionname, '_msl_nonfiltered_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
        xlabel('year')
        ylabel('Mean SSH (m)')
        title([regionname, ', Mean SSH(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),') '])
% %         num2str(round(mean_trend,2)), ' mm/y'
...
%         xlabel('year')
%         ylabel('Mean SSH (m)')
%         title([regionname ', Mean cmems SSH(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),'), ',num2str(round(mean_cmems_trend,2)), ' mm/y'])
...
%     %         end
%         end
%         %     plot(squeeze(mean(mean(comb_interped_data,1,'omitnan'),2,'omitnan')))
%         isize = size(comb_interped_data,1)
%         jsize = size(comb_interped_data,2)
%         lsize = size(comb_interped_data,3)
%         comb_yearly_interped_data=reshape(comb_interped_data,[isize, jsize, 12, lsize/12]);
%         mean_yearly_interped_data=squeeze(mean(mean(mean(comb_yearly_interped_data,1,'omitnan'),2,'omitnan'),3,'omitnan'));
%         trendtime=1:length(xData2);
%         p=polyfit(trendtime,mean_yearly_interped_data(1:length(xData2))',1);
%         yearly_interped_trend=p(1);
%         yearly_interped_trend = yearly_interped_trend * 1000.0; %% m/y -> mm/y
% 
%         yearly_msl=squeeze(mean(mean(mean(comb_yearly_interped_data,1,'omitnan'),2,'omitnan'),3,'omitnan'));
...
% 
% 
%         yearly_corrected_msl=squeeze(mean(mean(mean(comb_yearly_interped_data,1,'omitnan'),2,'omitnan'),3,'omitnan'));
...
%         comb_yearly_cmems_data=reshape(comb_cmems_data,[isize, jsize, 12, lsize/12]);
%         mean_yearly_cmems_data=squeeze(mean(mean(mean(comb_yearly_cmems_data,1,'omitnan'),2,'omitnan'),3,'omitnan'));
%         trendtime=1:length(xData2);
%         p=polyfit(trendtime,mean_yearly_cmems_data(1:length(xData2))',1);
%         yearly_cmems_trend=p(1);
%         yearly_cmems_trend = yearly_cmems_trend * 1000.0; %% m/y -> mm/y
% 
%         yearly_cmems_msl=squeeze(mean(mean(mean(comb_yearly_cmems_data,1,'omitnan'),2,'omitnan'),3,'omitnan'));
