...
for regionind=1:length(all_region)
    ...
    if (strcmp(system_name,'PCWIN64'))
        % % for windows
        figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\SSH\3rd_year\figure\CMEMS\',regionname,'\'); % % where figure files will be saved
...
%         inputmonth = 1:yeardays(inputyear(yearij));
        ...
        %         ncinfo('E:\Data\Observation\OIssh\monthly\cmems_monthly1983_11.nc');
            ...
            if tempmonth==1
                cmems_daily_err=ncread(cmemsfilename,'err',[cmems_lon_min(1) cmems_lat_min(1) 1], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) 31]);
                ...
                cmems_daily_err=ncread(cmemsfilename,'err',[cmems_lon_min(1) cmems_lat_min(1) sum(eomday(tempyear,1:tempmonth-1))+1], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) eomday(tempyear,tempmonth)]);                
            ...
            cmems_err = nanmean(cmems_daily_err,3);
...
%             cmems_data = ncread(cmemsfilename,varname,[cmems_lon_min(1) cmems_lat_min(1) tempmonth], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) 1]);
    ...
    for loni=1:size(cmems_data,1)
        for lati=1:size(cmems_data,2)
            cmems_sla_var(loni,lati)=var(comb_cmems_data(loni,lati,:));
        end
    end
    ...
    cmems_errvarid=netcdf.defVar(ncid, 'cmems_err', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,cmems_errvarid,'long_name','cmems_err');
    netcdf.putAtt(ncid,cmems_errvarid,'units','m ');
    netcdf.putAtt(ncid,cmems_errvarid,'source','Formal mapping error');
    ...
    cmems_sla_varvarid=netcdf.defVar(ncid, 'cmems_sla_var', 'NC_FLOAT', [lon_dimid lat_dimid]);
    netcdf.putAtt(ncid,cmems_sla_varvarid,'long_name','cmems_sla_var');
    netcdf.putAtt(ncid,cmems_sla_varvarid,'units','mm /year');
    ...
    netcdf.putVar(ncid, cmems_errvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_cmems_err);    
    ...
    netcdf.putVar(ncid, cmems_sla_varvarid, [0 0], [len_lon len_lat], cmems_sla_var);
