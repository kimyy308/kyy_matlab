...
close all;
clearvars '*' -except regionind2 testnameind2 all_region2 all_testname2 testname sodatestname
% % % 
% % % Read Model SST
% % % interp
% % % get RMS
...
for folding=1:1
        jpgname=strcat(outfile, '_', testname, '_',regionname, '_msl_yearly_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
%         if (exist(jpgname , 'file') ~= 2)
