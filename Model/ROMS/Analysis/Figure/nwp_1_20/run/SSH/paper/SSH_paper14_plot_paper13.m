...
%             msst_filt=msst_filt-mean(msst_filt);    
            p2=polyfit(1:length(msst_filt),msst_filt,1);
            p2=p2(1)*12.0;
            ...
            xlabel('year')
            ylabel('Mean SSH (m)')
            title([regionname, ', Mean SST(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),'), ',num2str(round(p2(1),3)), ' ^oC/y'])
            datetick('x','yyyy','keepticks')
            axis tight;
...
%             mean_sst_trend=mean(sst_trend(:), 'omitnan');
            p2=polyfit(1:length(msst),msst,1);
            p2=p2(1)*1000.0*12.0;
            ...
            ylabel('Mean SSH (m)')
            mean_trend=ncread(filename,'mean_trend');
            title([regionname, ', Mean SSH(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),'), ',num2str(round(p2(1),3)), ' ^oC/y'])
            datetick('x','yyyy','keepticks')
            axis tight;
            ylim(meansstlev)
            set(msstplot,'LineWidth',2);
...
corr2(m_interped_sla(logical(~isnan(m_interped_ssh).*~isnan(m_cmems_sla))), m_cmems_sla(logical(~isnan(m_interped_ssh).*~isnan(m_cmems_sla))))
