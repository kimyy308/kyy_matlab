    ...
    for regionind=1:length(all_region)
        ...
        for yearij = 1:length(inputyear)
            for monthij = 1:length(inputmonth)
                ...
                filename = strcat(filedir, 'NWP', testname, '_ssh_trend_', num2str(inputyear(1),'%04i'), ...
...
%                     lat=lat(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
                ...
                data_info = ncinfo(filename, [testname, '_ssh']);  %% [lon lat depth time] -> [1601 1201 33 1]

                data = ncread(filename,[testname, '_ssh'],[lon_min(1) lat_min(1) (yearij-1)*12+monthij], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
                data=data.*mask_soda;
                if readsst==1
