...
% all_testname2 = {'IPSL-CM6A-LR', 'MIROC6', 'NorESM2-LM'};
% all_testname2 = {'MPI-ESM1-2-LR'};
% all_testname2 = {'CNRM-CM6-1-HR','MPI-ESM1-2-HR'};
all_testname2 = {'MPI-ESM1-2-HR'};
% all_testname2 = {'CNRM-CM6-1-HR'};
...
%         sshdifflev = [40 70];
        ...
        inputyear1 = [1994:2014]; % % should put year from 1994 to 2017
        inputyear2 = [1994:2014]; % % should put year from 1994 to 2017
...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
        ...
        if (strcmp(system_name,'PCWIN64'))
            ...
            param_script =['C:\Users\kyy\Dropbox\source\matlab\Model\ROMS\Analysis\Figure\nwp_1_10\run\fig_param\fig_param_kyy_CMIP6_', regionname, '.m']
            CMIP6dir='E:\Data\Model\CMIP6\';
            filedir = strcat(CMIP6dir, varname, '\', scenname, '\Omon\', testname, '\'); % % where data files are        elseif (strcmp(system_name,'GLNXA64'))
            uodir = strcat(CMIP6dir, 'uo', '\', scenname, '\Omon\', testname, '\'); % % where data files are        elseif (strcmp(system_name,'GLNXA64'))
            vodir = strcat(CMIP6dir, 'vo', '\', scenname, '\Omon\', testname, '\'); % % where data files are        elseif (strcmp(system_name,'GLNXA64'))
        end
        variable='SSH';
        run(param_script);
        ...
        if (exist(pngname , 'file') ~= 2)        
            for yearij=1:length(inputyear2)
                ...
                for monthij=1:length(inputmonth)
                    ...
                    ufilename=[uodir, 'NWPCMIP6_uo_trend_1994_2014.nc'];
                    vfilename=[vodir, 'NWPCMIP6_vo_trend_1994_2014.nc'];
                    
                    if (exist('lon_rho' , 'var') ~= 1)
%                             lon= ncread(filename, 'lon');
%                             lat= ncread(filename, 'lat');
%                             [lat_rho, lon_rho]=meshgrid(lat,lon);
                        lon_rho = ncread(ufilename,'lon',[1, 1],[inf, inf]);
                        lat_rho = ncread(ufilename,'lat',[1, 1],[inf, inf]);
                    ...
                    data_info = ncinfo(ufilename, 'CMIP6_uo'); 
                    indij=(tempyear-1994)*12+tempmonth;
                    u = ncread(ufilename,'CMIP6_uo',[lon_min(1) lat_min(1) indij], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
                    v = ncread(vfilename,'CMIP6_vo',[lon_min(1) lat_min(1) indij], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
            ...
            if (exist(pngname , 'file') ~= 2)        
                ...
                for yearij=1:length(inputyear1)
                    ...
                    for monthij=1:length(inputmonth)
                        tempmonth=inputmonth(monthij);
                        monthstr=num2str(tempmonth, '%02i');
                        filename=[filedir, 'NWPCMIP6','_','ssh_trend_', num2str(min(inputyear1),'%04i'), '_', num2str(max(inputyear1),'%04i'),'.nc'];
                        CMIP6_varname=['CMIP6','_',varname];
%                         if (exist('lon_rho' , 'var') ~= 1)
...
%                             lat= ncread(filename, 'lat');
%                             [lat_rho, lon_rho]=meshgrid(lat,lon);
                            lon_rho = ncread(filename,'lon',[1, 1],[inf, inf]);
                            lat_rho = ncread(filename,'lat',[1, 1],[inf, inf]);
                            [lon_min, lon_max, lat_min, lat_max] = findind_Y(1/20, lonlat(1:4), lon_rho, lat_rho);
                            cut_lon_rho = lon_rho(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
                            cut_lat_rho = lat_rho(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
                        end
                        data_info = ncinfo(filename, CMIP6_varname); 
                        
                        indij=(tempyear-1994)*12+tempmonth;
                        if (strcmp(variable,'SST')==1 || strcmp(variable,'SSS')==1)
                            data = ncread(filename,CMIP6_varname,[lon_min(1) lat_min(1) indij], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);  %% cut horizontal area [x,y,z] (wider than target area) 
