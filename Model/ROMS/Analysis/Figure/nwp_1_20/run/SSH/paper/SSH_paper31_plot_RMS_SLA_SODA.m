...
%             'ssh_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
        filename = ['E:\Data\Reanalysis\SODA\', testname, '\',testname,'_',regionname, ...
...
%                     eval(['m_pcolor(lon_cmems', '''', ',lat_cmems', '''',',squeeze(corr_', nc_varname, '(:,:)', '''', '));']);
    ...
    %                 caxis(colorbar_lev);
                    ...
                    matname = ['E:\Data\Reanalysis\SODA\',testname,'\',testname,'_',regionname, ...
...
% start-------------------- SLA_RMS_corrected plot
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_rms_corrected_', 'sla','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2)        
                run(param_script);
