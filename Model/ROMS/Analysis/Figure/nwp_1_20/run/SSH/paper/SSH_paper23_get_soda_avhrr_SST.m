...
for testnameind=1:length(all_testname)
    for regionind=1:length(all_region)
        ...
        for yearij = 1:length(inputyear)
            for monthij = 1:length(inputmonth)
                ...
                filename = strcat(filedir, ...
                        'NWP3soda_temp_trend_', num2str(min(inputyear),'%04i'), '_', num2str(max(inputyear),'%04i'), '.nc');
                % read model data
                if (exist('lon')==0)
        ...
        %             mask_model = inpolygon(lon,lat,polygon_ind(:,1),polygon_ind(:,2));
                ...
                data_info = ncinfo(filename, sodavarname);  %% [lon lat depth time] -> [1601 1201 33 1]
                
                varind=(yearij-1)*12 + monthij;
                data = ncread(filename,sodavarname,[lon_min(1) lat_min(1) varind], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
