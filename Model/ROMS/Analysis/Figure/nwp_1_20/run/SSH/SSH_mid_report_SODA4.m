...
for regionind=1:length(all_region)
    ...
    dl=1/20;
    % for snu_desktop
    testname='SODA_3_4_2'   % % need to change
    ...
    if (strcmp(system_name,'PCWIN64'))
        ...
        sodadir='E:\Data\Observation\OIsst\monthly\';
    elseif (strcmp(system_name,'GLNXA64'))
    ...
    for yearij = 1:length(inputyear)
        ...
        %         ncinfo('E:\Data\Observation\OIsst\monthly\soda_monthly1983_11.nc');
            ...
            soda_data = ncread(sodafilename,varname,[soda_lon_min(1) soda_lat_min(1)  1 tempmonth], [soda_lon_max(1)-soda_lon_min(1) soda_lat_max(1)-soda_lat_min(1) 1 1]);
    ...
    for k=1:12
        ...
        for i=1:len_lon
            for j=1:len_lat
                ...
                clim_soda_trend_divided(i,j,k)=p(1) ;
            end
        end
    end
...
%     save([filedir,regionname,'soda_sst_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);

    ncid = netcdf.create(strcat(filedir,regionname,'soda_sst_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc'),'NETCDF4');
    ...
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'type', ['soda(OIsst) _ ', testname, 'monthly sst trend file']);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'title', [' soda sst trend (',num2str(inputyear(1)),'-',num2str(inputyear(end)),') ']);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'source', [' soda(OIsst) satellite data ' ]);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
    ...
    soda_sstvarid=netcdf.defVar(ncid, 'soda_sst', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,soda_sstvarid,'long_name','soda_sst');
    netcdf.putAtt(ncid,soda_sstvarid,'units','^o ');

    soda_sst_filteredvarid=netcdf.defVar(ncid, 'soda_sst_filtered', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,soda_sst_filteredvarid,'long_name','soda_sst_filtered');
    netcdf.putAtt(ncid,soda_sst_filteredvarid,'units','^o ');
    ...
    netcdf.putAtt(ncid,soda_trendvarid,'units','^o /year');
    ...
    netcdf.putAtt(ncid,soda_trend_filteredvarid,'units','^o /year');
    ...
    netcdf.putAtt(ncid,clim_soda_trend_dividedvarid,'units','^o /year');
    ...
    netcdf.putAtt(ncid,mean_soda_trendvarid,'units','^o /year');
    ...
    netcdf.putAtt(ncid,mean_soda_trend_filteredvarid,'units','^o /year');
    ...
    netcdf.putAtt(ncid,mean_clim_soda_trend_dividedvarid,'units','^o /year');

    clim_sodavarid=netcdf.defVar(ncid, 'clim_soda_sst', 'NC_FLOAT', [lon_dimid lat_dimid clim_time_dimid]);
    netcdf.putAtt(ncid,clim_sodavarid,'long_name','clim_soda_sst');
    netcdf.putAtt(ncid,clim_sodavarid,'units','^o ');
    ...
    netcdf.putVar(ncid, soda_sstvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_soda_data);
    netcdf.putVar(ncid, soda_sst_filteredvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_soda_data_filtered);
