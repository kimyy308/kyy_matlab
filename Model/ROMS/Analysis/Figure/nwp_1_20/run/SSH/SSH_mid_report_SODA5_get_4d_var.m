...
for regionind=1:length(all_region)
    ...
    save([filedir,regionname,'soda_',varname,'_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);

    ncid = netcdf.create(strcat(filedir,regionname,'soda_',varname,'_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc'),'NETCDF4');
    ...
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'type', ['soda _ ', testname, 'monthly ', varname ' trend file']);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'title', [' soda ', varname,' trend (',num2str(inputyear(1)),'-',num2str(inputyear(end)),') ']);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'source', [' soda data ' ]);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
    ...
    soda_sshvarid=netcdf.defVar(ncid, ['soda_', varname], 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,soda_sshvarid,'long_name', ['soda_', varname]);
    ...
    netcdf.putAtt(ncid,soda_ssh_filteredvarid,'long_name',['soda_',varname,'_filtered']);
    ...
    clim_sodavarid=netcdf.defVar(ncid, ['clim_soda_',varname], 'NC_FLOAT', [lon_dimid lat_dimid clim_time_dimid]);
