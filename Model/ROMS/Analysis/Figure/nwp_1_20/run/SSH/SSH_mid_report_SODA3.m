...
% all_region ={'NWP', 'NWP2', 'ES', 'SS', 'YS', 'ECS'}
all_region ={'GLO'}
...
% for regionind=1:length(all_region)
    ...
    inputyear = [1980:2009]; % % put year which you want to plot [year year ...]
...
%     lonlat(4)=max(refpolygon(:,2));
    ...
    elseif (strcmp(system_name,'GLNXA64'))
        ...
        sodadir='/data2/kimyy/Reanalysis/SODA/SODA_3_4_2/monthly/';
    end
...
% 
                soda_lon = ncread(sodafilename,'xt_ocean');
                soda_lat = ncread(sodafilename,'yt_ocean');
% 
...
%                 end
            ...
            soda_data = ncread(sodafilename,varname,[1 1 tempmonth], [len_lon len_lat 1]);
...
%             soda_data=soda_data.*mask_soda;
    ...
    save([filedir,regionname,'_soda_ssh_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
