...
inputyear = [2006:2080]; % % put year which you want to plot [year year ...]
...
jpgname=strcat(outfile, 'NIFS_', 'surface_salt_trend_', ...
    num2str(min(NIFS6.year(1,1,1,1,1)),'%04i'),'_',num2str(NIFS6.year(end,1,1,1,1),'%04i'), '.tif'); %% ~_year_month.jpg
...
%     std(std(NIFS6.salt_surf_comb_trend,'omitnan'),'omitnan')
    ...
    titlename = strcat('Salt trend, ','NIFS',',(',num2str(NIFS6.year(1,1,1,1,1),'%04i'),'-',num2str(NIFS6.year(end,1,1,1,1),'%04i'),'), ','M=',num2str(round(mean_trend,4)));  %% + glacier contribution
...
jpgname=strcat(outfile, 'NIFS_', '75m_salt_trend_',num2str(NIFS6.year(1,1,1,1,1),'%04i'),'_',num2str(NIFS6.year(end,1,1,1,1),'%04i'), '.tif'); %% ~_year_month.jpg
if (exist(jpgname , 'file') ~= 2)
    m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
    hold on;
    for i=1:size(NIFS6.salt,2)  %% line
...
for i =1:size(NIFS6.year,1) 
    tempyear=NIFS6.year(i,1,1,1);
    for month=1:6
        xData_NIFS((6*(i-1))+month) = datenum([num2str(tempyear),'-',num2str(month*2,'%02i'),'-15',]);
    end
end
...
for numline=1:size(NIFS6.line,2)
    if (isnan(sum(NIFS6.salt_surf_line_comb(:,numline),'omitnan'))==1)
        ...
        p=polyfit(xData_NIFS(~idx),NIFS6.salt_surf_line_comb(~idx,numline)',1);
        line_trend2=xData_NIFS*p(1)+p(2);
        msaltplot=plot(xData,MODEL6.salt_surf_line_comb(:,numline)','color','k');
        hold on
        msaltplot2=plot(xData_NIFS,NIFS6.salt_surf_line_comb(:,numline)','o','color','r','markerfacecolor','k','markersize',2);

%         msaltplot2=plot(xData,line_trend2,'Color','r')
        
        if (max(inputyear) <= NIFS6.year(end,1,1,1,1))
...
for numline=1:size(NIFS6.line,2)
    if (isnan(sum(NIFS6.salt75_line_comb(:,numline),'omitnan'))==1)
        ...
        p=polyfit(xData_NIFS(~idx),NIFS6.salt75_line_comb(~idx,numline)',1);
        line_trend2=xData_NIFS*p(1)+p(2);
        msaltplot=plot(xData,MODEL6.salt75_line_comb(:,numline)','color','k');
        hold on
        msaltplot2=plot(xData_NIFS,NIFS6.salt75_line_comb(:,numline)','o','color','r','markerfacecolor','k','markersize',2);
