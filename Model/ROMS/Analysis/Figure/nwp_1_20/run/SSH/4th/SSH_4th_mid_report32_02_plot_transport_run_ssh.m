...
[~, ~, raw] = xlsread('Z:\내 드라이브\Data\Observation\Transport_Korea\obs_tsushima_197001_201209_ORIGIN_old.xls','TWC','A2:H553');
...
year_obs_30y=obs_ks(find(obs_ks(:,1)<2006 & obs_ks(:,1)>1975),1);
month_obs_30y=obs_ks(find(obs_ks(:,1)<2006 & obs_ks(:,1)>1975),2);
tr_obs_30y=obs_ks(find(obs_ks(:,1)<2006 & obs_ks(:,1)>1975),5);
...
set(lgd,'FontSize',20, 'Location', 'northwest');
...
for i=1:length(year)
    figdir =strcat('Z:\내 드라이브\MEPL\project\SSH\4th_year\figure\nwp_1_20\',testname,'\transport\'); % % where figure files will be saved
    if (exist(strcat(figdir) , 'dir') ~= 7)
        mkdir(strcat(figdir));
    end 
    saveas(gcf,[figdir, 'ES_transp_',num2str(year(1)),'_',num2str(year(end)),'.png'],'png');
end
hold off;
close all;
...
title(['Model monthly mean EJS transports(',testname,')'],'fontsize',20);
xlabel('Time (Year)','color','k','FontSize',20,'fontweight','bold');
ylabel('Volume Transport (sv)','color','k','FontSize',20,'fontweight','bold');
