...
%         testname=all_testname2{testnameind2}    % % need to change
        ...
        msldata{1}=comb_interped_data;
        
        testname='test54';
        load(['G:\Data\Model\ROMS\nwp_1_20\',testname,'\run\',testname,'_',regionname, ...
            'ssh_recon_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
        msldata{2}=comb_interped_data;

        testname='test55';
        load(['G:\Data\Model\ROMS\nwp_1_20\',testname,'\run\',testname,'_',regionname, ...
            'ssh_recon_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
        msldata{3}=comb_interped_data;
        
        testname='test56';
        load(['G:\Data\Model\ROMS\nwp_1_20\',testname,'\run\',testname,'_',regionname, ...
            'ssh_recon_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
        msldata{4}=comb_interped_data;
        
        testname='ens03';
        load(['G:\Data\Model\ROMS\nwp_1_20\',testname,'\run\',testname,'_',regionname, ...
            'ssh_recon_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
        msldata{5}=comb_interped_data;
...
%             filedir = strcat('G:\Data\Model\ROMS\nwp_1_20\', testname, '\run\'); % % where data files are
        ...
        std(mean(mean(comb_recon_data,'omitnan'),'omitnan'))
        ...
        recon_msl=squeeze(mean(mean(comb_recon_data,1,'omitnan'),2,'omitnan'));
...
%         lgd=legend('Model Ensemble','Reconstructed SSH');
        for i=1:length(msldata)
            ...
            set(mslplot2{i}, 'Color', [0.8 0.8 0.8])
            set(get(get(mslplot{i},'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
        ...
        jpgname=strcat(outfile, '_', 'all', '_',regionname, '_msl_seasonal_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
        xlabel('year')
        ylabel('Mean SSH (m)')
...
% %         num2str(round(mean_trend_filtered,2)), ' mm/y'
        ...
        lgd=legend(['Model Ensemble : ', num2str(tr_ens),'mm/year'], ['Reconstructed SSH : ', num2str(tr_recon),'mm/year']);
...
%         comb_yearly_interped_data_filtered=reshape(comb_interped_data,[isize, jsize, 12, lsize/12]);
...
%         comb_yearly_recon_data_filtered=reshape(comb_recon_data,[isize, jsize, 12, lsize/12]);
