...
for testnameind=1:length(all_testname)
    for regionind=1:length(all_region)
        ...
        comb_recon_clim_divided=reshape(comb_recon_data,[len_lon, len_lat, 12, length(inputyear)]);
        ...
        clim_interped_trend_divided(1:len_lon,1:len_lat,1:12)=NaN;
        for k=1:12
            ...
            for i=1:len_lon
                for j=1:len_lat
                    ...
                    clim_interped_trend_divided(i,j,k)=p(1) * 1000.0 ;
                end
            end
        end
        
        clim_recon_trend_divided(1:len_lon,1:len_lat,1:12)=NaN;
        for k=1:12
            clim_trendtime(:,k)=inputyear(1)+(k-1)/12 : 1 : inputyear(end)+(k-1)/12;
            for i=1:len_lon
                for j=1:len_lat
        ...
        clim_model_trend_dividedvarid=netcdf.defVar(ncid, 'clim_model_trend_divided', 'NC_FLOAT', [lon_dimid lat_dimid clim_time_dimid]);
        netcdf.putAtt(ncid,clim_model_trend_dividedvarid,'long_name','clim_model_trend_divided');
        netcdf.putAtt(ncid,clim_model_trend_dividedvarid,'units','mm /year');
        ...
        netcdf.putVar(ncid, clim_model_trend_dividedvarid, [0 0 0], [len_lon len_lat length(climtime)], clim_interped_trend_divided);
