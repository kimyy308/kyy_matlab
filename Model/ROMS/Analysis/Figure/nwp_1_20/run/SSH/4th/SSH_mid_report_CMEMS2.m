...
%         'cmems_ssh_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
    ...
    if (strcmp(system_name,'PCWIN64'))
        % % for windows
        figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\SSH\5th_year\figure\CMEMS\',regionname,'\'); % % where figure files will be saved
        param_script ='C:\Users\kyy\Dropbox\source\matlab\Model\ROMS\Analysis\Figure\nwp_1_20\run\fig_param\fig_param_kyy_AKP4.m'
...
%     outfile = strcat(figdir,regionname);
    ...
    variable='SSH';
     pngname=strcat(outfile, '_', 'cmems','_',regionname, '_clim_', variable,'_',num2str(min(inputyear),'%04i'), ...
...
%             end
%         end
        mean_data=squeeze(mean(comb_cmems_adt,3,'omitnan'));
...
%                     mean_data=mean_data-min(mean_data(:));
        ...
        m_pcolor(double(cmems_lon),cmems_lat',mean_data');
