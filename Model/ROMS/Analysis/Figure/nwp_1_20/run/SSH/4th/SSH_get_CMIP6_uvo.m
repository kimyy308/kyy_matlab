...
for testnameind=1:length(all_testname)
    for regionind=1:length(all_region)
        ...
        elseif (strcmp(system_name,'GLNXA64'))
%             figrawdir =strcat('/home/kimyy/sst_project_source/fig/',testname,'/',regionname,'/'); % % where figure files will be saved
            param_script ='/home/kimyy/SSH_project_source/fig_param_kyy_EKB_RMS.m'
            CMIP6dir='/data1/CMIP/cmip6/historical_extHDD/';
            filedir = strcat(CMIP6dir, varname, '/historical/Omon/', testname, '/'); % % where data files are
...
%                     CMIP6_lat = ncread(CMIP6filename,lat_name, [CMIP6_lon_min(1), CMIP6_lat_min(1)], [CMIP6_lon_max(1)-CMIP6_lon_min(1), CMIP6_lat_max(1)-CMIP6_lat_min(1)]);
                ...
                if(ind_reverse_lon==0 && ind_reverse_lat==0)
                    CMIP6_data = ncread(CMIP6filename,varname,[CMIP6_lon_min(1) CMIP6_lat_min(1) 1 tind], [CMIP6_lon_max(1)-CMIP6_lon_min(1)+1 CMIP6_lat_max(1)-CMIP6_lat_min(1)+1 1 1]);
                else
                    CMIP6_data2 = ncread(CMIP6filename, varname, [1 1 1 tind], [inf inf 1 1]);
                    if (ind_reverse_lon==1)
                        for loni=1:size(CMIP6_lon,1)
                            CMIP6_data3(loni,:,1)=CMIP6_zos2(size(CMIP6_lon,1)-loni+1,:,1) ;
                        end
                        CMIP6_zos=CMIP6_data3(:,CMIP6_lat_min(1):CMIP6_lat_max(1),1);
                    end
...
%                             CMIP6_zos3(:,lati,1)=CMIP6_zos2(:,lati,1) ;
                            CMIP6_data3(:,lati,1)=CMIP6_data2(:,size(CMIP6_data2,2)- CMIP6_lat_min(1)-lati+1,1) ;
                        end
                        CMIP6_data=CMIP6_data3(CMIP6_lon_min(1):CMIP6_lon_max(1),:,1);
                    end
                    clear CMIP6_data2 CMIP6_data3
                end
...
%                 comb_CMIP6_zostoga(ind) = CMIP6_zostoga;
    ...
    %     end 
        ...
        ncid = netcdf.create(strcat(filedir,regionname,'CMIP6_',varname,'_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc'),'NETCDF4');
        ...
        CMIP6_sshvarid=netcdf.defVar(ncid, ['CMIP6_', varname], 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
        netcdf.putAtt(ncid,CMIP6_sshvarid,'long_name',['CMIP6_', varname]);
        netcdf.putAtt(ncid,CMIP6_sshvarid,'units','m/s ');

        CMIP6_ssh_filteredvarid=netcdf.defVar(ncid, ['CMIP6_',varname,'_filtered'], 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
        netcdf.putAtt(ncid,CMIP6_ssh_filteredvarid,'long_name',['CMIP6_', varname, '_filtered']);
        netcdf.putAtt(ncid,CMIP6_ssh_filteredvarid,'units','m/s/y ');
        ...
        netcdf.putAtt(ncid,CMIP6_trendvarid,'units','m/s/y');
        ...
        netcdf.putAtt(ncid,CMIP6_trend_filteredvarid,'units','m/s/y');
        ...
        netcdf.putAtt(ncid,clim_CMIP6_trend_dividedvarid,'units','m/s/y');
        ...
        netcdf.putAtt(ncid,mean_CMIP6_trendvarid,'units','m/s/y');
        ...
        netcdf.putAtt(ncid,mean_CMIP6_trend_filteredvarid,'units','m/s/y');
        ...
        netcdf.putAtt(ncid,mean_clim_CMIP6_trend_dividedvarid,'units','m/s/y');

        clim_CMIP6varid=netcdf.defVar(ncid, ['clim_CMIP6_', varname], 'NC_FLOAT', [lon_dimid lat_dimid clim_time_dimid]);
        netcdf.putAtt(ncid,clim_CMIP6varid,'long_name',['clim_CMIP6_', varname]);
        netcdf.putAtt(ncid,clim_CMIP6varid,'units','m/s/y ');
