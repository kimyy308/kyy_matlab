...
model_depth = ncread(filename,'h',[1 920], [980 1]);
...
model_depth_z2=diff(model_depth_z,1);
model_depth_z3=repmat(model_depth_z2',1,1,348);
load ('E:\Data\Model\ROMS\nwp_1_20\test42\run\NWP_u_all_time_1980_2008.mat')

testname='test42'
discharge_u=squeeze(mean(mean(comb_north_data,1,'omitnan'),2,'omitnan'));
comb_north_data_u=comb_north_data;
load ('E:\Data\Model\ROMS\nwp_1_20\test42\run\NWP_v_all_time_1980_2008.mat')

testname='test42'
% discharge_v=squeeze(mean(mean(comb_north_data,1,'omitnan'),2,'omitnan'));
comb_north_data_v=comb_north_data;
...
% plot(squeeze(mean(mean(comb_north_data,1,'omitnan'),2,'omitnan')))

hor_dist=m_lldist([115 115.05], [15 15])*1000;

% discharge=squeeze(mean(mean(comb_north_data.*comb_north_data_v.*model_depth_z3.*hor_dist,1,'omitnan'),2,'omitnan'));

% discharge=squeeze(sum(sum(comb_north_data.*comb_north_data_v.*model_depth_z3.*hor_dist,1,'omitnan'),2,'omitnan'));
discharge=squeeze(sum(sum(comb_north_data.*model_depth_z3.*hor_dist,1,'omitnan'),2,'omitnan'));
...
hold off
jpgname=strcat(outfile,testname, '_salt_north_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
xlabel('year')
ylabel('sum of salt at the boundary (meter^2)')
title(['northern boundary salinity, ',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i')])
