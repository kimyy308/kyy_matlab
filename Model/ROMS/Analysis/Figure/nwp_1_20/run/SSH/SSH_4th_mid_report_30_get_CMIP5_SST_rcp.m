...
% %  get reconstructed sst. compare model and resst. save. 

all_region ={'NWP','ES', 'SS', 'YS', 'AKP2'}
% all_region ={'AKP2', 'ES'}
...
for testnameind=1:length(all_testname)
    for regionind=1:length(all_region)
        ...
        inputyear = [2006:2075]; % % put year which you want to plot [year year ...]
...
%                 refpolygon(2,2)=lonlat(4);
        ...
        for yearij = 1:length(inputyear)
            for monthij = 1:length(inputmonth)
                ...
                varind = (tempyear - fyear_start)*12 + tempmonth;
...
%                     end
                ...
                data = ncread(filename,varname,[lon_min(1) lat_min(1) 1 varind], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1 1]);
        ...
        trend_filtered = trend_filtered; %% m/y -> mm/y
...
%         end 
        ...
        save([outputdir,testname,'_',regionname,'sst_trend_rcp45_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat'], '-v7.3', '-nocompression');

        ncid = netcdf.create(strcat(outputdir, testname,'_',regionname, '_sst_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc'),'NETCDF4');
        ...
        netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
            'type', ['NWP 1/20 _ ', testname, 'model, recon monthly sst analysis file']);
        netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
            'title', ' monthly sst analysis (1993-2009) ');
        netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        ...
        netcdf.putVar(ncid, sst_filteredvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_data_filtered);
        ...
        netcdf.putVar(ncid, clim_sst_trendvarid, [0 0 0], [len_lon len_lat length(climtime)], trend_clim);
        netcdf.close(ncid);
    end
end
