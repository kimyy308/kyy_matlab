...
% % % % % % % % Ensemble of 5 members (WOA initial)
...
% start-------------------- decadal current plot
    pngname=strcat(outfile, '_ENSw_',regionname, '_clim_uv_',num2str(min(inputyear1),'%04i'), ...
    ...
    tmp.tifname=strcat(dirs.figdir, 'RCM_ENSw', '_clim_uv_',num2str(min(RCM_info.years),'%04i'), ...
    ...
    for testnameind2=1:length(all_testname2)
        ...
        matname = [matdir, testname, '_', regionname, '_', variable, '_mean_', num2str(min(inputyear1),'%04i'), '-', num2str(max(inputyear1),'%04i'),  ...
    ...
    titlename = strcat('UV mean, ','ENSw',',(',num2str(min(inputyear1),'%04i'),'-',num2str(max(inputyear1),'%04i'),') ');  %% + glacier contribution
...
%                 system(['magick ', pngname, ' -trim ', pngname]);
    enssavefilename = [dirs.enssavedir, 'ENSw_', regionname, '_', variable, '_mean_', num2str(min(inputyear1),'%04i'), '-', num2str(max(inputyear1),'%04i'),...
                    '_', num2str(min(RCM_info.months),'%04i'), '-', num2str(max(RCM_info.months),'%04i'), '.mat'];
...
% start-------------------- decadal SST, SSH, SSS plot
    for varind2=1:length(all_var2)
        ...
        pngname=strcat(outfile, '_ENSw_',regionname, '_clim_',variable, '_', num2str(min(inputyear1),'%04i'), ...
            '_',num2str(max(inputyear1),'%04i'),  ...
                    '_', num2str(min(RCM_info.months),'%04i'), '-', num2str(max(RCM_info.months),'%04i'),'.tif'); %% ~_year_month.jpg
        ...
        tmp.tifname=strcat(dirs.figdir, 'RCM_ENSw', '_clim_', variable, '_',num2str(min(RCM_info.years),'%04i'), ...
                     '_',num2str(max(RCM_info.years),'%04i'),  ...
                    '_', num2str(min(RCM_info.months),'%04i'), '-', num2str(max(RCM_info.months),'%04i'),'.tif'); %% ~_year_month.jpg
        run(param_script);
        for testnameind2=1:length(all_testname2)
...
%             ssh_correction_for_fig = Func_0014_SSH_correction_for_pcolor('RCM_ENSw_historical');
            ssh_correction_for_fig = Func_0017_SSH_correction_for_CMIP6_RMSE('RCM_ENSw_historical');
        ...
        m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
        titlename = strcat(variable, ' mean, ','ENSw',',(',num2str(min(inputyear1),'%04i'),'-',num2str(max(inputyear1),'%04i'),') ');  %% + glacier contribution
        title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
