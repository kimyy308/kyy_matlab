...
%             filedir = strcat('E:\Data\Model\ROMS\nwp_1_20\output\', testname, '\run\packed_monthly\'); % % where data files are
            ...
            griddir = strcat('D:\Data\Model\ROMS\nwp_1_20\backup_surf\'); % % where data files are
...
%                                 'pck_', testname, '_monthly_', num2str(tempyear,'%04i'), '_', num2str(tempmonth,'%02i'), '.nc');
                        ...
                        longridname = strcat(griddir, ...
                                'NWP_pck_ocean_lon_rho_NWP.nc');
                        latgridname = strcat(griddir, ...
                                'NWP_pck_ocean_lat_rho_NWP.nc');

                        % read model data
                        if (exist('lon')==0)
                            modelinfo=ncinfo(longridname);
                            lon = ncread(longridname,'lon_rho',[1 1],[inf,1]);
                            lat = ncread(latgridname,'lat_rho',[1 1],[1,inf]);
                            ...
                            lon = ncread(longridname,'lon_rho', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
                            lat = ncread(latgridname,'lat_rho', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
...
% 
%                         tttttt= ncread('D:\Data\Model\ROMS\nwp_1_20\test57\run\test57_TEST_ssh_trend_2006_2100.nc', 'trend_filtered');
%                         pcolor(tttttt'); shading flat; colorbar;
