...
%     inputyear1 = [1985:2014]; % % put year which you want to plot [year year ...]
    ...
    inputyear2 = [2030]; % % put year which you want to plot [year year ...]
...
% start-------------------- decadal current plot
    pngname=strcat(outfile, '_diff_ENSg_',regionname, '_clim_uv_',num2str(min(inputyear1),'%04i'), ...
    ...
    tmp.tifname=strcat(dirs.figdir, 'diff_RCM_ENSg', '_clim_uv_',num2str(min(RCM_info.years),'%04i'), ...
    '_',num2str(max(RCM_info.years),'%04i'), ...
                    '_', num2str(min(RCM_info.months),'%04i'), '-', num2str(max(RCM_info.months),'%04i'),'.tif'); %% ~_year_month.jpg
    
    early_enssavefilename = [dirs.enssavedir, 'ENSg_', regionname, '_', variable, ...
    ...
    late_enssavefilename = [dirs.enssavedir, 'ENSg_', regionname, '_', variable, ...
...
%     v_rho=v_rho(:,1:size(mask_model,2)).*mask_model;  
    ...
    titlename = strcat('UV diff, ','ENSg',',(',num2str(min(inputyear2),'%04i'),'-',num2str(max(inputyear1),'%04i'),') ');  %% + glacier contribution
...
% start-------------------- decadal SST, SSH, SSS plot
    for varind2=1:length(all_var2)
        ...
        pngname=strcat(outfile, '_diff_ENSg_',regionname, '_clim_',variable, '_', num2str(min(inputyear1),'%04i'), ...
            '_',num2str(max(inputyear1),'%04i'), ...
                    '_', num2str(min(RCM_info.months),'%04i'), '-', num2str(max(RCM_info.months),'%04i'),'.tif'); %% ~_year_month.jpg
...
%         end
        ...
        m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
        titlename = strcat(variable, ' diff, ','ENSg',',(',num2str(min(inputyear2),'%04i'),'-',num2str(max(inputyear1),'%04i'),') ');  %% + glacier contribution
        title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
