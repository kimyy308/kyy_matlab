...
for testnameind2=1:length(all_testname2)
    ...
    %     trendlev = [-3 3];  %% trend lev
        ...
        inputyear2 = [1993:2014]; % % put year which you want to plot [year year ...]
        season='all';
        switch(season)
            case 'all'
...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
            figrawdir =strcat('Z:\내 드라이브\MEPL\project\SSH\6th_year\figure\cmip6\',testname,'\',regionname,'\'); % % where figure files will be saved
            ...
            for yearij=1:length(inputyear1)
                ...
                for monthij=1:length(inputmonth)
                    ...
                    if (exist('lon_rho' , 'var') ~= 1)
                        ...
                        lon_rho=ncread(ufilename, lonname);
                        lat_rho=ncread(ufilename, latname);
...
%             if (exist(pngname , 'file') ~= 2)        
    ...
    %                 ref_vec_y_range = round(ref_vec_y_ind-(m_quiver_y_interval/2)) : round(ref_vec_y_ind-(m_quiver_y_interval/2))+m_quiver_y_interval;
                ...
                cmip6_land_vec=ones(size(mean_u));
                cmip6_land_vec(isnan(mean_u))=1;
                cmip6_land_vec(isnan(mean_v))=1;
                cmip6_land_vec(isfinite(mean_u))=NaN;
                cmip6_land_vec(isfinite(mean_v))=NaN;
                save([cmip6dir,regionname, '_', testname, '_cmip6_land_vec','.mat'], 'cmip6_land_vec');
                ...
                switch(season)
                    case 'all'
                        save([cmip6dir,regionname,'_',testname, '_cmip6_vec_',yearstr_min, '_', yearstr_max, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'u_rho', 'v_rho');
                    case 'summer'
                        save([cmip6dir,regionname,'_',testname, '_cmip6_vec_summer_',yearstr_min, '_', yearstr_max, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'u_rho', 'v_rho');                        
                    case 'winter'
                        save([cmip6dir,regionname,'_',testname, '_cmip6_vec_winter_',yearstr_min, '_', yearstr_max, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'u_rho', 'v_rho');                                                
                end
...
% % % 
% % %                         varname='uo';
% % %                         filedir = strcat(cmip6dir, varname, '\historical\interp\', testname, '\'); % % where data files are
% % %                         flag_file_in = false;
% % %                         list = dir( [ filedir, '\', varname, '*' ]); 
% % %                         for kk = 1 : length( list )
% % %                             fname_in    = list(kk).name;
...
% % %                         vfilename=[filedir, '\', fname_in];
% % %                         
% % % %                         zosfiledir=strcat(cmip6dir, 'zos', '\historical\interp\', testname, '\');
% % % %                         flag_file_in = false;
...
%             if (exist(pngname , 'file') ~= 2)        
                ...
                cmip6_land=ones(size(mean_data));
                cmip6_land(isnan(mean_data))=1;
                cmip6_land(isfinite(mean_data))=NaN;
                save([cmip6dir,regionname, '_', testname, '_cmip6_land','.mat'], 'cmip6_land');
                
                yearstr_min=num2str(inputyear1(1));
                yearstr_max=num2str(inputyear1(end));
                save([cmip6dir,regionname,'_',testname, '_cmip6_', variable,'_',yearstr_min, '_', yearstr_max, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'mean_data');
...
% %                     monthstr=num2str(tempmonth, '%02i');
% % 
% %                     filedir = strcat(cmip6dir, 'thetao', '\historical\Omon\', testname, '\'); % % where data files are
% %                     flag_file_in = false;
