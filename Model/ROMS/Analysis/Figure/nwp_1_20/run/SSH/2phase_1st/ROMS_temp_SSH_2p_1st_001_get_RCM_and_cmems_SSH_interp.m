...
% % %         switch region
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
%             figrawdir =strcat('D:\OneDrive - ������б�\MEPL\project\MICT_pollack\4th_year\figure\nwp_1_20\',testname,filesep,regionname,filesep); % % where figure files will be saved
...
%             filedir = strcat('E:\Data\Model\ROMS\nwp_1_20\output\', testname, '\run\packed_monthly\'); % % where data files are
            ...
            cmemsdir='Z:\�� ����̺�\Data\Observation\CMEMS\';
        elseif (strcmp(system_name,'GLNXA64'))
            param_script ='/home/kimyy/Dropbox/source/matlab/Model/ROMS/Analysis/Figure/nwp_1_20/run/fig_param/fig_param_kyy_EKB_RMS.m';
%             filedir = strcat('E:\Data\Model\ROMS\nwp_1_20\output\', testname, '\run\packed_monthly\'); % % where data files are
            filedir = strcat('/data1/kimyy/Model/ROMS/nwp_1_20/backup_surf/', testname, '/run/zeta/'); % % where data files are
            griddir = strcat('/data1/kimyy/Model/ROMS/nwp_1_20/output/', testname, '/run/packed_monthly/'); % % where data files are
            savedir = strcat('/data1/kimyy/Model/ROMS/nwp_1_20/output/', testname, '/run/'); % % where data files are
            cmemsdir='/data1/kimyy/Observation/CMEMS/';
        end
...
%                         filename = strcat(filedir, num2str(tempyear,'%04i'), filesep, ...
%                                 'pck_', testname, '_monthly_', num2str(tempyear,'%04i'), '_', num2str(tempmonth,'%02i'), '.nc');
                        ...
                        gridname = strcat(griddir, num2str(tempyear,'%04i'), filesep, ...
                                'pck_', testname, '_monthly_', num2str(tempyear,'%04i'), '_', num2str(tempmonth,'%02i'), '.nc');
                ...
                for i=1:cmems_lonsize_cut
                    for j=1:cmems_latsize_cut
                        if isfinite(cmems_sla_yearly(i,j,1))
                            ...
                            [f_exp, gof_exp] = fit(single(trendtime_yearly'),single(squeeze(cmems_sla_yearly(i,j,:))+corr_for_exp),'exp1');
