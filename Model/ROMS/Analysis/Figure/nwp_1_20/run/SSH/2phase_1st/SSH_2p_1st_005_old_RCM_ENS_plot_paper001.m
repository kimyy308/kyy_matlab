...
% start-------------------- absolute trend plot
        ...
        while (fig_flag)
            ...
            tmp.tifname=strcat(dirs.figdir, 'RCM_ENS', '_abs_ssh_trend_',num2str(min(RCM_info.years),'%04i'), ...
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)            
                ...
                titlename = strcat('RCM-ENS', ',(',num2str(min(inputyear),'%04i'),'-', ...
...
% % % %             GCM trend
                tmp.tifname=strcat(dirs.figdir, 'GCM_ENS', '_abs_ssh_trend_',num2str(min(RCM_info.years),'%04i'), ...
                    '_',num2str(max(RCM_info.years),'%04i'), '.tif'); %% ~_year_month.jpg
                for testnameind2=1:length(all_testname2)
                    testname=all_testname2{testnameind2}    % % need to change
                    gcmtestname = Func_0004_get_GCMname_from_RCM(testname);
                ...
                titlename = strcat('GCM-ENS', ',(',num2str(min(inputyear),'%04i'),'-', ...
                    num2str(max(inputyear),'%04i'),'), ','M=',num2str(round(m_value,2)), ' mm/y');  %% + glacier contribution
