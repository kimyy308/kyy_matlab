        ...
        for yearij = 1:length(inputyear)
            for monthij = 1:length(inputmonth)
                ...
                filename = strcat(filedir, num2str(tempyear,'%04i'), '\', ...
                                'pck_', testname, '_temp_monthly_', num2str(tempyear,'%04i'), '_', num2str(tempmonth,'%02i'), '.nc');
                % read model data
                if (exist('lon')==0)
                    ...
                    lon = ncread(gridname,'lon_rho',[1 1],[inf,1]);
                    lat = ncread(gridname,'lat_rho',[1 1],[1,inf]);
...
%         grid off
        ...
        save([savedir,regionname,'sst_rms_and_bias_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);

        ncid = netcdf.create(strcat(savedir, testname, '_',regionname,'_rms_clim_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc'),'NETCDF4');
        ...
        netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
            'title', ' monthly SST RMS/BIAS (1985-2014) ');
        netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
