...
%     trendlev = [-3 3];  %% trend lev
    ...
    % for snu_desktopd
    inputyear1 = [2040]; % % put year which you want to plot [year year ...]
...
% start-------------------- earlier decadal current plot
    pngname=strcat(outfile, '_ENS3_',scenname, '_interp_',regionname, '_clim_uv_',num2str(min(inputyear1),'%04i'), ...
    ...
    tmp.tifname=strcat(dirs.figdir, 'GCM_ENS3', '_clim_uv_',num2str(min(RCM_info.years),'%04i'), ...
    ...
    titlename = strcat('UV mean, ','ENS3',',(',num2str(min(inputyear1),'%04i'),'-',num2str(max(inputyear1),'%04i'),') ');  %% + glacier contribution
    ...
    enssavefilename = [dirs.enssavedir, 'ENS3_', regionname, '_', variable, '_mean_', num2str(min(inputyear1),'%04i'), '-', num2str(max(inputyear1),'%04i'), '.mat'];
...
% start-------------------- earlier decadal SST, SSS plot
    for varind2=1:length(all_var2)
        ...
        pngname=strcat(outfile, '_ENS3_',scenname, '_interp_',regionname, '_clim_', variable, '_',num2str(min(inputyear1),'%04i'), ...
        '_',num2str(max(inputyear1),'%04i'), '.tif'); %% ~_year_month.jpg
        ...
        tmp.tifname=strcat(dirs.figdir, 'GCM_ENS3', '_clim_', variable, '_',num2str(min(RCM_info.years),'%04i'), ...
                     '_',num2str(max(RCM_info.years),'%04i'), '.tif'); %% ~_year_month.jpg
        run(param_script);
        for testnameind2=1:length(all_testname2)
            testname=all_testname2{testnameind2}
...
%                         data = ncread(ufilename,varname,[lon_min(1) lat_min(1) 1 tind_u], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1 1]);  %% cut horizontal area [x,y,z] (wider than target area)
        ...
        if (strcmp(variable,'SSH')==1)
            ...
            ssh_correction_for_fig = Func_0014_SSH_correction_for_pcolor('GCM_ENS3_historical');
        ...
        m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
        titlename = strcat(variable, ' mean, ','ENS3',',(',num2str(min(inputyear1),'%04i'),'-',num2str(max(inputyear1),'%04i'),') ');  %% + glacier contribution
        title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
