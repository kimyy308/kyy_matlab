...
%     end
    ...
    fpath = ['/data2/RCM/CMIP6/Model/ROMS/nwp_1_20/cut_ES/', testname, '/'];
    flist = dir(fpath);
...
%     load([stdpath, stdname])
    ...
    for year = syear : eyear
        ...
        for month = 1 : 12
            ...
            filename = ['pck_ES_',testname,'_monthly_',yy,'_',mm,'.nc'];
...
%             savepath = ['/data1/ykang/CMIP6/output/',testname,'/stddepth/monthly/',yy,'/'];
            savepath = ['/data2/RCM/CMIP6/Model/ROMS/nwp_1_20/cut_ES_stddepth_mat/', testname, '/'];
            ...
            save(savename,'temp','salt','u','v','w','RCM_grid');
            
            RCM_time.ftime= datenum(year,month,15) - datenum(1900,12,31);
            
            % % % % % %         make ncfile
            ncsavepath = ['/data2/RCM/CMIP6/Model/ROMS/nwp_1_20/cut_ES_stddepth/', testname, '/', yy, '/'];

            savencname = [ncsavepath,testname,'_monthly_std_',yy,'_',mm,'.nc'];
            ...
            netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
                'title', [' monthly SSH analysis (', num2str(min(year)), '-', num2str(max(year)) ,') ']);
            netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
