...
%     testname='test49'   % % need to change
    ...
    if (strcmp(system_name,'PCWIN64'))
        ...
        MyOceandir='E:\Data\Observation\OIsst\monthly\';
    elseif (strcmp(system_name,'GLNXA64'))
    ...
    for yearij = 1:length(inputyear)
        ...
        %         ncinfo('E:\Data\Observation\OIsst\monthly\MyOcean_monthly1983_11.nc');
            ...
            MyOcean_data = ncread(MyOceanfilename,varname, ...
                ...
                [MyOcean_lon_max(1)-MyOcean_lon_min(1) MyOcean_lat_max(1)-MyOcean_lat_min(1) 1 1]);
    ...
    for i=1:len_lon
        for j=1:len_lat
            ...
            MyOcean_trend_filtered(i,j)=p(1);
        end
    end
    ...
    for k=1:12
        ...
        for i=1:len_lon
            for j=1:len_lat
                ...
                clim_MyOcean_trend_divided(i,j,k)=p(1);
            end
        end
    end
    ...
    save([filedir,regionname,'MyOcean_sst_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);

    ncid = netcdf.create(strcat(filedir,regionname,'MyOcean_sst_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc'),'NETCDF4');
    ...
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'type', ['MyOcean _ ', 'monthly sst trend file']);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'title', [' MyOcean sst trend (',num2str(inputyear(1)),'-',num2str(inputyear(end)),') ']);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
        'source', [' MyOcean(OIsst) satellite data ' ]);
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
    ...
    MyOcean_sstvarid=netcdf.defVar(ncid, 'MyOcean_sst', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,MyOcean_sstvarid,'long_name','MyOcean_sst');
    netcdf.putAtt(ncid,MyOcean_sstvarid,'units','mm ');

    MyOcean_sst_filteredvarid=netcdf.defVar(ncid, 'MyOcean_sst_filtered', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,MyOcean_sst_filteredvarid,'long_name','MyOcean_sst_filtered');
    netcdf.putAtt(ncid,MyOcean_sst_filteredvarid,'units','mm ');
    ...
    clim_MyOceanvarid=netcdf.defVar(ncid, 'clim_MyOcean_sst', 'NC_FLOAT', [lon_dimid lat_dimid clim_time_dimid]);
    netcdf.putAtt(ncid,clim_MyOceanvarid,'long_name','clim_MyOcean_sst');
    ...
    netcdf.putVar(ncid, MyOcean_sstvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_MyOcean_data);
    netcdf.putVar(ncid, MyOcean_sst_filteredvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_MyOcean_data_filtered);
