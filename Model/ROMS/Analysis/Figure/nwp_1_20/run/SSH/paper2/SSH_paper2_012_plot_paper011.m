...
%  start-------------------- absolute trend difference plot
        ...
        while (fig_flag)
            jpgname=strcat(outfile, '_', testname,'_',regionname, '_absolute_ssh_trend_diff_cmems_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                lon_cmems=ncread(cmemsfilename,'lon_cmems');
                lat_cmems=ncread(cmemsfilename,'lat_cmems');
...
% start-------------------- absolute trend_corrected plot
        ...
        while (fig_flag)
            jpgname=strcat(outfile, '_', testname,'_',regionname, '_absolute_cmems_corrected_ssh_trend_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
...
% start-------------------- RMS plot
        ...
        while (fig_flag)
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_cmems_rms_', 'SSH','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                run(param_script);
                for yearij=1:length(inputyear)
...
% start-------------------- RMS_corrected plot
        ...
        while (fig_flag)
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_rms_cmems_corrected_', 'SSH','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                run(param_script);
                for yearij=1:length(inputyear)
...
% start-------------------- BIAS plot
        ...
        while (fig_flag)
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_cmems_bias_', 'SSH','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                run(param_script);
                for yearij=1:length(inputyear)
...
% start-------------------- model pattern correlation    
        ...
        while (fig_flag)
            ...
            rcm_interped_ssh = ncread(rcminterpedfilename, 'interped_ssh');
            m_rcm_interped_ssh = mean(rcm_interped_ssh,3,'omitnan');
            rcm_mask = m_rcm_interped_ssh;
            rcm_mask(isfinite(rcm_mask))=1;
            
%             abc=reshape(ans,[104,90])
            
            m_cmems_adt=mean(cmems_adt,3,'omitnan');
            m_interped_ssh=mean(interped_ssh,3,'omitnan').*rcm_mask;
            m_cmems_sla=mean(cmems_sla,3,'omitnan');
            m_interped_sla=mean(interped_sla,3,'omitnan');
            
            aaa= m_interped_ssh(logical(~isnan(m_interped_ssh).*~isnan(m_cmems_adt).*~isnan(m_rcm_interped_ssh)));
            bbb= m_cmems_adt(logical(~isnan(m_interped_ssh).*~isnan(m_cmems_adt).*~isnan(m_rcm_interped_ssh)));
            ccc = m_rcm_interped_ssh(logical(~isnan(m_interped_ssh).*~isnan(m_cmems_adt).*~isnan(m_rcm_interped_ssh)));
            plot(aaa); hold on; plot(bbb); plot(ccc); legend('gcm', 'cme', 'rcm'); hold off;
...
%             m_cmems_trend_filtered=mean(cmems_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(cmems_trend_filtered))));
             pcolor(m_interped_ssh'); shading flat; colorbar; caxis([-0.2 1.0]); colormap jet
             pcolor(m_cmems_adt'); shading flat; colorbar; caxis([-0.2 1.0]); colormap jet
...
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-mean(mean_data(:),'omitnan'));
                ...
                save([cmip5dir,regionname,'_',testname, '_model_', 'det_std','_',yearstr_min, '_', yearstr_max, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'model_sla_std');
...
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-mean(mean_data(:),'omitnan'));
                ...
                yearstr_min=num2str(inputyear(1));
                yearstr_max=num2str(inputyear(end));
                save([cmip5dir,regionname,'_',testname, '_model_linear_', 'det_std','_',yearstr_min, '_', yearstr_max, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'model_sla_std');
...
% start-------------------- u_rms
        ...
        while (fig_flag)
            ...
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                ...
                m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                titlename = strcat('rms-u,','gcm',',(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),  ', ', num2str(round(mean(rms_u_rho(:),'omitnan'),2)), ') ');  %% + glacier contribution
                title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
...
% start-------------------- v_rms
        ...
        while (fig_flag)
            ...
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                ...
                [v_rho_corr, v_rho_pval] = corrcoef(mean_interped_v_rho(logical(~isnan(mean_interped_v_rho).*~isnan(mean_cmems_v_rho))), mean_cmems_v_rho(logical(~isnan(mean_interped_v_rho).*~isnan(mean_cmems_v_rho))));
...
%                 save([filedir,regionname, '_cmems_linear_', 'det_std','_',yearstr_min, '_', yearstr_max, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'model_sla_std');
                ...
                m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                titlename = strcat('rms-v,','gcm',',(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),  ', ', num2str(round(mean(rms_v_rho(:),'omitnan'),2)), ') ');  %% + glacier contribution
                title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
