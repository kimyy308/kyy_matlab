...
% %  get cmemsstructed vel. compare model and revel. save. 
...
for scenind=1:length(all_scenname)
...
%         trend_filt=ncread('E:\Data\Model\CMIP5\vel\rcp45\interp\ens07\ens07_SScmems_interped_vel_trend_2006_2100.nc','interped_trend_filtered');
%         trend_filt=ncread('J:\Data\Model\CMIP5\vel\rcp45\interp\ens08\ens08_AKP4_vel_analysis_2006_2100.nc','trend_filtered');
%         m_trend_filt=mean(trend_filt(:), 'omitnan')
...
% % %         get model data and cmemsstructed data      
        ...
        while (fig_flag)
            run(param_script);
            matname = [filedir,scenname, '_', testname,'_',regionname,'model_cmems_vel_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat'];

            if (exist(matname , 'file') ~= 2 || fig_flag==2)   
                ufilename = strcat(filedir,  ...
                                scenname,'_', testname, '_NWP_CMIP5_ssu_analysis_', num2str(inputyear(1),'%04i'), '_', num2str(inputyear(end),'%04i'), '.nc');
                vfilename = strcat(filedir,  ...
                            scenname, '_', testname, '_NWP_CMIP5_ssv_analysis_', num2str(inputyear(1),'%04i'), '_', num2str(inputyear(end),'%04i'), '.nc');

%                 load(ufilename)
                comb_u_rho = ncread(ufilename, 'raw_ssu');
                comb_spatial_mean_u_rho=ncread(ufilename, 'clim_ssu');
                lon_u=ncread(ufilename, 'lon_rho');
                lat_u=ncread(ufilename, 'lat_rho');
%                 load(vfilename)
                comb_v_rho = ncread(vfilename, 'raw_ssv');
                comb_spatial_mean_v_rho=ncread(vfilename, 'clim_ssv');
                lon_v=ncread(vfilename, 'lon_rho');
                lat_v=ncread(vfilename, 'lat_rho');
                
%                 all_region ={'NWP'}
                ind=1;
%                 filedir=strcat('E:\Data\Model\CMIP5\surface\'); % % where data files are
        ...
        % % % %                 % read Reconstructed DATA
                        cmemsfilename = strcat(cmemsdir,'cmems_nwp_ssh_', num2str(inputyear(1),'%04i'),'.nc');
                        if (exist('cmems_lon')==0)
                            ...
                            comb_spatial_meaninterped_u_rho=(zeros([cmems_lonsize_cut,cmems_latsize_cut,12]));
                            comb_spatial_meaninterped_v_rho=(zeros([cmems_lonsize_cut,cmems_latsize_cut,12]));   
                        ...
                        interped_u_rho = griddata(double(lon_u), double(lat_u), squeeze(double(comb_u_rho(:,:,ind))),double(cmems_lon2),double(cmems_lat2)).*mask_cmems;   
                        interped_v_rho = griddata(double(lon_v), double(lat_v), squeeze(double(comb_v_rho(:,:,ind))),double(cmems_lon2),double(cmems_lat2)).*mask_cmems;   
                        
                        cmems_u_rho = ncread(cmemsfilename, 'ugos', [cmems_lon_min(1), cmems_lat_min(1), monthij], [cmems_lonsize_cut, cmems_latsize_cut 1]);
                        cmems_v_rho = ncread(cmemsfilename, 'vgos', [cmems_lon_min(1), cmems_lat_min(1), monthij], [cmems_lonsize_cut, cmems_latsize_cut 1]);
                        
                        rms_u_rho = rms_u_rho+ ((interped_u_rho - cmems_u_rho).^2) / (length(inputyear) * length(inputmonth));
                        rms_v_rho = rms_v_rho+ ((interped_v_rho - cmems_v_rho).^2) / (length(inputyear) * length(inputmonth));
                        
                        comb_cmems_u_rho(:,:,ind) = cmems_u_rho;
                        comb_cmems_v_rho(:,:,ind) = cmems_v_rho;
                        
%                         comb_speed(:,:,ind) = sqrt(single(comb_u_rho(:,:,ind)).^2 + single(comb_v_rho(:,:,ind)).^2);

                        comb_interped_u_rho(:,:,ind) = interped_u_rho;
                        comb_interped_v_rho(:,:,ind) = interped_v_rho;
                        comb_interped_speed(:,:,ind) = sqrt(single(interped_u_rho).^2 + single(interped_v_rho).^2);
                        comb_spatial_meaninterped_u_rho(:,:,monthij)=comb_spatial_meaninterped_u_rho(:,:,monthij)+interped_u_rho/double(length(inputyear));
                        comb_spatial_meaninterped_v_rho(:,:,monthij)=comb_spatial_meaninterped_v_rho(:,:,monthij)+interped_v_rho/double(length(inputyear));
                        
                        ind = ind + 1;
                        toc;
                    end
                end
                rms_u_rho = sqrt(rms_u_rho);
                rms_v_rho = sqrt(rms_v_rho);
                 save(matname, 'filedir','comb_u_rho', 'comb_v_rho', ...
                    'cmems_lonsize_cut', 'cmems_latsize_cut', 'comb_spatial_mean_u_rho', 'comb_spatial_mean_v_rho', ...
                    'comb_cmems_u_rho', 'comb_cmems_v_rho', 'rms_u_rho', 'rms_v_rho', ...
                    'mask_cmems', 'comb_interped_u_rho', 'comb_interped_v_rho', 'comb_spatial_meaninterped_u_rho', 'comb_spatial_meaninterped_v_rho', ...
                    'cmems_lon2', 'cmems_lat2', 'lon_u', 'lat_u', 'lat_u', 'lat_v', 'comb_interped_speed', 'mfname', '-v7.3');
            else
                load(matname);
            end
            fig_flag=0;
...
% % %         sea level analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, '_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            ...
            % % %         make ncfile
                ...
                netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
                    'type', ['NWP 1/20 _ ', testname, 'model, cmems monthly vel analysis file']);
                netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
                    'title', [' monthly vel analysis (', num2str(min(inputyear)), '-', num2str(max(inputyear)) ,') ']);
                netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'), ...
                ...
                raw_velvarid=netcdf.defVar(ncid, 'raw_vel', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
                netcdf.putAtt(ncid,raw_velvarid,'long_name','raw_vel');
                netcdf.putAtt(ncid,raw_velvarid,'units','m');

                vel_filteredvarid=netcdf.defVar(ncid, 'vel_filtered', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
                netcdf.putAtt(ncid,vel_filteredvarid,'long_name','vel_filtered');
                netcdf.putAtt(ncid,vel_filteredvarid,'units','m');
                ...
                clim_velvarid=netcdf.defVar(ncid, 'clim_vel', 'NC_FLOAT', [lon_dimid lat_dimid clim_time_dimid]);
                netcdf.putAtt(ncid,clim_velvarid,'long_name','clim_vel');
                netcdf.putAtt(ncid,clim_velvarid,'units','m');

                clim_vel_trendvarid=netcdf.defVar(ncid, 'clim_vel_trend', 'NC_FLOAT', [lon_dimid lat_dimid clim_time_dimid]);
                netcdf.putAtt(ncid,clim_vel_trendvarid,'long_name','clim_vel_trend');
                netcdf.putAtt(ncid,clim_vel_trendvarid,'units','m');
                ...
                netcdf.putVar(ncid, raw_velvarid, [0 0 0], [len_lon_model len_lat_model length(ftime)], comb_data);
                netcdf.putVar(ncid, vel_filteredvarid, [0 0 0], [len_lon_model len_lat_model length(ftime)], comb_data_filtered);
                ...
                netcdf.putVar(ncid, clim_velvarid, [0 0 0], [len_lon_model, len_lat_model length(climtime)], comb_spatial_meanmodel);
                netcdf.putVar(ncid, clim_vel_trendvarid, [0 0 0], [len_lon_model, len_lat_model length(climtime)], trend_clim);
                netcdf.close(ncid);
            end
            fig_flag=0;
        end
...
% % %     cmems sea level trend analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'cmems_vel_rms_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
...
%                 cmems_sla=comb_cmems_data;
                cmems_u_rho = comb_cmems_u_rho;
                cmems_v_rho = comb_cmems_v_rho;
...
%                 clim_cmems_sla=mean(cmems_sla_divided,4,'omitnan');
                cmems_u_rho_divided=reshape(cmems_u_rho,[size(cmems_u_rho,1), size(cmems_u_rho,2), 12, length(inputyear)]);
                clim_cmems_u_rho=mean(cmems_u_rho_divided,4,'omitnan');
                cmems_v_rho_divided=reshape(cmems_v_rho,[size(cmems_v_rho,1), size(cmems_v_rho,2), 12, length(inputyear)]);
                clim_cmems_v_rho=mean(cmems_v_rho_divided,4,'omitnan');
...
%                 mean_clim_cmems_trend_divided=mean(mean(clim_cmems_trend_divided,'omitnan'),'omitnan');
            ...
            % % %         make ncfile
                ...
                cmems_u_rhovarid=netcdf.defVar(ncid, 'cmems_u_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid time_dimid]);
                netcdf.putAtt(ncid,cmems_u_rhovarid,'long_name','cmems_u_rho');
                netcdf.putAtt(ncid,cmems_u_rhovarid,'units','m/s ');
                
                cmems_v_rhovarid=netcdf.defVar(ncid, 'cmems_v_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid time_dimid]);
                netcdf.putAtt(ncid,cmems_v_rhovarid,'long_name','cmems_v_rho');
                netcdf.putAtt(ncid,cmems_v_rhovarid,'units','m/s ');
                
                interped_u_rhovarid=netcdf.defVar(ncid, 'interped_u_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid time_dimid]);
                netcdf.putAtt(ncid,interped_u_rhovarid,'long_name','interped_u_rho');
                netcdf.putAtt(ncid,interped_u_rhovarid,'units','m/s ');
                
                interped_v_rhovarid=netcdf.defVar(ncid, 'interped_v_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid time_dimid]);
                netcdf.putAtt(ncid,interped_v_rhovarid,'long_name','interped_v_rho');
                netcdf.putAtt(ncid,interped_v_rhovarid,'units','m/s ');
...
%                 netcdf.putAtt(ncid,cmems_sla_filteredvarid,'units','m ');
                ...
                clim_cmems_u_rhovarid=netcdf.defVar(ncid, 'clim_cmems_u_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid clim_time_dimid]);
                netcdf.putAtt(ncid,clim_cmems_u_rhovarid,'long_name','clim_cmems_u_rho');
                netcdf.putAtt(ncid,clim_cmems_u_rhovarid,'units','m ');
                
                clim_cmems_v_rhovarid=netcdf.defVar(ncid, 'clim_cmems_v_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid clim_time_dimid]);
                netcdf.putAtt(ncid,clim_cmems_v_rhovarid,'long_name','clim_cmems_v_rho');
                netcdf.putAtt(ncid,clim_cmems_v_rhovarid,'units','m ');
                
                rms_u_rhovarid=netcdf.defVar(ncid, 'rms_u_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid]);
                netcdf.putAtt(ncid,rms_u_rhovarid,'long_name','rms_u_rho');
                netcdf.putAtt(ncid,rms_u_rhovarid,'units','m/s');
                
                rms_v_rhovarid=netcdf.defVar(ncid, 'rms_v_rho', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid]);
                netcdf.putAtt(ncid,rms_v_rhovarid,'long_name','rms_v_rho');
                netcdf.putAtt(ncid,rms_v_rhovarid,'units','m/s');
...
%                 netcdf.putAtt(ncid,clim_cmems_trend_dividedvarid,'units','mm /year');
                ...
                netcdf.putVar(ncid, cmems_u_rhovarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], comb_cmems_u_rho);
                netcdf.putVar(ncid, cmems_v_rhovarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], comb_cmems_v_rho);
                netcdf.putVar(ncid, interped_u_rhovarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], comb_interped_u_rho);
                netcdf.putVar(ncid, interped_v_rhovarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], comb_interped_v_rho);
%                 netcdf.putVar(ncid, cmems_sla_filteredvarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], comb_cmems_data_filtered);
                netcdf.putVar(ncid, clim_cmems_u_rhovarid, [0 0 0], [len_lon_cmems len_lat_cmems length(climtime)], clim_cmems_u_rho);
                netcdf.putVar(ncid, clim_cmems_v_rhovarid, [0 0 0], [len_lon_cmems len_lat_cmems length(climtime)], clim_cmems_v_rho);
%                 netcdf.putVar(ncid, cmems_trendvarid, [0 0], [len_lon_cmems len_lat_cmems], cmems_trend);
                netcdf.putVar(ncid, rms_u_rhovarid, [0 0], [len_lon_cmems len_lat_cmems], rms_u_rho);
                netcdf.putVar(ncid, rms_v_rhovarid, [0 0], [len_lon_cmems len_lat_cmems], rms_v_rho);
...
% % %     interped sea level trend analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, scenname, '_', testname,'_',regionname, 'cmems_interped_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
           if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
                len_lon_cmems=size(cmems_lon2,1);
                len_lat_cmems=size(cmems_lon2,2);
                             
               cmems_sla_mean = zeros(size(cmems_lon2));
               interped_speed=comb_interped_speed;

                interped_speed_divided=reshape(interped_speed,[size(cmems_lon2,1), size(cmems_lon2,2), 12, length(inputyear)]);
                clim_interped_speed=mean(interped_speed_divided,4,'omitnan');
                for t=1:length(inputyear)
                    interped_speed_filtered(:,:,(t-1)*12+1:(t-1)*12+12)=single(interped_speed(:,:,(t-1)*12+1:(t-1)*12+12)-clim_interped_speed);
                end

                % % %         interped trend 
                comb_interped_clim_divided=reshape(comb_interped_speed,[cmems_lonsize_cut, cmems_latsize_cut, 12, length(inputyear)]);

                interped_trend(1:cmems_lonsize_cut,1:cmems_latsize_cut)=NaN;
                for i=1:cmems_lonsize_cut
                    for j=1:cmems_latsize_cut
                        p=polyfit(trendtime,squeeze(comb_interped_speed(i,j,:))',1);
                ...
                comb_spatial_meaninterped_speed=mean(comb_interped_clim_divided, 4, 'omitnan');
        % % %         interped trend (seasonal filtered)
                for t=1:length(inputyear)
                    comb_interped_speed_filtered(:,:,(t-1)*12+1:(t-1)*12+12)=comb_interped_speed(:,:,(t-1)*12+1:(t-1)*12+12)-comb_spatial_meaninterped_speed;
                end
                ...
                for i=1:cmems_lonsize_cut
                    for j=1:cmems_latsize_cut
                        p=polyfit(trendtime,squeeze(comb_interped_speed_filtered(i,j,:))',1);
...
% %                end
            ...
            % % %         make ncfile
                ...
                interped_velvarid=netcdf.defVar(ncid, 'interped_vel', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid time_dimid]);
                netcdf.putAtt(ncid,interped_velvarid,'long_name','interped_vel');
                netcdf.putAtt(ncid,interped_velvarid,'units','m/s ');
                
                interped_vel_filteredvarid=netcdf.defVar(ncid, 'interped_vel_filtered', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid time_dimid]);
                netcdf.putAtt(ncid,interped_vel_filteredvarid,'long_name','interped_vel_filtered');
                netcdf.putAtt(ncid,interped_vel_filteredvarid,'units','m/s ');
                ...
                clim_interped_vel_trendvarid=netcdf.defVar(ncid, 'clim_interped_vel_trend', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid clim_time_dimid]);
                netcdf.putAtt(ncid,clim_interped_vel_trendvarid,'long_name','clim_interped_vel_trend');
                netcdf.putAtt(ncid,clim_interped_vel_trendvarid,'units','m/s');
                ...
                netcdf.putVar(ncid, interped_velvarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], comb_interped_speed);
                netcdf.putVar(ncid, interped_vel_filteredvarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], interped_speed_filtered);
                ...
                netcdf.putVar(ncid, clim_interped_vel_trendvarid, [0 0 0], [len_lon_cmems len_lat_cmems 12], interped_trend_clim);
...
% % %     interped sea level correlation analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'cmems_interped_vel_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
                cmemsfilename = strcat(filedir, testname,'_',regionname, 'cmems_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
               ...
               interpedfilename = strcat(filedir, testname,'_',regionname, 'cmems_interped_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
               ...
               comb_interped_data = ncread(interpedfilename, 'interped_vel');
               comb_interped_clim_divided=reshape(comb_interped_data,[cmems_lonsize_cut, cmems_latsize_cut, 12, length(inputyear)]);

               interped_vel=comb_interped_data;
                for sla_i=1:size(cmems_sla,1)
                    for sla_j=1:size(cmems_sla,2)
                        interped_sla_mean(sla_i,sla_j)=mean(interped_vel(sla_i,sla_j,:),'omitnan');
                        interped_sla(sla_i,sla_j,:)=interped_vel(sla_i,sla_j,:)-(interped_sla_mean(sla_i,sla_j)-cmems_sla_mean(sla_i,sla_j));
                    end
                end
    ...
    % % %   correlation coefficient between model_climatology and cmems_climatology 
            ...
            % % %   correlation coefficient between climatological vel and climatological cmems vel 
            for i=1:cmems_lonsize_cut
                for j=1:cmems_latsize_cut
                    for k=1:12
...
% % %     low pass filtered interped sea level trend analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'low_pass_filtered_cmems_interped_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
                cmemsfilename = strcat(filedir, testname,'_',regionname, 'cmems_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
                lon_info=ncinfo(cmemsfilename, 'lon_cmems');
                len_lon_cmems=lon_info.Dimensions(1).Length;
...
% % %     low pass filtered interped sea level correlation analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'low_pass_filtered_cmems_interped_vel_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
                cmemsfilename = strcat(filedir, testname,'_',regionname, 'cmems_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
                lon_info=ncinfo(cmemsfilename, 'lon_cmems');
                len_lon_cmems=lon_info.Dimensions(1).Length;
...
% % %     detrended sea level correlation analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'detrended_cmems_interped_vel_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
               cmemsfilename = strcat(filedir, testname,'_',regionname, 'cmems_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
                lon_info=ncinfo(cmemsfilename, 'lon_cmems');
                len_lon_cmems=lon_info.Dimensions(1).Length;
...
% % %     moving averaged interped sea level trend analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'moving_averaged_cmems_interped_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
                %                cmemsfilename = strcat(filedir, testname,'_',regionname, 'cmems_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
%                 lon_info=ncinfo(cmemsfilename, 'lon_cmems');
                len_lon_cmems=size(cmems_lon2,1);
...
% % %     moving averaged interped sea level correlation analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'moving_averaged_cmems_interped_vel_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
                cmemsfilename = strcat(filedir, testname,'_',regionname, 'cmems_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
                lon_info=ncinfo(cmemsfilename, 'lon_cmems');
                len_lon_cmems=lon_info.Dimensions(1).Length;
...
% % %   correlation coefficient between model_vel_filtered_detrended and cmems_filtered_detrended
...
%         save([filedir,testname,'_',regionname,'vel_trend_corr_coef_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat'], '-v7.3');
