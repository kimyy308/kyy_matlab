...
%         m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
        ...
        for ind_sta=1:length(PSMSL_obs)
            ...
            model_lon(ind_sta)=lon_rho(ind_sta_model_lon(ind_sta),ind_sta_model_lat(ind_sta));
            model_lat(ind_sta)=lat_rho(ind_sta_model_lon(ind_sta),ind_sta_model_lat(ind_sta));
        end
...
% % %         msl time series (seasonal filtered)  
    ...
    %                 end
            ...
            %         msl=squeeze(mean(mean(comb_data_filtered,1,'omitnan'),2,'omitnan'));
                    ...
                    xlabel('year')
                    ylabel('Mean SSH (m)')
                    title([valid_name{ind_sta}, ', Mean SSH(',num2str(min(inputyear),'%04i'), ...
