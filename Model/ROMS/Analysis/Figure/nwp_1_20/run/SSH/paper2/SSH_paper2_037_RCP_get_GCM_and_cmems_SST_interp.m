...
%         trend_filt=ncread('E:\Data\Model\CMIP5\sst\rcp45\interp\ens07\ens07_SScmems_interped_sst_trend_2006_2100.nc','interped_trend_filtered');
%         trend_filt=ncread('J:\Data\Model\CMIP5\sst\rcp45\interp\ens08\ens08_AKP4_sst_analysis_2006_2100.nc','trend_filtered');
...
%                 load(ufilename)
                comb_data = ncread(filename, 'raw_sst');
                comb_spatial_mean_data=ncread(filename, 'clim_sst');
...
% % %     interped sea level trend analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, scenname, '_', testname,'_',regionname, 'cmems_interped_sst_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
           if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
               ...
               interped_sst=comb_interped_data;
                ...
                interped_sst_divided=reshape(interped_sst,[size(cmems_lon2,1), size(cmems_lon2,2), 12, length(inputyear)]);
                clim_interped_sst=mean(interped_sst_divided,4,'omitnan');
                for t=1:length(inputyear)
                    interped_sst_filtered(:,:,(t-1)*12+1:(t-1)*12+12)=single(interped_sst(:,:,(t-1)*12+1:(t-1)*12+12)-clim_interped_sst);
                end
        ...
        % % %         interped trend (seasonal filtered)
                for t=1:length(inputyear)
                    comb_interped_sst_filtered(:,:,(t-1)*12+1:(t-1)*12+12)=comb_interped_data(:,:,(t-1)*12+1:(t-1)*12+12)-comb_spatial_meaninterped_data;
                end
...
% %                end
            ...
            % % %         make ncfile
                ...
                netcdf.putVar(ncid, interped_sst_filteredvarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], interped_sst_filtered);
