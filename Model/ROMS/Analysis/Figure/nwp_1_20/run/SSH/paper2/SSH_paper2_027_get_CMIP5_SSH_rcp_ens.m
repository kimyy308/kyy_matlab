...
%                 refpolygon(2,2)=lonlat(4);
        ...
        if (strcmp(system_name,'PCWIN64'))
            ...
            filedir = strcat('J:\Data\Model\CMIP5\', varname, '\rcp45\interp\', testname, '\'); % % where data files are
        elseif (strcmp(system_name,'GLNXA64'))
        end
...
%                 end
                filename = strcat(filedir, varname, '_interp_', testname, '_rcp45_r1i1p1_', yearstr, '.nc');
...
%         save([filedir,testname,'_',regionname,'ssh_trend_rcp45_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat'], '-v7.3', '-nocompression');
