...
%             figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\MICT_pollack\4th_year\figure\nwp_1_20\',testname,'\',regionname,'\'); % % where figure files will be saved
        ...
        for folding=1:1
            ...
            fig_flags{11,1}='get temp & salt, calculate zosto';
            fig_flags{11,1}='steric sea level analysis';
            fig_flags{11,1}='steric sea level data nc save';
            fig_flags{11,1}='RCM and GCM steric sea level interped data nc save';
...
%                             lon_west = abs(lon - (lonlat(1)));
%                             min_lon_west=min(lon_west);
%                             lon_east = abs(lon - (lonlat(2)));
%                             min_lon_east=min(lon_east);
%                             lat_south = abs(lat - (lonlat(3)));
%                             min_lat_south=min(lat_south);
%                             lat_north = abs(lat - (lonlat(4)));
%                             min_lat_north=min(lat_north);
        ...
        % % % %                 % read Reconstructed DATA
                        cmemsfilename = strcat(cmemsdir,'cmems_nwp_ssh_', num2str(1993,'%04i'),'.nc');
...
% %                         comb_spatial_meancmems(:,:,monthij)=comb_spatial_meancmems(:,:,monthij)+cmems_adt/double(length(inputyear));        %                 comb_spatial_meancmems(:,:,monthij)=comb_spatial_meancmems(:,:,monthij)+cmems_adt/double(length(inputyear));
                        ...
                        interped_data_nomask = griddata(double(lon), double(lat), double(data_nomask),double(cmems_lon2),double(cmems_lat2));   
             
                        model_land=ones(size(interped_data_nomask));
                        model_land(isnan(interped_data_nomask))=1;
                        model_land(isfinite(interped_data_nomask))=NaN;
                        save([filedir,regionname, '_', testname, '_model_interped_land','.mat'], 'model_land');
                ...
                if (strcmp(testname, 'test57') || strcmp(testname, 'test58') || strcmp(testname, 'test59') || strcmp(testname, 'test60') || strcmp(testname, 'ens08'))
                    load('D:\Data\Model\CMIP5\zos\rcp45\post_corr_rcp45.mat');
                    corr_rcp_clim=reshape(corr_rcp,[length(corr_rcp)/12, 12]);
                    corr_rcp_clim_mean=mean(corr_rcp_clim,1);
                    
                    for loni=1:size(comb_data,1)
                        for lati=1:size(comb_data,2)
                            comb_data(loni,lati,:)=squeeze(comb_data(loni,lati,:))+squeeze(corr_rcp((min(inputyear)-2006)*12+1 : (max(inputyear)-2006)*12+12));
                            comb_spatial_meanmodel(loni,lati,:)=squeeze(comb_spatial_meanmodel(loni,lati,:))+squeeze(corr_rcp_clim_mean)';
                        end
                    end
                    for loni=1:size(comb_interped_data,1)
                        for lati=1:size(comb_interped_data,2)
                            comb_interped_data(loni,lati,:)=squeeze(comb_interped_data(loni,lati,:))+squeeze(corr_rcp((min(inputyear)-2006)*12+1 : (max(inputyear)-2006)*12+12));
                            comb_spatial_meaninterped(loni,lati,:)=squeeze(comb_spatial_meaninterped(loni,lati,:))+squeeze(corr_rcp_clim_mean)';
                        end
                    end
                end
                save(matname, 'mask_model', 'len_lon_model', 'len_lat_model', 'comb_spatial_meanmodel', 'comb_data', ...
                    'cmems_lonsize_cut', 'cmems_latsize_cut', 'comb_spatial_meaninterped', ...
                    'mask_cmems', 'comb_interped_data', ...
...
% % %         sea level analysis
        ...
        while (fig_flag)
            ...
            % % %         trend (seasonal filtered)
                ...
                sla_divided=reshape(comb_data,[len_lon_model, len_lat_model, 12, length(inputyear)]);
                sla_yearly=squeeze(mean(sla_divided,3,'omitnan')).*100;  %% m -> cm
                                
                % % %         interped exponential fitting yearly
                if max(inputyear)<=2005
                    tic;
                    corr_for_exp=abs(2*(min(sla_yearly(:))));
                    for i=1:len_lon_model
                        for j=1:len_lat_model
                            if isfinite(sla_yearly(i,j,1))
                                [f_linear, gof_linear] = fit(trendtime_yearly',squeeze(sla_yearly(i,j,:)),'poly1');
                                sla_yearly_linear_fit(i,j,1:length(trendtime_yearly))=f_linear(trendtime_yearly);
                                sla_yearly_linear_fit_rsquare(i,j)=gof_linear.rsquare;
                                sla_yearly_linear_fit_rmse(i,j)=gof_linear.rmse;
                            else
                                sla_yearly_linear_fit(i,j,1:length(trendtime_yearly))=NaN;
                                sla_yearly_linear_fit_rsquare(i,j)=NaN;
                                sla_yearly_linear_fit_rmse(i,j)=NaN;
                            end
                        end
                    end
                    toc;
                    ...
                    for i=1:len_lon_model
                        for j=1:len_lat_model
                            if isfinite(sla_yearly(i,j,1))
                                [f_exp, gof_exp] = fit(trendtime_yearly',squeeze(sla_yearly(i,j,:)),'exp1');
                                sla_yearly_exp_fit(i,j,1:length(trendtime_yearly))=f_exp(trendtime_yearly);
                                sla_yearly_exp_fit_rsquare(i,j)=gof_exp.rsquare;
                                sla_yearly_exp_fit_rmse(i,j)=gof_exp.rmse;
                            else
                                sla_yearly_exp_fit(i,j,1:length(trendtime_yearly))=NaN;
                                sla_yearly_exp_fit_rsquare(i,j)=NaN;
                                sla_yearly_exp_fit_rmse(i,j)=NaN;
                            end
                        end
                    end
                    toc;
...
%                 disp('climatological trend complete') 
            ...
            % % %         make ncfile
                ...
                yearly_time_dimid = netcdf.defDim(ncid, 'yearly_time', length(trendtime_yearly));
                ...
                sla_yearlyvarid=netcdf.defVar(ncid, 'sla_yearly', 'NC_FLOAT', [lon_dimid lat_dimid yearly_time_dimid]);
                netcdf.putAtt(ncid,sla_yearlyvarid,'long_name','sla_yearly');
                netcdf.putAtt(ncid,sla_yearlyvarid,'units','m');
                
                if max(inputyear)<=2005 
                    sla_yearly_linear_fitvarid=netcdf.defVar(ncid, 'sla_yearly_linear_fit', 'NC_FLOAT', [lon_dimid lat_dimid yearly_time_dimid]);
                    netcdf.putAtt(ncid,sla_yearly_linear_fitvarid,'long_name','sla_yearly_linear_fit');
                    netcdf.putAtt(ncid,sla_yearly_linear_fitvarid,'units','m');
                else
                    sla_yearly_exp_fitvarid=netcdf.defVar(ncid, 'sla_yearly_exp_fit', 'NC_FLOAT', [lon_dimid lat_dimid yearly_time_dimid]);
                    netcdf.putAtt(ncid,sla_yearly_exp_fitvarid,'long_name','sla_yearly_exp_fit');
                    netcdf.putAtt(ncid,sla_yearly_exp_fitvarid,'units','m');
                end
...
%                 netcdf.putAtt(ncid,clim_ssh_trendvarid,'units','m');
                ...
                netcdf.putVar(ncid, sla_yearlyvarid, [0 0 0], [len_lon_model len_lat_model length(trendtime_yearly)], sla_yearly);
                if max(inputyear)<=2005 
                    netcdf.putVar(ncid, sla_yearly_linear_fitvarid, [0 0 0], [len_lon_model len_lat_model length(trendtime_yearly)], sla_yearly_linear_fit);                                    
                else
                    netcdf.putVar(ncid, sla_yearly_exp_fitvarid, [0 0 0], [len_lon_model len_lat_model length(trendtime_yearly)], sla_yearly_exp_fit);                
                end
...
%                cmems_trend_filtered=ncread(cmemsfilename, 'cmems_trend_filtered');
               ...
               interped_ssh=comb_interped_data;
                ...
                interped_sla_yearly=squeeze(mean(interped_sla_divided,3,'omitnan')).*100;  %% m -> cm
                interped_ssh_divided=reshape(interped_ssh,[size(cmems_lon2,1), size(cmems_lon2,2), 12, length(inputyear)]);
                interped_ssh_yearly=squeeze(mean(interped_ssh_divided,3,'omitnan')).*100;  %% m -> cm
                ...
                for i=1:cmems_lonsize_cut
                    for j=1:cmems_latsize_cut
                        for k=1:size(interped_sla_yearly,3)
...
%                         interped_trend_yearly(i,j)=p(1) * 1000.0 ;
                ...
                toc;
%                 pcolor(interped_sla_yearly_exp_fit(:,:,end)')
%                 pcolor(interped_sla_yearly(:,:,end)')
%                 
%                 
                for i=1:cmems_lonsize_cut
                    for j=1:cmems_latsize_cut
...
% %                end
            ...
            % % %         make ncfile
                ...
                clim_interped_ssh_trendvarid=netcdf.defVar(ncid, 'clim_interped_ssh_trend', 'NC_FLOAT', [lon_cmems_dimid lat_cmems_dimid clim_time_dimid]);
                netcdf.putAtt(ncid,clim_interped_ssh_trendvarid,'long_name','clim_interped_ssh_trend');
                netcdf.putAtt(ncid,clim_interped_ssh_trendvarid,'units','m');
...
% %                 netcdf.putAtt(ncid,corrected_interped_sshvarid,'units','m ');
                ...
                netcdf.putAtt(ncid,interped_sla_yearlyvarid,'units','cm ');
                ...
                netcdf.putAtt(ncid,interped_sla_yearly_detrendedvarid,'units','cm ');
...
%                 netcdf.putVar(ncid, corrected_interped_sshvarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], comb_interped_data_corrected);
                ...
                netcdf.putVar(ncid, interped_sla_yearly_exp_fit_detrendedvarid, [0 0 0], [len_lon_cmems len_lat_cmems length(trendtime_yearly)], interped_sla_yearly_detrended);                
...
% %                 netcdf.putVar(ncid, corrected_interped_slavarid, [0 0 0], [len_lon_cmems len_lat_cmems length(ftime)], corrected_interped_sla);
                ...
                netcdf.putVar(ncid, clim_interped_ssh_trendvarid, [0 0 0], [len_lon_cmems len_lat_cmems 12], interped_trend_clim);
...
%                             eval(['comb_cmems_',num2str(nyear),'y_movmean(i,j,end-6*nyear+1:end)=NaN;'])
                    ...
                    eval(['trendtime_',num2str(nyear),'y_movmean=trendtime;'])
                    eval(['trendtime_',num2str(nyear),'y_movmean(1:6*nyear)=NaN;'])
                    eval(['trendtime_',num2str(nyear),'y_movmean(end-6*nyear+1:end)=NaN;'])
                end
                ...
                for nyeari=1:length(nyears)
                    ...
                    for i=1:cmems_lonsize_cut
                        for j=1:cmems_latsize_cut
                            eval(['temp_data=comb_interped_',num2str(nyear),'y_movmean(i,j,:);']);
                            eval(['p=polyfit(trendtime_', num2str(nyear), 'y_movmean(isfinite(temp_data)),temp_data(isfinite(temp_data)),1);']);
                            eval(['trend_',num2str(nyear),'y_movmean','(i,j)=p(1) * 1000.0 ;']);
                        end
                    end
                end
%                 temp_data=comb_interped_1y_movmean(50,50,:);
%                 p=polyfit(trendtime_1y_movmean(isfinite(temp_data)),temp_data(isfinite(temp_data)),1)
               disp('movmean trend complete') 
            ...
            % % %         make ncfile
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    ...
                    eval([nc_varname,'varid=netcdf.defVar(ncid,','''', ...
                        nc_varname,'''',',', '''', 'NC_FLOAT','''',',', '[lon_cmems_dimid lat_cmems_dimid]);'])
                    eval(['netcdf.putAtt(ncid,', nc_varname,'varid,', '''', 'long_name','''', ',', '''', nc_varname,'''',');'])
                    eval(['netcdf.putAtt(ncid,', nc_varname,'varid,', '''', 'units'    ,'''', ',', '''', 'mm/y', '''', ');']);
                    end
                end
...
% % %         get ts & calculate zosto
    ...
    %                          pcolor(T_src(:,:,40)); shading flat; colorbar
    %                         pcolor(P(:,:,1)'); shading flat; colorbar
    %                         pcolor(rho_src(:,:,40)'); shading flat; colorbar
    %                         pcolor(zosto(:,:,1)'); shading flat; colorbar
...
%                 if (exist(landname , 'file') ~= 2 || fig_flag==2)   
%                     load('D:\Data\Model\ROMS\nwp_1_20\test53\run\AKP4_test53_model_land.mat')
...
%         pcolor(comb_zosto2(:,:,1)'); shading flat; colorbar; colormap jet;
%         pcolor(zosto_2(:,:)'); shading flat; colorbar; colormap jet;
%         figure;
%         pcolor(comb_zosto(:,:,1)'); shading flat; colorbar; colormap jet;
%         pcolor(comb_zosto(:,:,360)'-comb_zosto(:,:,12)'); shading flat; colorbar; colormap(bwrmap); caxis([-0.3 0.3]);
%         plot(squeeze(mean(mean(comb_zosto.*mask_model,1,'omitnan'),2,'omitnan')))
%         
%         abc=comb_zosto.*mask_model;
%         filename = 'J:\Data\Model\ROMS\nwp_1_20\test55\run\2005\test55_monthly_2005_01.nc';
%         mask_rho=ncread(filename,'mask_rho',[lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
%         mask_rho(mask_rho<1)=NaN;
%         abcd=comb_zosto.*mask_model.*mask_rho;
%         plot(squeeze(mean(mean(comb_zosto.*mask_model.*mask_rho,1,'omitnan'),2,'omitnan')))
% 
%         
...
%                 comb_interped_zosto_correct(:,:,noise_ind) = (comb_interped_zosto_correct(:,:,noise_ind-1) +  comb_interped_zosto_correct(:,:,noise_ind+2))/2.0;
%                 comb_interped_ssh_correct(:,:,noise_ind) = (comb_interped_ssh_correct(:,:,noise_ind-1) +  comb_interped_ssh_correct(:,:,noise_ind+2))/2.0;
%                 
%                 for i=1:size(comb_interped_zosto,1)
%                     for j=1:size(comb_interped_zosto,2)
%                         raw_zosto=squeeze(comb_interped_zosto(i,j,:));
%                         std5_zosto = 4.* std(raw_zosto(1:12));
%                         diff_zosto = diff(raw_zosto);
%                         
%                         raw_ssh= squeeze(comb_interped_ssh_correct(i,j,:));
%                         diff_ssh = diff(raw_ssh);
%                         if diff_zosto(end)> std5_zosto
%                             raw_zosto(end) = raw_zosto(end-1) + diff_zosto(end-1);
%                             raw_ssh(end) = raw_ssh(end-1) + diff_ssh(end-1);
%                             diff_zosto(end)=diff_zosto(end-1);
%                         end
%                         raw_zosto(find(diff_zosto>std5_zosto)+1)= (raw_zosto(find(diff_zosto>std5_zosto)) + raw_zosto(find(diff_zosto>std5_zosto)+2))./2.0;
%                         raw_ssh(find(diff_zosto>std5_zosto)+1)= (raw_ssh(find(diff_zosto>std5_zosto)) + raw_ssh(find(diff_zosto>std5_zosto)+2))./2.0;
% 
%                         continuity_error=diff(find(diff_zosto>std5_zosto));
%                         continuity_error(continuity_error==1)=55555;
%                         if continuity_error==55555
...
%                 mask_rho = ncread(rawsshfilename, 'trend');
%                 mask_rho(isfinite(mask_rho))=1;
%                 raw_grid_zosto = comb_zosto(:,:,1) .* mask_rho;
%                 interped_mask_zosto = griddata(double(lon), double(lat), double(raw_grid_zosto),double(lon_glo),double(lat_glo));   
%                 interped_mask_zosto(isfinite(interped_mask_zosto))=1;
