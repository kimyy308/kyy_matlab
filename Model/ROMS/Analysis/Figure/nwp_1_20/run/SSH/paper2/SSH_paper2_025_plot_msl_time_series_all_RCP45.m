...
% all_testname2 = {'test11', 'test12'};
RCM_testnames = {'test57', 'test58', 'test59', 'test60', 'ens07'};
GCM_testnames = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens07'};
...
if (strcmp(system_name,'PCWIN64'))
    % % for windows
    figrawdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\SSH\5th_year\figure\all\',regionname,'\'); % % where figure files will be saved
...
for folding=1:1
        jpgname=strcat(outfile, '_', testname, '_',regionname, '_msla_seasonal_filtered_cmems_', ...
...
%             constant_cor=corrcoef(RCM_interped_sla_seasonal_filtered(5,:),cmems_sla_seasonal_filtered);
%             txt1=text(xData2(5), min(meanplotlev2)+diff(meanplotlev2)/16.0 ,['R(RCM) = ', num2str(round(constant_cor(1,2),2))], 'FontSize', 20); 
%             constant_cor=corrcoef(GCM_interped_sla_seasonal_filtered(5,:),cmems_sla_seasonal_filtered);
...
for folding=1:1
        jpgname=strcat(outfile, '_', testname, '_',regionname, '_msla_yearly_mean_cmems_', ...
            num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
%         if (exist(jpgname , 'file') ~= 2)
%             for varind=1:length(inputyear)*12
%                 msl_filt(varind)=squeeze(mean(mean(ncread(filename,'ssh_filtered',[1 1 varind], [inf inf 1]),1,'omitnan'),2,'omitnan'));
...
%             constant_cor=corrcoef(RCM_interped_sla_yearly_mean(5,:),cmems_sla_yearly_mean);
%             txt1=text(xData2(5), min(meanplotlev2)+diff(meanplotlev2)/16.0 ,['R(RCM) = ', num2str(round(constant_cor(1,2),2))], 'FontSize', 20); 
%             constant_cor=corrcoef(GCM_interped_sla_yearly_mean(5,:),cmems_sla_yearly_mean);
%             txt1=text(xData2(10), min(meanplotlev2)+diff(meanplotlev2)/16.0 ,['R(GCM) = ', num2str(round(constant_cor(1,2),2))], 'FontSize', 20); 
% 
            set(gcf,'PaperPosition', [0 0 36 12]) 
            hold off
