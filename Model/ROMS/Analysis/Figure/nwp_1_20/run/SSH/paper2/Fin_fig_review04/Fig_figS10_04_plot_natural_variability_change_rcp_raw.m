...
for regionind2=1:length(all_region2)
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
            figrawdir =strcat('D:\Research\Ph_D_course\2020_SSH_CMIP5_decadal_variation_around_korea_using_downscaling_ocean_model\fin_fig_r4\'); % % where figure files will be saved
        ...
        tifname=strcat(figrawdir, 'figS10_04','.tif'); %% ~_year_month.jpg
        
        test_text={'(A) RCM-IPSL-L', '(B) RCM-IPSL-M', '(C) RCM-Nor', '(D) RCM-MPI','(E) RCM-IPSL-L', '(F) RCM-IPSL-M', '(G) RCM-Nor', '(H) RCM-MPI'};
%         test_text2={'(RCP 4.5 - Historical)', '(RCP 4.5 - Historical)', '(RCP 4.5 - Historical)', '(RCP 4.5 - Historical)', ...
%             '(RCP 8.5 - Historical)', '(RCP 8.5 - Historical)', '(RCP 8.5 - Historical)', '(RCP 8.5 - Historical)'};
        test_text2={'(RCP 4.5)', '(RCP 4.5)', '(RCP 4.5)', '(RCP 4.5)','(RCP 8.5)', '(RCP 8.5)', '(RCP 8.5)', '(RCP 8.5)'};
        for i=1:8
            test_text3{i}='(Future - Past)';
        end
...
%             all_rcp_std(:,:,testnameind) = rcp_std;
            all_diff_det_std(:,:,testnameind) = diff_det_std;
...
%             land_model_interped=get_land_grid(filename,lonlat,lon_rho,lat_rho);
            ...
            mean_data=all_diff_det_std(:,:,testnameind);
            
            ysmask=Func_0005_get_mask_from_polygon(lon_rho, lat_rho, ys_khoapolygon);
...
% load([tidedir,regionname, '_', testname, '_model_interped_land','.mat']);
           ...
           if testnameind==4
            ...
            txt{testnameind,3}=m_text(119, 46, test_text3{testnameind}, 'FontSize', m_grid_fontsize+4); 
            RCM_lon{testnameind}=lon_rho;
            RCM_lat{testnameind}=lat_rho;
            RCM_SSH_std_future{testnameind}=mean_data;
        end
%      ---------- RCP 4.5
...
%         saveas(gcf,tifname,'tif');
        ...
        hold off;
        close all;
        save (['D:\research\Ph_D_course\2020_SSH_CMIP5_decadal_variation_around_korea_using_downscaling_ocean_model', ...
            '\data_dryad\DataS10_Natural_variaibility_RCP.mat'], ...
            'RCM_lon', 'RCM_lat', 'RCM_SSH_std_future', ...
