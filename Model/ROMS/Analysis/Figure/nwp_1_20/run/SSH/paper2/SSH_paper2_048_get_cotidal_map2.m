...
for testnameind2=1:length(all_testname2)
    for regionind2=1:length(all_region2)
        for yearind2=1:length(inputyear)
            ...
            outputdir=['/data1/kimyy/Model/ROMS/nwp_1_20/output/', testname, '/run/',num2str(tempyear)];
            ...
            lonfilename=['/data1/kimyy/Model/ROMS/nwp_1_20/output/', testname, '/run/',num2str(tempyear),'/ocean_his_lon_rho_AKP4', '.nc'];
            latfilename=['/data1/kimyy/Model/ROMS/nwp_1_20/output/', testname, '/run/',num2str(tempyear),'/ocean_his_lat_rho_AKP4', '.nc'];
...
%             Model.amp_O1(1:lon_max(1)-lon_min(1)+1,1:lat_max(1)-lat_min(1)+1)=NaN;
            ...
            for lon_loop=lon_min(1) : lon_max(1)
                for lat_loop=lat_min(1) : lat_max(1)
                    loni=lon_loop-lon_min(1)+1;
                    lati=lat_loop-lat_min(1)+1;
                    filename=[outputdir,'/ocean_his_', varname, '_AKP4_', num2str(1, '%04i'), '.nc'];
...
%                             dayi
            ...
            toc;
            save([outputdir,'/', testname, '_', 'harmonic_analysis_',varname, '_', regionname, '_', num2str(tempyear, '%04i'), '.mat'])

            ncoutfilename=[outputdir,'/', testname, '_', 'harmonic_analysis_', varname, '_', regionname, '_', num2str(tempyear, '%04i'), '.nc'];
