...
%         lon_glo = lon_cmems(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
%         lat_glo = lat_cmems(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
...
%                         all_m2_amp(1:size(temp_tcon,1), 1:size(temp_tcon,2), temp_testind)=temp_tcon(:,:,tide_info.index(1),1).*100;
            temp_diffname=[temp_filedir, temp_testname, '_local_vuln_sl_', num2str(max(inputyear),'%04i'), '-', num2str(min(inputyear),'%04i'), '.mat'];
            clear slr_ind
            load(temp_diffname)
        ...
        [mean_ysecs_total_slr(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(ysecs_total_slr, lon_glo, lat_glo);
        [mean_es_total_slr(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(es_total_slr, lon_glo, lat_glo);
        max_ysecs_total_slr(testnameind,1) = max(ysecs_total_slr(:));
        max_es_total_slr(testnameind,1) = max(es_total_slr(:));
        lon_glo(find(ysecs_total_slr==max(ysecs_total_slr(:))))
        lat_glo(find(ysecs_total_slr==max(ysecs_total_slr(:))))
        lon_glo(find(es_total_slr==max(es_total_slr(:))))
        lat_glo(find(es_total_slr==max(es_total_slr(:))))
