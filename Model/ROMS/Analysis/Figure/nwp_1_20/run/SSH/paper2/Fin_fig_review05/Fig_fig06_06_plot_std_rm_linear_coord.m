...
for regionind2=1:length(all_region2)
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
%             figrawdir =strcat('D:\MEPL\project\SSH\5th_year\figure\paper2\fin_fig_r5\'); % % where figure files will be saved
        ...
        hold on;
        tifname=strcat(figrawdir, 'fig06_06','.tif'); %% ~_year_month.jpg
...
%         load(['D:\Data\Model\ROMS\nwp_1_20\',testname,'\run\',regionname,'sst_rms_and_bias_',num2str(inputyear(1),'%04i'),'_',num2str(inputyear(end)),'.mat']);
        ...
        load([filedir, 'AKP4_cmems_linear_det_std_1993_2005.mat']);
...
%         mean_data= mean_data .* mask_model;    
        ...
        m_grid('fontsize', m_grid_fontsize, 'tickdir', m_grid_tickdir_type,  'box', m_grid_box_type, ...
                'xticklabels', [120,  130, 140], 'xtick',[120, 130, 140], 'xaxislocation', 'top', ...
        ...
        CMEMS_SSH_std = model_sla_std;
        %         EKWC 
                    [indw, inde, inds, indn]=findind_Y(1/20, [131 131 38 38], cut_lon_rho, cut_lat_rho);
                    model_sla_std(indw:inde, inds:indn); mean(ans(:),'omitnan')   
...
%      ---------- GCM-IPSL-L
        ...
        for testnameind=1:4
            ...
            load([cmip5dir,regionname,'_',testname, '_model_linear_', 'det_std','_',yearstr_min, '_', yearstr_max, '.mat']);
            ...
            %         EKWC 
                    [indw, inde, inds, indn]=findind_Y(1/20, [131 131 38 38], cut_lon_rho, cut_lat_rho);
                    mean_data(indw:inde, inds:indn); 
                    ekwcstd(testnameind)=mean(ans(:),'omitnan');  
            
            std_all(testnameind)= mean(mean_data(:), 'omitnan');
            m_proj(m_proj_name,'lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
            ...
            GCM_SSH_std{testnameind}=mean_data;
...
%         testnameind=5;
            ...
            load([filedir,regionname,'_',testname, '_model_linear_', 'det_std','_',yearstr_min, '_', yearstr_max, '.mat']);
            mask_model = double(inpolygon(cut_lon_rho,cut_lat_rho,refpolygon(:,1),refpolygon(:,2)));
            mask_model(mask_model==0)=NaN;
            ...
            RCM_SSH_std{testnameind-4}=mean_data;
            ...
            ysmask=Func_0005_get_mask_from_polygon(cut_lon_rho, cut_lat_rho, ys_khoapolygon);
            ys_std=mean_data.*ysmask;
            [mean_ys_std(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(ys_std, cut_lon_rho, cut_lat_rho);
...
%         saveas(gcf,tifname,'tif');
        ...
        hold off;
        close all;
        save ('D:\research\Ph_D_course\2020_SSH_CMIP5_decadal_variation_around_korea_using_downscaling_ocean_model\data_dryad\Data06_SSH_std.mat', ...
            'CMEMS_lon', 'CMEMS_lat', 'CMEMS_SSH_std', ...
            'GCM_lon', 'GCM_lat', 'GCM_SSH_std', ...
            'RCM_lon', 'RCM_lat', 'RCM_SSH_std', ...
