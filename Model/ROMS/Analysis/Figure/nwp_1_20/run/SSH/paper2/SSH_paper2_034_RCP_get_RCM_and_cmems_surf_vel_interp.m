...
%         if (strcmp(testname,'test61')==1 || strcmp(testname,'test62')==1 || strcmp(testname,'test63')==1 || strcmp(testname,'test64')==1 || strcmp(testname,'ens09')==1)
%             drivename='H';
%         elseif (strcmp(testname,'test65')==1 || strcmp(testname,'test66')==1 || strcmp(testname,'test67')==1 || strcmp(testname,'test68')==1 || strcmp(testname,'ens10')==1)
%             drivename='G';
%         end
        switch testname
...
% % %         get model data and cmems data      
            ...
            if (exist(matname , 'file') ~= 2 || fig_flag==2)   
                for yearij = 1:length(inputyear)
                    for monthij = 1:length(inputmonth)
                        ...
                        varname ='u';
                        u = ncread(filename,varname,[lon_min(1) lat_min(1) modelinfo.Dimensions(2).Length 1], [lon_max(1)-lon_min(1) lat_max(1)-lat_min(1)+1 1 1]);
                        u_rho=u2rho_2d(u')';
                        u_rho=u_rho.*mask_model;
                        
                        varname ='v';
                        v = ncread(filename,varname,[lon_min(1) lat_min(1) modelinfo.Dimensions(2).Length 1], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1) 1 1]);
                        v_rho=v2rho_2d(v')';
                        v_rho=v_rho.*mask_model;
                        
                        len_lon_model = size(u_rho,1);
                        len_lat_model = size(u_rho,2);

                        if (exist('comb_spatial_meanmodel')==0)
                            comb_spatial_mean_u_rho=(zeros([len_lon_model,len_lat_model,12]));
                            comb_spatial_mean_v_rho=(zeros([len_lon_model,len_lat_model,12]));   
                            del=(zeros([len_lon_model,len_lat_model,12]));
                        end

                        comb_u_rho(:,:,ind) = single(u_rho);
                        comb_v_rho(:,:,ind) = single(v_rho);
                        comb_speed(:,:,ind) = sqrt(single(u_rho).^2 + single(v_rho).^2);
                        comb_spatial_mean_u_rho(:,:,monthij)=comb_spatial_mean_u_rho(:,:,monthij)+u_rho/double(length(inputyear));
                        comb_spatial_mean_v_rho(:,:,monthij)=comb_spatial_mean_v_rho(:,:,monthij)+v_rho/double(length(inputyear));
        ...
        %                     comb_spatial_meanmodel=(zeros([length(cmems_lon),length(cmems_lat),12]));
                        ...
                        interped_u_rho = griddata(double(lon), double(lat), u_rho, double(cmems_lon2),double(cmems_lat2));   
                        interped_v_rho = griddata(double(lon), double(lat), v_rho, double(cmems_lon2),double(cmems_lat2));   
%                         interped_uv_rho = [interped_u_rho, interped_v_rho];
...
%                         cmems_uv_rho = [cmems_u_rho, cmems_v_rho]; % x 2y
...
%                         comb_interped_uv_rho(:,:,ind) = interped_uv_rho;
                ...
                save(matname, 'mask_model', 'len_lon_model', 'len_lat_model', 'comb_u_rho', 'comb_v_rho', ...
                    ...
                    'cmems_lon2', 'cmems_lat2', 'lon', 'lat', 'comb_speed', 'comb_interped_speed', '-v7.3');
            else
                load(matname);
            end
            fig_flag=0;
...
% % %     interped sea level trend analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir,  testname,'_',regionname, 'cmems_interped_vel_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
