...
% all_region ={'GLO'};
% all_region ={'NWP', 'AKP4', 'ES_KHOA','YS_KHOA', 'SS_KHOA'};
% all_region ={'NWP', 'AKP4', 'ES_KHOA','YS_KHOA', 'SS_KHOA'};

% all_region ={'ES_KHOA','YS_KHOA', 'SS_KHOA'};

% all_testname = {'test11', 'test12'};
% all_testname = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens09'};
% all_testname = {'ens09'};

% all_testname = {'ens10'};
all_testname = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens08'};

% all_testname = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens10'};
...
%         trend_filt=ncread('E:\Data\Model\CMIP5\zos\rcp45\interp\ens07\ens07_SScmems_interped_ssh_trend_2006_2100.nc','interped_trend_filtered');
%         trend_filt=ncread('J:\Data\Model\CMIP5\zos\rcp45\interp\ens08\ens08_AKP4_ssh_analysis_2006_2100.nc','trend_filtered');
                ...
                for yearij = 1:length(inputyear)
                    for monthij = 1:length(inputmonth)
                        ...
                        filename = strcat(filedir,  ...
                                'zos_interp_', testname, '_', scenname, '_r1i1p1_', num2str(tempyear,'%04i'), '.nc');
                        % read model data
...
%                             lat = ncread(filename,'lat_rho', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
        ...
        % % % %                 % read Reconstructed DATA
                        switch(regionname)
                            case('GLO') %% North western Pacific
                                cmemsfilename = strcat(cmemsdir,'cmems_glo_ssh_', num2str(inputyear(1),'%04i'),'0101.nc');                                
                            otherwise
...
% %                         comb_spatial_meancmems(:,:,monthij)=comb_spatial_meancmems(:,:,monthij)+cmems_adt/double(length(inputyear));        %                 comb_spatial_meancmems(:,:,monthij)=comb_spatial_meancmems(:,:,monthij)+cmems_adt/double(length(inputyear));
                        ...
                        cmip5_land=ones(size(interped_data_nomask));
                        cmip5_land(isnan(interped_data_nomask))=1;
                        cmip5_land(isfinite(interped_data_nomask))=NaN;
                        save([cmip5dir,regionname, '_', testname, '_cmip5_interped_land','.mat'], 'cmip5_land');
                ...
                save(matname, 'mask_model', 'len_lon_model', 'len_lat_model', 'comb_spatial_meanmodel', 'comb_data', ...
                    ...
                    'mask_cmems', 'comb_interped_data', 'comb_spatial_meaninterped', ...
...
% % %     cmems sea level trend analysis
        ...
        while (fig_flag)
            ncoutfilename = strcat(filedir, testname,'_',regionname, 'cmems_ssh_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
            if (exist(ncoutfilename , 'file') ~= 2 || fig_flag==2)   
