...
for regionind2=1:length(all_region2)
        ...
        tifname=strcat(figrawdir, 'fig10_03','.tif'); %% ~_year_month.jpg
...
%             landdir = strcat('G', ':\Data\Model\ROMS\nwp_1_20\', 'test65', '\run\'); % % where data files are
            ...
            if testnameind<=rcmcount
                load([rcmdir, testname,'_',regionname, '_rcm_slc_hist_rcp.mat']);
                all_slc_raw(:,:,testnameind) = rcm_slc.*100;
                all_mean_slc_raw(testnameind,1) = mean_rcm_slc.*100;
            else
                all_slc_raw(:,:,testnameind) = mean(all_slc_raw(:,:,rcmcount-3:rcmcount),3);
                all_mean_slc_raw(testnameind,1) = mean(all_mean_slc_raw(rcmcount-3:rcmcount));
            end
            mean_data_glo = all_slc(:,:,testnameind);
            mean_data = all_slc_raw(:,:,testnameind);
            ...
            ysecs_slc=all_slc(:,:,testnameind).*ysecsmask;
            es_slc=all_slc(:,:,testnameind).*esmask;
            [mean_all_slc(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(mean_data_glo, lon_glo, lat_glo);
            [mean_ysecs_slc(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(ysecs_slc, lon_glo, lat_glo);
            [mean_es_slc(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(es_slc, lon_glo, lat_glo);
            ysecs_std_all_slc(testnameind,1) = std(ysecs_slc(:),'omitnan');
            es_std_all_slc(testnameind,1) = std(es_slc(:),'omitnan');
            ...
            ysecsmask_raw=Func_0005_get_mask_from_polygon(lon_rho, lat_rho, ysecs_khoapolygon);
            esmask_raw=Func_0005_get_mask_from_polygon(lon_rho, lat_rho, es_khoapolygon);
            ysecs_slc_raw=all_slc_raw(:,:,testnameind).*ysecsmask_raw;
            es_slc_raw=all_slc_raw(:,:,testnameind).*esmask_raw;
            [mean_all_slc_raw(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(mean_data, lon_rho, lat_rho);
            [mean_ysecs_slc_raw(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(ysecs_slc_raw, lon_rho, lat_rho);
            [mean_es_slc_raw(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(es_slc_raw, lon_rho, lat_rho);
            ysecs_std_all_slc_raw(testnameind,1) = std(ysecs_slc_raw(:),'omitnan');
            es_std_all_slc_raw(testnameind,1) = std(es_slc_raw(:),'omitnan');
            es_max_all_slc_raw(testnameind,1) = max(es_slc_raw(:));
            es_min_all_slc_raw(testnameind,1) = min(es_slc_raw(:));
...
%                         'labelspacing', 50000,'Rotation', 0,'fontweight', 'bold');
            ...
            txt{testnameind,2}=m_text(118, 49, test_text{testnameind}, 'FontSize', m_grid_fontsize+4); 
            RCM_lon{testnameind-5}=lon_rho;
            RCM_lat{testnameind-5}=lat_rho;
            RCM_SLR{testnameind-5}=mean_data;
        end
...
%             pos_sb{testnameind} = pos_sb{testnameind} + correction_large_fig;
            ...
            DIFF_SLR{testnameind-10}=mean_data;
        end
...
%         saveas(gcf,tifname,'tif');
        ...
        hold off;
        close all;
        save ('Z:\내 드라이브\research\Ph_D_course\2020_SSH_CMIP5_decadal_variation_around_korea_using_downscaling_ocean_model\data_dryad\Data10_SLR_rcp85.mat', ...
            'GCM_lon', 'GCM_lat', 'GCM_SLR', ...
            'RCM_lon', 'RCM_lat', 'RCM_SLR', ...
            'DIFF_lon', 'DIFF_lat', 'DIFF_SLR', ...
