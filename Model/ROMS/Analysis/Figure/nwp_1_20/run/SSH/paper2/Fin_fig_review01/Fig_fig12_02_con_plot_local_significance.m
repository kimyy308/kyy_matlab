...
for regionind2=1:length(all_region2)
        ...
        hold on;
        tifname=strcat(figrawdir, 'fig12_02','.tif'); %% ~_year_month.jpg
...
%                         all_m2_amp(1:size(temp_tcon,1), 1:size(temp_tcon,2), temp_testind)=temp_tcon(:,:,tide_info.index(1),1).*100;
        ...
        shading(gca,m_pcolor_shading_method);   
        hold on
        [C{testnameind,2},h{testnameind,2}]=m_contour(cut_lon_rho',cut_lat_rho', mean_data', [10:5:70], 'color','k', ...
                    'linewidth', 1, 'linestyle', '-', 'parent', ax{testnameind,2});
                clabel(C{testnameind,2},h{testnameind,2},'FontSize',m_grid_fontsize-2,'Color','k', ...
                    'labelspacing', 50000,'Rotation', 0,'fontweight', 'bold');        colormap(ax{testnameind,2},jet);
