...
% cmems_2y_lowpass_sla = ncread(RCM_cmemsfilenames{1}, 'cmems_2y_lowpass');
% cmems_5y_lowpass_sla = ncread(RCM_cmemsfilenames{1}, 'cmems_5y_lowpass');
...
for folding=1:1
        jpgname=strcat(outfile, '_', testname, '_',regionname, '_msla_seasonal_filtered_', ...
...
%             cmems_1y_mean=mean(cmems_sla_seasonal_filtered(1:12*5));
            ...
            for testind=1:length(RCM_testnames)
                ...
                RCM_interped_sla_seasonal_filtered(testind,:)=RCM_interped_sla_seasonal_filtered(testind,:)+(cmems_1y_mean-RCM_interped_1y_mean(testind));
                GCM_interped_sla_seasonal_filtered(testind,:)=GCM_interped_sla_seasonal_filtered(testind,:)+(cmems_1y_mean-GCM_interped_1y_mean(testind));
...
%             cmap2 = get(mslplot2_all{5},'defaultaxescolororder');
            ...
            constant_cor=corrcoef(RCM_interped_sla_seasonal_filtered(length(RCM_testnames),:),cmems_sla_seasonal_filtered);
            txt1=text(xData2(5), min(meanplotlev2)+diff(meanplotlev2)/16.0 ,['R(RCM) = ', num2str(round(constant_cor(1,2),2))], 'FontSize', 20); 
            constant_cor=corrcoef(GCM_interped_sla_seasonal_filtered(length(RCM_testnames),:),cmems_sla_seasonal_filtered);
...
for folding=1:1
        jpgname=strcat(outfile, '_', testname, '_',regionname, '_msla_yearly_mean_', ...
            num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
%         if (exist(jpgname , 'file') ~= 2)
%             for varind=1:length(inputyear)*12
%                 msl_filt(varind)=squeeze(mean(mean(ncread(filename,'ssh_filtered',[1 1 varind], [inf inf 1]),1,'omitnan'),2,'omitnan'));
...
%             cmems_1y_mean=mean(cmems_sla_seasonal_filtered(1:12*5));
            ...
            hold on
            cmems_1y_mean=mean(cmems_sla_yearly_mean(1:5));                        
            for testind=1:length(RCM_testnames)
...
%             cmap2 = get(mslplot2_all{5},'defaultaxescolororder');
            ...
            set(gca,'FontSize',15);
            grid on
            lgd=legend('RCM-ens','GCM-ens','SAT');
            set(lgd,'FontSize',15);
%             set(lgd,'Position',[0.13 0.88, 0.775, 0.03]);
            set(lgd,'Position',[0.13 0.25, 0.775, 0.03]);

            set(lgd,'Orientation','horizontal');
            constant_cor=corrcoef(RCM_interped_sla_yearly_mean(length(RCM_testnames),:),cmems_sla_yearly_mean);
            txt1=text(xData2(7), min(meanplotlev2)+diff(meanplotlev2)/16.0 ,['R(RCM) = ', num2str(round(constant_cor(1,2),2))], 'FontSize', 20); 
...
%             cmems_1y_mean=mean(cmems_sla_seasonal_filtered(1:12*5));
            ...
            for testind=1:length(RCM_testnames)-1
                ...
                GCM_interped_sla_yearly_mean(testind,:)=GCM_interped_sla_yearly_mean(testind,:)-(cmems_1y_mean-GCM_interped_1y_mean(testind));
%                 mslplot_all{testind}=plot(xData2,RCM_interped_sla_yearly_mean(testind,:),'r')
                mslplot2_all{testind}=plot(xData2,GCM_interped_sla_yearly_mean(testind,:).*100,'b')
                gcm_mse(testind,:)=(GCM_interped_sla_yearly_mean(testind,:).*100-cmems_sla_yearly_mean.*100).^2
                gcm_tempcorr=corrcoef(GCM_interped_sla_yearly_mean(testind,:).*100, cmems_sla_yearly_mean.*100);
                corr_gcm(testind)=gcm_tempcorr(1,2);
            end
            rms_gcm=sqrt(mean(gcm_mse,2));
            mslplot=plot(xData2,cmems_sla_yearly_mean.*100.0,'k')
...
%             cmems_1y_mean=mean(cmems_sla_seasonal_filtered(1:12*5));
            ...
            for testind=1:length(RCM_testnames)-1
                ...
                RCM_interped_sla_yearly_mean(testind,:)=RCM_interped_sla_yearly_mean(testind,:)+(cmems_1y_mean-RCM_interped_1y_mean(testind));
                GCM_interped_sla_yearly_mean(testind,:)=GCM_interped_sla_yearly_mean(testind,:)-(cmems_1y_mean-GCM_interped_1y_mean(testind));
                mslplot_all{testind}=plot(xData2,RCM_interped_sla_yearly_mean(testind,:).*100,'r')
%                 mslplot2_all{testind}=plot(xData2,GCM_interped_sla_yearly_mean(testind,:),'b')
                rcm_mse(testind,:)=(RCM_interped_sla_yearly_mean(testind,:).*100-cmems_sla_yearly_mean.*100).^2
                rcm_tempcorr=corrcoef(RCM_interped_sla_yearly_mean(testind,:).*100, cmems_sla_yearly_mean.*100);
                corr_rcm(testind)=rcm_tempcorr(1,2);
            end
            rms_rcm=sqrt(mean(rcm_mse,2));
            mslplot=plot(xData2,cmems_sla_yearly_mean.*100.0,'k')
