...
for regionind2=1:length(all_region2)
        ...
        hold on;
        tifname=strcat(figrawdir, 'R1Q3C9_M2','.tif'); %% ~_year_month.jpg
...
% start-------------------- validiation point dot
        ...
        amplev=([0 max(table2array(tcon_obs(:,tidalind)))]);
        ...
        hold on;
        amp_4con=sum(table2array(tcon_obs(:,tidalind)),2);
...
%         obs_4con =  sum(all_tcon_obs,2);
        model_4con= all_tcon_model(:,tidalind-4);
        obs_4con =  all_tcon_obs(:,tidalind-4);
...
%                     title(['all test', ', 4con amp, ', num2str(tempyear,'%04i')])
        all_4con = [obs_4con model_4con];
        maxval=max(all_4con(:));
        xlim([0 maxval])
        ylim([0 maxval])
        axis equal;
%         set(ax{2}, 'style', 'equal')
...
%                     lgd=legend(all_testname2);
        ...
        set(lgd,'FontSize',m_grid_fontsize-2);
%         set(lgd,'Position',[0.75 0.20, 0.20, 0.03]);
        set(lgd,'Position',[0.75 0.30, 0.20, 0.03]);  % for equal plot
