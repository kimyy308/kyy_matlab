...
% all_testname2 = {'test57', 'test58','test59' ,'test60', 'test65', 'test66', 'test67','test68', 'ens08','ens10'};
all_testname2 = {'ens08','ens10'};
...
%         varname ='zeta'
    ...
    % % %         flag configuration
        for folding=1:1
            ...
            fig_flags{8,1}='decadal Surface variable difference plot';
        end
        for flagi=1:9
            fig_flags{flagi,2}=0;
        end
        ...
        while (fig_flag)
            for varind2=1:length(all_var2)
                ...
                if (exist(pngname , 'file') ~= 2 || fig_flag==2)        
                    ...
                    set(gcf,'PaperPosition', [paper_position_hor paper_position_ver paper_position_width paper_position_height]) 
                    saveas(gcf,pngname,'tif');
                    pngname2= strcat('Z:\내 드라이브\MEPL\project\SSH\5th_year\figure\GIS\', testname,'_',regionname, '_clim_', variable,'_',num2str(min(inputyear1),'%04i'), ...
                    '_',num2str(max(inputyear1),'%04i'), '.tif'); %% ~_year_month.jpg
                    saveas(gcf,pngname2,'tif');
                    save(['Z:\내 드라이브\MEPL\project\SSH\5th_year\figure\GIS\', testname, '_', variable,'_',num2str(min(inputyear1),'%04i'), ...
                    '_',num2str(max(inputyear1),'%04i'), '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'mean_data');
                    close all;
        ...
        while (fig_flag)
            for varind2=1:length(all_var2)
                ...
                if (exist(pngname , 'file') ~= 2 || fig_flag==2)        
                    ...
                    set(gcf,'PaperPosition', [paper_position_hor paper_position_ver paper_position_width paper_position_height]) 
                    saveas(gcf,pngname,'tif');
                    pngname2= strcat('Z:\내 드라이브\MEPL\project\SSH\5th_year\figure\GIS\', testname,'_',regionname, '_clim_', variable,'_',num2str(min(inputyear2),'%04i'), ...
                    '_',num2str(max(inputyear2),'%04i'), '.tif'); %% ~_year_month.jpg
                    saveas(gcf,pngname2,'tif');
                    save(['Z:\내 드라이브\MEPL\project\SSH\5th_year\figure\GIS\', testname, '_', variable,'_',num2str(min(inputyear2),'%04i'), ...
                    '_',num2str(max(inputyear2),'%04i'), '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'mean_data');
                    close all;
                    clear lon_rho mean_data
                end
            end
...
% start-------------------- earlier decadal SST, SSS plot
        ...
        while (fig_flag)
            for varind2=1:length(all_var2)
                variable=all_var2{varind2};
                pngname=strcat(outfile, '_', testname,'_',regionname, '_diff_', variable, '.tif'); %% ~_year_month.jpg
                if (exist(pngname , 'file') ~= 2 || fig_flag==2)        
                    run(param_script);
                    for yearij=1:length(inputyear1)
                        tempyear=inputyear1(yearij);
                        ...
                        for monthij=1:length(inputmonth)
                            ...
                            if (exist('mean_data1' , 'var') ~= 1)
                                mean_data1=zeros(size(data));
                            end
                            mean_data1=mean_data1 + (data / (length(inputyear1) * length(inputmonth)));
                        end
                    end
                    mask_model = double(inpolygon(cut_lon_rho,cut_lat_rho,refpolygon(:,1),refpolygon(:,2)));
                    mask_model(mask_model==0)=NaN;
                    mean_data1 = mean_data1.*mask_model;
                    ...
                    for yearij=1:length(inputyear2)
                        ...
                        for monthij=1:length(inputmonth)
                            ...
                            if (exist('mean_data2' , 'var') ~= 1)
                                mean_data2=zeros(size(data));
                            end
                            mean_data2=mean_data2 + (data / (length(inputyear2) * length(inputmonth)));
                        end
                    end
                    mask_model = double(inpolygon(cut_lon_rho,cut_lat_rho,refpolygon(:,1),refpolygon(:,2)));
                    mask_model(mask_model==0)=NaN;
                    mean_data2 = mean_data2.*mask_model;
                    ...
                    m_pcolor(cut_lon_rho',cut_lat_rho',diff_data');
                    shading(gca,m_pcolor_shading_method);   
                    ...
                    m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                    titlename = strcat(variable, ' diff , ',testname); 
                    title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
                    ...
                    set(h,'fontsize',colorbar_fontsize);
                    title(h,colorbar_title,'fontsize',colorbar_title_fontsize);
                    caxis(colorbar_diff_lev);

                    set(gcf, 'PaperUnits', 'points');
                    set(gcf, 'PaperSize', [hor_paper_size_x, hor_paper_size_y]);
                    set(gcf,'PaperPosition', [paper_position_hor paper_position_ver paper_position_width paper_position_height]) 
                    saveas(gcf,pngname,'tif');
                    pngname2= strcat('Z:\내 드라이브\MEPL\project\SSH\5th_year\figure\GIS\', testname,'_',regionname, '_diff_', variable, '.tif'); %% ~_year_month.jpg
                    saveas(gcf,pngname2,'tif');
                    save(['Z:\내 드라이브\MEPL\project\SSH\5th_year\figure\GIS\', 'diff_', testname, '_', variable, '.mat'], 'cut_lon_rho', 'cut_lat_rho', 'mean_data1', 'mean_data2', 'diff_data');
                    close all;
                    clear lon_rho mean_data1 mean_data2 diff_data
                end
            end
            fig_flag=0;
        end
