...
for regionind2=1:length(all_region2)
        ...
        hold on;
        tifname=strcat(figrawdir, 'fig06_04','.tif'); %% ~_year_month.jpg
...
%      ---------- GCM-IPSL-L
        ...
        for testnameind=1:4
            ...
            param_script ='C:\Users\user\Dropbox\source\matlab\Model\ROMS\Analysis\Figure\nwp_1_20\run\fig_param\fig_param_kyy_EKB_RMS.m';
            filedir = strcat('D:\Data\Model\ROMS\nwp_1_20\', testname, '\run\'); % % where data files are
%  'D:\Data\Model\CMIP5\zos\rcp85\interp\IPSL-CM5A-LR\AKP4_IPSL-CM5A-LR_model_det_std_2006_2100.mat'
            run(param_script);
            m_grid_fontsize=m_grid_fontsize-9;
            load([cmip5dir,regionname, '_', testname, '_cmip5_interped_land','.mat']);
            load([cmip5dir,'\zos\', scenname, '\interp\', testname, '\', regionname,'_',testname, '_model_', 'det_std','_',yearstr_min, '_', yearstr_max, '.mat']);
            ...
            m_proj(m_proj_name,'lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
            sb{testnameind}=subplot(8,4,[1+8*(testnameind-1) 2+8*(testnameind-1) 5+8*(testnameind-1) 6+8*(testnameind-1)]);  % Auto-fitted to the figure.
...
%         testnameind=5;
            ...
            m_proj(m_proj_name,'lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
            sb{testnameind}=subplot(8,4,[3+8*(testnameind-5) 4+8*(testnameind-5) 7+8*(testnameind-5) 8+8*(testnameind-5)]);  % Auto-fitted to the figure.
            pos_sb{testnameind}=get(sb{testnameind}, 'pos'); % Get the position.
