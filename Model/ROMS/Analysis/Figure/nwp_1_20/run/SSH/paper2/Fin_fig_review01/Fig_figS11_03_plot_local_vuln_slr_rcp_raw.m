...
for regionind2=1:length(all_region2)
        ...
        tifname=strcat(figrawdir, 'figS11_03','.tif'); %% ~_year_month.jpg
...
%             all_rcp_std(:,:,testnameind) = rcp_std;
            ...
            local_vuln_slr(290,325) = NaN; % error point
...
%             mean_data=mean_data .*mask_model;
            ...
            ysecs_slc=all_local_vuln_slr(:,:,testnameind).*ysecsmask;
            es_slc=all_local_vuln_slr(:,:,testnameind).*esmask;
            [mean_all_slc(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(mean_data, lon_rho, lat_rho);            
            [mean_ysecs_slc(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(ysecs_slc, lon_rho, lat_rho);
            [mean_es_slc(testnameind,1), error_status] = Func_0011_get_area_weighted_mean(es_slc, lon_rho, lat_rho);
