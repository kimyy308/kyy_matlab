...
% % %         flag configuration
        for folding=1:1
            ...
            fig_flags{27,1}='time_series_sla_reconstructed_seasonal';
            fig_flags{28,1}='amplitude_seasonal_ssh_reconstructed';
            ...
            fig_flags{32,1}='time_series_reconstructed_plot';
            fig_flags{33,1}='RMS_ssh_plot';
            fig_flags{34,1}='RMS_corrected_ssh_plot';
            fig_flags{35,1}='BIAS_ssh_plot';
            fig_flags{36,1}='variance_reconstructed_sla_plot';
            fig_flags{37,1}='std_reconstructed_sla_plot';
            fig_flags{38,1}='variance_reconstructed_sla_seasonal_plot';
            fig_flags{39,1}='variance_model_sla_plot';
            fig_flags{40,1}='variance_model_sla_seasonal_plot';
            fig_flags{41,1}='variance_reconstructed_sla_lowpass_plot';
            ...
            fig_flags{47,1}='variance_reconstructed_sla_movmean_plot';
...
%         std(mean(mean(comb_data_filtered,'omitnan'),'omitnan'))
        ...
        corrfilename = strcat(filedir, testname,'_',regionname, 'interped_ssh_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
        lpffilename = strcat(filedir, testname,'_',regionname, 'low_pass_filtered_interped_ssh_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
        lpf_corrfilename = strcat(filedir, testname,'_',regionname, 'low_pass_filtered_interped_ssh_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
        movfilename = strcat(filedir, testname,'_',regionname, 'moving_averaged_interped_ssh_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
        mov_corrfilename = strcat(filedir, testname,'_',regionname, 'moving_averaged_interped_ssh_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
        detrendfilename = strcat(filedir, testname,'_',regionname, 'detrended_interped_ssh_corr_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
        ...
        recon_trend=ncread(reconfilename, 'recon_trend');
...
% start-------------------- corr_interped plot
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                corr_interped=corr_interped.*recon_mask;
                lon_recon=ncread(reconfilename,'lon_recon');
                lat_recon=ncread(reconfilename,'lat_recon');

                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                m_pcolor(lon_recon',lat_recon',squeeze(corr_interped(:,:)'));
...
% start-------------------- corr_interped_filtered plot
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                m_pcolor(lon_recon',lat_recon',squeeze(corr_interped_filtered(:,:)'));
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
...
% % % % start-------------------- corr_seasonal plot
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                lon_recon=ncread(filename,'lon_recon');
                lat_recon=ncread(filename,'lat_recon');

                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                m_pcolor(lon_recon',lat_recon',squeeze(corr_spatial_mean(:,:)'));
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
...
% start-------------------- corr_interped_detrended plot
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                m_pcolor(lon_recon',lat_recon',squeeze(corr_interped_detrended(:,:)'));
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
            ...
            for nyeari=1:length(nyears)
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    ...
                    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                        ...
                        eval(['corr_',nc_varname, '=corr_', nc_varname, '.*recon_mask;']);
                        ...
                        m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                        hold on;
                        eval(['m_pcolor(lon_recon', '''', ',lat_recon', '''',',squeeze(corr_', nc_varname, '(:,:)', '''', '));']);
                        shading(gca,m_pcolor_shading_method);
                        m_gshhs_i('color',m_gshhs_line_color);
                        m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        ...
        while (fig_flag)
            ...
            for nyeari=1:length(nyears)
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    nc_varname_prefix=nc_varname_prefixes{nc_varnameij};
                    nc_varname=[nc_varname_prefix, num2str(nyear),'y_lowpass'];
                    jpgname=strcat(outfile, '_', testname,'_',regionname, '_corr_corrected_lowpass_', nc_varname, '_',num2str(min(inputyear),'%04i'), ...
                        '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
                    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
            ...
            for nyeari=1:length(nyears)
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    ...
                    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                        ...
                        eval(['corr2_',nc_varname, '=corr2_', nc_varname, '.*recon_mask;']);
                        lon_recon=ncread(filename,'lon_recon');
                        lat_recon=ncread(filename,'lat_recon');

                        m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                        hold on;
                        eval(['m_pcolor(lon_recon', '''', ',lat_recon', '''',',squeeze(corr_', nc_varname, '(:,:)', '''','-','corr2_', nc_varname, '(:,:)', '''',  '));']);
                        shading(gca,m_pcolor_shading_method);
                        m_gshhs_i('color',m_gshhs_line_color);
                        m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
                        eval(['mcorr=mean(corr_',nc_varname,'(:),', '''', 'omitnan', '''',');']);
            ...
            for nyeari=1:length(nyears)
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    ...
                    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                        ...
                        eval(['m_pcolor(lon_recon', '''', ',lat_recon', '''',',n_mean_corr_diff','''',  ');']);
...
% start-------------------- diff mask between lowpassed corr and rms 
    ...
    %                     eval(['m_pcolor(lon_recon', '''', ',lat_recon', '''',',mean_corr_diff','''',  ');']);
                        m_pcolor(lon_recon', lat_recon', diff_mask');
                        shading(gca,m_pcolor_shading_method);
                        m_gshhs_i('color',m_gshhs_line_color);
                        m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
                        eval(['mcorr=mean(corr_',nc_varname,'(:),', '''', 'omitnan', '''',');']);
...
% start-------------------- diff between lowpassed corr and rms 
    ...
    %                     eval(['m_pcolor(lon_recon', '''', ',lat_recon', '''',',mean_corr_diff','''',  ');']);
                        m_pcolor(lon_recon', lat_recon', diff_value');
                        shading(gca,m_pcolor_shading_method);
                        m_gshhs_i('color',m_gshhs_line_color);
                        m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
                        eval(['mcorr=mean(corr_',nc_varname,'(:),', '''', 'omitnan', '''',');']);
        ...
        while (fig_flag)
            ...
            for monthij = 1:12
                ...
                if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                    ...
                    m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                    hold on;
                    m_pcolor(lon_recon',lat_recon',squeeze(corr_clim(:,:,monthij))');
                    shading(gca,m_pcolor_shading_method);
                    m_gshhs_i('color',m_gshhs_line_color);
                    m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
            %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
...
%  start-------------------- absolute trend difference plot
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                trend_diff = interped_trend_filtered-recon_trend_filtered;
                m_pcolor(double(lon_recon)',lat_recon',trend_diff(:,:)');
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
...
% start-------------------- absolute trend_corrected plot
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                correction_trend=mean(recon_trend_filtered(:), 'omitnan')-mean_trend_filtered;
...
% start-------------------- lowpassed msl time series
        ...
        while (fig_flag)
            nyears=[2];
%                         nyears=[2,3,5];
            nc_varname_prefixes={'recon_', 'interped_'};
            for nyeari=1:length(nyears)
                nyear=nyears(nyeari);
                for nc_varnameij=1:length(nc_varname_prefixes)
                    nc_varname_prefix=nc_varname_prefixes{nc_varnameij};
                    ...
                    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                        for varind=1:length(inputyear)*12
                            ...
                            elseif (strcmp(nc_varname_prefix, 'recon_')==1)
                                temp_raw_msl=ncread(reconfilename,'recon_sla',[1 1 varind], [inf inf 1]);
                            end
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    ...
                    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                        for varind=1:length(inputyear)*12
                            ...
                            elseif (strcmp(nc_varname_prefix, 'recon_detrended_')==1)
                                temp_raw_msl=ncread(filename,'recon_sla',[1 1 varind], [inf inf 1]);
                            end
                            raw_msl(varind)=squeeze(mean(temp_raw_msl(:),'omitnan'));
                        end
                        msl=msl-mean(msl, 'omitnan');    
...
% start-------------------- seasonal msl time series
    ...
    %         mean_clim_ssh=mean_clim_ssh-mean(mean_clim_ssh);
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                lon_recon=ncread(reconfilename, 'lon_recon');
                lat_recon=ncread(reconfilename, 'lat_recon');
                len_lon=size(lon_recon,1);
                len_lat=size(lat_recon,2);
...
% start-------------------- recon seasonal msl
        ...
        while (fig_flag)
            ...
            climoutfile = strcat(climdir,regionname);
            jpgname=strcat(climoutfile, '_', testname, '_',regionname, '_seasonal_recon_msl_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '_',  num2str(monthij, '%02i'), '.jpg'); %% ~_year_month.jpg
    ...
    %         mean_clim_ssh=mean_clim_ssh-mean(mean_clim_ssh);
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                lon_recon=ncread(filename, 'lon_recon');
                lat_recon=ncread(filename, 'lat_recon');
                len_lon=length(lon_recon);
                len_lat=length(lat_recon);
                clim_ssh=ncread(filename, 'clim_recon_ssh');
                clim_ssh(clim_ssh>1000000)=NaN;
                recon_trend=ncread(filename, 'recon_trend');
                recon_mask=ones(size(recon_trend));
                recon_mask(isnan(recon_trend))=NaN;
                clim_ssh = clim_ssh .* recon_mask;
                for ijij=1:12
...
% start-------------------- recon seasonal msl amplitude
        ...
        while (fig_flag)
            ...
            climoutfile = strcat(climdir,regionname);
            jpgname=strcat(climoutfile, '_', testname, '_',regionname, '_seasonal_amp_recon_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '_',  num2str(monthij, '%02i'), '.jpg'); %% ~_year_month.jpg
    ...
    %             pcolor(amp_clim_ssh'*100)
                ...
                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                m_pcolor(lon_recon',lat_recon',squeeze(amp_clim_ssh(:,:)'*100.0));
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
                titlename = strcat('seasonal amp, ','recon',',(',num2str(min(inputyear),'%04i'),'-', ...
    ...
    %             title(h,'mm/y','fontsize',colorbar_title_fontsize);
                ...
                disp(' ')
                disp(['clim_', 'recon seasonal msl amplitude', ' plot is created.'])
                disp(' ')
                disp([' File path is : ',jpgname])
                disp(' ')
...
% start-------------------- seasonal msl amplitude diff
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                len_lon=length(lon_recon);
                len_lat=length(lat_recon);
                clim_recon_ssh=ncread(filename, 'clim_recon_ssh');
                clim_recon_ssh(clim_recon_ssh>1000000)=NaN;
                comb_interped_data=ncread(filename,'interped_ssh');
                comb_interped_clim_divided=reshape(comb_interped_data,[len_lon, len_lat, 12, length(inputyear)]);
                clim_ssh=mean(comb_interped_clim_divided,4,'omitnan');
                clim_ssh(clim_ssh>1000000)=NaN;
                ...
                recon_mask(isnan(recon_trend))=NaN;
                clim_ssh = clim_ssh .* recon_mask;
                clim_recon_ssh=clim_recon_ssh .* recon_mask;
    ...
    %             mean_clim_ssh=mean_clim_ssh-mean(mean_clim_ssh);
                for loni=1:size(clim_ssh,1)
                    for lati=1:size(clim_ssh,2)
                        amp_clim_ssh(loni,lati)=(max(clim_ssh(loni,lati,:))-min(clim_ssh(loni,lati,:)))/2.0;
                        amp_clim_recon_ssh(loni,lati)=(max(clim_recon_ssh(loni,lati,:))-min(clim_recon_ssh(loni,lati,:)))/2.0;
                    end
                end
    %             pcolor(amp_clim_ssh'*100)
                ...
                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                m_pcolor(lon_recon',lat_recon',amp_clim_recon_ssh(:,:)'*100-amp_clim_ssh(:,:)'*100);
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
                titlename = strcat('seasonal amp diff, ',testname,',(',num2str(min(inputyear),'%04i'),'-', ...
                    num2str(max(inputyear),'%04i'),'), ','M=',num2str(round(mean(amp_clim_recon_ssh(:)*100.0, 'omitnan')-mean(amp_clim_ssh(:)*100.0, 'omitnan'),2)));  
...
% start-------------------- recon MSL plot
        ...
        while (fig_flag)
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_recon_', 'SSH','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
                ...
                for yearij=1:length(inputyear)
                    ...
                    for monthij=1:length(inputmonth)
                        ...
                        if (exist('lon_min' , 'var') ~= 1)
                            lon_recon=ncread(reconfilename, 'lon_recon');
                            lat_recon=ncread(reconfilename, 'lat_recon');
%                             [lat_rho, lon_rho]= meshgrid(lat_recon, lon_recon);
                            [lon_min, lon_max, lat_min, lat_max] = findind_Y(1/20, lonlat(1:4), lon_recon, lat_recon);
                            cut_lon_rho = lon_recon(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
                            cut_lat_rho = lat_recon(lon_min(1):lon_max(1), lat_min(1):lat_max(1));
                        end
%                         data_info = ncinfo(filename, varname); 
                        data(:,:)=ncread(reconfilename,'recon_sla',[lon_min(1) lat_min(1) varind], ...
                ...
                recon_msl=mean(mean_data(:), 'omitnan');
...
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-recon_msl);
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-0.45);
                ...
                m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                titlename = strcat(variable, ' mean, ','recon',',(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),') ');  %% + glacier contribution
                title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
...
%                         data_info = ncinfo(filename, varname); 
                        data(:,:)=ncread(filename,'recon_adt',[lon_min(1) lat_min(1) varind], ...
                            [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
...
%                         data_info = ncinfo(filename, varname); 
                        ...
                        if (exist('sq_diff' , 'var') ~= 1)
                            ...
                            sq_diff=sq_diff + ((model_data-model_msl)*100-(data-recon_msl)*100).^2;
                    end
                end
...
%                         data_info = ncinfo(filename, varname); 
                        ...
                        if (exist('mean_bias' , 'var') ~= 1)
                            ...
                            mean_bias=mean_bias + ((model_data-model_msl)*100-(data-recon_msl)*100)/(length(inputyear) * length(inputmonth));
                    end
                end
...
% start-------------------- recon variance plot
        ...
        while (fig_flag)
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_recon_', 'VAR','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                run(param_script);
...
%                             [lat_rho, lon_rho]= meshgrid(lat_recon, lon_recon);
                ...
                recon_sla=ncread(reconfilename,'recon_sla', [lon_min(1) lat_min(1) 1], ...
                            [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 inf]);
                for loni=1:size(recon_sla,1)
                    for lati=1:size(recon_sla,2)
                        recon_sla_var(loni,lati)=var(recon_sla(loni,lati,:).*100);
                    end
                end
%                 recon_msl=mean(mean_data(:), 'omitnan');
                m_proj(m_proj_name,'lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
                ...
                mask_model(mask_model==0)=NaN;
%                 mean_data= mean_data .* mask_model;
                recon_sla_var=recon_sla_var .* mask_model;
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-recon_msl);
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-0.45);
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-mean(mean_data(:),'omitnan'));
                m_pcolor(cut_lon_rho',cut_lat_rho',recon_sla_var');
                ...
                m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                titlename = strcat(variable, ' VAR, ','recon',',(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),') ');  %% + glacier contribution
                title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
...
% start-------------------- recon std plot
        ...
        while (fig_flag)
            ...
            pngname=strcat(outfile, '_', testname,'_',regionname, '_recon_', 'STD','_',num2str(min(inputyear),'%04i'), ...
                '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                run(param_script);
%                 for yearij=1:length(inputyear)
...
%                         data_info = ncinfo(filename, varname); 
%                         data(:,:)=ncread(filename,'recon_sla',[lon_min(1) lat_min(1) varind], ...
%                             [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
%                         if (exist('mean_data' , 'var') ~= 1)
%                             mean_data=zeros(size(data));
%                         end
...
%                     end
%                 end
                recon_sla=ncread(filename,'recon_sla', [lon_min(1) lat_min(1) 1], ...
                            [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 inf]);
                for loni=1:size(recon_sla,1)
                    for lati=1:size(recon_sla,2)
                        recon_sla_var(loni,lati)=var(recon_sla(loni,lati,:).*100);
...
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-0.45);
%                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-mean(mean_data(:),'omitnan'));
                m_pcolor(cut_lon_rho',cut_lat_rho',sqrt(recon_sla_var)');
                ...
                m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                titlename = strcat(variable, ' STD, ','recon',',(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),') ');  %% + glacier contribution
                title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
...
% start-------------------- recon variance_seasonal plot
    ...
    %                 end
                    for yearij=1:length(inputyear)
                        recon_sla(:,:,yearij)=ncread(filename,'recon_sla', [lon_min(1) lat_min(1) (yearij-1)*12+tempmonth], ...
                                    [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
                    end
    ...
    %                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-mean(mean_data(:),'omitnan'));
                    ...
                    m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                    titlename = strcat(variable, ' VAR, ','recon,', calendarname{tempmonth}(1:3), ',(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),') ');  %% + glacier contribution
                    title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
...
% start-------------------- recon variance_lowpass plot
        ...
        while (fig_flag)
            ...
            for nyeari=1:length(nyears)
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    ...
                    pngname=strcat(outfile, '_', testname,'_',regionname, '_recon_', nc_varname,'_','VAR_',num2str(min(inputyear),'%04i'), ...
                        '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
                    if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                        run(param_script);
                        if (exist('lon_min' , 'var') ~= 1)
                        ...
                        eval(['recon_sla', '=ncread(filename,', '''', nc_varname,'''',');']);
%                         recon_sla=ncread(filename,'recon_sla', [lon_min(1) lat_min(1) 1], ...
%                                     [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 inf]);
                        recon_sla=recon_sla(:,:,nyear*6+1:end-(nyear*6)-1);
                        for loni=1:size(recon_sla,1)
                            for lati=1:size(recon_sla,2)
                                recon_sla_var(loni,lati)=var(recon_sla(loni,lati,:));
                            end
                        end
        %                 recon_msl=mean(mean_data(:), 'omitnan');
                        m_proj(m_proj_name,'lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
        ...
        %                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-0.45);
        %                 m_pcolor(cut_lon_rho',cut_lat_rho',mean_data'-mean(mean_data(:),'omitnan'));
                        m_pcolor(cut_lon_rho',cut_lat_rho',recon_sla_var'.*100);
                        ...
                        m_grid('fontsize', m_grid_fontsize, 'box', m_grid_box_type, 'tickdir', m_grid_tickdir_type);
                        titlename = strcat(nc_varname, ' VAR, ','recon',',(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),') ');  %% + glacier contribution
                        title(titlename,'fontsize',m_pcolor_title_fontsize);  %%title
...
% start-------------------- model pattern correlation    
        ...
        while (fig_flag)
            recon_adt=ncread(filename,'recon_adt');
            interped_ssh=ncread(filename,'interped_ssh');
            recon_sla=ncread(filename,'recon_sla');
            interped_sla=ncread(filename,'interped_sla');
            m_recon_adt=mean(recon_adt,3,'omitnan');
            m_interped_ssh=mean(interped_ssh,3,'omitnan');
            m_recon_sla=mean(recon_sla,3,'omitnan');
            m_interped_sla=mean(interped_sla,3,'omitnan');
            m_interped_trend_filtered=mean(interped_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered))));
            m_recon_trend_filtered=mean(recon_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered))));

            corr2(m_interped_ssh(logical(~isnan(m_interped_ssh).*~isnan(m_recon_adt))), m_recon_adt(logical(~isnan(m_interped_ssh).*~isnan(m_recon_adt))))
            corr2(interped_sla(logical(~isnan(m_interped_ssh).*~isnan(m_recon_sla))), m_recon_sla(logical(~isnan(m_interped_ssh).*~isnan(m_recon_sla))))
            corr2(interped_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered))) - m_interped_trend_filtered, ...
                recon_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered)))- m_recon_trend_filtered)
        ...
        % start-------------------- model pattern correlation (yearly)
            ...
            if (exist(pngname , 'file') ~= 2 || fig_flag==2)      
                for yearij=1:length(inputyear)
                    ...
                    yearly_recon_adt=ncread(filename,'recon_adt',[1 1 (yearij-1)*12+1], [inf inf 12]);
                    yearly_interped_ssh=ncread(filename,'interped_ssh',[1 1 (yearij-1)*12+1], [inf inf 12]);
                    yearly_recon_sla=ncread(filename,'recon_sla',[1 1 (yearij-1)*12+1], [inf inf 12]);
                    yearly_interped_sla=ncread(filename,'interped_sla',[1 1 (yearij-1)*12+1], [inf inf 12]);

                    m_recon_adt=mean(yearly_recon_adt,3,'omitnan');
                    m_interped_ssh=mean(yearly_interped_ssh,3,'omitnan');
                    m_recon_sla=mean(yearly_recon_sla,3,'omitnan');
                    m_interped_sla=mean(yearly_interped_sla,3,'omitnan');
        %             m_interped_trend_filtered=mean(interped_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered))));
        %             m_recon_trend_filtered=mean(recon_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered))));

                    ssh_corr(yearij) = corr2(m_interped_ssh(logical(~isnan(m_interped_ssh).*~isnan(m_recon_adt))), m_recon_adt(logical(~isnan(m_interped_ssh).*~isnan(m_recon_adt))));
                    sla_corr(yearij) = corr2(interped_sla(logical(~isnan(m_interped_ssh).*~isnan(m_recon_sla))), m_recon_sla(logical(~isnan(m_interped_ssh).*~isnan(m_recon_sla))));
        %             corr2(interped_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered))) - m_interped_trend_filtered, ...
        %                 recon_trend_filtered(logical(~isnan(interped_trend_filtered).*~isnan(recon_trend_filtered)))- m_recon_trend_filtered)
                end
                plot(ssh_corr);
        ...
        while (fig_flag)
            ...
            for nyeari=1:length(nyears)
                ...
                for nc_varnameij=1:length(nc_varname_prefixes)
                    nc_varname_prefix=nc_varname_prefixes{nc_varnameij};
                    nc_varname=[nc_varname_prefix, num2str(nyear),'y_movmean'];
                    jpgname=strcat(outfile, '_', testname,'_',regionname, '_corr_movmean_', nc_varname, '_',num2str(min(inputyear),'%04i'), ...
                        '_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
                    if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                        eval(['corr_',nc_varname, '=ncread(mov_corrfilename,','''','corr_', nc_varname,'''',');']);
                        eval(['corr_',nc_varname, '=corr_', nc_varname, '.*recon_mask;']);
...
% start-------------------- recon variance_movmean plot
        ...
        %                             [lat_rho, lon_rho]= meshgrid(lat_recon, lon_recon);
                        ...
                        eval(['recon_sla', '=ncread(movfilename,', '''', nc_varname,'''',');']);
%                         recon_sla=ncread(filename,'recon_sla', [lon_min(1) lat_min(1) 1], ...
%                                     [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 inf]);
                        recon_sla=recon_sla(:,:,nyear*6+1:end-(nyear*6)-1);
                        for loni=1:size(recon_sla,1)
                            for lati=1:size(recon_sla,2)
                                recon_sla_var(loni,lati)=var(recon_sla(loni,lati,:).*100.0);
                            end
                        end
        %                 recon_msl=mean(mean_data(:), 'omitnan');
                        m_proj(m_proj_name,'lon',[lonlat(1) lonlat(2)],'lat',[lonlat(3) lonlat(4)]);
...
% start-------------------- cmems_relative trend plot (bwrmap)
        ...
        while (fig_flag)
            jpgname=strcat(outfile, '_', testname,'_',regionname, '_cmems_relative_ssh_trend_bwrmap_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                if (exist('lon_min' , 'var') ~= 1)
...
% start-------------------- cmems absolute trend plot (bwrmap)
        ...
        while (fig_flag)
            outfile = strcat(figdir,regionname);
            jpgname=strcat(outfile, '_', testname,'_',regionname, '_cmems_absolute_ssh_trend_bwrmap_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                if (exist('lon_min' , 'var') ~= 1)
                    lon_cmems=ncread(cmemsfilename, 'lon_cmems');
...
% start-------------------- recon_relative trend plot (bwrmap)
        fig_flag=fig_flags{51,2};
        while (fig_flag)
            jpgname=strcat(outfile, '_', testname,'_',regionname, '_recon_relative_ssh_trend_bwrmap_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                if (exist('lon_min' , 'var') ~= 1)
                    lon_recon=ncread(reconfilename, 'lon_recon');
...
%                             [lat_rho, lon_rho]= meshgrid(lat_recon, lon_recon);
                ...
                hold on;
                recon_trend_filtered=ncread(reconfilename, 'recon_trend_filtered');
                mean_trend_filtered=mean(recon_trend_filtered(:),'omitnan');
                m_pcolor(double(cut_lon_rho)',cut_lat_rho',squeeze(recon_trend_filtered(:,:)'-mean_trend_filtered));
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
        %         titlename = strcat(regionname,', SSH trend, ','Mean=',num2str(round(mean_trend_filtered,2)),'mm/y');
                titlename = strcat('SSH trend(abs), ', 'recon', ',(',num2str(min(inputyear),'%04i'),'-', ...
                    num2str(max(inputyear),'%04i'),'), ','M=',num2str(round(mean_trend_filtered,2)), ' mm/y');  %% + glacier contribution
...
% start-------------------- recon absolute trend plot (bwrmap)
        fig_flag=fig_flags{52,2};
        while (fig_flag)
            outfile = strcat(figdir,regionname);
            jpgname=strcat(outfile, '_', testname,'_',regionname, '_recon_absolute_ssh_trend_bwrmap_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.tif'); %% ~_year_month.jpg
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                if (exist('lon_min' , 'var') ~= 1)
                    lon_recon=ncread(reconfilename, 'lon_recon');
...
%                             [lat_rho, lon_rho]= meshgrid(lat_recon, lon_recon);
                ...
                m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
                hold on;
                m_pcolor(double(cut_lon_rho)',cut_lat_rho',squeeze(recon_trend_filtered(:,:)'));
                shading(gca,m_pcolor_shading_method);
                m_gshhs_i('color',m_gshhs_line_color);
                m_gshhs_i('patch',m_gshhs_land_color);   % gray colored land
                mean_trend_filtered=mean(recon_trend_filtered(:),'omitnan');
