...
%         sshdifflev = [40 70];
        ...
        inputyear2 = [2005:2005]; % % put year which you want to plot [year year ...]
...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
%             figrawdir =strcat('F:\OneDrive - 서울대학교\MEPL\project\SSH\5th_year\figure\nwp_1_20\',testname,'\',regionname,'\'); % % where figure files will be saved
...
%                         mean_data=mean_data + (data / (length(inputyear2) * length(inputmonth)));
                ...
                switch(regionname)
                    case('NWP')
                        pngname=strcat(outfile, 'fig01_01.tif');
                        m_pcolor(cut_lon_rho',cut_lat_rho', -mean_data');
                    case('AKP4') %% North western Pacific
                        pngname=strcat(outfile, 'fig01_02.tif'); 
