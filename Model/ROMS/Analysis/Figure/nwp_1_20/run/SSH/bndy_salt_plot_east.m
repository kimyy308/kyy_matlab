...
model_depth = ncread(filename,'h',[980 1], [1 920]);
...
testname='test42';
discharge_u=squeeze(mean(mean(comb_east_data,1,'omitnan'),2,'omitnan'));
comb_east_data_u=comb_east_data;
load ('E:\Data\Model\ROMS\nwp_1_20\test42\run\NWP_v_all_time_1980_2008.mat')

testname='test42'
% discharge_v=squeeze(mean(mean(comb_east_data,1,'omitnan'),2,'omitnan'));
comb_east_data_v=comb_east_data;
...
% plot(squeeze(mean(mean(comb_east_data,1,'omitnan'),2,'omitnan')))

hor_dist=m_lldist([115 115], [15 15.0483])*1000;

% discharge=squeeze(mean(mean(comb_east_data.*comb_east_data_v.*model_depth_z3.*hor_dist,1,'omitnan'),2,'omitnan'));

% discharge=squeeze(sum(sum(comb_east_data.*comb_east_data_u.*model_depth_z3.*hor_dist,1,'omitnan'),2,'omitnan'));
discharge=squeeze(sum(sum(comb_east_data.*model_depth_z3.*hor_dist,1,'omitnan'),2,'omitnan'));
...
hold off
jpgname=strcat(outfile,testname, '_salt_east_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
xlabel('year')
ylabel('sum of salt at the boundary (meter^2)')
title(['eastern boundary salinity, ',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i')])
