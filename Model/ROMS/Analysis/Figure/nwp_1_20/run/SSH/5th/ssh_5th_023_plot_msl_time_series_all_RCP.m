...
for testind=1:length(RCM_rcp26_testnames)
    ...
    RCM_rcp26_cmemsfilenames{testind} = strcat(filedir, testname,'_',regionname, 'cmems_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp26_interpedfilenames{testind} = strcat(filedir, testname,'_',regionname, 'cmems_interped_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp26_movfilenames{testind} = strcat(filedir, testname,'_',regionname, 'moving_averaged_cmems_interped_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
...
for testind=1:length(RCM_rcp45_testnames)
    ...
    drivename='D';
    filedir=strcat(drivename, ':\Data\Model\ROMS\nwp_1_20\', testname, '\run\');
    RCM_rcp45_modelfilenames{testind} = strcat(filedir, testname,'_',regionname, '_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp45_cmemsfilenames{testind} = strcat(filedir, testname,'_',regionname, 'cmems_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp45_interpedfilenames{testind} = strcat(filedir, testname,'_',regionname, 'cmems_interped_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp45_movfilenames{testind} = strcat(filedir, testname,'_',regionname, 'moving_averaged_cmems_interped_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
end
...
for testind=1:length(RCM_rcp85_testnames)
    ...
    drivename='D';
    filedir=strcat(drivename, ':\Data\Model\ROMS\nwp_1_20\', testname, '\run\');
    RCM_rcp85_modelfilenames{testind} = strcat(filedir, testname,'_',regionname, '_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp85_cmemsfilenames{testind} = strcat(filedir, testname,'_',regionname, 'cmems_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp85_interpedfilenames{testind} = strcat(filedir, testname,'_',regionname, 'cmems_interped_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
    RCM_rcp85_movfilenames{testind} = strcat(filedir, testname,'_',regionname, 'moving_averaged_cmems_interped_ssh_trend_', ...
        num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.nc');
end
...
nyears=[1,2,5];
for testind=1:length(RCM_rcp26_testnames)
    RCM_rcp26_interped_sla(testind,:,:,:) = ncread(RCM_rcp26_interpedfilenames{testind}, 'interped_sla');
    ...
    RCM_rcp26_interped_sla_yearly_mean(testind,:)=mean(RCM_rcp26_interped_sla_3d(testind,:,:),2,'omitnan') .* 100; % (m -> cm)
...
for testind=1:length(RCM_rcp45_testnames)
    RCM_rcp45_interped_sla(testind,:,:,:) = ncread(RCM_rcp45_interpedfilenames{testind}, 'interped_sla');
    ...
    RCM_rcp45_interped_sla_yearly_mean(testind,:)=mean(RCM_rcp45_interped_sla_3d(testind,:,:),2,'omitnan') .* 100; % (m -> cm)
...
for testind=1:length(RCM_rcp85_testnames)
    RCM_rcp85_interped_sla(testind,:,:,:) = ncread(RCM_rcp85_interpedfilenames{testind}, 'interped_sla');
    ...
    RCM_rcp85_interped_sla_yearly_mean(testind,:)=mean(RCM_rcp85_interped_sla_3d(testind,:,:),2,'omitnan') .* 100; % (m -> cm)
...
for folding=1:1
        jpgname=strcat(outfile, '_all_scen_',regionname, '_RCM_msla_yearly_mean_', ...
           ...
           %% Hue, Saturation, Value
            ...
            for testind=1:length(RCM_rcp26_testnames)
                ...
                RCM_rcp26_interped_sla_yearly_mean(testind,:)=RCM_rcp26_interped_sla_yearly_mean(testind,:)+(cmems_1y_mean-RCM_rcp26_interped_1y_mean(testind));
            ...
            for testind=1:length(RCM_rcp45_testnames)
                ...
                RCM_rcp45_interped_sla_yearly_mean(testind,:)=RCM_rcp45_interped_sla_yearly_mean(testind,:)+(cmems_1y_mean-RCM_rcp45_interped_1y_mean(testind));
            ...
            for testind=1:length(RCM_rcp85_testnames)
                ...
                RCM_rcp85_interped_sla_yearly_mean(testind,:)=RCM_rcp85_interped_sla_yearly_mean(testind,:)+(cmems_1y_mean-RCM_rcp85_interped_1y_mean(testind));
...
% start-------------------- msl time series (5 movmean) (RCM)
for folding=1:1
        jpgname=strcat(outfile, '_all_scen_',regionname, '_RCM_msla_yearly_5ymovmean_', ...
            num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
           ...
           %% Hue, Saturation, Value
            ...
            for testind=1:length(RCM_rcp26_testnames)
                ...
                RCM_rcp26_interped_sla_yearly_mean(testind,:)=RCM_rcp26_interped_sla_yearly_mean(testind,:)+(cmems_1y_mean-RCM_rcp26_interped_1y_mean(testind));
                if testind==length(RCM_rcp26_testnames)
                    tempmovmean=movmean(RCM_rcp26_interped_sla_yearly_mean(testind,:),5);
                    mslplot_rcp26_all{testind}=plot(xData2(1:pastlen),tempmovmean(1:pastlen),'Color', cmap_rcp26)
                    mslplot_rcp26_all{testind+1}=plot(xData2(pastlen:end),tempmovmean(pastlen:end),'Color', cmap_rcp26)
                else
                    mslplot_rcp26_all{testind}=plot(xData2,movmean(RCM_rcp26_interped_sla_yearly_mean(testind,:),5),'Color', cmap_rcp26)                    
                end
            ...
            for testind=1:length(RCM_rcp45_testnames)
                ...
                RCM_rcp45_interped_sla_yearly_mean(testind,:)=RCM_rcp45_interped_sla_yearly_mean(testind,:)+(cmems_1y_mean-RCM_rcp45_interped_1y_mean(testind));
                if testind==length(RCM_rcp45_testnames)
                    tempmovmean=movmean(RCM_rcp45_interped_sla_yearly_mean(testind,:),5);
                    mslplot_rcp45_all{testind}=plot(xData2(1:pastlen),tempmovmean(1:pastlen),'Color', cmap_rcp45)
                    mslplot_rcp45_all{testind+1}=plot(xData2(pastlen:end),tempmovmean(pastlen:end),'Color', cmap_rcp45)
                else
                    mslplot_rcp45_all{testind}=plot(xData2,movmean(RCM_rcp45_interped_sla_yearly_mean(testind,:),5),'Color', cmap_rcp45)                    
                end
                if testind == length(RCM_rcp45_testnames)
                    set(mslplot_rcp45_all{testind}, 'Linewidth', 9)
                    set(mslplot_rcp45_all{testind+1}, 'Linewidth', 9)
            ...
            for testind=1:length(RCM_rcp85_testnames)
                ...
                RCM_rcp85_interped_sla_yearly_mean(testind,:)=RCM_rcp85_interped_sla_yearly_mean(testind,:)+(cmems_1y_mean-RCM_rcp85_interped_1y_mean(testind));
                if testind==length(RCM_rcp85_testnames)
                    tempmovmean=movmean(RCM_rcp85_interped_sla_yearly_mean(testind,:),5);
                    mslplot_rcp85_all{testind}=plot(xData2(1:pastlen),tempmovmean(1:pastlen),'Color', cmap_rcp85)
                    mslplot_rcp85_all{testind+1}=plot(xData2(pastlen:end),tempmovmean(pastlen:end),'Color', cmap_rcp85)
                else
                    mslplot_rcp85_all{testind}=plot(xData2,movmean(RCM_rcp85_interped_sla_yearly_mean(testind,:),5),'Color', cmap_rcp85)                    
                end
                if testind == length(RCM_rcp85_testnames)
                    set(mslplot_rcp85_all{testind}, 'Linewidth', 9)
                    set(mslplot_rcp85_all{testind+1}, 'Linewidth', 9)
...
% end-------------------- msl time series (5 movmean) (RCM)
