...
all_testname2 = {'test61', 'test62','test63','test64','ens09','test57', 'test58','test59','test60','ens08','test65', 'test66','test67','test68',  'ens10'};
...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
        ...
        matname = [filedir, testname, '_', regionname, '_', 'SSH', '_exp_slr_sta_', num2str(min(inputyear),'%04i'), '-', num2str(max(inputyear),'%04i'), '.mat'];
        if (exist(matname , 'file') ~= 2)
            ...
            slr(1:ind_sta)=NaN;
            for ind_sta=1:size(tidal_st,1)
                [f_exp, gof_exp] = fit(trendtime_yearly',squeeze(yearly_model_msl(:,ind_sta)),'exp1');
                model_msl_yearly_exp_fit(1:length(trendtime_yearly), ind_sta)=f_exp(trendtime_yearly);
                slr(ind_sta)=(model_msl_yearly_exp_fit(end,ind_sta) - model_msl_yearly_exp_fit(1,ind_sta)) * 100.0;
            end
...
%                 jpgname=strcat(outfile, '_', testname, '_',tidal_name{ind_sta}, '_exp_slr_msl_', ...
...
%             end
            save(matname, 'tidal_st', 'model_msl_yearly_exp_fit', 'slr', 'model_trend', 'ind_sta_model_lon', 'ind_sta_model_lat');
        else
            load(matname);
        end
end
