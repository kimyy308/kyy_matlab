...
% all_region2 ={'NWP'}
for scennameind2=1:length(scennames)
for testnameind2=1:length(all_testname2)
    for regionind2=1:length(all_region2)
        close all;
        clearvars '*' -except regionind2 testnameind2 all_region2 all_testname2 all_var2 scennames scennameind2
        % % % 
        % % % Read Model SST
        % % % interp
        % % % get RMS
        % % % get BIAS
        scenname = scennames{scennameind2};
...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
        ...
        if (strcmp(system_name,'PCWIN64'))
            % % for windows
            figrawdir =strcat('Z:\내 드라이브\MEPL\project\SSH\5th_year\figure\CMIP5\',testname,'\',scenname, '\', regionname,'\'); % % where figure files will be saved
...
% start-------------------- earlier decadal current plot
        ...
        %                 ref_vec_y_range = round(ref_vec_y_ind-(m_quiver_y_interval/2)) : round(ref_vec_y_ind-(m_quiver_y_interval/2))+m_quiver_y_interval;
                    ...
                    save(matname, 'u_rho','v_rho', 'cut_lon_rho', 'cut_lat_rho', '-append');
                else
                    load(matname);
                end
                ...
                if (exist(pngname , 'file') ~= 2 || fig_flag==2)        
                    ...
                    if (exist(matname , 'file') ~= 2 || fig_flag==2)
                        for yearij=1:length(inputyear1)
                            ...
                            for monthij=1:length(inputmonth)
                                ...
                                elseif (strcmp(variable,'BT')==1)
                                    data = ncread(filename,varname,[lon_min(1) lat_min(1) inf tind], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1 1]);  %% cut horizontal area [x,y,z] (wider than target area) 
                                end
                        ...
                        elseif(strcmp(variable, 'SSH'))   
                            load(['D:\Data\Model\CMIP5\sealevel_mass\post_sle_', scenname, '.mat']);
                            tyear=2006;
                            ...
                            for kk = 1 : length( list )
                                ...
                                fyear_start = str2num( fyear_str{1}(1:4) );
                                fyear_end   = str2num( fyear_str{2}(1:4) );
                                if( tyear >= fyear_start && tyear <= fyear_end &&     ...                 
                                        strcmp( fname_split{2}, 'Omon' ) &&         ...
                                        strcmp( fname_split{3}, testname ) &&      ...                 
                                        strcmp( fname_split{4}, scenname ) )
                                    flag_file_in = true;            break;
                            ...
                            filename=[filedir, '\', fname_in];
                            zostoga = ncread(filename,'zostoga',[1], [1140]);
                            sle_ind=(inputyear1(1)-2006)*12+1 : (inputyear1(end)-2006)*12+12;
                            mean_corr_rcp = mean(corr_rcp(sle_ind));
                            mean_zostoga = mean(zostoga(sle_ind));
                            switch(testname)
                                case('NorESM1-M') %% North western Pacific
                                    mean_data=mean_data+(mean_corr_rcp-corr_rcp(1));
                                otherwise
                                    mean_data=mean_data+(mean_corr_rcp-corr_rcp(1)) + (mean_zostoga-zostoga(1));                                    
                            end
                ...
                if (exist(pngname , 'file') ~= 2 || fig_flag==2)        
                    ...
                    if (exist(matname , 'file') ~= 2 || fig_flag==2)
                        ...
                        elseif(strcmp(variable, 'SSH'))   
                            ...
                            filename=[filedir, '\', fname_in];
                            zostoga = ncread(filename,'zostoga',[1], [1140]);
                            sle_ind=(inputyear2(1)-2006)*12+1 : (inputyear2(end)-2006)*12+12;
                            mean_corr_rcp = mean(corr_rcp(sle_ind));
                            mean_zostoga = mean(zostoga(sle_ind));
                            switch(testname)
                                case('NorESM1-M') %% North western Pacific
                        ...
                        save(matname, 'mean_data', 'cut_lon_rho', 'cut_lat_rho', '-append');
                    else
                        load(matname);
                    end
                    mask_model = double(inpolygon(cut_lon_rho,cut_lat_rho,refpolygon(:,1),refpolygon(:,2)));
