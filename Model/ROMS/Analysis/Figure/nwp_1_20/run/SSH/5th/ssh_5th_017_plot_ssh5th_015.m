...
all_testname2 = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens08', 'ens09', 'ens10'};
% all_testname2 = {'ens08', 'ens09', 'ens10'};

% all_testname2 = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens08'};
% all_testname2 = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens09'};
% all_testname2 = {'ens09', 'ens10'};

% all_testname2 = {'IPSL-CM5A-LR', 'IPSL-CM5A-MR', 'NorESM1-M', 'MPI-ESM-LR', 'ens10'};
...
% all_region2 ={'NWP', 'AKP4', 'ES_KHOA','YS_KHOA', 'SS_KHOA'};
% all_region2 ={'GLO'};
all_region2 ={'AKP4'};

% all_region2 ={'TEST'}

all_scenname2={'rcp26', 'rcp45', 'rcp85'};

for scennameind2=1:length(all_scenname2)
for testnameind2=1:length(all_testname2)
    for regionind2=1:length(all_region2)
        close all;
        clearvars '*' -except regionind2 testnameind2 all_region2 all_testname2 all_scenname2 scennameind2
        % % % 
        % % % Read Model SST
        % % % interp
        % % % get RMS
...
%                     '_ssh_analysis_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.nc'];
         ...
         scenname=all_scenname2{scennameind2};
         switch testname
                case {'ens09'}
                    scenname='rcp26';
                case {'ens08'}
...
% start-------------------- model slr-ratio index plot (from exp fit yearly SLR)
        ...
        while (fig_flag)
            ...
            if (exist(jpgname , 'file') ~= 2 || fig_flag==2)
                ...
                scenname_his='historical';
                his_period = [1993 2005];
                hisdir = strcat('D:\Data\Model\CMIP5\zos\', scenname_his, '\interp\', testname, '\'); % % where data files are
                interpedfilename_his = strcat(hisdir, testname,'_',regionname, ...
                ...
                for loni=1:size(model_sla_his_yearly_detrended,1)
                    for lati=1:size(model_sla_his_yearly_detrended,2)
                        model_sla_his_std(loni,lati)=std(model_sla_his_yearly_detrended(loni,lati,:).*100.0);
                    end
                end
