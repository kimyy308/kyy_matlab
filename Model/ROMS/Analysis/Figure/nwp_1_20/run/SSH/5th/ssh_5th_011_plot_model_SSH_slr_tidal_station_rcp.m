...
%             end
    ...
    % start-------------------- plot trend along station
        round(slr',2)
        outfile = strcat(figdir,'slr_station_');
        jpgname=strcat(outfile, '_', testname, '_', ...
                num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
        if (exist(jpgname , 'file') ~= 2)
            trendplot=plot(slr,'-o','MarkerFaceColor','k')
            xticks(1:21)
            xticklabels(tidal_name)
            xtickangle(45)
            xlabel('Station')
            ylabel('slr (cm)')
            title([testname, ' exp fit slr (',num2str(min(inputyear),'%04i'), ...
                    '-',num2str(max(inputyear),'%04i'),'), ','M = ',num2str(round(mean(slr),2)), ' cm'])
%             axis tight;
            ylim(slrplotlev)
            set(gcf,'PaperPosition', [0 0 36 12]) 
