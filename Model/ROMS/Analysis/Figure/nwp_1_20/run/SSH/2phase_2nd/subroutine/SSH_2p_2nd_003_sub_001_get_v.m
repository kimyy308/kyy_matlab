...
disp('subroutine v_2p_2nd_003_sub_001_get_v') 
disp([RCM_info.testname, ', ', RCM_info.regionname, ', ', ...
...
[RCM_info.days, tmp.error_status] = Func_0020_get_day_from_season(RCM_info.months, 0);

tmp.elapsed=toc(tmp.lap_time_j);
tmp.totlap= (length(RCM_info.years)*length(RCM_info.days));
...
if (exist(RCM_info.matname , 'file') ~= 2 || flags.fig_tmp==2)
    RCM_info.size_t=length(RCM_info.years)*length(RCM_info.days);
    RCM_info.size_years=length(RCM_info.years);
    RCM_info.size_days=length(RCM_info.days);
...
%% print present time
            tmp.elapsed=toc(tmp.lap_time_j);
            tmp.templap= (yearij-1)*length(RCM_info.days)+dayij-1;
...
%% set data filename (raw grid, ES)
            switch(RCM_info.testname)
                otherwise
                    ...
                    'pck_ES_', RCM_info.testname, '_daily_', num2str(tmp.year,'%04i'), ...
                    '_', num2str(tmp.day,'%04i'), '.nc');
            end
...
%% get RCM data
            RCM_data.tmp_data = ncread(RCM_info.filename, tmp.variable, [RCM_grid.ind_w RCM_grid.ind_s, 40, 1], ...
                    [RCM_grid.size_lon_rho RCM_grid.size_lat_rho, 1, 1]);
...
%                 RCM_data.yearly_maxind=NaN(RCM_grid.size_lat_rho, RCM_info.size_years);
            ...
            RCM_data.yearly_mean(:,:,yearij)=RCM_data.yearly_mean(:,:,yearij) + ...
                RCM_data.tmp_data / double(RCM_info.size_days);
