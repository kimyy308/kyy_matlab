...
% % set figure dir
RCM_info.figdir=['D:\MEPL\project\SSH\7th_year(2022)\figure\nwp_1_20\all\adm_trend'];

if (exist(RCM_info.figdir , 'dir') ~= 7)
    mkdir(RCM_info.figdir);
end
...
RCM_info.matname_mean_data_trend = [RCM_info.savedir,'ENS_adm','_',RCM_info.regionname, '_RCM_ssh_mean_data_trend_', ...
    num2str(min(RCM_info.years),'%04i'),'_',num2str(max(RCM_info.years),'%04i'), '_', RCM_info.season,'.mat'];
...
hold off
tifname= [RCM_info.figdir, '\RCM_station_trend_ENS_adm_hist_',...
...
set(gcf,'PaperPosition', [0 0 30 15]) 
hold off
tifname= [RCM_info.figdir, '\GCM_station_trend_ENS_adm_hist_',...
    num2str(min(RCM_info.years),'%04i'),'_',num2str(max(RCM_info.years),'%04i'), '.tif'];
saveas(gcf,tifname,'tif'); RemoveWhiteSpace([], 'file', tifname);
close all;
