% %  Updated 12-Jan-2022 by Yong-Yub Kim, make


% start-------------------- earlier decadal current plot

dirs.figdir=[dirs.figrawdir,'surface', tmp.fs, tmp.regionname, tmp.fs, 'ekman_t', tmp.fs, ...
...
tmp.tifname=strcat(dirs.figdir, tmp.testname, '_clim_ekman_t_',num2str(min(RCM_info.years),'%04i'), ...
    '_',num2str(max(RCM_info.years),'%04i'), ...
                    '_', num2str(RCM_info.months(1),'%04i'), '-', num2str(RCM_info.months(end),'%04i'),'.tif'); %% ~_year_month.jpg
tmp.variable='EKMAN_T';
if (exist(tmp.tifname , 'file') ~= 2 || flags.fig_switch(8)==2)      
...
%                             [lon_v_min, lon_v_max, lat_v_min, lat_v_max] = findind_Y(1/20, lonlat(1:4), lon_v, lat_v);
        ...
        cut_f = ...
            RCM_grid.f(RCM_grid.lon_min(1):RCM_grid.lon_max(1), RCM_grid.lat_min(1):RCM_grid.lat_max(1));
    end
                
    if (exist(tmp.matname , 'file') ~= 2 || flags.fig_switch(8)==2)
...
%                         D:\Data\Model\ROMS\nwp_1_20\backup_surf\test2102\run\v\1985\pck_test2102_v_monthly_1985_01.nc
                switch tmp.testname
                    otherwise
                        tmp.sustrfilename=[dirs.filedir, 'sustr', tmp.fs, tmp.yearstr, filesep,'NWP_pck_', tmp.testname, '_sustr_monthly_', tmp.yearstr, '_', tmp.monthstr, '.nc'];
                        tmp.svstrfilename=[dirs.filedir, 'svstr', tmp.fs, tmp.yearstr, filesep,'NWP_pck_', tmp.testname, '_svstr_monthly_', tmp.yearstr, '_', tmp.monthstr, '.nc'];
                end
               
                
                tmp.data_info = ncinfo(tmp.sustrfilename, 'sustr'); 
                tmp.sustr = ncread(tmp.sustrfilename,'sustr', ...
                ...
                tmp.svstr = ncread(tmp.svstrfilename,'svstr', ...
                    [RCM_grid.lon_min(1) RCM_grid.lat_min(1) 1], ...
                ...
                if (isfield(tmp, 'mean_sustr') ~= 1)
                    tmp.mean_sustr=zeros(size(tmp.sustr));
                    tmp.mean_svstr=zeros(size(tmp.svstr));
                end
                tmp.mean_sustr=tmp.mean_sustr + (tmp.sustr / (length(RCM_info.years) * length(RCM_info.months)));
                tmp.mean_svstr=tmp.mean_svstr + (tmp.svstr / (length(RCM_info.years) * length(RCM_info.months)));
            end
        end
        sustr_rho = u2rho_2d(tmp.mean_sustr')';
        svstr_rho = v2rho_2d(tmp.mean_svstr')';
        ...
        rho0=1024;
        ekman_u_rho = svstr_rho ./ rho0 ./ cut_f;
        ekman_v_rho = -sustr_rho ./ rho0 ./ cut_f;
        ekman_u_rho(tmp.ref_vec_x_range,tmp.ref_vec_y_range)=param.m_quiver_ref_u_value;
        ekman_v_rho(tmp.ref_vec_x_range,tmp.ref_vec_y_range)=param.m_quiver_ref_v_value; 
        save(tmp.matname, 'ekman_u_rho','ekman_v_rho', 'cut_lon_rho', 'cut_lat_rho');
    else
        load(tmp.matname);
    end
    ...
    ekman_u_rho=ekman_u_rho(1:size(RCM_grid.mask_model,1),:).*RCM_grid.mask_model;
    ekman_v_rho=ekman_v_rho(:,1:size(RCM_grid.mask_model,2)).*RCM_grid.mask_model;  
    ...
    uvplot=m_quiver(cut_lon_rho(1:param.m_quiver_x_interval:end, 1:param.m_quiver_wind_y_interval:end)', ...
                    cut_lat_rho(1:param.m_quiver_x_interval:end, 1:param.m_quiver_wind_y_interval:end)', ...
                    ekman_u_rho(1:param.m_quiver_x_interval:end, 1:param.m_quiver_wind_y_interval:end)' * param.m_quiver_vector_size/5, ...
                    ekman_v_rho(1:param.m_quiver_x_interval:end, 1:param.m_quiver_wind_y_interval:end)' * param.m_quiver_vector_size/5, ...
                    'color', param.m_quiver_vector_color, 'AutoScale','off','LineWidth', param.m_quiver_LineWidth);

    m_text(param.m_quiver_ref_text_x_location, param.m_quiver_ref_text_y_location, param.m_quiver_ekman_ref_text, 'FontSize', param.m_quiver_ref_text_fontsize); 
