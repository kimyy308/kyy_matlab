disp('subroutine SSH_2p_2nd_010_sub_005_plot_bndy_GLORYS') 
...
for bndyij=1:length(RCM_info.bndy_directions)
    ...
    dirs.figdir=[dirs.figrawdir_GLORYS, tmp.fs, 'bndy', tmp.fs, tmp.variable_GCM, tmp.fs, ...
    ...
    tmp.tifname=[dirs.figdir, tmp.fs, 'GLORYS', '_', tmp.variable_GCM, '_', ...
    ...
    switch tmp.variable_GCM
        case 'so'
            [C,h2]=contour(RCM_grid.(['lonlat_', tmp.direction]), RCM_grid.(['stddepth_', tmp.direction]), ...
                    GCM_bndy_mean.([tmp.variable_GCM, '_stddepth_', tmp.direction, '_interped']), [30:0.5:35], 'color','k', ...
                'linewidth', 1.5, 'linestyle', '-');
            clabel(C,h2,'FontSize',13,'Color','k', ...
                'labelspacing', 50000,'Rotation', 0,'fontweight', 'bold');
        case 'thetao'
            [C,h2]=contour(RCM_grid.(['lonlat_', tmp.direction]), RCM_grid.(['stddepth_', tmp.direction]), ...
                    GCM_bndy_mean.([tmp.variable_GCM, '_stddepth_', tmp.direction, '_interped']), [0:5:35], 'color','k', ...
                'linewidth', 1.5, 'linestyle', '-');
            clabel(C,h2,'FontSize',13,'Color','k', ...
                'labelspacing', 50000,'Rotation', 0,'fontweight', 'bold');
        otherwise
    ...
    if min(RCM_info.years) == max(RCM_info.years)
        tmp.titlename = strcat(tmp.variable_GCM, ', ', RCM_info.season(1:3), ', ', 'GLORYS',',(',num2str(max(RCM_info.years),'%04i'),') ');                        
    else
        tmp.titlename = strcat(tmp.variable_GCM, ', ', RCM_info.season(1:3), ', ', 'GLORYS',',(',num2str(min(RCM_info.years),'%04i'),'-',num2str(max(RCM_info.years),'%04i'),') ');
    end
...
%     close all;
    ...
    pcolor(RCM_grid.(['lonlat_', tmp.direction])(:,1:tmp.ind_dep), RCM_grid.(['stddepth_', tmp.direction])(:,1:tmp.ind_dep), ...
        GCM_bndy_mean.([tmp.variable_GCM, '_stddepth_', tmp.direction, '_interped'])(:,1:tmp.ind_dep)); 
    shading(gca,param.m_pcolor_shading_method);   
    switch tmp.variable_GCM
        case 'so'
            [C,h2]=contour(RCM_grid.(['lonlat_', tmp.direction])(:,1:tmp.ind_dep), RCM_grid.(['stddepth_', tmp.direction])(:,1:tmp.ind_dep), ...
                    GCM_bndy_mean.([tmp.variable_GCM, '_stddepth_', tmp.direction, '_interped'])(:,1:tmp.ind_dep), [30:0.5:35], 'color','k', ...
                'linewidth', 1.5, 'linestyle', '-');
            clabel(C,h2,'FontSize',13,'Color','k', ...
                'labelspacing', 50000,'Rotation', 0,'fontweight', 'bold');
        case 'thetao'
            [C,h2]=contour(RCM_grid.(['lonlat_', tmp.direction])(:,1:tmp.ind_dep), RCM_grid.(['stddepth_', tmp.direction])(:,1:tmp.ind_dep), ...
                    GCM_bndy_mean.([tmp.variable_GCM, '_stddepth_', tmp.direction, '_interped'])(:,1:tmp.ind_dep), [0:5:35], 'color','k', ...
                'linewidth', 1.5, 'linestyle', '-');
            clabel(C,h2,'FontSize',13,'Color','k', ...
                'labelspacing', 50000,'Rotation', 0,'fontweight', 'bold');
        otherwise
