...
for testnameind=1:length(RCM_info.name)
    for regionind=1:length(RCM_info.region)
        clearvars '*' -except regionind testnameind flags flg_lev param ...
            ...
            RCM_mean_data RCM_mean_trend GCM_mean_data GCM_mean_trend RCM_info GCM_info
...
%                     tmp.month = RCM_info.months(monthij);
        ...
        if (exist(RCM_info.matname , 'file') == 2)   
            ...
            error([RCM_info.matname, ' does not exist'])
        end
...
%                 CMEMS_grid.lon2, CMEMS_grid.lat2);
        ...
        if testnameind==1
            RCM_mean_data.ens.adm_div_all=zeros(1,length(RCM_mean_data.([RCM_info.testname]).adm_div_all));
            RCM_mean_data.ens.adm_div_all= ...
                RCM_mean_data.ens.adm_div_all + RCM_mean_data.([RCM_info.testname]).adm_div_all/length(RCM_info.name);
            GCM_mean_data.ens.adm_div_all=zeros(1,length(GCM_mean_data.([RCM_info.testname]).adm_div_all));            
            GCM_mean_data.ens.adm_div_all= ...
                GCM_mean_data.ens.adm_div_all + GCM_mean_data.([RCM_info.testname]).adm_div_all/length(RCM_info.name);
        else
            RCM_mean_data.ens.adm_div_all= ...
                RCM_mean_data.ens.adm_div_all + RCM_mean_data.([RCM_info.testname]).adm_div_all/length(RCM_info.name);
            GCM_mean_data.ens.adm_div_all= ...
        ...
        switch (RCM_info.regionname)
            case({'NWP', 'AKP4'})
                ...
                [tmp_fitobject, tmp_gof] = fit(RCM_time.trendtime_yearly', ...
                     squeeze(RCM_mean_data.([RCM_info.testname]).('adm_div_all'))'.*1000.0, 'poly1'); %% m/y -> mm/y
                RCM_mean_trend.([RCM_info.testname]).('adm_div_all') = tmp_fitobject.p1;


        %         RCM_mean_trend.([RCM_info.testname]).('adm_div_all') = ...
        %             RCM_mean_trend.([RCM_info.testname]).('adm_div_all') .* ...
        %             RCM_grid.mask_adm_div_all;
                [tmp_fitobject, tmp_gof] = fit(RCM_time.trendtime_yearly', ...
                     squeeze(GCM_mean_data.([RCM_info.testname]).('adm_div_all'))'.*1000.0, 'poly1'); %% m/y -> mm/y
                GCM_mean_trend.([RCM_info.testname]).('adm_div_all') = tmp_fitobject.p1;
        %         GCM_mean_trend.([RCM_info.testname]).('adm_div_all') = ...
        %             GCM_mean_trend.([RCM_info.testname]).('adm_div_all') .* ...
        %             GCM_grid.mask_adm_div_all;


        % % %         get mean sea-level ('adm_div_ES', 'adm_div_YS', 'adm_div_SS')
                RCM_info.subregions = {'adm_div_ES', 'adm_div_YS', 'adm_div_SS'};
    ...
    case({'NWP', 'AKP4'})
        ...
        for i=1:RCM_time.tlen
            for testnameind=1:length(RCM_info.name)
                RCM_info.testname = RCM_info.name{testnameind};
                if testnameind==1
                    tmp.RCM_ens_members=RCM_mean_data.([RCM_info.testname]).('adm_div_all')(i);
                    tmp.GCM_ens_members=GCM_mean_data.([RCM_info.testname]).('adm_div_all')(i);
                else
                    tmp.RCM_ens_members=[tmp.RCM_ens_members, RCM_mean_data.([RCM_info.testname]).('adm_div_all')(i)];
                    tmp.GCM_ens_members=[tmp.GCM_ens_members, GCM_mean_data.([RCM_info.testname]).('adm_div_all')(i)];
                end
...
for i=1:RCM_time.tlen
    RCM_mean_data.ens.([RCM_info.regionname, '_rand_lower_limit'])(i) = quantile(RCM_mean_data.ens.([RCM_info.regionname, '_rand'])(:,i), tmp.lowerlimit);
    RCM_mean_data.ens.([RCM_info.regionname, '_rand_upper_limit'])(i) = quantile(RCM_mean_data.ens.([RCM_info.regionname, '_rand'])(:,i), tmp.upperlimit);
    GCM_mean_data.ens.([GCM_info.regionname, '_rand_lower_limit'])(i) = quantile(GCM_mean_data.ens.([RCM_info.regionname, '_rand'])(:,i), tmp.lowerlimit);
    GCM_mean_data.ens.([GCM_info.regionname, '_rand_upper_limit'])(i) = quantile(GCM_mean_data.ens.([RCM_info.regionname, '_rand'])(:,i), tmp.upperlimit);
end
