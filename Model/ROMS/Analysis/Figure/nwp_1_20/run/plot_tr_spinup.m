...
%% before 97.Feb -> sea level difference (Takikawa and yoon, 2005)
%% 97.Feb ~ 2002.Feb -> ADCP (Takikawa et al, 2005)
%% 97.Feb ~ 2007.Feb -> ADCP (Fukudome et al, 2010)
...
for i=1:length(year)
    if refyear>=1980
        ...
        korea_obs_spinup((12*(i-1))+1:(12*(i-1))+12)=0;    %% get observation transport at the korea strait
    end
end
...
lgd=legend('Korea(Ref)','Korea(Model)','Tsugaru', 'Soya');
set(lgd,'FontSize',10);
...
xlabel('Time (month)','color','k','FontSize',17,'fontweight','bold');
ylabel('Volume Transport (sv)','color','k','FontSize',17,'fontweight','bold');
lgd=legend('Korea(Ref)','Korea(Model)','Taiwan', 'onshore(out(-))');
set(lgd,'FontSize',10);
set(lgd,'Position',[0.13 0.88, 0.775, 0.03]);
set(lgd,'Orientation','horizontal');
set(gca,'FontSize',13);
