...
% C  = Corr_ST_CW{51,1};
% C  = Corr_CS_Obs{51,1};
% C  = Corr_CS_White{51,1};     % size: [x y theta lag]
% C  = Corr_CS_Colored{51,1};   % size: [x y theta lag]
C  = Corr_CS_CW{51,1};          % 예시: 필요에 따라 ST용 변수로 교체하세요 (size: [x y lag]일 수 있음)
...
if hasTheta
    ...
    for k = 1:3
        ...
        colormap(gca, cmap);
        caxis(caxisRange);
        title(sprintf('Obs-Corr(\\theta, lag): NPP (t+\\tau) \\leftarrow %s (t)', names{k}));
        xlabel('Lead (months)');
        ylabel('\theta (month)');
