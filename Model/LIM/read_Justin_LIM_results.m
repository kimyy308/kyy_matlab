...
base_dir = ['/Volumes/kyy_raid/kimyy/Research/Postdoc/03_IBS/', ...
            '2025_LIM_BGC_prediction/manuscripts/Revision_round1/To_Yong_Yub/'];
...
for No = 1:50
    ...
    % 0. Load MAT file
    %% ---------------------------------------------------------
    matfile = sprintf('%sNPP_LIM_n_4_No_%d.mat', base_dir, No);
    ...
    % --- Y ---
    assert(iscell(Y) && isequal(size(Y), [12 24]), 'Y must be 12x24 cell');
    n_init_year = size(Y{1,1}, 2);
    assert(size(Y{1,1},1) == n_state, 'Y cell state dim mismatch');

    %% ---------------------------------------------------------
    % 2. Create NetCDF file
    %% ---------------------------------------------------------
    ncfile = sprintf('%sNPP_LIM_n_4_No_%d.nc', base_dir, No);
    if isfile(ncfile)
        delete(ncfile);
    end
            ...
            for s = 1:numel(modes)
                ...
                if ismember(mat_types{mt}, {'A','D'})
                    if strcmp(modes{s}, 'CS')
                        assert(isequal(size(V), [4 4 12]), [vname ' must be 4x4x12']);
                        varid.(vname) = netcdf.defVar(ncid, vname, 'double', ...
                            [dim.state_i, dim.state_j, dim.month]);
                    else
                        assert(isequal(size(V), [4 4]), [vname ' must be 4x4']);
                        varid.(vname) = netcdf.defVar(ncid, vname, 'double', ...
