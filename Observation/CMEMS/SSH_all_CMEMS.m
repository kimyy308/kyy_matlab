...
for regionind=1:length(all_region)
    ...
    switch(regionname)
        ...
        case('NWP2') %% North western Pacific
            lonlat = [114, 165, 14, 53];  %% whole data area
            refpolygon(1,1)=lonlat(1);
            refpolygon(2,1)=lonlat(2);
            refpolygon(1,2)=lonlat(3);
            refpolygon(2,2)=lonlat(4);
...
%             refpolygon(2,2)=lonlat(4);
    ...
    elseif (strcmp(system_name,'GLNXA64'))
        ...
        filedir = strcat('/data2/kimyy/Observation/CMEMS/'); % % where data files are
        cmemsdir='/data2/kimyy/Observation/CMEMS/';
    end
...
%         inputmonth = 1:yeardays(inputyear(yearij));
        ...
        %         ncinfo('E:\Data\Observation\OIssh\monthly\cmems_monthly1983_11.nc');
            ...
            if tempmonth==1
                ...
                cmems_daily_ugos=ncread(cmemsfilename,'ugos',[cmems_lon_min(1) cmems_lat_min(1) 1], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) 31]);
                cmems_daily_vgos=ncread(cmemsfilename,'vgos',[cmems_lon_min(1) cmems_lat_min(1) 1], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) 31]);
                ...
                cmems_daily_ugos=ncread(cmemsfilename,'ugos',[cmems_lon_min(1) cmems_lat_min(1) sum(eomday(tempyear,1:tempmonth-1))+1], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) eomday(tempyear,tempmonth)]);
                cmems_daily_vgos=ncread(cmemsfilename,'vgos',[cmems_lon_min(1) cmems_lat_min(1) sum(eomday(tempyear,1:tempmonth-1))+1], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) eomday(tempyear,tempmonth)]);
            ...
            cmems_ugos = nanmean(cmems_daily_ugos,3);
            cmems_vgos = nanmean(cmems_daily_vgos,3);
...
%             cmems_data = ncread(cmemsfilename,varname,[cmems_lon_min(1) cmems_lat_min(1) tempmonth], [cmems_lon_max(1)-cmems_lon_min(1) cmems_lat_max(1)-cmems_lat_min(1) 1]);
            ...
            comb_cmems_ugos(:,:,ind) = cmems_ugos;
            comb_cmems_vgos(:,:,ind) = cmems_vgos;
...
%     save([filedir,regionname,'cmems_ssh_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
    ...
    cmems_ugosvarid=netcdf.defVar(ncid, 'cmems_ugos', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,cmems_ugosvarid,'long_name','Absolute geostrophic velocity: zonal component');
    netcdf.putAtt(ncid,cmems_ugosvarid,'units','m/s ');
    
    cmems_vgosvarid=netcdf.defVar(ncid, 'cmems_vgos', 'NC_FLOAT', [lon_dimid lat_dimid time_dimid]);
    netcdf.putAtt(ncid,cmems_vgosvarid,'long_name','Absolute geostrophic velocity: meridian component');
    netcdf.putAtt(ncid,cmems_vgosvarid,'units','m/s ');
    ...
    netcdf.putVar(ncid, cmems_ugosvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_cmems_ugos);
    netcdf.putVar(ncid, cmems_vgosvarid, [0 0 0], [len_lon len_lat length(ftime)], comb_cmems_vgos);
