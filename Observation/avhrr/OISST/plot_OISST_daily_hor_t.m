function status=plot_OISST_daily_hor_t(testname, outfile, filedir, section, tempyear,inputmonth, inputday, shadlev, conlev,  param_script)
% clear all;close all;
%==========================================================================
% %  Updated 18-Aug-2019 by Yong-Yub Kim 
...
for monthij=1:length(inputmonth)
    ...
    for dayij=1:length(inputday)
        ...
        filedir=[rawfiledir, num2str(tempyear, '%04i'), num2str(tempmonth,'%02i'),'\',];
        filename = strcat(filedir, testname, '.',num2str(tempyear,'%04i'), num2str(tempmonth, '%02i'), num2str(tempday,'%02i'), '.nc');
    ...
    % % %     plot
        ...
        titlename = strcat('OISST, (',num2str(tempday, '%02i'),'d, ', char(calendarname(tempmonth)), ', ', num2str(tempyear),')');
        ...
        jpgname=strcat(outfile, '_', testname, '_', num2str(tempyear,'%04i'), '_', num2str(tempmonth,'%02i'), num2str(tempday,'%02i'), '.jpg'); %% ~_year_month.jpg
        grid off;
        drawnow;
