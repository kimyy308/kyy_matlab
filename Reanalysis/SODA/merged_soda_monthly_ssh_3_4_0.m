...
sodadir='/data1/kimyy/ext_hdd/Reanalysis/SODA/SODA_3_4_0/';
sodaprefix='soda3.4.0_mn_ocean_reg_';
...
lon = ncread(strcat(sodadir,sodaprefix,num2str(2015,'%4i'),'.nc'),'longitude');
lat = ncread(strcat(sodadir,sodaprefix,num2str(2015,'%4i'),'.nc'),'latitude');
for year=minyear:maxyear
    ...
    for month=1:12
        ...
        ssha_aviso(:,:,month,year-minyear+1)=griddata(double(lon_aviso),double(lat_aviso),double(tempssha)',double(lon),double(lat)'); %% [x y v'(y,x) xv yv]
    end
    year
end
...
lon_data_info = ncinfo(strcat(sodadir,sodaprefix,num2str(year,'%4i'),'.nc'), 'longitude');
lat_data_info = ncinfo(strcat(sodadir,sodaprefix,num2str(year,'%4i'),'.nc'), 'latitude');
